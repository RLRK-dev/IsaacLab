...
> Every sim/proxy verdict must **state its conservatism direction** for the question being asked. A sim is *conservative* for a question if it makes success **harder** than reality; *non-conservative* if it makes success **easier**. Consequence: a **non-conservative PASS needs real-world (or higher-fidelity) confirmation** before it transfers; a **conservative FAIL is conclusive** (if it fails even in the easy case, it fails in the hard case) — **but only when the test METHOD is faithful** (it exercised the real scenario's load path/dynamics, not a convenient proxy; a method whose unfaithfulness biases toward *failure* downgrades the FAIL to inconclusive-pending-faithful-retest, and a *success*-biasing unfaithfulness cannot rescue a FAIL). Faithfulness is the method/load-path axis, distinct from this conservatism (fidelity) axis; for a FAIL banked conclusive, name the one method/load-path delta from the real scenario, mark each load-bearing mechanism candidate engaged-or-bypassed, and state the discriminating re-test. A verdict that omits its conservatism direction is incomplete.
