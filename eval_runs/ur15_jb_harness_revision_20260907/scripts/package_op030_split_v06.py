...
PLAN = "data/op030_split_shots_v06.json"
MOTION = "data/op030_split_animation_v06.npz"
PREPARED = "data/op030_split_animation_v06.json"
NATIVE_AUDIT = "audit/op030_split_native_v06.json"
DOCUMENT = "OP030_三ST工程・部品確認書_v06.md"
PAGE = "review_OP030_split_v06.html"
README = "README_OP030_split_v06.md"
PACKAGE_AUDIT = "audit/op030_split_package_v06.json"
INVENTORY = "DELIVERY_SHA256.json"
REVIEW_STATUS = (
    "ケーブル設置のBをコンベアの反対側へ移し、A・B・Cを左右交互に配置しました。"
    "製品の向きと搬送高さを保ち、Bの把持・設置動作を新しい配置に合わせています。"
)
SCRIPT_FILES = (
    "scripts/package_op030_split_v06.py",
    "scripts/animate_op030_split_v06.py",
...
def video_stem(view: str) -> str:
    """Return the fixed v06 filename stem for a review view."""
    return f"UR15_JB_OP030_split_{view}_v06"
...
<title>UR15 OP030｜支持部・内部ケーブル 3 ST v06</title><style>
...
<small>EV用ジャンクションボックス＋高電圧ハーネス / OP030 / v06</small>
<h1>千鳥配置の3 STで支持部・ケーブルを組み付ける</h1>
...
保存動作の時間はAが238.8秒、Cが80.967秒です。Bの新しい動作時間は工程・部品確認書に記載しています。
...
def readme_text(context: dict) -> str:
    ...
    return f"""# UR15 OP030 3 ST 工程確認 v06
...
Aの保存動作は7,165フレーム・238.8秒です。Bの新しい動作時間は同梱確認書を参照してください。
...
- ベイクコード: `scripts/animate_op030_split_v06.py` と同梱のimport先

19個のhelperと上記の静的モデル・対応表・保存動作2個を合わせた23入力を最小構成として扱います。
`scripts/op030_fixed_height_timeline_v04.py` は共通実装として旧名のまま再利用しています。
隔離フォルダーでの実再ベイク結果は、`audit/op030_split_layout_portable_rebake_v06.json` を参照してください。
...
python scripts/encode_op030_v02.py --folders regenerated_split_process --view process \\
  --plan {Path(PLAN).name} --basename UR15_JB_OP030_split_process_v06_regenerated
...
python scripts/encode_op030_v02.py --folders regenerated_split_wide --view wide \\
  --plan {Path(PLAN).name} --basename UR15_JB_OP030_split_wide_v06_regenerated
```
...
同梱render/encodeのv02というファイル名は再利用コードの名前です。入力planと出力名はv06を明示しています。
...
```bash
python scripts/package_op030_split_v06.py
```
