...
BANK = ROOT / "data/op030_split_c_front_motion_v05.npz"
AUDIT = ROOT / "audit/op030_c_front_replay_v05.json"
...
def prepare() -> None:  # noqa: C901 -- Preserve the archived authoring routine in this source checkpoint.
    ...
    context = Context(data["object_names"], MESH)
    routes_paths = [
        ROOT / "audit/op030_c_near_branch_survey_v05.json",
        ROOT / "audit/op030_c_near_common_branch_v05.json",
    ]
    inputs = [Path(__file__), MESH, MESH.with_suffix(".json"), *routes_paths]
    hashes = {str(p): digest(p) for p in inputs}
    reports = [json.loads(p.read_text()) for p in routes_paths]
    routes = {2: reports[0]["returns"], 1: reports[1]["records"][0]["records"]}
    ...
    if abs(receipt - np.asarray(routes[1][-1]["joints"][-1])).max() > 1e-6:
        raise ValueError("Receipt winding mismatch")
    root = data["poses"][0, 0]
    ...
    def control(pair):
        ...
        row["object_poses"] = row["object_poses"].copy()
        for index, name in enumerate(data["object_names"]):
            if "feeder" in str(name) and (name not in owners or owners[name] == 0):
                arm = int("M14" in str(name))
                row["object_poses"][index] = delta[arm] @ row["object_poses"][index]
