...
def summarize(ready: dict, results: dict, pinned: dict[str, str], stage_path: Path) -> dict:
    ...
    sources = {(stage_path / f"UR15_JB_OP030_split_{view}_v06_review.mp4").as_uri() for view in ("process", "wide")}
...
def main() -> None:
    ...
    parser.add_argument("--output_dir", type=Path, default=ROOT / "audit/op030_split_page_browser_v06")
    ...
    required += [f"UR15_JB_OP030_split_{view}_v06_review.mp4" for view in ("process", "wide")]
