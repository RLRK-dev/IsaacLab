...
const directory='/home/rlrk/Downloads/THREAD_HVJB_外組み工程_v01_20260916';
...
try{
  ...
  await evaluate(`document.getElementById('search').value='';document.getElementById('search').dispatchEvent(new Event('input'));document.querySelector('[data-evidence="0"]').click();window.scrollTo(0,0);`);
  await pause(200);
  ...
  const report={observed_at:new Date().toISOString(),directory,initial,evidence,filters,search:searched,mobile,missing_links:missing,javascript_errors:errors,scope:'Offline UI and source-link checks only; no physical acceptance'};
  const body=JSON.stringify(report,null,2)+'\n';
  await writeFile(join(root,'audit/hvjb_preassembly_browser_v01.json'),body,{flag:'wx'});
  await writeFile(join(directory,'browser_audit.json'),body,{flag:'wx'});
  console.log('PREASSEMBLY_BROWSER_OK',JSON.stringify({initial,evidence:evidence.length,filters,missing:missing.length,errors:errors.length,mobile}));
