...
"""Join v05 parallel arm motion and early loading on a fixed-height pallet [m, s]."""
...
def main():
    ...
    parser.add_argument("--b_factory", default="op030_split_b_v05:build_sequence")
    parser.add_argument("--c_factory", required=True)
    parser.add_argument("--static_manifest", type=Path, default=ROOT / "audit/op030_split_static_v04.json")
    parser.add_argument("--output", type=Path, default=ROOT / "data/op030_split_animation_v05.npz")
    ...
    if config.get("transport_clips", True):
        raise ValueError("v05 must not contain cable holding mechanisms")
    ...
    if static.get("retention", {}).get("clips"):
        raise ValueError("Cable holding mechanisms remain in the v05 static manifest")
    ledgers = {}
    for name in ("A", "C"):
    ...
    station_work_side["B"] = [state["root"][1, 0] >= 0 for state in states]
