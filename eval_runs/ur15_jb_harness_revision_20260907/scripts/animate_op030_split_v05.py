...
"""Bake v05 parallel arm motion and early loading with fixed transport height [m, s]."""
...
def configure_presentation_cameras():
    ...
    definitions = {
        ...
        "split_B_stock": ((-2.55, -0.55, 1.90), (-1.50, 0.60, 1.30), 28),
        "split_B_joint": ((-0.65, 1.65, 1.70), (0.02, 0.75, 0.93), 48),
...
def bake_native(data_path, output):
    ...
    def positioner(name, poses):
        # The linkage drivers read the carriage's local Z. Keeping the raised
        # base parent is essential: unparenting would change that coordinate.
        obj = bpy.data.objects[name]
        if obj.parent is None:
            raise ValueError("Fixed-height positioner lost its raised base")
    ...
    positioner(static["entry_lift"]["carriage"], translations(0, -1.15, data["op020_carriage"][:, 2, 3] - 0.392))
    ...
    for station in ("A", "B", "C"):
        ...
        positioner(cell["lift_carriage"], translations(0, cell["offset_y_m"], CONVEYOR_RISE + data["lift_" + station]))
    ...
    world(drawer, translations(data["drawer_B"], 2.3, 0))
    active = {row["uid"] for row in prepared["wires"]}
    for wire in static["wires"]:
        if wire["uid"] not in active:
            world(wire["uid"], translations(data["drawer_B"], 2.3, 0))
    wire_results = []
    ...
    if retention.get("clips"):
        raise ValueError("Removed cable restraints remain in v05")
    ...
    if failures:
        write_json(ROOT / "audit/op030_split_native_v05_readback_failed.json", failures)
    ...
    write_json(
        ROOT / "data/op030_split_shots_v05.json",
        dict(
            ...
            phase_prefix="",
            scope_caption="EV用ジャンクションボックス｜支持部取付・ケーブル設置・端子締結｜3ST動作検討モデル",
        ),
    )
    report = dict(
    ...
    write_json(ROOT / "audit/op030_split_native_v05.json", report)
    print("OP030_SPLIT_NATIVE_COMPLETE", str(output), len(frames), flush=True)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--data", type=Path, default=ROOT / "data/op030_split_animation_v05.npz")
    parser.add_argument("--output", type=Path, default=ROOT / "UR15_JB_OP030_split_v05.blend")
