...
"""Run the existing isolated Chrome checks for the v06 delivery page [s, px]."""
...
HELPERS = {
    "main": ROOT / "analysis/verify_op030_split_process_page.mjs",
...
def preflight(options: argparse.Namespace) -> dict:
    ...
    return {
        ...
        "note": "Executable, helper syntax and plan checks only; no claim about v06 video playback or page layout",
    }


def run_helper(key: str, options: argparse.Namespace, ready: dict) -> dict:
    """Run one unchanged v03 browser helper against explicit v06 inputs."""
    ...
    command = [
        ...
        "--label",
        f"OP030_split_v06_delivery_{key}",
    ]
...
def summarize(ready: dict, results: dict, pinned: dict[str, str], stage_path: Path) -> dict:
    ...
    sources = {(stage_path / f"UR15_JB_OP030_split_{view}_v06_review.mp4").as_uri() for view in ("process",)}
    ...
    checks = {
        ...
        "one_expected_process_video_source": len(videos) == 1 and {row["source"] for row in videos} == sources,
        "process_video_duration_matches_plan": len(videos) == 1
        and all(abs(row["duration_s"] - duration) <= 0.02 for row in videos),
        "process_video_actually_played": len(main.get("playback_results", [])) == 1
...
def main() -> None:
    ...
    parser.add_argument("--output_dir", type=Path, default=ROOT / "audit/op030_split_process_page_browser_v06")
    ...
    required += [f"UR15_JB_OP030_split_{view}_v06_review.mp4" for view in ("process",)]
