...
const directory='/home/rlrk/Downloads/THREAD_HVJB_写真部品照合_v03_20260915';
...
try{
  ...
  const wireIds=await evaluate('catalog.visible_wire_segments.map(r=>r.id)');
  const wireChecks=[];
  for(const id of wireIds){
    const state=await evaluate(`new Promise(resolve=>{document.getElementById('related').checked=true;choose(${JSON.stringify(id)});requestAnimationFrame(()=>requestAnimationFrame(()=>resolve({render:window.photoReviewRendered,markers:document.querySelectorAll('#endpoint-marks .end-marker').length,notes:document.querySelectorAll('#endpoints p').length,viewBox:document.getElementById('photo').getAttribute('viewBox'),expected:[...contextIds()].sort()})));})`);
    if(state.markers!==2||state.notes!==3||JSON.stringify([...state.render.shown_ids].sort())!==JSON.stringify(state.expected)||state.render.gl_error!==0)throw new Error('Wire context '+JSON.stringify({id,state}));
    wireChecks.push({id,...state});
  }
  const traces=await evaluate('({rows:document.querySelectorAll("#traces tr").length,retiredAbsent:!["WI31","WI32"].some(id=>byId[id]),expectedRegion:byId.I03.observations.overview.visibility})');
  if(traces.rows!==wireIds.length||!traces.retiredAbsent||traces.expectedRegion!=='occluded_expected_region')throw new Error(JSON.stringify(traces));
  const candidateChecks=[];
  for(const id of ['CC01','CC02']){
    const state=await evaluate(`new Promise(resolve=>{document.querySelector('[data-candidate="${id}"]').click();requestAnimationFrame(()=>requestAnimationFrame(()=>resolve({render:window.photoReviewRendered,markers:document.querySelectorAll('#endpoint-marks .end-marker').length,photoMarks:document.querySelectorAll('#marks .selected').length,expected:[...contextIds()].sort()})));})`);
    if(state.markers!==4||state.photoMarks!==2||JSON.stringify([...state.render.shown_ids].sort())!==JSON.stringify(state.expected))throw new Error('Candidate context '+JSON.stringify(state));
    candidateChecks.push({id,...state});
  }
  const detailTrace=await evaluate(`new Promise(resolve=>{choose('W15');document.getElementById('source-view').value='detail';document.getElementById('source-view').dispatchEvent(new Event('change'));requestAnimationFrame(()=>requestAnimationFrame(()=>resolve({render:window.photoReviewRendered,markers:document.querySelectorAll('#endpoint-marks .end-marker').length})));})`);
  if(detailTrace.render.source_view!=='detail'||detailTrace.markers!==2)throw new Error(JSON.stringify(detailTrace));
  await evaluate('window.scrollTo(0,0)');await pause(600);
  const wireScreenshot=await call('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});
  await writeFile(join(directory,'audit/browser_wire_entry.png'),Buffer.from(wireScreenshot.data,'base64'));
  await evaluate("document.getElementById('all').click()");
  const missing=[];
  ...
  const report={checked_at:new Date().toISOString(),initial,selected,content,mobile,sourceImages,uncovered,sourceSelection,wireChecks,traces,candidateChecks,detailTrace,missing_links:missing,
  ...
  await writeFile(fileURLToPath(new URL('../audit/hvjb_photo_browser_v03.json',import.meta.url)),text);
