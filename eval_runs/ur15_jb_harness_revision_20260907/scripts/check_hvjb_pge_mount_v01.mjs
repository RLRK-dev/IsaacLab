...
// Reuses the isolated Chrome/CDP workflow from check_hvjb_compact_hand_v01.mjs.
...
const profile = await mkdtemp(join(tmpdir(), 'hvjb-pge-mount-browser-'));
...
try {
  ...
  const root = "document.getElementById('hvjb-pge-mount-20260917')";
  for (let i = 0; i < 80; i++) {
  ...
  for (const width of [736, 320]) {
    ...
    for (const scheme of ['light', 'dark']) {
      ...
      for (const state of ['finger', 'interface']) {
        await evaluate(`${root}.querySelector('[data-mount-view="${state}"]').click()`);
        await pause(80);
        const observed = await evaluate(`(() => {
          ...
          return {state: r.dataset.view, width: r.clientWidth, documentWidth: document.documentElement.scrollWidth,
            fingers: +r.dataset.fingerCount, actuators: +r.dataset.actuatorCount,
            visibleFinger: r.querySelectorAll('[data-visible-finger]').length,
            visiblePads: r.querySelectorAll('[data-visible-pad]').length,
            pressed: [...r.querySelectorAll('button[aria-pressed="true"]')].map(n => n.dataset.mountView),
            faceMm: +(r.querySelector('[data-jaw-face-mm]')?.dataset.jawFaceMm || 0),
            screws: r.querySelectorAll('[data-jaw-screw]').length,
            locatorCenters: [...r.querySelectorAll('[data-locator]')].map(n => +n.dataset.uMm),
            circleDiameter: +(r.querySelector('[data-diameter-mm]')?.dataset.diameterMm || 0),
            outside, overlap};
        })()`);
        if (observed.state !== state || observed.fingers !== spec.finger_concept.finger_count || observed.actuators !== spec.finger_concept.actuator_count || observed.pressed.join() !== state) throw new Error(JSON.stringify(observed));
        if (observed.documentWidth > width || observed.outside.length || observed.overlap.length) throw new Error(JSON.stringify(observed));
        if (state === 'interface') {
          if (observed.faceMm !== spec.interface.nominal_face_size_mm[0] || observed.screws !== spec.interface.screw_axes_per_jaw) throw new Error('Moving jaw interface differs from recorded drawing');
          if (observed.circleDiameter !== spec.interface.round_locator_diameter_mm || JSON.stringify(observed.locatorCenters) !== JSON.stringify(spec.interface.locator_centers_uv_mm.map(p => p[0]))) throw new Error('Locator centers differ from recorded drawing');
        } else if (observed.visibleFinger !== 1 || observed.visiblePads !== 2) {
          throw new Error('Side view does not show the declared nearer finger with two pads');
        }
  ...
  console.log('PGE_MOUNT_BROWSER_OK', JSON.stringify({states: results.length, errors: errors.length}));
} finally {
