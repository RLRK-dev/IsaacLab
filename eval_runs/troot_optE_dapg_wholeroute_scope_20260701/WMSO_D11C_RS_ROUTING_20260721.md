...
## (b) 採択 D-1 への影響 — ✅**裁定済（2026-07-21 19:3x）= 格下げ**

⭐**Rs 逐語**「**落選した案を却下した理由に誤りがあったとき、採択された案は再度候補にかくさげ**」
⇒ **D-1 は 採択 → 候補。B1 は CLOSED → OPEN。** custody = `WMSO_RS_RULING_B_D1_DEMOTION_CUSTODY_20260721.md`。
⛔**残る確認 3 点**（同 custody §5）= ①却下されていた案（rank 2 への proof kind 追加）も候補に戻るか〔決定脚「D-3 に合流」は誤りと独立に成立〕②凍結 D1.1-B の status 表現（(a)=B により凍結物は編集しない）③**B1 を誰が・いつ選び直すか**（⛔pQ は自己起動しない）。
⚠**D1.1-C v2.5 は D-1 に非依存**（言及 5 箇所は全て §1.1 の経緯記録）⇒ 本裁定は v2.5 を止めない。依存は**後続版**（IN-1 / IN-3）。
