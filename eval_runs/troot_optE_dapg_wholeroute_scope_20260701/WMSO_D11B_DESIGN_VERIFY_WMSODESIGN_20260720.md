...
| **C2** locator（軸B・target-byte）= **rank3 のみ**（原形正）／ association（軸A・hash 値）= rank3+rank4 | 原形 TRUE（軸B）・当時は軸未明示ゆえ曖昧 | ✅ (a) `ProofItem.ref: str` 必須 ✓ (b) TENSOR_BINDING は 4 evidence grade で非空 ✓（UNKNOWN=空は無 proof で整合）(c) `artifact_hash == claim_target_hash` **直接**束縛 = FINAL_ARTIFACT_HASH/REPRODUCED_OUTPUT_HASH の 2 種＝rank3(HBR) ✓。**軸B(target-byte locator = ref が target 実体を解決) は rank3 のみ**（FINAL/REPRODUCED）／**軸A(hash association = ref は manifest/blob を解決し claim_target を列挙) は rank3+rank4**（TRAIN_RUN_MANIFEST 等）。⛔~~私の当初 REFINEMENT「rank4 確立ゆえ rank3 のみ不完全」~~ = **撤回（pN V2-B1）**: 2 軸を混同していた。B1 は軸B ゆえ **原形「rank3 のみ」が正** |
...
| **C4** 両 slot 同型 | 偽（訂正形で成立） | ✅ **原形 FALSE / 訂正形 TRUE** — OFFLINE_REPLAY: TB=not_applicable / NORM=required（他 2 profile は両 required）・proof_policy も EXACT で NORM のみ NORMALIZER_HASH+FINAL_ARTIFACT_HASH。**C4a（schema 層）= 同型**（両 slot とも ArtifactSlot ref無・claim_targets 束縛）/ **C4b（end-to-end）= 非対称**（profile 適用 + proof set: NORM の EXACT に FINAL_ARTIFACT_HASH＝rank4 直接 locator 在・TB 無 — §24 訂正表 V2-B3。⛔「locator gap 構造は同型」を無限定で書くのは誤り） |

**完全性**: custody record 質問（§2）は C1(ref無)+C2(rank3 locator)+C3(slice=SHADOW rank2)+C4(両slot同型)を逐語含む ⇒ **C1-C4 = Rs へ渡した事実主張の完全集合**（漏れ 0）。

**consequence（pQ 主張）評価**: 「C3 未決 ⇒ locator が覆う grade/profile が定まらない + C2 で軸B(locator)=rank3 のみ ⇒ 前提確定まで選択肢集合を確定できない」= **SOUND**。⭐**pQ refined consequence（正）**: **C3 は locator 設計の上流** — OFFLINE_REPLAY が TB を免除するため、C3（slice の profile 決定）が「そもそも TB の locator が slice に必要か」を左右する（3 profile 未決のままでも言える）。⇒ **本 artifact が選択肢を提示しないのは正しい**。⛔~~「私の C2/C3 refinement が consequence を強める」~~ = **撤回**（当該 refinement は pN V2-B1/B2 で誤りと判明・§24 訂正表）。

**NORM の rank4 locator = 測定済（deferred でない・V2-B3 の e2e 非対称 evidence）**: NORM の EXACT proof set は **FINAL_ARTIFACT_HASH（rank4 直接 locator = 軸B）**を含み TB の EXACT は含まない ⇒ **end-to-end で slot 間非対称**（C4b の根拠）。⚠依然 deferred = NORM の**全 grade 別 proof set の網羅測定**（本検証は EXACT のみ確認・rank1/2/3 は未網羅）。
