...
- **seat_seg = route が定める固定段**（C1 = **body30**、`CANONICAL_MOTION_TABLE_V1.md:128` 逐語「body30 が C1 groove 壁内に側方 seated & 保持(Y+0.150)」= STEP 9 secured-predicate 行）。⇒ **recording 不要**（現 `_pin_seat_seg` の recording 依存を route-defined 定数へ）。多 clip は各 clip の route-defined seat 段。
  - ⚠ **本 bullet は RE-SUPERSEDED（2026-07-17 §8.1 = `PIN_D_TRIGGER_CHARTER_VTDESIGN_20260717.md`、pointer 追記 = §8.1-1 の p5 授権により %12）**: 「固定段（body30）」は実測死 — 81-cell の C1 seat は per-cell {25..34}（`PIN_D_MATERIALS_RSTECHLEAD_20260717.md` §2）かつ「body30」は runtime のどの番号空間にも写像を持たない（同 §1）。裁定 = **identity 源は (B) recording 由来のまま + fire 対象 = identity body**（(A) fired-body は照合レグへ降格 — 観測の再死ゆえ）。step-table 自体は不変（呼称の地位確定のみ）。
