...
"""comp3 G1 E-probe: recording-aligned IK CONFIG at the UNCHANGED step-level cadence.

Rs GO 22:30 (E-probe -> D order): the A-probe excluded target-cadence as the mechanism -- the env
batched-IK SOLUTION is the L-side killer. Option E asks: does aligning the IK CONFIGURATION to the
recording's fix L while keeping the production drive structure (1x IK per RL step + 10-frame joint
chord) and production 4-sub?

On-disk config diff (the E delta; empty-diff STOP check passed -- two material differences):
  1. COLLISION objectives: recording solve_ik_dual INCLUDES the dual-arm link-avoid objectives
     (route_executor.py:626-630 -> newton_routing_utils.py:305-321 COLLISION_PAIRS spheres,
     COLLISION_AVOIDANCE default ON); the env batched solver has NONE (newton_route_env.py:879-890
     objectives list = pos L/R + rot L/R + jlimit only).
  2. ITERATIONS: recording IK_ITERATIONS=100 (route_executor.py:456); env IK_ITERATIONS_RL=30
     (newton_skill_env_base.py:99).
  Identical on both sides (kept): pos weight 1.0 / rot weight 0.5 + same Rx(-90) target quat /
  jlimit weight 10.0 / step_size 1.0 / warm-start = previous-solution chain.

Mechanism (probe-level, env files untouched): monkeypatch env._solve_ik_batch (single call site,
newton_route_env.py:1085) to call the RECORDING's own route_executor.solve_ik_dual verbatim (fresh
IKSolver per call, n_problems=1, collision objectives ON, iterations=100) with warmstart_jq = the env's
jq_starts (previous solution, fingers pinned OPEN -- both sides' warm-start discipline). Everything
else -- step_target cadence, 10-frame chord, grip staircase, 4-sub -- is UNCHANGED production. ik_resid
IS a valid tracking metric this run (step targets are driven normally).

Pre-registered interpretation grid (%12 22:30): L recovers -> E PROVEN (trainer = E, near-zero slowdown
candidate; B+D-b to fallback) / L fails -> B+D-b FINAL.
...
for _p in (str(_TIL), str(_TIL / "envs"), str(_TIL / "scripts")):
...
OUT_JSON = _EVAL_DIR / "comp3_g1eprobe_grasplift_result.json"
FRAMES_DIR = _EVAL_DIR / "comp3_g1eprobe_frames"
...
def main():
    print("[EPROBE] building flag-ON + g1_scene_align env (world_count=1, device=cuda:0 as-coded substrate) ...")
    ...
    # E-PROBE swap: env._solve_ik_batch (single call site :1085) -> the recording's solve_ik_dual
    # verbatim (fresh solver, collision objectives ON, iterations=100). Drive cadence/chord UNCHANGED.
    import route_executor as rex_mod

    scene_info = {"fk_model": env._fk_model, "fk_state": env._fk_state}
    n_solves = {"i": 0}

    def eprobe_solve_ik_batch(targets_left_np, targets_right_np, jq_starts_np):
        sol, _cost = rex_mod.solve_ik_dual(
            scene_info,
            np.asarray(targets_left_np[0], dtype=np.float32),
            np.asarray(targets_right_np[0], dtype=np.float32),
            warmstart_jq=jq_starts_np[0],
        )
        n_solves["i"] += 1
        return np.asarray(sol).reshape(1, -1)

    env._solve_ik_batch = eprobe_solve_ik_batch
    import os as _os

    assert _os.environ.get("COLLISION_AVOIDANCE", "1") != "0", "E-probe needs the recording default (ON)"
    print("[EPROBE] recording-aligned IK swap installed (solve_ik_dual: collision obj ON, iters=100)")
    ...
    result["preconditions_loud"] = {
        ...
        "e_probe": "PROBE-LEVEL IK-config alignment: env._solve_ik_batch swapped for the recording's"
        " solve_ik_dual verbatim (collision objectives ON + iterations=100; weights/target/step"
        " identical both sides). Drive cadence (1xIK/step + 10-frame chord) + 4-sub UNCHANGED."
        " ik_resid IS valid this run (normal drive path).",
    }
    ...
    for tag in ("ctx", "zoom"):
        name = {
            "ctx": "comp3_g1eprobe_grasplift_context_20260710.mp4",
            "zoom": "comp3_g1eprobe_grasplift_clawzoom_20260710.mp4",
        }[tag]
        out = VIDEO_DIR / name
        cmd = [
            "ffmpeg",
