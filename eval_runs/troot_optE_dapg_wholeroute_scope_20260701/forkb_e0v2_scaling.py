...
"""fork-B E0v2 -- full fresh re-run under D1 v0.3 sec 7 + the sec 7.1 exact bars (pN CONCUR-WITH-FIXES).
...
OUT = _EVAL / "forkb_e0v2_scaling_result.json"
E0DIR = _EVAL / "forkb_e0v2_runs"
BASE_SEED = 20260716
WARMUP_STEPS = 30
WINDOW_END = 230
...
def selftest_cpuzero():
    """Detector self-test: given an all-zero CPU sample set, the dead-instrument predicate MUST fire (exit 2)."""
    cpu_samples = [0.0] * 12
    dead = bool(cpu_samples and max(cpu_samples) == 0.0)
...
def _run_phase(run_tag, n_procs, n_cores):
    ...
    while any(p.poll() is None for p in procs):
        ...
        samples.append(row)
    out = {"rcs": [p.returncode for p in procs], "final": [], "prov": [], "samples_n": len(samples)}
    for i in range(n_procs):
        base = E0DIR / run_tag / f"proc_{i}"
        out["final"].append(json.loads((base / "status.json").read_text()) if (base / "status.json").exists() else {})
        out["prov"].append(
            json.loads((base / "provenance.json").read_text()) if (base / "provenance.json").exists() else {}
        )
...
def main():
    ...
    r = {
        "MARK": "E0v2 evidence -- D1 v0.3 sec 7 + sec 7.1 exact bars (bank 3a14e38c17/c79d066109); v1 untouched",
        "protocol": {
            "base_seed": BASE_SEED,
    ...
    for tag, n in (("n1_a", 1), ("n1_b", 1), ("n2", 2), ("n4", 4)):
        ...
        if ph["cpu_mean"] and all(c == 0.0 for c in ph["cpu_mean"] if c is not None):
    ...
    # post-run race bracket: re-hash every closure file recorded by any child
    changed = set()
    for tag in ("n1_a", "n1_b", "n2", "n4"):
        for pv in r["phases"][tag]["prov"]:
            for rel, load_sha in (pv.get("loaded_closure_sha256_at_load") or {}).items():
                p = d0cal._REPO / rel
                post = d0cal._sha256_file(p) if p.exists() else "(deleted)"
                if post != load_sha:
                    changed.add(rel)
    r["changed_during_run"] = sorted(changed)
    if changed:
        errors.append(f"RACE: sources changed during the run: {sorted(changed)}")
