...
"""comp3 G1 substep-10 diagnosis: G1-retry config with the drive loop at the RECORDING's contact solve.

Rs GO 15:33 (ISSUE-1 discrimination): the G1 retry (comp3_g1retry, lane-floor-fixed) latched the cage but
the cable paid AXIALLY through it during lift. Commanded geometry is recording-matched (confirmation run),
so the leading remaining delta = the recorded-staircase 10->4 substep contact-solve transfer (plan sec2
ISSUE-1, pre-registered direction-UNKNOWN; grip close is contact/force-limited). This probe re-runs the
IDENTICAL G1-retry configuration with a PROBE-LEVEL override of the RL drive loop's contact subdivision to
the recording's: RL_SIM_SUBSTEPS 4 -> 10 AND RL_SIM_DT DT/4 -> DT/10 (BOTH must move together -- the drive
loop integrates substeps x sim_dt per frame, newton_route_env.py:691-698; patching substeps alone would
integrate 2.5x DT per frame and break the timeline). With the pair patched, each frame is 10 x DT/10 == DT
== the recording runner's SIM_SUBSTEPS=10 @ SIM_DT exactly. The grip staircase mapping cf[t]+sub_i is
UNTOUCHED (sub_i indexes the drive-loop frames, PHYSICS_STEPS_PER_RL=10, not the inner substeps; desk-check
in the dispatch ping). Settle/P0 already run at the default 10-sub (calls without args) -> unchanged.
Production constants (task_config / newton_skill_env_base) are NOT modified: the override monkeypatches the
newton_route_env module globals, which :1129 reads at call time (SRG T.SIM_SUBSTEPS probe-override
precedent). Outputs are comp3_g1sub10_* (run-1 / retry evidence preserved).

Success => the remaining delta is CONFIRMED as the 4-sub contact resolution (substep 4->N campaign-level Rs
material per the SRG banked rule); failure => ISSUE-3 (10-frame interp lift transient) moves up; arm_q
direct drive is the next probe.
...
# PROBE-LEVEL substep override (Rs GO 15:33): drive-loop contact solve at the recording's subdivision.
# BOTH constants move together (frame time must stay DT; see module docstring). :1129 reads these module
# globals at call time -> the patch takes effect for every RL drive frame; settle/P0 use the no-arg
# defaults (already 10-sub) and are unaffected. task_config / base are NOT modified.
...
OUT_JSON = _EVAL_DIR / "comp3_g1sub10_grasplift_result.json"
FRAMES_DIR = _EVAL_DIR / "comp3_g1sub10_frames"
...
def main():
    print("[SUB10] building flag-ON + g1_scene_align env (world_count=1, device=cuda:0 as-coded substrate) ...")
    ...
    result["preconditions_loud"] = {
        ...
        "sub10_override": "PROBE-LEVEL RL_SIM_SUBSTEPS=10 + RL_SIM_DT=DT/10 (recording-matched contact"
        " solve; production constants untouched; grip staircase cf[t]+sub_i mapping unaffected --"
        " desk-checked)",
    }
    ...
    for tag in ("ctx", "zoom"):
        name = {
            "ctx": "comp3_g1sub10_grasplift_context_20260710.mp4",
            "zoom": "comp3_g1sub10_grasplift_clawzoom_20260710.mp4",
        }[tag]
        out = VIDEO_DIR / name
        cmd = [
            "ffmpeg",
