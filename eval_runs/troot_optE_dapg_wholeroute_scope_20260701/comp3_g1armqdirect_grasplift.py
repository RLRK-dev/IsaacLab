...
"""comp3 G1 arm_q-direct diagnosis: drive the arms with the RECORDED arm_q per physics frame.

Rs-approved sequence (loud; %12 dispatch 15:54): the sub10 probe confirmed ISSUE-1 (substep contact
transfer) as material -- R arm fully recovers at 10-sub -- but L still pays axially. This probe isolates
ISSUE-3 (the env drive path: batched-IK solution + 10-frame linear joint interp CHORD vs the recording's
per-frame arm path) at the PRODUCTION substep condition (4-sub; NO substep override -- 10-sub is already
explained by ISSUE-1, so the drive-path contribution must be measured at production contact solve).

Mechanism (probe-level, env files untouched): monkeypatch route_executor.apply_arm_only_write_perworld
(the drive loop's ONLY arm write site, called once per physics frame from _apply_actions_batch; the fn
consumes ONLY the arm cols {0-5,14-19} of jq_interp -- gripper coords stay servo-DYNAMIC) to substitute
the golden recording's arm_q row for the current frame: rec_f = min(frame_counter + 1, n_rec-1) (the
established sub-frame convention, +-1-frame bounded). The env's IK still runs but its output is bypassed
at the write site -> ik_resid in this run is NOT a tracking metric (commanded IK targets are not driven);
recorded loud in the JSON. Grip staircase replay identical (untouched). reset/broadcast paths use the
BROADCAST write fn -> unaffected.

Pre-registered interpretation grid (%12 15:54): L recovers -> ISSUE-3 (drive chord) CONFIRMED for L /
L still fails -> at 4-sub the drive path does not rescue L = ISSUE-1 dominates the L side (raises the
weight of the substep 4->N campaign decision). Video leg watches the L cage FORMATION quality (throat
crotch vs base-seat) in the close-window zoom. Outputs comp3_g1armqdirect_* (evidence preserved).
...
OUT_JSON = _EVAL_DIR / "comp3_g1armqdirect_grasplift_result.json"
FRAMES_DIR = _EVAL_DIR / "comp3_g1armqdirect_frames"
...
def main():
    print("[ARMQD] building flag-ON + g1_scene_align env (world_count=1, device=cuda:0 as-coded substrate) ...")
    ...
    # ARM_Q-DIRECT bypass: substitute the recorded arm_q row at the drive loop's single arm write site.
    ...
    def armq_direct_write(phys_jq, phys_jqd, jq_interp, arm_ow_q_idx, arm_ow_qd_idx):
        rec_f = min(frame_counter["i"] + 1, n_rec - 1)
        frame_counter["i"] += 1
        jq_over = np.asarray(jq_interp).copy()
        jq_over[0, :] = rec_arm_q[rec_f, :28]  # only arm cols {0-5,14-19} are consumed by the write
        return orig_write(phys_jq, phys_jqd, jq_over, arm_ow_q_idx, arm_ow_qd_idx)

    rex_mod.apply_arm_only_write_perworld = armq_direct_write
    print(f"[ARMQD] arm_q-direct bypass installed (n_rec={n_rec}; production 4-sub; IK bypassed at write)")
    ...
    result["preconditions_loud"] = {
        ...
        "armq_direct": "PROBE-LEVEL bypass: recorded arm_q drives the arms per physics frame at the"
        " write site (env IK+interp chord bypassed; production 4-sub; grip staircase untouched)."
        " ik_resid in THIS run is NOT a tracking metric (IK targets not driven) -- ignore it.",
    }
    ...
    for tag in ("ctx", "zoom"):
        name = {
            "ctx": "comp3_g1armqdirect_grasplift_context_20260710.mp4",
            "zoom": "comp3_g1armqdirect_grasplift_clawzoom_20260710.mp4",
        }[tag]
        out = VIDEO_DIR / name
        cmd = [
            "ffmpeg",
