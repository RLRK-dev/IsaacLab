...
- **⭐shadow bar の discharge 形 (ask C1、%12 confirm 依頼):** 正しい aligned HOLD (参照 6 frame 先行) は shadow 交差の ~1 step 前に発火して march を止めるため、**flag ON run では shadow の 15mm 交差は反実仮想**。discharge = (i) live shadow prefix ≡ **banked comp5 div_seg24 系列 EXACT (max diff 0.000000mm / 343 step)** — build が n=1 ground-truth 軌跡を再現 (ii) **banked 系列の交差 = t343 ∈ bar** (iii) aligned 発火の帰属 delta = **−1 step** (予算 ~0.6-0.7 step 系統 + ramp 離散化、within budget)。ruling の意図 (n=1 基準と apples-to-apples) はこの 3 述語で充足 — live 交差そのものを bar にすると「HOLD が正しく働くほど FAIL」の自己矛盾になる。
