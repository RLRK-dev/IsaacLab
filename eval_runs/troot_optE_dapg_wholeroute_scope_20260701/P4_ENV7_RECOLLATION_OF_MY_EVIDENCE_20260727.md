...
⚠⚠ **比べる量を訂正します (w2:p18 `-394`(3) の指摘・私が再計算して確認)**: 関節ごとの `stiffness` は **リンク長が違うと比べられません** — 判定系は 32×30 mm ・現行は 40×15 mm です。⇒ ⭐ **比較可能なのは連続体の曲げ剛性 `EI = K × SEG`** (task_config.py:146 の関係を逆に使う)。⛔ 私が §3-2 初版で出した「6 倍」「2.8 倍」は **長さの違う関節同士の比**でした。

| file | 関節 K | SEG | ⭐ **EI = K×SEG** | ⚠ 私の退役 head-note が言っていた値 |
| --- | --- | --- | --- | --- |
| `ur15_steps.py` / `_c1seat.py` / `_reaim.py` (判定系) | 0.12 | 0.030 | **0.00360** | 0.12 ⇒ ⭐ 正しい |
| `ur15_cell.py` / `ur15_route.py` (柔系) | 0.02 | 0.030 | **0.00060** | ⛔ **0.12 と書いてあった = 誤り** |
| 現行 `ur15_steps_wired.py` | 0.33333 | 0.015 | **0.00500** (= SSOT `:144`) | — |

⇒ ⭐ **正しい倍率**: 現行は判定系の **1.39 倍** ・柔系の **8.33 倍**。
⇒ ⛔ **per-joint K で言うと 2.8 倍 / 16.7 倍**に見えますが、⭐ **これはリンク長を混ぜた数**なので使いません。
