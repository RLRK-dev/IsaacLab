...
104. **m-p18-504 受領（01:4x）= p0 の指摘「推奨が採られると形 v1 (2) の述語が成立しない」は**正しい** — 当卓（形の発行者）が置き換えの文言を出す。⛔ 本 run は未認可・発火には Rs1 の新しい一語が要る・当卓は発火しない**
   - **当卓の独立確認（blob から）**: 3 knob はいずれも受入済 driver blob `84a372439c59` に read site を持つ — `UNWRAP_SOLVE` `:2168`（`near is not None` と連言）・`ARM_PATH` `:2242`（同）・`START_TRIES` `:2615` と `:2653`（既定 `"24"`）。3 件とも `_RM_ENV` `:49-53` に在る ⇒ 設定しても RUN_METRICS に残る。`TRACK_TOL` は**側ごとの導出値**（`:559` = `ARM_CLEARANCE / ARM_REACH[t]`）。起立誤差の印字 `:2785-2790` は **step loop `:3174` より前**。driver に「開始姿勢の後で止まる」knob は**無い**（`SystemExit(0)` は `:1214` と `:3061` の 2 か所で、それぞれ `P4_CLIP_DUMP` と `P4_RELEASE_ONLY` の別 probe）。
   - **形 v3（推奨が採られた場合の新 run の形・当卓の文言・面と順序は p6／採否と発火は Rs1）**:
     - **(N1) 対象** = **新しい 1 run**。⛔ 行 69 は履行済で、これは**その再走ではない**（object 同一・条件 3 点相違）。
     - **(N2) object** = `96e9ece175` の clean 木（driver blob `84a372439c59`・cell_spec blob `6bdf7ea4f9ca`・task_config blob 不変）= **#69 と byte 同一**。mount は C-2 既定（spread 0.280／tilt 20.0／crown r 0.110）。
     - **(N3) v1 (2) の「env knob は全て未設定と印字」を本 run では次の述語に置き換える**（⛔ 置換であって緩和ではない・#69 の形は遡及変更しない）: **設定する key = ちょうど 3 個** `UNWRAP_SOLVE=1`・`ARM_PATH=1`・`START_TRIES=240`／**残り 21 key ＋ `MUJOCO_GL` ＋ `CUDA_VISIBLE_DEVICES` は未設定**・執行前後に 26 行を印字（#69 と同じ形）／**機械照合** = `RUN_METRICS.json` の `ur15_steps.config.env_switches_set`（driver `:112`）が**ちょうどこの 3 対**であること・過不足は B4 返却。**「parameter 変更 0」の意味を明示** = **file は 1 byte も変えない**（object の sha で照合）・差分は**環境変数だけ**で、その差分は RUN_METRICS に残り後から読める。
     - **(N4) carry 非発火**: 3 key は carry trigger ①-⑧（item 97）のいずれでもない ⇒ **R0 targets は不変**（当卓・p0 §8.67・p6 の [DEFER-RECON] が独立に一致）。`GRASP_CENTRE_X`・`WORK_ROW_DY`・mount 系 override は未設定のまま。
     - **(N5) bar は 1 つだけ** = `[steps] STANDING ERROR L` の値が**同じ行に印字される TRACK_TOL 以下**（ramp gate が開く）。⚠ 値を焼き付けない（側ごとの導出値・#69 の印字は 5.14 mrad）。**他は全て報告**（bar なし）。
     - **(N6) 射程の限界**: driver は開始姿勢の後で止まらない ⇒ **run は進めるところまで進む**。起立誤差の印字は step loop より前なので**途中で止まっても bar は読める**。⛔ **本 run は「開始姿勢に到達したか」以外を主張しない** — 把持・route・貫通・工程の成否は bar の外で、近接カメラは現行 cell で **judge-fit でない**（item 100 (d)）。STEP 3 以降に何が起きても**記録のみ**。
     - **(N7) 実行と経路は #69 と同じ**: 執行 1 回・p0・clean archive・stdout/stderr は通常 file へ redirect・**全停止に札**（現行 3 分類＋「その他」は本文に機構を名指す）・産物は sha256 で p0 §8 に commit（run.log は force-add・同名）・動画は `~/Downloads` へ copy し hub が sha を pin・**返却規則 B4**（札／knob 印字／sha pin の欠落＋**env 集合の過不足**）・再走は返却後のみ。
     - **(N8) 不触**: route run (2)・D4′・09-07 WIP・07-29 記録・parameter・mount。
     - **(N9) 権限**: **本 run は未認可**。発火には **Rs1 の新しい一語**が要る。当卓は発火しない・p0 は**形の commit を受領してから**執行する。
     - **(N10) 実務注記（gate でない）**: `START_TRIES` 24 → 240 で開始姿勢の解探索が 10 倍 ⇒ #69 の 483 s より長くなる見込み。
   - m-p4-315 として p18 へ（relay 先 Rs1 = p19・p6・p0・cc p11・⛔ 映像卓 p1D には送らない）。
105. **m-p18-505 受領（01:4x）= p11 §17.35-17.36 @ `d84d88abb0`（489 行・当卓が blob で確認）: カメラ幾何の独立再導出が当卓と一致 ＋ 補完 1 点。当卓の処置 = **補完を採用**（納品物に追記 2 = `be9001f436`・01:44:10 実測）**
   - **採用の理由（当卓が自分で再計算）**: 半 span 44 mm は distance 0.40 m で**半角 6.28°**しか張らず、半画角 22.5° を満たす**最小 distance は 0.106 m**（半画角が覆う横幅は 0.40 m で ±0.166 m）⇒ **要件 (ii)（distance）は現行で既に充足**・候補から外す。⇒ **直すべきは (i) 注視点が両 pinch の中点であること と (iii) 方位の 2 つだけ**。当卓の要件表が 1 つ小さくなった = 設計の自由度が減る方向の訂正で、**推奨（取付は変えない・開始姿勢を直す）は不変**。
   - 同じ driver 行（`:2939`/`:3037`）を 2 卓が読んで同じ読みに着いた。⚠ ただし**同じ source の再導出であって独立計器ではない**（当卓・p11・hub の限定と同旨）。
106. **送信記録（01:44:54）**: item 104（形 v3）＋ item 105（カメラ補完の採用）＋ 二重反映の条件を **m-p4-315** として p18 へ。p18 transcript に record（1 件・01:45:00 実測）。本文 = scratchpad `p4_dispatch/m-p4-315.txt`。
107. **m-p18-507 受領（01:4x）= p11 §17.37 @ `aeed05b4f1`（496 行）の裏取り 2 点。(1) は採用（3 knob は carry の 8 入力を書かない・効くのは選ばれる開始姿勢だけ・⚠ 3 卓とも同じ blob を読んでおり独立計器ではない）。(2) は**読みが 1 点違う** — 当卓が driver の分岐を読み切ったので、形に **(N11)** として足す（根拠つき・p11 の court に差し戻さず当卓が発行する = 形の文言は当卓）**
   - **当卓が読んだ分岐（blob `84a372439c59` `:2330-2352`）**: `cands` = **収束許容を通った候補**（`:2161` 近傍の `pe > 0.002 or re_ > re_max` で `continue`）／`_strict` = そのうち衝突なし／`_fell_back = not _strict`／**`free = _strict or cands`**（`:2347`）／**`if not free: raise RuntimeError("no IK solution for …")`**（`:2349-2350`）。⇒ **clearance/経路で全部棄却されても run は止まらない** — driver は棄却された姿勢の中から選んで続行し、`:2356-2360` に loud な 1 行（`NOT ONE of N candidates cleared … read the next line's 'collision-free' as 'none'`）を印字する。`:2350` の raise は **候補が 1 つも収束許容に入らなかったとき**だけ発火する。
   - **(N11) 停止札と読みの事前宣言（新しい札は作らない = Rs1 の運用）**:
     - (a) **追従 gate の stall**（`:3996`）= item 86 の文の形（「その他」＋機構を名指す）。
     - (b) **`no IK solution`**（`:2350`）= **「controller の不収束」**。理由 = 発火条件が「収束許容を通った候補が 0」であり、§17.4 が「controller の不収束」に当てた R0 計器の主題そのものだから。⚠ **p11 の「その他」とはここだけ読みが違う**（p11 は候補の棄却で尽きた停止と読んだが、棄却で尽きても driver は止まらない ⇒ その停止は起きない）。
     - (c) **候補が全て棄却された場合は停止ではなく「読みの限定」**: `START` の solve で `:2356-2360` の loud 行が出たかどうかを **p0 が §8 に必ず記録する**。出ていたら、たとえ bar（起立誤差 ≤ TRACK_TOL）を満たしても「**clear な開始姿勢に到達した**」とは読めない（選ばれた姿勢が棄却済みだから）。⛔ これは B4 の返却理由ではなく、**bar の意味の限定**。⭐ `ARM_PATH=1` は clearance を厳しくするので、**増えるのは raise ではなく fall-back の方**。その意味で (c) は推奨 run で最も起こりやすい形。
   - m-p4-316 として p18 へ（短文・relay 先 Rs1 = p19・PLAN-KEEPER 1 卓・p0・cc p11）。
108. **送信記録（01:46:47）**: item 107（(N11) = 停止札と読みの事前宣言・p11 との読みの差異 1 点）を **m-p4-316** として p18 へ（relay 先 Rs1 = p19・PLAN-KEEPER 1 卓・p0・cc p11）。p18 transcript に record（1 件・01:46:53 実測）。本文 = scratchpad `p4_dispatch/m-p4-316.txt`（内容は item 107 が正・kickoff 側 commit = `698a593d7d`）。**Rs1 待ち = 1 件（推奨の採否）**・当卓は発火しない・run 0。
109. **m-p18-509 受領（01:4x・訂正・応答不要）= hub 自身の訂正**: m-p18-507 の「+5/−0」は誤りで正しくは **+7/−0**（p11 §17.37 ・489 → 496 行）。当卓が独立に再測 = `git diff --numstat` が **7 0**・親 489 行／子 496 行・`grep -c '^+[^+]'` は **6**（追加行の 1 本が空行）⇒ **同じ commit について 3 つの数が出る場所で、権威は numstat 1 つだけ**という hub の読みに当卓も同意する。結論は不変（3 knob は carry ①-⑧ を書かない・R0 targets 不変・当卓の (N11) も不変）。**当卓の記録は影響なし**（item 107 と m-p4-316 は行数 496 と commit を引いたが numstat は引いていない）。⭐ hub の自己診断「出力を表示させておきながら本文の見た目から数を作った」は、当卓が 09-20 に 2 度踏んだ形（item 78 B の期待値・item 93 の env key）と同型 ⇒ **当卓も「数は出力を貼る・文章から作らない」を運用に加える**（既に item 96 で hub の +18 を +29 と訂正した側だが、当卓の側でも同じ規律を明記する）。本日の hub の誤り 2 件目（1 件目 = pB 追記 4 を +18 と書いた件・当卓 item 96 が訂正）。
...
113. **m-p18-511 受領（01:5x・応答不要）= 当卓の cite 訂正（m-p4-317）が p0 へ relay され、**hub の独立な行表が当卓の確定表と完全一致**（hub は送信直前に blob `84a372439c59` の 10 行を再表示して照合・台帳 §1647）。⇒ **3 卓（当卓・hub・p0）が同じ blob で同じ行を読んでいる**ので、形 v3 の (N11) が指す行はこれで固まった（⚠ 同じ source の一致であって独立計器ではない・当卓の限定は不変）。hub 自身の観察「当卓の記録が訂正不要だったのは、便が名指した行でなく**自分が読んだ行を書いていたから**」= 当卓が item 110/111 で自分に課した規律と同じ。
   - **当卓の現在状態**: **Rs1 待ち = 1 件（推奨の採否）**。当卓から出ている未応答の依頼は 0 件。run 0・解錠なし・発火しない。採用された場合の次 = 形 v3 (N1)-(N11) を計画面に 1 卓だけが記載 → Rs1 の一語 → p0 が執行 1 回 → 着地 relay → 読み（bar は左腕の起立誤差のみ・(N11)(c) の loud 行の有無を併記）→ 当卓の語。
114. **m-p18-512／513 受領（01:5x）= #69 の受入済 log に停止の機構が最初から印字されていた。当卓が blob から独立に読み、p0・p11・hub の数と完全一致。**当卓の処置 = 札の「原因」節に上流を足す（class は不変）・読みの表を (bar, loud) で書き直す・推奨の根拠を訂正**（納品物 追記 3／4）**
   - **当卓の実測（run.log blob `c79fdfd1d410…` を当卓が直読）**: `:51`/`:54`/`:57` = 「start-pose IK L: NOT ONE of **13** candidates cleared」×3／`:60` = **13 solved / 0 collision-free（⛔ 0 -- all 13 put back）**・chosen pos 1.27 mm／`:62`/`:64` = 右腕 **15 solved / 3 collision-free**・棄却は the other arm ×12 が主／`:58` の全文 = 左の棄却内訳 **柱 11**（crown ×5・stem ×4＋2＋1）・**the other arm ×3**・**along the move ×1**／`:50` = settle traverse の arm↔arm **−54.6 mm**（移動中に両腕が深く相互貫入）／loud 行 **全 84**（aim L 65・aim R 16・start-pose L 3・start-pose R 0）。
   - **A. 札の refinement（class「その他」は不変・挿入・p0 §8 の既存行は変えない）**: 札の「原因」節に**上流**を足す — 「**左の開始姿勢は IK は収束したが 13 候補のいずれも clearance と経路を通らず、driver が棄却姿勢を採用した（run.log `:51`/`:54`/`:57`/`:60`）。rest 接触・3 関節の力飽和・ramp gate 不開・STEP 2 stall はその結果**」。⇒ item 86 の裁定（§17.4 の定義で「controller の不収束」でも「calibration 停止」でもない ⇒ その他）は**不変**。⛔ #69 の受入（item 95）も**不変**（射程 = 形の履行と 5 記録の一致）。
   - **B. (N11)(c) の確定文言（p11 の A/B/C を採用・当卓が #69 の位置づけを訂正）**: 読みは **(bar, loud) の対**で 3 分岐 — **A** bar 満たす＋loud 無し = clear な開始姿勢に到達（唯一「直った」と言える形）／**B** bar 満たす＋loud 有り = 棄却姿勢から始めて誤差だけ小さい（(c) の限定・「直った」とは読めない）／**C** bar 満たさない = #69 と同型（到達性か取付）。⚠ **#69 は C かつ loud 有り**（bar 不成立 662.68 対 5.14・左 start-pose の loud 3 回）— p11 の便は B を「#69 の既定」と書いたが bar を満たしていない。**p0 の §8 の必須記録 = ①両側それぞれの loud 行の有無 ②逐語（候補数 N 込み）③直後の collision-free の印字値**（p11 の 3 点をそのまま採用）。
   - **C. 推奨の根拠の訂正（納品物 追記 3・推奨自体は維持）**: 左の開始姿勢を殺しているのは **姿勢での柱との干渉（16 件中 11）**であって経路（1 件）ではない ⇒ **最も効くのは `START_TRIES`**（既定 24 で 13 収束・0 clear。同取付の 240 draw グリッドは **L solved 106 / L free 5** = `SPREAD_TILT_SWEEP_TRIES240_KINONLY.txt:54`）。⚠ `ARM_PATH=1` は棄却を厳しくするので**逆向きに効き得る**（08-02 は別中心で R の clear 4 → 0 を実測）・`UNWRAP_SOLVE=1` はその対。⇒ **3 つは 1 組**。`:50` の −54.6 mm は相手腕の経路検査を入れる独立の理由。
   - **D. 未測定の名指し（p11 (3) と同じ）**: **C-2 既定 cell に左の clear な開始姿勢が存在するか**は未測定。**240 draw は標本を増やすだけで存在証明ではない**。0 clear のままなら cell についての証拠であり、判断は Rs1（グリッドの通る 2 点のうち現行が最良なので、測定済みの取付に逃げ場は無い）。
   - **E. 当卓の失敗（記録）**: 当卓は #69 の run.log を `:41-44`・`:92-112`・`:362-402` と読んだが **`:45-66` を読まなかった**。停止の機構はそこに最初から印字されていた。pB・pZ・pC・hub も読んでいない ⇒ **5 記録の一致は「同じ場所を見た一致」だった**。⭐ **log は問いの在りかでなく log 自身の区画で読む**（driver は label つき block で印字する）。
115. **送信記録（01:55:37）**: item 114（札の refinement・(N11)(c) の確定文言 (bar, loud)・推奨の根拠の訂正・未測定の名指し・当卓の失敗）を **m-p4-318** として p18 へ（relay 先 Rs1 = p19・p0・PLAN-KEEPER 1 卓・cc p11）。p18 transcript に record（1 件・01:55:44 実測）。納品物の最新 = `38e404cf81`（141 行）。**Rs1 待ち = 1 件（推奨の採否）**・当卓は発火しない・run 0・解錠なし。
116. **m-p18-514 受領（01:5x・応答不要）= 当卓 m-p4-318 が p0 へ relay され、hub が (A)-(E) を自分でも実測して確認（run.log の `:50` `:58` `:62` `:93` とグリッド 54 行目 = 106 solved / **5 free** を hub 自身が実読）。⭐ **当卓の訂正（#69 は B でなく C かつ loud 有り）は Rs1 の判断面に挿入された**（hub が原文を残したまま見出し直下に置いた = 読み手が 1 項目しか読まない場合に備えて）。当卓が最も気にしていた点がこれで閉じた。**
   - **hub の自己申告（本日 4 件目・最も重い）**: hub は p11 の「B = #69 の既定」を Rs1 の判断面まで通した。⚠ hub は**同じ台帳の 1 つ前の節に `:93` の 662.68 対 5.14 を自分で記録**しており、**1 文の中で #69 を B と C の両方に置いていた**。訂正は挿入済。
   - **当卓の観察（記録のみ・裁定でない）**: 本日 1 日で同じ形の誤りが **4 卓**に出た — hub 4 件（うち 3 件が同形）・当卓 3 件（行番号）＋ 2 件（期待値・env key）・pB 3 件（cite）・p11 1 件（17.37 (a) 撤回）。**共通の形 = 測定は手元に在り、本文がそれと違う**（数を出力から貼らず、文章から作った）。⇒ 全卓が同じ対策に着いた（**出力をそのまま貼る**・**行は blob に pin**・**識別子に種別語**・**検査の単位は class**）。⚠ これは速度と便数の関数でもある（本日の便数は当卓だけで m-p4-305 から 318）。当卓は以後、1 便に**新しい数を入れるなら、その turn で出力を貼った数だけ**にする。
   - **状態**: **Rs1 待ち = 1 件（推奨の採否）**。当卓から出ている未応答の依頼 0 件。run 0・解錠なし・発火しない。
117. **m-p18-515 受領（01:5x・cc・当卓への必要応答なし）= 役割移管 5 件目 = **ARM-CONTROL-DESIGN `w2:p11` → `w2:p1H`（Codex）**（利用者逐語「herdrにおけるp11の役割を君に移管させたい」）。⚠ 先行 4 件と違い **p11 は稼働中で設計の語を出している最中** ⇒ hub は計画卓の前例（自分で受付終了を宣言して閉じる）を推奨した。当卓の処置 = **当卓から設計 court へ開いている項目を 1 回だけ出す**（引継ぎ書から落ちないように）**
   - **当卓の chain の現状**: 設計 = p11 → **p1H**／build = **p0（不変）**／検証 = pZ → **p1F**／log = pB → **p1E**／視覚 = pC → **p1D**／計画 = p6 → **p1G**／Rs1 = **p19**。⇒ **当卓（p4）と p0 以外は全て Codex 卓**になり、**hub が tool で配達検証できるのは p4 と p0 の 2 卓だけ**。
   - ⭐ **当卓の受入の述語はこの変化に依存しない**: 当卓は**便の数値で裁定せず、commit された object を自分で git から読む**（本日の #69 でも 5 記録を全て blob から再計算した）。⇒ **配達の検証不能は調整の速度に効くが、証拠には効かない**。⚠ ただし**「送った」を「届いた」と読まない**規律は一層効く（当卓 item 103）。
   - **当卓から設計 court へ開いている項目（引継ぎ書に載せるため・依頼ではなく一覧）**: ① **カメラ要件 (i)(iii) の実装**（注視点を両 pinch の中点から把持点か片腕へ／stem・crown と交差しない方位。**(ii) distance 0.40 m は充足済**・納品物 `38e404cf81` §4 と追記 2）— 当卓は source を触らない。② **§17.4 の 4 札化**は **Rs1 へ持つ提案として保持**（当卓は採らない・item 93 D／100 (a)・現行運用 = 3 分類＋「その他」本文に機構を名指す）。③ **§17.37 (a) の撤回**は受領済・**(N11) は当卓が発行**（item 107／114）。④ **#69 の札の原因節の refinement**（上流 = 棄却姿勢の採用）は当卓が発行済 — 設計面で異論があれば後継の語。⑤ **当卓からの未応答依頼 = 0 件**。
   - m-p4-319 として p18 へ（短文・relay 先 p11 と後継 p1H・cc p0・Rs1）。**Rs1 待ちは依然 1 件（推奨の採否）**。
118. **送信記録（01:59:12）**: item 117（設計 court へ開いている 5 項目の一覧）を **m-p4-319** として p18 へ（relay 先 p11 と後継 p1H・cc Rs1・p0）。p18 transcript に record（1 件・01:59:18 実測）。**本日の当卓の便 = m-p4-305 から 319 の 15 通**。**Rs1 待ち = 1 件（推奨の採否）**・当卓からの未応答依頼 0 件・run 0・解錠なし・発火しない。
119. **m-p18-516 受領（01:5x・cc・応答不要）= 当卓 m-p4-319（設計 court へ開いている 5 項目）が **両方の面に届いた**: ① 旧 p11 へ relay（**まだ受付中のうちに** = 引継ぎ書に取り込める時点）② 後継 p1H の共有 file `work/thread-p11-transfer-20260921/P18_TO_P1H.md` に配置（hub の sender が Codex 卓を拒否するため）。⇒ **落ちないようにという当卓の意図は custody として立った**（後で落ちていた場合、両面に届いていたことを示せる）。hub は 5 項目のいずれにも裁定しない。当卓は待ち・run 0・解錠なし・**Rs1 待ち = 1 件（推奨の採否）**。
120. **m-p18-517 / 518 / 519 / 520 受領（02:03-02:10）= ⭐ Rs1 が推奨 A を採用し新規 run 1 回を認可（当卓の Rs1 待ち = 0 件）／当卓の訂正 4 件／hub の未確定 1 件を当卓が実測で閉じる／hub 自身も p1J へ移管**
   - **(A) 裁定（当卓が全文読了・要旨で代えない）**: `work/thread-rs1-20260921/PROPOSAL_A_DISPOSITION.md`・**66 行**・sha256 `9228e03bccfb54c0c0f33344e8b7a3083235fcd851f1af1961c7d5c643908fbe`（当卓が `sha256sum` で実測一致）・**untracked ゆえ as-read**（Rs1 自身が「Git commit とは称さない」と明記）。**推奨 A を採用・`UNWRAP_SOLVE=1 ARM_PATH=1 START_TRIES=240` を一組とする新規 run 1 回を認可・実行は p0・追加の Rs1 一語は不要**。範囲 = `96e9ece175` の clean archive・driver blob `84a372439c…`・C-2 の取付/入力/controller/camera 同一・env 差分は 3 対のみ（残り 21 key と `MUJOCO_GL`・`CUDA_VISIBLE_DEVICES` 未設定）・⛔「設定差分ゼロ」とは書かない・終端または停止まで 1 回・**判定対象を開始姿勢から拡張しない**・B4 返却規則は維持・**返却でも自動再走しない**・追加 run は新しい裁定。⛔ **#69 の受入は閉じたまま**・route run (2)・D4′・WIP・camera 改修・取付変更は本裁定から発火しない。
   - **(B) 当卓の訂正 4 件を納品物へ挿入（追記 5 @ `60169c0a98`・+18/−0・159 行）**: ① **柱の棄却理由は 12**（crown 5＋stem 4＋wrist_2/stem 2＋**wrist_1/stem 1**）／相手腕 3／移動中 1 = **16** — 納品物 `:108` は**四項を列挙しながら和を 11 と書いた**（総計 16 が誤りを決める。11 なら 15）。② **理由の件数であって候補の内訳ではない**（driver `:2362-2364` に `not "poses"` と明記・当卓が blob で実読）。③ **`:50` の −54.6 mm は開始姿勢探索より前の HOME settle traverse** ⇒ 3 設定でその移動の干渉まで解消するとは言わない（順序は log の行順が独立に示す）。④ ⭐ **当卓の bar の境界を訂正**: driver `:2785-2790` は **`_hold_err >= TRACK_TOL` で閉じる**ので**等号は不成立側**。(N11) の「≤」は誤りで、**厳密に小さい**こと・判定は丸め値でなく**同じ行の `gate opens with N%` 節**で読む（裁定 `:43`）。⇒ **当卓の文言と裁定が食い違う箇所は裁定が優先する**（当卓が発行した形 v3・(N11) も同じ扱い）。
   - **(C) 12 への到達は Rs1 便の前**（p11 §17.46 の指摘 → 当卓が `:58` を再読）。⛔ ただし**同じ 1 行を複数卓が数えたことを独立計器の数に数えない**（裁定 `:56` と同旨・当卓 item 114 E の再掲）。
   - **(D) hub の未確定 1 件を当卓が実測で閉じた**: hub は「**m-p18-516（当卓の未了 5 件）が旧 p11 で処理されたかは決まらない**」と書いた（引継ぎ書 `:61` の未処理 0 件の内訳に 516 の名が無いため）。⭐ **決まる。処理されている** — `work/thread-p11-transfer-20260921/P11_HANDOFF_TO_P1H.md` `:30-36` が **§3b「p4 から設計 court へ開いている 5 件（m-p4-319 → m-p18-516・kickoff item 117 @ `3cee58767e`）」**として 5 件を逐条で収録（当卓が直読・untracked ゆえ **as-read**・02:02 mtime）。**hub は `:61` を読んで `:30` を読まなかった** — 当卓 item 114 E と同じ形（**問いの指す場所だけを読み、file を自分の節割りで読まなかった**）。切替の成立点 = 旧 p11 自身の宣言 **02:00:56**、当卓の m-p4-319 の到達 = **01:59:50** ⇒ **受付中に届いた**は成立（item 119 の記述はこの点で保たれ、さらに「届いた」から「引継ぎ書に載った」へ強まる）。
   - **(E) 517 が当卓に預けた「規則にするか」の問い — 当卓の語**: 3 卓が独立に名指した対処（他卓の便を自分の記録に通してから通す）は、**既に commit された規則として存在する**（`CLAUDE.md:247`「⛔ **他 pane の message の数値で裁定しない**（artifact を自分で見てから裁定する）」・当卓が on-disk を実読）。⛔ **足りなかったのは規則ではなく、その規則の対象**: 面から面へ渡ったのは**数値ではなく分類**（「B = #69 の既定」）で、条文の目的語（数値）に掛からなかった。⇒ **当卓は自卓の運用として、数値に限らず「他卓の便に含まれる artifact についての主張」全般に同じ検査を掛ける**（分類・札・件数・順序を含む）。⛔ **新しい規則は起票しない**（CLAUDE.md の改訂は Rs 専権・L3）。Rs1 が常設規則にするなら最小の変更は目的語を「他卓の便の主張」に広げることだが、**採否は Rs1 であり、当卓から 2 件目の判断待ちを積まない**。
   - **(F) hub 自身の移管**: `w2:p18` は **02:10:12 に受付終了**、以後の hub 宛は **`w2:p1J`**（Codex）。引継ぎ書 64 行・sha256 `50804608bc…09a3c442`。⭐ **進行中の 1 件（p0 の実行報告を Rs1 宛 file へ追記する債務）は §1 で明示的に p1J へ渡された** ⇒ **p0 は予定どおり 1 回執行してよい**。⚠ hub の未確定 = 518 の当卓 leg が `ABSORBED(unacked)`（当卓は本項で受領を記録する）。
   - **(G) 当卓の次**: **発火しない**。p0 の着地後、**裁定 `:42-48` の固定された読み**（左右それぞれの START loud の有無・逐語・候補数、最終 solved/collision-free、STANDING ERROR を一組で／aim の loud と混ぜない／必要行が無ければ**判定不能**として記録し成立扱いにしない）で (bar, loud) を読み、A/B/C を当卓の語として出す。**左だけの A を両腕 controller の動的成功にしない**・把持/工程/route の成功や衝突ゼロの受入は本 run から出さない・**V12 の独立 review / blind review / judge-fit は維持**。⛔ 映像卓 p1D へ数値を送らない。
   - **状態**: **Rs1 待ち = 0 件**（推奨の採否は本裁定で解消）・当卓からの未応答依頼 0 件・**run は p0 が 1 回**（当卓は実行しない）・解錠は裁定の範囲のみ・source 編集なし・parameter 不触。
