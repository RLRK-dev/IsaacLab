...
⭐ **たわみの桁（⚠ 見積り・pairing 未確定ゆえ確定値でない）**: `13.6 mrad × 0.649 m/rad ≈ 8.8 mm` / `54.4 mrad × 0.224 m/rad ≈ 12.2 mm`。⇒ **どちらの組み合わせでも 2 mm 閾値の 4〜6 倍**。
⛔ **確定値として使わない理由 2 つ**: ①**どの関節が 27.22 を負うか未測**（H-3.1 待ち）②**実際は 6 関節の寄与の和**（符号によって増減する）。⇒ **H-3.1 の per-joint 値と J の per-joint 列を対応付けて初めて確定する。**
⇒ ⭐ **設計上の含意（暫定）**: `ke` を **5 倍程度**上げる必要が出る公算。⇒ §5.5.B の予算（`ζ ≥ 1` を保つ `β·α < 8.16`）と**同じ桁で衝突する** ⇒ **「`ke` だけ上げる」枝は余裕を食い潰す**可能性が高く、**「`ke` と `kd` を同時に上げる」枝**が有力（`T_lag = 0.2 s` 不変・`ζ` は改善）。⛔ **H-3.1 が出るまで採らない。**
...
⚠ **UNDEFINED TERM: 「W-b」** — 本 project 内に**定義 artifact が見当たらない**（p11 実測: `ARM_CONTROL_PD1_PREREG_V14_WB_DRAFT_…:12` は「振付＝再工事 W-b」と**参照**するが、W-b 自体の定義・waypoint 集合・owner を定めた doc は特定できていない）。⇒ **(3) は「定義と owner が確定するまで着手不能」**。⛔ 私は W-b を定義しない（振付の owner 課題）。
...
### 5.5.2 ✅ **解決済**（`908ac46745`・2026-07-26）— H-4 が ABSENT だった件

⭐ 下記の指定どおり **SSOT index で解決**され、3 参照点が実測された（結果 = §5.5.C-2）。p0 逐語 `resolution` = 「**SSOT index (stride 14, EE +5, pads +9/+13) — NOT name search**」。⚠ 記録として以下を残す（同型の再発防止）。
