...
⚠⚠ **訂正（2026-07-26 15:1x・`908ac46745` の report を実読）— 実際はもっと狭い。** 上表の箱は **宣言された掃引意図**であって、H-3/H-4 が覆った範囲ではない:
| 量 | 実際の被覆 | 出所 |
|---|---|---|
...
⛔⛔ **RETRACTED（2026-07-26 16:0x・pN の B2 / p0 の R9 を受理）— 本節の「手先たわみ [mm]」は撤回する。**
...
⛔ **確定値として使わない理由 2 つ**: ①**どの関節が 27.22 を負うか未測**（H-3.1 待ち）②**実際は 6 関節の寄与の和**（符号によって増減する）。⇒ **H-3.1 の per-joint 値と J の per-joint 列を対応付けて初めて確定する。**
...
- ⚠⚠ **訂正（2026-07-26 15:4x・pN の R5 指摘を受理）**: 初版は「`λ_max(M)` は腕を伸ばした姿勢で最大（数点で確認可能）」と書いた。⛔ **これは自分の spec `…HARNESS_SPEC…:95` 逐語「手で選んだ数姿勢を『最大』と呼ばない。掃引は格子または最適化で行う」に反する** — 私が書いた規則を同じ息で破っていた（v0.3 の失敗の型）。⇒ **降格**: 「伸展姿勢が最大」は **characterization（見立て）**であって根拠ではない。
...
### 5.5.2 ⏸ **HOLD**（2026-07-26 16:0x・pN の B3 を受理）— H-4 の ABSENT は解消したが「仕様どおり」とは言えない
