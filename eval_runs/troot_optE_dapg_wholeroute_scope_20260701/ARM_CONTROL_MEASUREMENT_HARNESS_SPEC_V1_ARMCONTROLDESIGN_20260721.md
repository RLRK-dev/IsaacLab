# 腕制御 測定ハーネス 仕様 v1.9 — p0 実装 / pZ 検証（p11 ARM-CONTROL-DESIGN, 起草 2026-07-21）
...
- ⚠ **把持状態の宣言**: cable を掴んだ状態を含むか否かを、出力に必ず明記する。
  ⛔⛔ **HOLD（pN **B16**・2026-07-26）**: 旧文「含まないなら**慣性は過小側** ＝ ζ は**楽観側**」は **無条件の方向の断定ゆえ HOLD**。⇒ ⭐ **現在の state = 「出力値は free-arm（＝宣言した測定状態）のもの。cable を掴んだ状態が effective な `M` / `c` / `K` および `ζ` に与える方向・量は UNVERIFIED」。**
  ⚠ **解析式は「仮定つきの条件付き注記」としてのみ保持する**（⛔ 無条件の結論として使わない）: 私の下界 `ζ_min ≥ (c/2)·√(λ_min(K_e)/λ_max(M))` が「掴むと `ζ` の下界が下がる」を与えるのは、**①減衰と剛性が不変 ②付加負荷が effective `M` に半正定値として加算され `λ_max(M)` が単調増加する** 等の仮定の下だけ。⛔ **たわむ cable ＋ 接触ではこれらの仮定を私は検証していない**（接触を介した結合は `c` や `K` も変え得る）。
...
- ⚠ 把持中の cable 慣性は `M` に入らない。⛔⛔ **訂正 v1.9（pN **B26**・B16 の伝播完了）**: 旧文の **「⇒ ζ は楽観側」は無条件の方向の断定ゆえ撤回**。⇒ **出力に明記するのは「値は free-arm（宣言した測定状態）のもの」まで**。**掴んだたわむ cable ＋ 接触の下で `ζ` がどちらへどれだけ動くかは UNVERIFIED**（§2 の把持状態宣言と同旨）。
...
- ⇒ これと **H-4 の evidence-grade Jacobian**（**将来 authority が確定する success-evaluation point** のもの・**方向つき 3 成分 ＋ 認可 envelope**）を合成して初めて **手先のたわみ [mm]** が出る。⛔⛔ **訂正（pN **B18**）**: 旧文は **`J-a` を active に固定していた**ので撤回 — **`J-a` は conditional**（authority が確定する点が `J-a` だった場合）。⛔ **proxy（縮約 ＋ 1 姿勢）から合成しない**（**B15**）。
...
- ⛔ `pinch` site も**単独では不可** — `collapse_fixed_joints` で `wrist_3_link` に剛体固定されており、**8 本の gripper DOF の列がゼロ**になる（v0.4 ISSUE 8）。⛔ **v1.9 訂正（B30・AC-5 と同一境界）**: 旧文「使う場合は**その旨と誤差の向き**を明記」のうち、**「誤差の向き」の要求は撤回**。⇒ **報告するのは「gripper DOF の寄与を含むか否か」（＝包含の有無）**。**向きは、それを示す証拠が無い限り UNVERIFIED と書く**（⛔ 過小/過大を断定させない）。
...
⇒ ⭐ **同じ「指先」という語が 2 つの点を指し、pin された参照オフセットどうしの差は 34.8〜55.7 mm。** ⇒ **各参照点で **evidence-grade の Jacobian**（＝**方向つき 3 成分 ＋ 認可 envelope**）**を測る必要がある**（⭐**B15**: **proxy = 現行 H-4 の縮約 ＋ 1 姿勢の値は測定済**。⛔ そこから bar を導かない）。**
...
| **J-c** | `EE_TO_PINCH_TIP_CLOSED = 0.2757` 相当の爪先（**ラベルの付いた参照点**・`task_config.py:321` のラベルは「pad TIP drop; コ f1ext claw tip」） | ⛔ **v1.8 訂正（**B21**）**: 旧「**J-a と J-b の差を定量化して報告するため**」は **J-c 自身の用途になっていなかった**ので撤回。⇒ **J-c は 3 つ目のラベル付き参照点**であり、要る理由は **J-c と J-a / J-b を proxy として並べて比較するため** |
...
H-2 の `ζ = 2.857` は **`M` に cable 慣性を含まない「自由腕」の値**（cable は接触結合ゆえ `M` に現れない）。⛔⛔ **訂正 v1.9（B26）**: 旧文 **「⇒ 把持中は実効慣性が増え ζ は下がる（楽観側）」は無条件の断定ゆえ撤回** — **掴んだたわむ cable ＋ 接触の下では effective `M` / `c` / `K` と `ζ` の方向・量は UNVERIFIED**（B16）。⛔ 自由腕の値を把持 phase に流用できない。
...
- ⛔⛔ **v1.9 訂正（B26）**: 旧「**判定に使う形**」として置いていた **8.16 倍の枝判定は、task acceptance にも枝の決定にも使わない。** ⇒ **未検証仮定つきの diagnostic としてのみ保持**（仮定 = **`c` / `K` 不変・付加負荷が effective `M` に PSD 加算され `λ_max(M)` 単調増加**。**たわむ cable ＋ 接触では未検証**）。⭐ **保持するのは直接観測のほう** — 上の **phase 別 step-response から読む `ζ_grasped`**（⛔ 方向を仮定せず、観測された行き過ぎ量から読む）。
  📎 **条件付き diagnostic（上記仮定の下でのみ）**: `ζ ∝ M^(−1/2)` より、把持で ζ が 1 を割るには **実効慣性が `2.857² ≈ 8.16 倍`** に増える必要がある。⇒ 参考として「8.16 倍を超えたか否か」を併記してよいが、⛔ **これで枝を決めない**（旧文は決めていた）。⛔ **削除（B29）**: この直後に残っていた旧括弧「（設計側の枝がこれで決まる）」は、**「決めない」と書いた同じ行で決めており矛盾**していたので**削除**。
...
| AC-5 | Jacobian の参照 frame が**明記**され、**gripper DOF の寄与が含まれるか否か（＝包含の有無）**が述べられている。⛔ **v1.9 訂正（B26 / B10）**: 旧文が同じ項目で要求していた **「誤差の向き」は分離する** — **包含の有無は出力から言えるが、task に効く感度に対して過小か過大かの向きは UNVERIFIED**。⇒ **向きを述べるなら「UNVERIFIED」と書くこと。⛔ 過小/過大を断定させない。** |
