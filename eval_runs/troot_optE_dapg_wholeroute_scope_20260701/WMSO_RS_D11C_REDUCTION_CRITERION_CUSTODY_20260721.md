...
| U-4 | 2026-07-21 **15:3x / 17:3x** | **推奨で良い**（2 回・cycle-3 実施と routing 方針に対する承認） |
...
## 6. 追補（cycle-3 G-8）— cycle-3 実施の authority

cycle-2 判定記録は「skill の max-2-cycles に到達 ⇒ **cycle-3 の要否は Rs 裁量**（自己起動しない）」と定めた。**cycle-3 は Rs 逐語「推奨で良い」→「susumete」で承認された**（U-4）。⚠ v2.2 起草時にこの記録を作っていなかったのは **cycle-2 F-10 と同型の再発**（Rs 発話に依拠する権威を、同一ターンで記録しなかった）。本追補で解消する。
⚠ U-4 は **手順の承認**であって、routing (a)-(d) の**中身の裁定ではない**。
