# W0-e: seat/guide offset-follow 修正 spec v0.8 (Rs「A」15:1x 授権 — locked file 編集承認)
...
- **F-1b′ (Y 位相退避、経験則 = 座屈 mode の trigger 回避):** **operand [v0.8 分割]: dy := config `CABLE_XY_OFFSET[1]` [mm]、config 導出のみ・node 状態読取禁止** (fix-⑤ :3950 と同 gating 系; state 位相は未較正 + bright-line 抵触)。**trigger-B1 (phase-periodic): floor_mod(dy,15) ∈ [2.5,6.5]** (Python `%`; −10→5。grid 3 周期 corroborate: {−10,+5,+20} 行 27/27 fail) → **退避 δ_target=7.5** (最短符号方向)。**trigger-B2 (absolute、P-1 で位相周期性 REFUTED): dy ∈ [11.5, 13.25 暫定 — P-4/P-5 で縁 pin、P-6 で負側 mirror 判定]** → **退避 dy_target=10.0** (下方、phase-10 帯 = grid 全 dx 27/27 実証最厚 zone; 上方 13.5/14 は dx≤−10 挙動未証で不採用)。**\|Δy\| ≤ 7.5mm (H2)、実必要 ≤5.0** (B1 max +5 / B2 max −3)。適用点 = 降下前・壁上端より上で完了。**achieved-phase log (退避後 pre-seat の nearest-node Δy) + \|achieved−commanded\| < 1mm を RUN-2c 受入に**。評価注記: 87/87 走査は **in-sample fit** (規則を構築した点集合上の走査)、out-of-sample = dy10.5 の 1 cell → RUN-2c/2d の数値受入で介入を初実行。B1 margin = 区間 [1.5,2.5]mm (単点 6.0 O は n=1)。帯定数の provenance = cuda:0 + 本 build sha (PREREG に stamp、migration 時は {4.5,7.5,12.5} spot-check)。**scope 条項: F-1b は帯 trigger の回避のみ — F-1a と AND。F-1b の必要性自体を RUN-2c (F-1a 単独 → +F-1b) で帰属** (F-1a が斜行類も救う場合、F-1b は conservative 退避として維持されるが機構主張はしない)。
...
0. **pre-build probe (build 前、既存 runner):** ✅**P-1 dy=−2.5 = PASS 828.6 (falsifier 発火 → B2 absolute 化)** / ✅**P-2 dy=13.5 = PASS 829.0・P-3 dy=14.0 = PASS 829.0 (UB<13.5)** / ✅**reach probe = ALL-CLEAR (C1 4隅 + C2 x±22、全 8 target 残差 0.00mm、route-faithful solve_ik_dual)** [CC3-CH6] / **P-4 dy=11.5・P-5 dy=13.0 (B2 縁 pin)・P-6 dy=−12.5 (負側 mirror) → 完了後 build 開始**。単発 run で可 (cuda:0 同 device 決定論 banked: r-v1 7/7 + sha epoch EXACT)。
