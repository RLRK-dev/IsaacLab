...
### F-9.7 追補 — leg 3: 陽性対照、および bar の**導出** (2026-07-14 07:0x、%12。script = `f9_positive_control_seat_predicate.py`)

F-9 本文の根拠は **(i) 機構** (自由軸 dy の量子化床 7.32mm > bar 3.0mm) と **(ii) 24/81 recount** の 2 本だった。本追補は **第 3 の独立レグ**を足し、かつ **F-9.5(1) の bar 根拠を 1 点訂正する**。

#### (a) 陽性対照 — 「物理的に着座している」既知集合に env の式を当てる

対照集合 = **producer が着座と判定した 58 cell** (`cable_c1_nonpin_final_mm < 0` ∧ `c2_seated_honest`)。producer は `mj_geomDistance` = **連続な面間距離、節点量子化なし・weld 除外**。正常な計器なら **58/58 発火**する。

| env 述語 | file:line | 発火 | 本来 |
|---|---|---|---|
| `p3` (G3 latch, C1) | `:1549` | **17/58 (29%)** | 58/58 |
| `p5` (G5 latch, C2) | `:1551` | **19/58 (33%)** | 58/58 |
| `_c2_seated_honest` (G6 連言項) | `:1312-1319` | **25/58 (43%)** | 58/58 |

分解 (なぜそうなるか):

| 軸 | C1 | C2 | 意味 |
|---|---|---|---|
| \|dy\| (**自由軸・指示対象なし**) | med **3.83** / max **7.38** | — | **距離を支配。** 予測した量子化床 ±7.32mm と一致 |
| \|dx\| (**致命軸**) | med 0.94 / max **1.94** | med 0.21 / max **0.37** | 実際の着座品質。**極めて良好** |

⇒ **F-9 の機構仮説は陽性対照で確認された。** env の式は、物理的に正しい着座の **57-71% を棄却する**。
⚠ **本レグが試すのは「式」であって「build」ではない** (`_seat_metrics` は節点座標の純関数ゆえ recording 上で評価可能)。別 build は節点を動かすが **40 節点/15mm の離散化は同じ** ⇒ 量子化床は不変 ⇒ **欠陥は build 非依存。**

#### (b) ⭐訂正 — F-9.5(1) の bar 根拠は「較正」であって「導出」ではなかった (%12 own)

F-9.5(1) は「**bar 3.0mm 据置** (実測 |dx| max 1.94mm ⇒ 3mm は妥当)」と書いた。これは **データに対する較正**であって**拘束からの導出ではない** — **F-9 自身が弾劾している手つき**である (定数は built model から読め)。

**溝の幾何が、データを一切使わずに bar を決める:**

```
groove 内半径   6mm   (create_clip.py:48 GROOVE_INNER_D = 0.012 → :72 groove_r = D/2)
cable 半径      4mm   (task_config.py:137 CABLE_RADIUS = 0.004)
─────────────────────
横方向クリアランス 2.0mm  = 着座した cable が取り得る |dx| の上界 (剛体束縛)
```

⇒ 実測 max |dx| = **1.94mm** は、bar の**出所ではなく独立な検算**になる。**CAD 断面と 58 本の実測軌道が 0.06mm で一致** — これが修正計器の陽性対照である。

**値について (conservatism 方向):** 2.0mm は**剛体**上界。MuJoCo の soft contact は荷重下の cable を壁へめり込ませる (producer の wall 距離は **−1.12mm** まで負) ⇒ **bar を 2.0mm ちょうどに置くと真に着座した cable を false-FAIL し得る**。
⇒ **推奨: 値 3.0mm は据置、根拠を差し替える。**

> **3.0mm = 2.0mm 幾何クリアランス + 1.0mm solver 貫入許容** — 「`T_GROOVE` の再利用」ではない。

同じ数値・挙動不変・**bar は一切緩めない** (§運用15 conservatism 方向を満たす) が、定数が **借用から導出へ**変わる。⚠ `T_GROOVE = groove_r/2` は **seated-position 定数**であって**横方向クリアランス定数ではない** — 3.0mm の従来の唯一の出所がこれだった (= cross-axis 定数流用、F-9 が指弾する class そのもの)。

#### (c) 新規 WARN — shipped の C2 z 帯は余裕が薄い

`C2_SETTLE_Z_TOL_MM = 3.0` に対し、**golden 自身の最終フレームで |z_gap| max = 2.74mm = 予算の 91% (余裕 0.26mm)**。
⇒ env は**別 build** であり同じ bar を通さねばならない。**margin review 対象**として起票する (F-9 の量子化欠陥とは別件・独立)。

#### (d) 本追補が変えないこと

- **0.716 (58/81) は生存** — F-9.3 のまま。producer の計器は量子化フリーで、本追補の |dx| (max 1.94mm ≪ 溝) がそれを再確認した。
- **B3b-B7 = STOP 継続。Rs 裁定待ち。実装しない。** 成功条件の変更 = 直交 DESIGN-GATE + spec = Rs 専権。

*%12 — 2026-07-14 07:0x。leg 3 = `f9_positive_control_seat_predicate.py` (再現可能、読んだ artifact の絶対パスを印字する)。*
