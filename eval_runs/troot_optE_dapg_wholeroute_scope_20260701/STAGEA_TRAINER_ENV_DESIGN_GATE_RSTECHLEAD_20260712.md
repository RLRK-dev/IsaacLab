...
> ## 🔒⭐⭐⭐ §20-Z. **SUPERSEDING RULING (RS-TECH-LEAD %12、2026-07-14 19:3x) — 本節の *警報* は前提が偽だった。Rs 動画 GT で解除する。**
>
> **Rs 逐語 (2026-07-14、`~/Downloads/BLIND_DO_NOT_JUDGE__p1b_c1_xsec_3a717010.mp4` = cell `2037_x-20_y-15_Fon` の C1 断面動画を直接見て、私に対して):**
> > 「これは**ケーブルが C1 の溝に入っているし C1 の底にもついているから見た目上は ok**。**C2 へのケーブル誘導、押し込みもできている**。ここにもどてくれ」
> 同日 17:13「**C1のケーブル固定はもんだいない**」/ 18:30「**おれがうまくいっているといっただろうが**」/ 18:33「『溝は試されていない』**溝をためすってなんだよ？**」
>
> ⇒ ⛔⛔⛔ **§20.1 の negative control 前提「Rs が目視で却下した cell (`2037_x-20_y-15_Fon` =「cable off C1」)」は【偽】である。** Rs が実際に判定したのは **その同じ cell** であり、判定は **OK (着座している)**。
> ⇒ ⛔ **∴ 以下は全て土台を失う (撤回):**
> - 「**全数値が pass と読むのに Rs の目は落とした ⇒ lateral groove-capture を測れる numeric は存在しない**」(07-12 banked、`harness/state/ANCHOR_STEPTABLE_ALIGNMENT_p4p5_20260712.md:101-107`) ⇒ **前提が逆。numeric と human は *一致* していた。不一致は一度も観測されていない。**
> - 「\|dx\| 計器は Rs 却下 cell を弁別できない ⇒ 計器失格」⇒ **却下 cell が存在しない ⇒ 判定不能ではなく、*正しく PASS していた*。**
> - 「**溝の保持力は一度も試されていない**」(C1 収容計器 spec v0.9-v0.11 の headline) ⇒ **Rs が 18:33 に明示的に否定。**
>
...
> ⇒ 🔒 **campaign 前の修正対象は 上表 3 件のみ。計器の新規研究 program は不要。**
> ⇒ ⭐ **Rs が指定した次の作業 (17:18 逐語)**:「**ピンが打たれる前に『本当に溝に居るか』を確かめる」これはあたりまえだ**」= **seat-trigger** (canonical 工程表 STEP 9 = *側方 seated* を確認してから C1 クランプ。現行実装は frame counter で打っている)。
>
> ⚠ **以下の §20.0 / §20.1 は歴史として残す (削除しない) が、§20.1 の「Rs 却下」を前提とする主張は本 §20-Z が supersede する。**
