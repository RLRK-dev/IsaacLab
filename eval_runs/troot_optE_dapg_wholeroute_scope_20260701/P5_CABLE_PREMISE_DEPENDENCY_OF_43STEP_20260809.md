...
## 4b. 追記 2026-08-09 17:58 JST — **§3(c) の「二面の原因」に custody の順序が付いた（(c) は測定で解散）**（挿入のみ・m-p18-255 を受け、pin 2 本は自卓で再現してから書いた）

§3(c) の記述「**同じ quarantine に、面ごとに別の原因が書いてある**」は**面の記述として真のまま**。ただし **2 つの原因は対等ではないことが測定された** — 読み手が「原因は 2 説ある」とだけ持ち出さないよう、順序を本節で運ぶ:

| 面 | 初出（自卓で再現した pin） | 位置づけ |
|---|---|---|
| **LEDGER の機構原因**（spring-follow + kinematic hold） | **`e05efab04a` 2026-07-02 07:14**（tracking 初日の blob `:42` に既在・pickaxe はこの導入 commit のみを返す = **一度も消えていない**） | **decision-of-record**（記録上の原因） |
| **spec の 1-DOF 帰属**（`:31`） | **`4de2c9fd42` 2026-07-15 02:09**（1 file・**+4/−0** — **13 日後**に invariant 5 の正当化として追加） | **後付けの gloss**（quarantine の決定記録ではない） |

- ⇒ **(c) は Rs の裁定でなく測定で解散**（Rs 逐語の趣旨 = 「隔離したのは私ではないから、理由を私に聞いても何も得られない」— 原因は記録の中に在った）。**修理 = spec `:31` の cite を LEDGER の機構行へ repoint する 1 行**（p11 の統合 draft に同乗・04-Specs は着地承認まで read-only）。
- ⭐ **cite は内容で**: 当該文は初日 `:42` → 本日 `:63` に移動している。**行番号でなく「fidelity-QUARANTINED の文」で指す**（本 file 冒頭方針と同じ）。
- ⛔ **変わらないもの**: 物理的な真の原因は**誰も主張していない**（未測のまま）／quarantine 自体は**解除されていない**／§3(c) の 43-step 面の帰結（z のみ述語の非保守性）と「面を言わずに一文で書くと偽になる」は**そのまま有効**。
