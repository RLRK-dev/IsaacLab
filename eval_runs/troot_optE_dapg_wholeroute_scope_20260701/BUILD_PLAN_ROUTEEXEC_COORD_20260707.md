...
- **⭐%12 06:13 VERIFIED CORRECT + 承認**: UR5e 6-DOF ゆえ per-arm 14 = arm{0-5}+gripper{6-13}、2-arm で **`_ARM_OVERWRITE_IDX=[0,1,2,3,4,5,14,15,16,17,18,19]`** (arm 12 coords)、gripper={6-13,20-27}(16)。build で `_ARM_OVERWRITE_IDX` を module 定義し :738 exclusion (`phys_jq[jq0 + _ARM_OVERWRITE_IDX]`) + grip cmd per-world qd (`joint_target_pos[_arm_qd_start[w] + drivers]`) に使用せよ。
...
- **⭐%12 06:13 posture SET = measure-first at 4-substep**: grip verdict を **4-substep landing-control probe に gate**。**substep decouple (4→10) = deferred evidence-gated fallback** (4-substep probe が grip drop/slip を示した時のみ escalate) — 理由 (a) env physics を無証拠で preemptive 変更しない (2.5× throughput + env-core 変更 cost) (b) 実 deployment regime (4) で測る (c) 失敗実証時のみ二度目 churn。**Rs-approval (WARN build-approval + SRG measured-grip verdict + substep decouple) = build-launch gate (production-launch-gate) へ defer、5体 を block しない** (§運用2 HIGH-COST-GATE = 5体+RULE-CHECK 後)。%12 が flag-flip disposition の efficacy-under-flag (proven=10-substep) を own、SRG gate 機能 confirm。

### §11.3 predicate re-validation under void (Issue 2, HIGH)
grasp_actuation=True の void は cable rest-Z under C1 を変える (cable center = CLIP1_Y=0.150 = void center:1520、middle ~120mm が void span unsupported) → C1-retention (z<0.840 `_c1_retention_m`:762-778) / seat metrics (:780-791) / `_cable_z_rest`:402 が void-substrate で異なる。⚠ **§運用28 reconcile**: monolith 0.716 は **void 上** (build_scene gA=True, test:3715) + **同 predicate** で earn ゆえ predicate は **void-calibrated**; env-core SOLID ⑨a′ 25/81 は **offline proxy = 別 substrate**。→ fold: 「Layer-B re-BASELINE」を sharpen = **void substrate = PROVEN route substrate (0.716)**、B⑨a′(solid) は **OFF-path のみ** guard。Layer B が void predicate を live 実測、Layer A byte-repro が monolith void route との一致で担保。「env-core solid-table 認定が void に transfer」を assume しない。 **⭐%12 06:13**: I2 void-predicate re-val = **Layer B ⑨b online numerator に内包** (void = on-path 実測、B⑨a′ solid = off-path guard のみ)。redesign 不要。
...
## §12. v1.7 fold — 5体 round3 DECIDE=FAIL fold (⭐governing; %12 rulings 07:23; F1-F15)

**DECIDE = FAIL (accepted CRITICAL F1 + 5 HIGH); gate が write-site exclusion gap を code 前捕捉 (v1.5 inert 再発防止)。全 foldable、mechanism redesign 無。§12 が affected items を governs、superseded 先行 spec は pointer。ledger = scratchpad/verify5_round3_DECIDE.md。**

### §12.1 F1+F2+F10 = write-site exclusion STRUCTURAL FIX via AR-reuse (SUPERSEDES §10.1/§11.1 exclusion spec)
**F1 CONFIRMED (%12 3-site baseline)**: 全 gripper-repin site (phys_jq/phys_jqd arm+gripper slice write) = `_broadcast_arm_jointq`:387-388 (callers :396/:470/:485) + reset :617-618 + RL-drive :738-739。v1.5/v1.6 は :738 のみ scope → :387/:617 で gripper OPEN 再pin = **v1.5 inert 再発**。
**FIX = wholesale REUSE AR L3-landed 構造 pattern (§運用28 verified `newton_aerial_regrasp_mujoco_env.py`)**:
- `_GRIPPER_COORDS_LOCAL`:200 = {6-13,20-27} programmatic (SSOT test:1622-28) / `_ARM_OVERWRITE_LOCAL`:201 = {0-5,14-19} (complement、hardcode でない)
- per-world driver dofs `_l_driver_dofs`/`_r_driver_dofs`:421-423 = `_arm_qd_start[w]+driver`
- vectorized arm-only re-pose maps `_arm_ow_q_idx`/`_arm_ow_qd_idx`/`_arm_ow_src`:426-433 (gripper {6-13,20-27} を全 write から除外)
- fail-loud servo-seed assert:435-441 (各 driver DOF が build-time OPEN target を持つか、wrong-offset silent 失敗を捕捉)
- write pattern:475 `phys_jq[_arm_ow_q_idx]=fk_jq[_arm_ow_src]` (+ qd analog) = arm-only、gripper untouched
- grip cadence primitive `_set_gripper_target(_all_l_driver_dofs, ctgt)`:739/747/809 (cage/ramp/close)
**route の :387/:617/:738 全3 site に AR arm-ow map pattern 適用 (q AND qd)** = 構造的解決 (write-site-gap 再発根絶、3-round churn antidote)。§10.1「全16除外」+ §11.1 per-world fancy-index spec は本 AR-reuse に **SUPERSEDED** (concrete proven impl)。

### §12.2 F14+F15 = reset_to_phase SCALAR 維持、per-world DEFER to trainer (⭐Rs option A; REVERT item⑨ widening)
- route-executor は `reset_to_phase(k: int)` **SCALAR 維持**。item⑨ の `k:int|Tensor` widening = **REVERT** (公開API境界変更を回避 → CC4 F15 governance 懸念 **DISSOLVE**)。
- per-world state 復元 = **内部処理** (scalar k、per-world state slice — §5 (i)-(x) list は per-world slice で維持、署名は scalar)。
- per-world k ARGUMENT (curriculum start-mix) + auto-reset done-path curriculum wiring = **trainer node D-2 へ defer** (per-world が実際必要な場所; F14 item⑥ もここ)。
- ∴ 本 node で公開API境界変更なし。B⑨a′ の contract-change guard 部分は不要化 (B⑨a′ は **flag-flip byte-identity guard** として維持)。§5/§10.2-item⑨ の widening spec + env-core §6 の v1→v1.1 bump = **REVERT** (本 node contract 不変)。
- ✅ CONFIRM (%12 要請): DoD③ ⑨b (81-cell online numerator) は scalar reset (全 world phase-0、per-world 変動 = XY offset のみ、phase-k でない) で走る → **scalar interface 十分**。

### §12.3 F5 = reset banked phase-k GRIP target (SUPERSEDES item⑦ blanket-OPEN)
`reset_to_phase(k scalar)` は banked phase-k GRIP target を復元 (OPEN for phase-0、banked-CLOSED for G≥grasp)、blanket-OPEN でない。gripper joint_q + `control.joint_target_pos[drivers]` を banked grip state に復元 (§5 list、per-world slice)。item⑦「reset drivers to OPEN_RAD」= **SUPERSEDED** (gripped-k で A②/1-step 破壊回避、CC4 CH2/CC5 CH2)。

### §12.4 F4 = invariant-specific anti-revert check (SUPERSEDES §10.2① generic AST)
§10.2① の「semantic-equivalence check (AST/behavior)」= generic ゆえ off-grid invariant revert 検出不能 (Layer A on-grid のみ) → **INVARIANT-SPECIFIC assert**: (a) argmin over `cable_bodies[:,1]` Y (NOT fixed X target) (b) `_R_ty==c2y+_GHS` (88mm span、§0 FOUNDATIONAL) (c) `C2_TILT_SIGN` default 0 (test:5128/5290/5148)。+ banner「MIRROR OF Rs-LOCKED」+ golden hash。rewritten anti-revert logic の hand-edit = STOP→BLOCKED_FOR_USER (Rs)。invariant-specific check 構築不能なら foundational-invariant STOP。

### §12.5 F6 = §11.3 substrate over-claim SOFTEN (endorse)
§11.3「void = proven 0.716 substrate」= FALSE: env-core Layer-B scene = `add_support_clips=True` (4 clips base:1789-1811 @void loci) + C1 no-spacer ≠ monolith 0.716 (no-support + C1 SPACER riser test:1201-04、**void+support+no-spacer NEVER built**、AR は support=False)。→ soften「shares table VOID **only**; support-clips + C1-spacer = **measured deltas**」。ALL substrate delta を B③ divergence ledger 列挙。§10.4 /geometric-design に **4 support clips 追加** (現状 C1 clip のみ)。⚠ %12: void geometric PASS は grasp 機構 (void byte-id) に有効、grip EFFICACY = full-scene delta ゆえ measured (SRG) → F6 は measure-first 補強、矛盾なし。

### §12.6 F11 = explicit-solver-pass (endorse, SUPERSEDES monkeypatch + item⑧)
copy の C2 re-grasp rotated IK = `ik_move_both(solver=<rot_solver>)` explicit arg (sig 同一 verified: `solve_ik_dual`:1854 / `_solve_ik_dual_rot`:5171 = `(scene_info,target_left,target_right,warmstart_jq=None)`) → route_executor globals monkeypatch 撤去、**item⑧ (try/finally) 消滅**、①(locals)-§2(globals) tension 解消。§2 manifest globals-mutation 撤回。⚠ guard: smoke cell で byte-id confirm。

### §12.7 F3/F7/F8/F9/F12/F13 = fold (per disposition)
- **F3 (contact-path, §11.4 MED→HIGH)**: env `collide+contacts`:376-77 vs monolith None-path:1835 → grip = Layer-B GPU のみ観測、**first Layer-B smoke = grip-contact-source parity** (contact count/pad-cable pair introspection) を grip verdict 前に。Layer-A GREEN ≠ env grip works。
- **F7 (§10.2③ reword)**: 「verbatim replay」→「frame-indexed cadence @4-substep, time-compressed 2.5× (214×4=856 vs 2140 substep), closure convergence unproven」。grip efficacy を Layer A から bank 禁止。
- **F8 (byte-neutral scope)**: F14 で widening REVERT ゆえ scalar interface で byte-neutral は自明 (int(k) tensor-crash 懸念 moot)。
- **F9 (VOID guard loud-assert)**: base:1738 module-global SOLVER_BACKEND gate → **build-time loud assert** `grasp_actuation → SOLVER_BACKEND=='mujoco'` (§11.6 補強、import-order regression silent-SOLID 防止)。
- **F12 (flag-gate grip-cmd)**: `joint_target_pos` write を flag-on path に gate、flag-off no-op assert。
- **F13 (citation)**: servo mirror = test:1479-1557 (not 1336-1421 Franka/VBD path); VOID = test:1085-1139。§10.1 cite 訂正。

### §12.8 re-verify = TARGETED (⭐%12 07:23; full panel 不要 — extraction/geo/contract 本 round 0c)
4-lens: (a) **write-site completeness** (3-site baseline exhaustive、他 gripper-write site 無き事 + AR-reuse 全カバー を challenge) (b) **F10 AR-reuse 忠実性** (c) **F4 invariant-assert** (no-kinematic-trick servo≠pin + argmin-Y/88mm/tilt assert) (d) **F5 reset 正当性** (banked grip target)。→ re-verify DECIDE で %12 ping back。

## §13. v1.8 fold — round4 build-faithful fold (⭐governing; %12+Rs GO 07:53 empirical-shift; G1-G13)

**⭐Rs 経験的シフト承認 (option A、first-code 承認)。§13 が build-faithful spec を governs、DECIDE round4 の全 finding fold。次 = RULE-CHECK → BUILD Layer A thin slice (経験的 validation)。⚠ Layer A thin slice = low-cost (10h training でない) ゆえ full production-launch-gate 不要 (Rs first-code 承認で足る); full PLG は後段 trainer run + SRG measured-grip verdict 用。**

### §13.0 recurring-class 構造的 close (triple、gripper-coverage 再発根絶、%12 07:53)
1. **F10 = AR proven exclusion 機構 reuse** (毎回 re-derive しない、`_ARM_OVERWRITE_LOCAL`)。
2. **G7 = 単一 SSOT `_GRIPPER_COORDS_LOCAL` から exclusion AND restore を co-derive** (両者 divergence = F1[exclusion]→G7[restore] の再発源、単一 source で根絶)。
3. **byte-repro = empirical guard** (どの coord も漏れれば re-pin → byte-id 失敗で必ず検出、paper 列挙より強い guard)。

### §13.1 G1 CRIT = 3-pattern write-site (SUPERSEDES §12.1 単一 AR:475)
route の 3 site は heterogeneous ゆえ AR の 2 実 pattern に正しく grounding (%12 verified):
- **:387 `_broadcast_arm_jointq` → AR:475 broadcast**: `phys_jq[_arm_ow_q_idx]=fk_jq[_arm_ow_src]` (single pose all-world、`_arm_ow_src=_ARM_OVERWRITE_LOCAL*world_count`)。
- **:738 `_apply_actions_batch` → AR:1575 per-world**: `phys_jq[_arm_ow_q_idx]=jq_interp[:,_ARM_OVERWRITE_LOCAL].reshape(-1)` (+qd :1576)、per-world IK target 保存。
- **:617 reset → F5 banked-restore (§13.3、NOT exclusion)**。
両 write-loop pattern は `_arm_ow_q_idx`/`_arm_ow_qd_idx` (gripper 除外 map) を共有。

### §13.2 G3 HIGH = dual-arm choreography ADAPT (NOT blind L-freeze copy)
AR:1560/1570 の L-arm freeze は AR **aerial-hold 固有** (L が cable を空中保持、R のみ re-grasp)。route = active dual-arm C1→C2 routing (両腕 move)。→ AR の hold/freeze を **route の実 dual-arm choreography に ADAPT** (route の C1→C2 arm roles を build 時 locked monolith `_run_mujoco_grasp_route` から確認、両腕の per-phase hold 意味論を保つ)、**literal blind-copy 禁止** (§0 DUAL-ARM FOUNDATIONAL)。build 時に route arm roles を実読して adapt。

### §13.3 G2 HIGH = :617 = F5 banked-restore (resolves F10-vs-F5)
:617 (reset one-shot) は exclusion でなく **F5 banked-restore**: banked gripper q (all-16) + `control.joint_target_pos[drivers]` banked target を write (AR:1090+1097-1104 pattern)。one-shot ゆえ inert 非発生 (servo が written q から drive)。exclusion は per-frame-loop (:387 grip-window / :738) のみ。

### §13.4 G7 HIGH (root-cause) = §5(xi) all-16 restore co-derived from SSOT
§5 restore list に **(xi) gripper joint_q, all-16 coords `_GRIPPER_COORDS_LOCAL`={6-13,20-27} ← banked phase-k grip state** (drivers + followers 両方) 追加。**restore-set ≡ §13.1 exclusion-set (同一 16、ONE SSOT `_GRIPPER_COORDS_LOCAL` から co-derive)** = §13.0-2 構造的 close の core。builder は (i)-(x) list から gripper を omit しない。

### §13.5 G4/G5/G6 = invariant + test spec-precision (unit-testable code = empirical)
- **G4 (HIGH)**: §12.4 invariant (a) split → **(a1)** `_kR=argmin(|cbq[:,1]−_R_ty|)` (picker) **AND (a2)** `target_X == cbq[_kR,0]` (bow-follow、NOT fixed c2x) + behavioral: target_X が 81-grid で varies (fixed-X air-grip bug 検出、test:5128-30)。
- **G5 (HIGH)**: §12.4 (b) `_R_ty==c2y+_GHS` は VACUOUS TAUTOLOGY (:5134 def) → **numeric pin** `abs(2·_GHS−0.088)<1e-9` + both endpoints (`_R_ty==c2y+_GHS` AND `_La[1]==c2y−_GHS` :5092) + `_GHS` resolves `GRIP_HALF_SPAN` (60mm backup `task_config.py.pre_3c_backup:121` revert 検出)。
- **G6 (HIGH)**: A②/1-step divergence test は EE/target/phase のみ = cable-hold に BLIND → **G3-G6 cell で cable-hold quantity 追加** (cable body_q at grasp points / follower joint_q / grip-hold predicate 対 monolith G-boundary+1 tol)。A②(iii) が claim(B) を discharge 可能に。

### §13.6 G8-G13 = MED/LOW fold
- G8: route imports に `GRIPPER_JOINT_RANGE` + `ARM_DOF` 追加。
- G9: cable_bodies provenance pin + runtime `|_picked_dy|≤tol` (:5137)。
- G10: F4 = target-static assert ≠ contact proof → Layer A byte-repro (on-grid) + Layer B force gate (B③/B④ SRG) と PAIRED 明記。
- G11: ⑨b = 81 worlds × 1 episode、horizon 測定、no-mid-measurement auto-reset (fresh-process reference mirror); done-path (:1136) は gripper を phase-0 OPEN banked に reset (reset-correctness、curriculum でなく)。
- G12: q-AND-qd load-bearing (両 map必須); dead FINGER_OPEN writes :695-96/:724-27 delete (gA=True で no-op、latent re-pin surface); grip-cmd single-writer = `_set_gripper_target` (F12 flag-off no-op assert)。
- G13: §5(v) `episode_length_buf←phase-k` は arm target AND step-1 grip schedule 両方を govern (framing)。

### §13.7 build path (⭐Rs first-code 承認、empirical validation)
v1.8 fold → **RULE-CHECK** → **BUILD Layer A thin slice** (byte-repro on AR-reuse、monolith build_scene substrate、env-core 非依存、**cuda:0 canonical only** [device-fragile [[project-canonical-route-device-fragile-cpu-vs-cuda]]])。= 経験的 validation: no-repin + per-world + AR-fidelity を byte-id で実証。Layer A 先行、Layer B (env-core substrate + SRG 4-substep measured-grip) 後続。⚠ full production-launch-gate 不要 (Layer A low-cost、Rs first-code 承認)、full PLG は後段 trainer run。ping back: (1) v1.8 fold + RULE-CHECK (2) Layer A byte-repro (byte-id 58/81 EXACT + no-repin 実証) + build 開始報告。

### §13.8 GPU parallelization (⭐Rs directive %12 07:58; 2 GPU / ≤8 proc, device-fragility reconciled)
GPU topology (両 idle、%12 nvidia-smi 確認; CLAUDE.md「cuda:2」= stale enumeration、物理 index 0/1 のみ):
- **cuda:0 = index0 A6000 48GB = byte-repro canonical (device-fragile target)**。
- **cuda:1 = index1 PRO 4000 24GB = independent-workload-ONLY** (route 3/3 MISMATCH 実績 [[project-canonical-route-device-fragile-cpu-vs-cuda]])。
⚠ **HARD 制約 (byte-repro correctness)**:
- **byte-repro (Layer A 58/81 EXACT) = cuda:0 ONLY**。route device-fragile ゆえ cuda:1 = MISMATCH → byte-id 不成立。
- **byte-repro を cuda:0+cuda:1 split 禁止** (cross-device = 各 cell 結果が device 依存 → 「byte-repro FAIL」誤読の silent-invalid)。
- 81-grid は **cuda:0 内で ≤4 process split** して parallelize (device-consistent、A6000 48GB は ≤4 proc)。
reconciliation (≤8 proc / 2 GPU 有効活用):
- **cuda:0: byte-repro 81-grid を ≤4 process 並列** (Layer A、device-consistent、高速化 × device-fragility 保全)。
- **cuda:1: ≤4 process independent work** (byte-identity を cuda:0 reference に要求しないもの = Layer B env-core self-consistent smoke / device-qual probe / 後段 DR sweep prep)。⚠ cuda:1 結果を cuda:0 canonical と byte 比較しない (cross-device invalid)。
GPU hygiene (§運用GPU): 各 proc `CUDA_VISIBLE_DEVICES=N` 必須 (未設定 = cuda:0 不要 context); launch 前 `nvidia-smi --query-compute-apps` (現両 idle); A6000 ≤4 / PRO 4000 ≤4; 旧 proc 残存 → §運用20 kill 承認要 (自律 kill 禁止)。
Layer A byte-repro 結果報告に **device=cuda:0 + proc 分割数** 含める (%12 要請)。

---

*%11 COORD (w2:p3) 起草 2026-07-07 / rev v1.1 (00:32) / rev v1.2 (01:22 pre-check 2 CRIT: 2-layer DoD + gripper-servo + HIGH4/MED5/MED6) / **rev v1.3 (re-pre-check BLOCK fold: CRIT1 自前 IK stack verbatim copy 確定 [1-ULP monkeypatch、import-reuse 撤回] + Issue2 explicit-solver 不採用 + Issue3 Layer A 実行モデル [run_route self-driving / step_target facade] + Issue4 env-core ⑨a′ 再regression [B⑨a′] + Issue5 citation :724-727 + Issue6 reset cable-state+_per_world_fk_jq + Issue7 A② bank-vs-monolith spot-check)**。PAPER-ONLY / INVARIANTS 不触 / locked runner 不触 (global mutate 禁止) / task_config 不触。**Layer A (verdict+trajectory byte-identity, 自前 IK stack) = 先祖返り guard / Layer B = substrate-transfer 実測 (非保守)**。**/pre-check 3 round: round1 BLOCK (substrate mismatch→2-layer) → round2 BLOCK (IK 1-ULP→自前 copy) → round3 WARN (内部整合 2 HIGH fold: line76/23 import-reuse 矛盾撤去 + transitive-closure manifest + two-copy drift gate; 0 CRIT = 収束)**。→ %12 re-checkpoint PASS → **5体 [VERIFY] = FAIL (4 CRIT/HIGH, NHA=CHANGE_JUSTIFIED)** → **v1.4 完成**: governance-independent (CRIT1/2/4 + MED5-8,10-12) + **env-core-source (CRIT3/MED9) = ⭐Rs 決定 A (cross-node 3 箇所 flag-gate default-off、`bad0d25204`) で fold** + /force-design skip (力 inherited)。→ re-pre-check = BLOCK (1 CRIT servo-kinematic + 1 HIGH ee_tgt-GPU + 2 MED) → **v1.5 fold**: servo 機構訂正 (`_ARM_OVERWRITE_IDX` gripper 除外 + PD schedule、旧「:731-742 不編集」= kinematic-close 抵触ゆえ撤回) + ee_tgt HARD L∞=0 を CPU-det phase 限定 (C2 re-grasp argmin は GPU 由来) + grip-hold viability 軸 (%12) + stale text 訂正。→ **次 = full 5体 再走** (%12 02:37、FAIL 復帰 + env-source 新 surface) → **full 5体#2 = FAIL (LOAD-BEARING CRIT: env-core grasp_actuation 欠落 → v1.5 servo inert)** → **rev v1.6 (servo-scope escalation DISPOSITION Rs=A `9b9a41604c` + %12 plan-ACK 05:23): v1.5 drive-loop servo 撤去 → `grasp_actuation=True` flag-flip [proven base:1586-1740 build_scene mirror, route:339 kwarg default-off, 4th flag] = 3 findings 同時解消 [inert-servo/CC4#5/finding#5 全16除外] + full 5体#2 HIGH/MED 10項 fold [§10.2] + %12 sequencing [Layer A byte-repro front-load] + design-gate /geometric-design table-void 新 pass [§10.4] + 5体 re-verify delta=Layer B [§10.5]。⭐§13 が v1.8 build-faithful spec governs (⭐%12+Rs GO 07:53 empirical-shift → RULE-CHECK → BUILD Layer A thin slice、first-code 承認)** → **full 5体 round3 = FAIL (accepted CRIT F1 write-site exclusion gap + 5 HIGH; gate が code 前捕捉) → rev v1.7 (%12 rulings 07:23、§12 governs): F1+F2+F10 = AR-reuse structural fix (全3 write-site :387/:617/:738 q+qd 除外、`newton_aerial_regrasp`:200-201/:421-441 wholesale reuse) / F14+F15 = reset_to_phase SCALAR + per-world defer-trainer (item⑨ widening REVERT、F15 dissolve) / F4 invariant-specific / F5 banked grip target (item⑦ superseded) / F6 substrate-soften / F11 explicit-solver-pass (item⑧ 消滅) + F3/F7-F9/F12/F13。→ targeted re-verify (4-lens)**。build 非進行。*
