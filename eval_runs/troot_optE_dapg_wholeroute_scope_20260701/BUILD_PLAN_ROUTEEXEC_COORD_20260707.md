...
- **⚠ two-copy IK drift standing gate (v1.3, re-pre-check Issue3):** IK stack が copied (oracle in-file + route_executor copy) ゆえ将来 drift 可。oracle 側は Rs-LOCKED/ANTI-REVERT (frozen :5290) → **route_executor copy のみ drift 可** (= route-executor change)。standing rule: **route_executor.py の IK stack or oracle が変わったら Layer A byte-repro (cuda:0 canonical) を再走** (Layer A は build-time DoD だが本 rule で回帰 gate 化)。
