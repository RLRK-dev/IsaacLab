...
**⛔ Q2 env-core ownership — v1.4 HOLD (5体 CRIT3/CC4-CH1, %12 E3 own):** gripper-servo + C2-clip + route-swap は env-core source 3 箇所編集 = env-design **Rs専権**。%12 の旧「%12 authorize で COMPLETE 維持」= 越権 + under-scope (servo default-off だけでは ⑨a′ 保全不能、§9 HOLD bullet 参照)。→ **%12 が Rs へ G1/G2 escalate 中、本節は Rs 決定まで HOLD**。決定後に認可方式 (cross-node flag-gate edit vs env-core re-open) + legacy-config ⑨a′ 保証を確定して反映。**governance-independent fix (CRIT1/2/4 + MED5-8,10-12) は本 HOLD と独立に v1.4 fold 済 (%12 GO 02:29)**。

**⚠ IK stack transitive-closure manifest (v1.3, re-pre-check Issue2 — 「4 関数」は undercount):** route_executor.py に **verbatim copy** = {`ik_move_both`:1958 / `solve_ik_dual`:1814 / `_solve_ik_dual_rot`:5171 / `_rot_quat_rx`:5156 / `_cable_local_pitch`:5161} + monkeypatch install/restore (:5214-5215/:5343 を **route_executor の globals** に対して実行) + **自前 `_demo_rec` binding** (:1780、copied physics_step:1846 + ik_move_both:1981 が参照)。**import-reuse** (IK 非依存、byte-safe) = {`physics_step`:1790 / `get_ee_positions`:1945 / `update_kinematic_bodies`:1752 / geom helpers / IK objective classes / 定数 `DEVICE`/`IK_ITERATIONS`/`IK_STEP_SIZE`/`STEPS_PER_CM`/`MAX_MOVE_STEPS`/`SIM_SUBSTEPS`/`SIM_DT`/`JOINTS_PER_ARM` 等}。5体 [VERIFY] で transitive closure を grep 検証 (byte-faithful 集合の完全性)。
...
| # | 項目 | bar / 判定 |
|---|---|---|
| **A① byte-repro (PRIMARY)** | RouteExecutor `run_route()` を **monolith substrate (SIM_SUBSTEPS=10 + copied IK + 実 grip) 経由**で canonical 81-grid → **⚠ v1.4 (5体 CRIT2, %12 E2 own): 2 量で bar 分離** — **(i) HARD L∞=0 = CPU-deterministic `ee_tgt_pos_l/r` のみ** (=抽出対象の route logic そのもの、note_targets:160; ≥1 不一致 = 抽出 infidelity = build FAIL) **∧ (ii) `joint_q` = tolerance-band** (GPU IK 出力ゆえ C2 re-grasp:5145-5343 は GPU#562-stochastic → L∞=0 不能、GPU-stochastic cell は verdict と同じ flag+carry) **∧ (iii) verdict strict_v2 EXACT** (安定 cell)。**前提 = A①-pre self-repro (下記 §4)**。「conservative-definite」は ee_tgt leg 限定 (joint_q tol-band は noise-floor 内)。env-core 非経由ゆえ抽出のみ isolate。 |
...
| # | 項目 | bar / 判定 |
|---|---|---|
| **B③ ⑨b online-numerator** | env-core substrate (**4-substep 維持** + gripper servo dynamic close = §2 drive-loop 拡張) residual≡0 × 81 live → **strict_v2 実測** (58/81 と**仮定禁止**、%12 Q3)。⚠ **v1.4 (5体 CC3-CH3): divergence を per-axis 符号付きで報告** — {grip-force / seat-depth / reach} 各軸を **conservative (env-core<oracle=RL が recover) / non-conservative (env-core>oracle=over-optimism、高 fidelity/実機 確認要) / unknown-要判定**でタグ (旧「recover すべき量」= deficit 方向を presume していた誤り撤回; 4-substep の softer contact は seating を非保守に易化し得る)。**substep 4→10 決定 = Rs-level post-data** (今は 4 維持で empirically-gate) |
...
- **⚠ invariant-enforcement scope 訂正 (v1.4, 5体 CC4-CH2):** 「byte-repro が invariant を structural enforce」は **Layer A (抽出 vs monolith) 限定**。servo/C2/4-substep の Layer B path は byte-repro 非経由ゆえ別基底要: **DUAL-ARM = servo が per-arm `grip_2` 2-vec で両腕駆動** + **no-kinematic-trick = gripper coord は dynamic position-drive (joint_q.assign 上書きしない)** を Layer B/B⑨a′ check に明記 (§9「byte-repro enforces」の blanket claim 撤回)。
- **⛔ HOLD (v1.4, 5体 CRIT3 + CC4-CH1, %12 E3 own): env-core-source fix は Rs 判断待ち。** gripper-servo + C2-clip + route-swap は env-core `_apply_actions_batch`/scene/`self._route` の **source 3 箇所編集** = env-design **Rs専権** (%12 の旧「%12 authorize」= 越権、E3 own)。%12 が Rs へ **G1/G2 escalate** 中 (認可方式 = cross-node edit vs env-core re-open)。**⑨a′「COMPLETE intact」は servo default-off だけでは不成立** (scene C1-only:344 + C2 geometric-proxy:961 + stub:309 → C2-clip/route-swap も flag-gate 必須、legacy config {C1-only+stub+servo-off} が ⑨a′ 25/81 EXACT 再現を保証、C2-scene run は「Layer-B re-BASELINE (new predicate)」改称)。**MED9 blast-radius grep (共有 base の grip/AR/AC byte-unchanged) も本 HOLD に同梱**。Rs 決定後に方式込みで v1.4 反映 → CRIT4 core-loop 変更ゆえ re-verify 寄り。

---

*%11 COORD (w2:p3) 起草 2026-07-07 / rev v1.1 (00:32) / rev v1.2 (01:22 pre-check 2 CRIT: 2-layer DoD + gripper-servo + HIGH4/MED5/MED6) / **rev v1.3 (re-pre-check BLOCK fold: CRIT1 自前 IK stack verbatim copy 確定 [1-ULP monkeypatch、import-reuse 撤回] + Issue2 explicit-solver 不採用 + Issue3 Layer A 実行モデル [run_route self-driving / step_target facade] + Issue4 env-core ⑨a′ 再regression [B⑨a′] + Issue5 citation :724-727 + Issue6 reset cable-state+_per_world_fk_jq + Issue7 A② bank-vs-monolith spot-check)**。PAPER-ONLY / INVARIANTS 不触 / locked runner 不触 (global mutate 禁止) / task_config 不触。**Layer A (verdict+trajectory byte-identity, 自前 IK stack) = 先祖返り guard / Layer B = substrate-transfer 実測 (非保守)**。**/pre-check 3 round: round1 BLOCK (substrate mismatch→2-layer) → round2 BLOCK (IK 1-ULP→自前 copy) → round3 WARN (内部整合 2 HIGH fold: line76/23 import-reuse 矛盾撤去 + transitive-closure manifest + two-copy drift gate; 0 CRIT = 収束)**。→ %12 re-checkpoint PASS → **5体 [VERIFY] = FAIL (4 CRIT/HIGH, NHA=CHANGE_JUSTIFIED)** → **v1.4 governance-independent fold 済 (CRIT1/2/4 + MED5-8,10-12), ⛔env-core-source (CRIT3/MED9) = Rs G1/G2 待ち HOLD**。build 非進行。*
