...
- 記録 = 2026-09-06 00:31 UTC（`date -u` 実測・v0.2.8 で更新）。本 file は package の読み方の案内であり、規範文書ではない。
- 状態: **REVIEW CANDIDATE v0.2.8・未 bank・two-key 未・Rs 未裁定・gate PASS を主張しない**。impl / training / closed-loop authority / production / freeze / slice = CLOSED のまま。
...
| `05_WMSO_RUNTIME_SPEC_…md` | **runtime spec v0.2.8**（boundary-only・TERMINAL boundary・checkpoint 切替なし。AuthorityManager / CommandGateway / CAS / lease / SafeHold / audit） |
| `06_WMSO_INDUSTRIAL_PROFILE_…md` | **industrial deployment profile v0.2.8**（cell 固有条件を content-addressed にし、凍結意味論を狭める方向にしか使えない） |
...
- **v0.2.8 本文への再レビュー・再 verify は未実施**。round 1→5 で HIGH は 5 → 3 → 5 → 2 → 0。「open = 0」は宣言しない（05 §12・06 §10 に open が残る。round 5 の起草既定 = 11_ §11.1 末尾）。「open = 0」は宣言しない（05 §12・06 §10 に open が残る）。
...
1. v0.2.8 を two-key review に回す（Rs 決定 2026-09-05: 外部 AI（GPT-astra）の独立レビューを two-key 判断の材料にする — 依頼文 = `review_records/rs_consult/Q_GPTastra_*.md`・v0.2.8 に pin。round 5 で HIGH = 0 に到達したため AI 再 round は一旦止める）。
2. round 4 / 5 で起草既定として置いた値・選択の確認（11_ §10.1 / §11.1 末尾に一覧: head-check 副場合の disposition・`P_VALIDITY_UNBOUNDED` の kind 集合・base pose 許容 / validity 上限の値・identity 系 diagnostic item の必須化・role registry head の発行者・gateway の audit 同期 block）。
3. 裁定後に残る open: OPP-16（timing baseline の発行主体 = RT0 / cell safety court と値の根拠）・OPP-17 / 05 OP-21（「every motion で両腕」の実行適合性は composition court）・OPP-13 の role 実名（pS / pY 等の割当）・OP-19 の周期診断項目（baseline の diagnostic_items）。
4. `contracts_v3` を起票しない判断（07）の追認。
