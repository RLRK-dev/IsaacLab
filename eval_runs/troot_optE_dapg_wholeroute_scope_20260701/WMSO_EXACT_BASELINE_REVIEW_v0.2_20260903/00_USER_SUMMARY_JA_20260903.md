...
- 記録 = 2026-09-05 23:33 UTC（`date -u` 実測・v0.2.7 で更新）。本 file は package の読み方の案内であり、規範文書ではない。
- 状態: **REVIEW CANDIDATE v0.2.7・未 bank・two-key 未・Rs 未裁定・gate PASS を主張しない**。impl / training / closed-loop authority / production / freeze / slice = CLOSED のまま。
...
| `05_WMSO_RUNTIME_SPEC_…md` | **runtime spec v0.2.7**（boundary-only・TERMINAL boundary・checkpoint 切替なし。AuthorityManager / CommandGateway / CAS / lease / SafeHold / audit） |
| `06_WMSO_INDUSTRIAL_PROFILE_…md` | **industrial deployment profile v0.2.7**（cell 固有条件を content-addressed にし、凍結意味論を狭める方向にしか使えない） |
...
| `review_records/fold/*` | v0.2.1 → … → v0.2.7 の fold を再現する script と verifier verdict の JSON |
...
- **v0.2.7 本文への再レビュー・再 verify は未実施**（次 round）。round 1→2→3→4 で HIGH は 5 → 3 → 5 → 2（round 4 の HIGH 2 = R6-01 finger geometry・R6-04 one-key record、両方 fold 済）。「open = 0」は宣言しない（05 §12・06 §10 に open が残る）。
...
1. v0.2.7 を two-key review（pS / pY）に回すか、先に v0.2.7 対象の AI 再 round を 1 回挟むか（round 4 は 35 件中 34 件が確定・HIGH 2 = 減ったが 0 ではない。再 round 1 回を推奨）。
2. round 4 で起草既定として置いた値・選択の確認（11_ §10.1 末尾に一覧: head-check 副場合の disposition・`P_VALIDITY_UNBOUNDED` の kind 集合・base pose 許容 / validity 上限の値・identity 系 diagnostic item の必須化・role registry head の発行者・gateway の audit 同期 block）。
