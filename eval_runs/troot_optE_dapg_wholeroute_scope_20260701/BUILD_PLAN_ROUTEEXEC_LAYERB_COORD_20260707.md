...
**rev:** **v0.2.2** (2026-07-07 16:5x — comp1 DONE+%12-VERIFY-PASS 433ba6b246; folds 2 %12-confirmed comp2 findings: **(A) qvel=0 data-forced** [recording carries NO joint_qd — §8/§3 corrected: plan's earlier "qpos/qvel from recording" was a wrong premise; arm qd substrate-zeroed = EXACT, gripper qd→0 servo re-accel ⑦(b); ⑦(a) qvel-leg = self-consistency vs banked-0, NOT vs-recorded] + **(B) cable-fork consistency SURFACED** [reset_to_phase(k≥1) restores arm+gripper only; env-core reset re-seeds cable to phase-0 → k≥1 fork cable-mismatch; ⑦ split refined §8/§3; cable re-seed = env-level, ⑦(b)/trainer stage; PENDING %12 scope ruling]). v0.2.1 = 021df057bc / v0.2 = 39176db220 / v0.1 = a51825bd28. ⚠ **minimal Stage-A {comp1,comp2} BUILD-AUTHORIZED (no-GPU, structural); comp3-8 Stage-B HOLD.** DESIGN-GATE skip-recorded (Stage-A structural: reward/env/success/physics/geometry 不変, flag-gated default-off). → [RULE-CHECK] Tier0 → BUILD (chunk+ast-verify+commit+ping) → §5 STATIC gate → %12 verify.
...
- **⑦(a) restore-fidelity** (Stage-A, no-GPU, comp1 DONE 433ba6b246): reset_to_phase(k) restored **arm+all-16-gripper** L∞, static (no rollout). qpos-leg = restored **vs recorded arm_q** at phase-k boundary (真 fidelity, ≤1mm; measured 0). qvel-leg = restored **== banked-0** (self-consistency + sentinel write-proof; recording has NO qvel so "vs-recorded" is undefined, %12-confirmed 16:57). ⚠ **cable NOT in ⑦(a)** (arm+gripper only). **⑦(b)** (Stage-B, live): 1-step-after-reset divergence (grasp_actuation=ON) **+ cable-fork consistency** (env-level cable re-seed from recorded cable_xyz — see §8 (B)).
...
## §8. state_bank + ⑦ (CC6 split; v0.2.2 folds 2 %12-confirmed findings A/B)
- comp2 builds `_state_bank[k]` = phase-k banked {arm+all-16-gripper **qpos**, arm+gripper **qvel=0**, grip_target} from the recording (Q3: recording carries phase_id per-frame → phase-k boundary = first frame at mapped phase k). `build_state_bank_from_recording()` (route_executor.py, comp1 DONE 433ba6b246) → RouteExecutor.reset_to_phase(:3093) restores via apply_banked_restore (BUILT).
- **(A) qvel=0 = DATA-FORCED (%12-confirmed 16:57):** the recording's `_STACK_KEYS` (route_demo_recorder.py:38-51) = ee_pos/quat/ee_tgt/**arm_q (positions=full joint_q)**/grip_cmd/cable_xyz/quat — **NO velocity/joint_qd field**. So banked qvel=0. Sound: arm joint_qd is substrate-zeroed every step (_broadcast_arm_jointq:388 / apply_arm_only_write_*) = EXACT; gripper qd unrecorded → 0 (POSITION servo re-accelerates live, ⑦(b)). `grip_target` = recorded `grip_cmd` (COMMANDED driver target [rad], cols [L,R]), DISTINCT from the servo-lagged ACTUAL gripper joint_q restored from arm_q (measured: grip_cmd 0.7407 vs joint_q ~0.71). ⇒ plan's earlier "qpos/qvel from recording" was a WRONG premise (no qvel exists); corrected here.
- **(B) cable-fork consistency = SURFACED+RULED (%12 16:57 investigated → %12 RULING 17:13 = DEFER to Stage-B ⑦(b) as a REQUIREMENT; %12 §運用28 CONFIRMED via _reset_worlds:591 docstring 'AUTHORITATIVE joint_q seeding of arm + cable' to P0):** env-core `_reset_worlds` (:583-627) re-seeds the FREE-root cable to the SETTLED (phase-0) pose via `derive_cable_joint_q_from_tangents`+`seed_cable_joint_state`. `reset_to_phase(k≥1)` overwrites arm+gripper ONLY → the cable STAYS at phase-0 while the gripper is banked-CLOSED at phase-k ⇒ a k≥1 curriculum fork grasps empty air (cable-mismatch). Answer = **(b)** cable MUST be banked+restored for a consistent fork; DATA exists (recorded `cable_xyz`/`cable_quat` at the boundary; also `arm_q[28:74]`=cable joint_q). ⚠ the cable restore is **ENV-LEVEL** (via the existing per-world offset-aware `seed_cable_joint_state`, NOT a direct route_executor joint_q assign — the free-root world-offset would misplace world≥1). SCOPE: `reset_to_phase(k≥1)` is NOT rolled out in ANY Stage-A path (env reset = reset_to_phase(0)=no-op; ⑦(a) test = arm+gripper isolation) → the mismatch does not manifest in Stage-A. comp2 state_bank stays arm+gripper (minimal-Stage-A: no un-exercised data). **⛔ STAGE-B ⑦(b) REQUIREMENT (not a note — %12 17:13; forgetting = empty-grip mismatch on every k≥1 curriculum fork):** (1) extend `build_state_bank_from_recording` to ALSO capture the phase-k boundary `cable_xyz`/`cable_quat`; (2) wire an ENV-LEVEL `seed_cable_joint_state(banked cable_xyz, per-world offset-aware)` into the curriculum-fork reset path (the k≥1 branch, alongside the arm+gripper restore). Mirrored in node LOUD-CARRY.
- **⑦(a)** (Stage-A no-GPU, comp1 DONE): restored arm+gripper qpos vs recorded (真 fidelity) + qvel==banked-0 (self-consistency) L∞ ≤ 1mm/1mm/s (static unit; measured 0). **⑦(b)** (Stage-B): 1-step-after-reset divergence (grasp_actuation=ON, live) + cable-fork consistency (B). SCALAR k (per-world curriculum → trainer, §13.2).
