...
- **(B) cable-fork consistency = SURFACED (%12 16:57, INVESTIGATED, PENDING %12 scope ruling):** env-core `_reset_worlds` (:621-627) re-seeds the FREE-root cable to the SETTLED (phase-0) pose via `derive_cable_joint_q_from_tangents`+`seed_cable_joint_state`. `reset_to_phase(k≥1)` overwrites arm+gripper ONLY → the cable STAYS at phase-0 while the gripper is banked-CLOSED at phase-k ⇒ a k≥1 curriculum fork grasps empty air (cable-mismatch). Answer = **(b)** cable MUST be banked+restored for a consistent fork; DATA exists (recorded `cable_xyz`/`cable_quat` at the boundary; also `arm_q[28:74]`=cable joint_q). ⚠ the cable restore is **ENV-LEVEL** (via the existing per-world offset-aware `seed_cable_joint_state`, NOT a direct route_executor joint_q assign — the free-root world-offset would misplace world≥1). SCOPE: `reset_to_phase(k≥1)` is NOT rolled out in ANY Stage-A path (env reset = reset_to_phase(0)=no-op; ⑦(a) test = arm+gripper isolation) → the mismatch does not manifest in Stage-A; it is a ⑦(b)/trainer-curriculum completion. `build_state_bank` MAY be extended to also carry banked `cable_xyz` now (data-ready) — %12's scope call.
