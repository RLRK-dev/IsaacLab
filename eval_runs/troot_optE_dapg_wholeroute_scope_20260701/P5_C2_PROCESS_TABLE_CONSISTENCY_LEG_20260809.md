...
- ⛔ **(c1) は 2 つに割れる（2026-08-09 16:47 訂正・p18 §1314 §2 が契機。⚠ 直下の 16:44 行の「どちらも静的には無理」は本行が上書きする）**: 私は (c1) を「経路が空いているか」と 1 つに書いたが、**指令された経路と、実際に追従した経路は別の物**（§4g で自分が見つけた commanded ≠ realised が、そのまま当たる）。
  - **(c1a) 指令された整定 span（零姿勢→HOME）の経路 clearance** = ⭐ **run 不要で測れる**。計器が既に持つ **along-path 列**（連続姿勢間の関節空間補間・`mj_forward` + `mj_geomDistance` のみ）を、この span に当てるだけ。⇒ **既存の authorization の内側**（新しい instrument も新しい許可も要らない）。⚠ **薄い側は台の +19.8 mm** で、両端からは見えない。
