...
## 4n. 追記 2026-08-09 16:27 JST — **§4f の −1.0 mm を別機構で再測。主張は生き残り、方法には穴が在った（発火せず）**

**契機** = p18 §1303 の家族法「**Re-measure your own predicate with a DIFFERENT MECHANISM before closing a discrepancy with an explanation**」＋ 同卓の自白（`head -4` で深い行を落とした）。

- ⚠ **私の元の取り方** = `grep 'STEP 2 ARM-TO-ARM…' | head -1` ⇒ **同じ形の穴**（深い所の行を落とす）。
- ✅ **別機構での再測**（regex の文字クラスも `head` も使わず、python の文字列 index で全行走査）: **4 file とも `n=1` / value = `-1.0`**、positive control は合成行から `-9.9` を返す。⇒ **§4f の「4 本とも −1.0 mm・crown 非依存」は生き残る**、しかも等級が上がる（「最初の 1 本を見た」→「**1 本しか無いことを数えた**」）。
- ⛔ **正直な等級**: **穴は在った。発火しなかっただけ**（当該行が各 file 1 本だったため）。⇒ ⭐ **「結果が正しかった」は「方法が健全だった」ではない。**
