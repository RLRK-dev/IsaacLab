...
| (b) | **測られる 2 値（後継・owner つき）** = ①計器の **STEP-1 行**（**HOME@C-2 の実リンク間 clearance**）／②**zero-pose 診断**（`qpos = 0` 両腕） | ✅ **測定済 2026-08-09 16:39**（p0 の承認済み計器・`mj_forward` のみ・`mj_step` 0・各行が geom 対を名指す。⭐ **producer と半径 2026-08-09 17:17 追記**: `kinonly_step_solve.py:272 def closest(m, d, A, B, cutoff: float = 0.5)` で、:338-340 の 3 呼出はすべて**既定の 0.5 m** を使う。⭐ **実測値かどうかの判別は「内側に見える」ではなく ceiling との厳密不等**: この closest() は prefilter を持たず `mj_geomDistance` に cutoff を素通しする（:294・本 turn 実読）⇒ **全対が 0.5 m より遠いと cutoff そのものが距離として返り、pair 名まで付く**（pZ が自前 2 球 model で実測: 真の間隙 1.980 m → 0.500000 が返る）⇒ **飽和した行は正確に +500.0 と印字される**。**本行の 4 値はすべて < 500.0 ⇒ 実測値**。⚠ **ただし +491.3 は ceiling の 98.3%（残り 8.7 mm）** — 「余裕が大きい」と読まない・**将来の表で 500.0 ちょうどは飽和**（p0 の prefilter-or-None 修正が入るまで、ceiling 付近の行はこの注記を継承する）。〔⚠ **在位置訂正 2026-08-09 17:25**: 本注記の元の文は「**全値が半径のはるか内側（最小 19.8 mm ≪ 500 mm）⇒ 不在・sentinel の境界から遠い**」— ⛔ **最小値で全称を支えていた**（491.3 は境界から 8.7 mm しか離れていない）。判別を quantifier から厳密不等（< 500.0）へ差し替え。〕⛔ **B5（wired の settle recorder が 16 mm を宣言しつつ 2 つの query に跨る件）は本行に届かない — producer が別**）: **② qpos=0 → arm↔arm +491.3 mm（L_base#2 ↔ R_base#41）／arm↔table +19.8 mm（R_forearm#44 ↔ table_top）= CLEAR**／**① HOME 対照 → +194.2 / +80.1 = CLEAR**。⇒ **零姿勢は干渉していない ⇒ 分岐は純サーボ・Rs への初期条件差し戻しは無し** |
...
〔⚠ **在位置更新 2026-08-09 16:42**: (b) は元々 **PENDING** と書いていた。**測定済になった PENDING は、最上位に在る間ずっと「まだ」と読ませる**ので、§4m 冒頭方針どおり在位置で値に置き換えた（原文の語は本注記に保存）。併せて **旧 (c) を (c1)/(c2) に分けた** — 「到達するか」と「経路が空いているか」は**別の未測**で、1 行に畳むと片方が消える。⚠ **2 値は「再組立された cell」での測定**（p0 の pre-stated 限界）であり、driver が建てる cell の測定ではない。〕

- ⛔ **(c1) は 2 つに割れる（2026-08-09 16:47 訂正・p18 §1314 §2 が契機。⚠ 直下の 16:44 行の「どちらも静的には無理」は本行が上書きする）**: 私は (c1) を「経路が空いているか」と 1 つに書いたが、**指令された経路と、実際に追従した経路は別の物**（§4g で自分が見つけた commanded ≠ realised が、そのまま当たる）。
  - **(c1a) 零姿勢→HOME の関節空間補間線の clearance** = ⚠ **run 不要で計算できる。ただし *片側にしか効かない*。**〔⛔ **格下げ 2026-08-09 16:52・p11 の訂正語を採用**: 逐語「**A STEP INPUT HAS NO COMMANDED PATH; the interpolation is a reference line, a hit informs, a clear does not**」。**サーボは *目標* を受け取るのであって *軌道* を受け取らない** ⇒ **関節空間の直線は計器が引いた参照線であって、機械が指令された経路ではない**。⇒ ⛔ **当たれば情報／空いていても何も言えない。** ⇒ **私が 16:47 に書いた「run 不要で *測れる*」は言い過ぎ** — 測れるのは参照線であって経路 clearance ではない。⭐ **これは私が今夜 別件で立てた片側則（`deleted=0` は結論的／`deleted≥1` は無情報）と同じ形で、向きが逆**。⚠ **p11 の instrument 側読みを「私の行が支持する」と送ったのも同じ理由で撤回**（`m-p4-224`）。〕⚠ **薄い側は台の +19.8 mm** で、両端からは見えない ⇒ **参照線が当たるなら、それは強い警告**。
  - **(c1b) 実際に追従した経路の clearance** = ⛔ **run が要る**。producer = 整定ループの走行中最小 clearance（p0 任意の by-product）。⇒ **pZ の第 3 節（followed traverse は未測かつ gated）はここに対応する。**
  ⇒ ⭐ **帰結**: 私の行の未測は **3 つ**（(c1a) 測定可・未実施／(c1b) run 待ち／(c2) run 待ち）。⛔ **(c1a) を run 待ちに数えるのは、閉じられる穴を閉じないままにすること。**
- ⭐ **(c1b)(c2) が閉じる条件（2026-08-09 16:44 追記・⚠ (c1a) には当たらない = 上行参照）**: **どちらも「静的には無理／実 run でのみ測れる」で同じ**。⇒ **両方とも最初の authorized wired run で閉じる**。**producer は既に名前が付いている** — **(c2)** = driver 既存の「**reached N% of the way**」印字／**(c1)** = **整定ループが arm↔env の走行中最小 clearance を geom 対つきで印字する 1 行**（p18 §1313 §2 の by-product 案・⛔ 受入行ではなく p0 の任意）。⇒ ⭐ **その 1 行が入れば、(c1) は run が許可された日に *副産物として* 埋まる。入らなければ、同じ値を得るのに 1 反復かかる。** ⛔ 私は要件化しない（court = p11/pZ、実装の選択は p0）。
- ⛔ **gate は再開しない** — **dep-1 = 「p5 のレグが履行済み」。gate 自体は計器の表（未達）に移っている**。本節は履行への**注記**であって取り消しではない。〔⚠ **在位置訂正 2026-08-09 16:29**: 本行は元々「**dep-1 は discharged のまま**」と書いていた。p18 が同じ圧縮（`dep-1 = p5 discharged, gate waits on instrument + Rs` → `dep-1 (discharged)`）を自卓の欠陥として撤回し、**主語が「レグ」から「gate」へすり替わる**と指摘 ⇒ **私の行も同じ語を運んでいた**ので、主語を戻した。⭐ **原文は本注記に保存**。⛔ **挿入でなく在位置で直した理由** = 状態を述べる行は、**古い読みが最上位に残っている間ずっと正しく見える**（下に訂正を足すだけでは、そこへ到達しない読み手を救えない）。⛔ **①/② の裁定は p4** — 私は自分の語法だけを直し、gate は動かさない。〕
