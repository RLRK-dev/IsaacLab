# NOTE: pre-change V6 artifact. Runs ONLY at the I3/I4 commit PARENT (old _c1_escape_after_seat
# signature); breaks loudly at the post-fix commit -- see gate2_i3i4_v6_precheck_oldbehavior_output.txt.
"""V6 fails-without-fix: OLD (pre-I3/I4) behavior of the three new fixtures, captured at HEAD."""
import sys
...
a = np.zeros((40, 3)); a[:, 0] = 0.37; a[:, 1] = 0.30; a[:, 2] = 0.829
a[5] = (C1X + 0.020, C1Y + 0.010, 0.829)
a[6] = (C1X + 0.020, C1Y - 0.010, 0.829)

# I3(b) false escape: routed identity crossing (segs 26-27... nodes 27-28) in-lateral (dx=0) but z OUT of
# band (0.90); stray (segs 5-6) IN-band z at +70mm dev. OLD global picker prefers in-band -> stray -> 70>60 -> True.
b = np.zeros((40, 3)); b[:, 0] = 0.37; b[:, 1] = 0.30; b[:, 2] = 0.90
...
c = np.zeros((40, 3)); c[:, 2] = 0.829
for i in range(40):
    c[i, 0] = C2X + 0.060
# routed side: nodes 0..27 ascending y (node order), pin at 27 y=0.150; below-pin span descends toward C2Y
for i in range(28):
    c[i, 1] = 0.150 - (27 - i) * 0.015  # node 27 = 0.150, node 17 = 0.0, node 0 = -0.255
# feed side: nodes 28..39 descend in y from 0.150 through C2Y=0, IN-groove x
for i in range(28, 40):
    c[i, 1] = 0.150 - (i - 27) * 0.015  # node 37 = 0.0 -> crossing near seg 36/37
    c[i, 0] = C2X
c[27, 0] = C2X + 0.060  # pin node stays on routed x

e = env27()
# OLD I3 guard behavior (reads global _crossing_x_dev)
...
# OLD I4 seat behavior (both-sides walk)
dx_c, z_c = e._seat_metrics(c, nre._C2_XY)
print(f"I4 feed-drape OLD: dx={dx_c:.4f}, z={z_c:.3f}, seated={e._seated_in_groove(dx_c, z_c)} (expect True = wrong credit)")
