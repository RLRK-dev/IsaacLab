# [RS-TECH-LEAD2 → OPS-SUP-CODEX] WMSO D0 — factual read-only inventory **v4** (records fix R5–R7)

- supersedes v3 `…_v3.md` (sha `7276c6fb…`); **records-only fix per pN v3 RECORDS-ONLY HOLD (R5/R6/R7)**. node `T-WMSO`; verify `w2:pN`; custody `w2:p6`.
- prepared_at: 2026-07-18 15:14 JST · repo HEAD `86a28f1521` (advisory; as_read_sha256 is the pin) · **read-only, no run, no gate PASS**.
- pN v3 verdict: **substantive axes PASS** (commit/hash, candidate counts, trichotomy, A/B/C, R4 wording, 54-source post-bracket 0-mismatch); this v4 corrects only the 3 exact-command/record items R5/R6/R7 — all conclusions unchanged.
...
**Dedicated recovery/fallback SKILL = ABSENT_IN_DECLARED_CLOSURE** (R7: exact query case-insensitive across all 4 skill taxonomies:
`grep -rniE 'recovery|fallback' thread_isaac_lab/skills/step_table.py thread_isaac_lab/models/skill_adapter.py thread_isaac_lab/models/skill_adapter_with_prediction.py thread_isaac_lab/skills/scripted_skills.py`
→ **rc=1 (0 hits)**; SkillName enum = the 9 listed).
...
- `grep -rnE --include='*.py' '\bHz\b' thread_isaac_lab/{envs,orchestrator,skills,configs}/` → rc=0, **exactly 2 lines**: `envs/hook_hanging_env.py:989` and `:991` (derived-Hz prints in the unrelated `ManagerBasedEnv`).
- `grep -rniE --include='*.py' 'latency|deadline|real.?time|wall.?clock|hertz' thread_isaac_lab/{envs,orchestrator,skills,configs}/` → **rc=1 (0 matches)**.
- (R5 records fix: the v3 `-rncE` form (no `--include`) emitted a 0-count line for every file and matched 3 `__pycache__/*.pyc` binaries, and case-insensitive `Hz` also hit lowercase `hz` = geometry half-height e.g. `task_config.py:91 hz=2.5mm`; the `--include='*.py'` source-only form above is the exact record.) Conclusion unchanged: no wall-clock deadline exists;
...
| Skill-level anti-thrash (hysteresis/dwell/switch-penalty) | ABSENT_IN_DECLARED_CLOSURE | R6: `grep -rniE 'hysteresis\|dwell\|switch.?penalt\|thrash\|min.?switch\|cooldown' thread_isaac_lab/orchestrator/ thread_isaac_lab/skills/ thread_isaac_lab/envs/newton_route_env.py` → **rc=0, 11 hits all `newton_route_env.py` `_c1_pin_dwell`** pin-latch (unrelated) |
