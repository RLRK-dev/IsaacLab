...
| 5 / 6 | run form, converged | **CONFIRMED** | `solve_ik(t, tgt, tries=None, iters=300, seed=1, near=None, other=None, re_max=0.05, quiet=True)`; converged := returned; per-row tag none on all 34 denominator rows; `pe` re-checked on a throwaway `MjData` (kinematics only; vs mine ≤ 4.5e-13 mm L / 7.6e-13 mm R, appendix B), `re` not re-checked (stated) |
