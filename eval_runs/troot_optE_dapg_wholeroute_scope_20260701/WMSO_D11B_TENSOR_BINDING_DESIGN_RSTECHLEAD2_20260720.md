# WMSO D1.1-B `tensor_binding` — DESIGN (v5)
...
- gate 位置: §5 chain の **design**。経緯 = cycle-1 debate FAIL → v2 → pS PASS → **pN exact-pin HOLD B1-B5** → v3 → **cycle-2 debate 実施済（max-2-cycles 到達）** → v4 → pS 再 PASS → **pN exact-pin HOLD R1-R3** → 本 v5。次 = **pS 再 verify → pN exact-pin 再判定 → ⛔Rs の B1 裁定（A/A′/B）→ freeze**。**impl/training/authority = CLOSED 継続**。
