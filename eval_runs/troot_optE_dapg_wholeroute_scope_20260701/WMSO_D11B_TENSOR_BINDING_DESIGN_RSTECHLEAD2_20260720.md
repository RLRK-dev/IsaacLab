# WMSO D1.1-B `tensor_binding` — DESIGN (v13)
...
- gate 位置: §5 chain の **design**。経緯 = cycle-1 debate FAIL → v2 → pS PASS → **pN exact-pin HOLD B1-B5** → v3 → **cycle-2 debate 実施済（max-2-cycles 到達）** → v4 → pS 再 PASS → **pN exact-pin HOLD R1-R3** → v5 → pS 再 PASS（12:51） → **著者自検出 S-1 の fold**（v6）→ **pN exact-pin HOLD H1-H3** → v6.1 → **two-key 充足**（pS `251e8c8516f2…` 13:29 / pN exact-pin PASS-CLOSE 14:35、transcript `a31498dd0582…` @ `ccff616b89`）→ Rs 裁定 A′（14:39）→ v7 → v7.1 → ⛔**Rs が A′ を破棄（15:5x・逐語「誤りが前提であるならそれは当然破棄にしろ」）** → 本 **v8**。**B1 = ⛔OPEN に復帰**。次 = **訂正済みの前提で A/A′/B を Rs へ再上程 → 再裁定 → 設計 fold → pS → pN → freeze**。⚠v7/v7.1 の設計軸 PASS は無効。**v6.1 の two-key は A′ 以前ゆえ破棄の巻き添えではない**が、**その v6.1 自身が破棄の原因となった偽記述を含み、両 key がそれを通していた**（§12 v8）。**impl/training/authority = CLOSED 継続**。
- ⚠**順序規律（pN H1・著者の手順誤り）**: 本行が拘束する順序は **pS → pN の逐次**であるにもかかわらず、著者は v6 を **pS と pN へ同時 dispatch**（13:10）し順序を bypass した。pN は v6 pin を検証したが **pS の v6 readback が banked でない**ため PASS-CLOSE は成立しない。⇒ 以後 **final pin への pS addendum を bank してから pN へ回す**（並行 dispatch を行わない）。
...
3. **B 完了前**: TensorBindingSpec artifact が存在しないため、KNOWN hash を**正当に**供給できない。⚠**v1 はこれを「構造的」と称したが、正しくは「§4 の hash-identity 検査が走って初めて構造になる」**（§12 D-5）。⛔**v4 追記（cycle-2 CC2-CH2）**: その供給経路は v3 §4 で**撤回**されたため、**DC-1 の「構造性」は §10 の B-1 解決に BLOCKED**。v2 の「供給経路を §4 で指名する」という結び句は失効。→ ⛔**v8 で BLOCKED に復帰**（v7 の解除は破棄された A′ 裁定に依存していたため）。〔以下は A′ を採る場合の記述として保持〕 ~~v7 で BLOCKED 解除~~: 供給経路が確定した（`ref` = `EvidenceRecord.source_ref` / `expected_sha256` = `claim_target_hash`、§4 表）。**DC-1 は再び構造的**である — B 完了前は TensorBindingSpec artifact が存在せず、`resolve_artifact` が解決すべき実体が無いため、KNOWN hash を正当に供給する経路が**存在しない**（虚偽宣言は `E_PROOF_ARTIFACT_UNRESOLVED` で fail-closed）。⚠**構造性の根拠は「artifact 不在」であり「locator 不在」ではない**点に注意（後者は A′ で解消済）。
...
**hash / 供給（v2 追加・§12 D-5／**v7 で A′ に合わせ整理**）**: ~~`E_BINDING_ARTIFACT_UNRESOLVED`~~ → **撤回**（解決失敗は frozen **`E_PROOF_ARTIFACT_UNRESOLVED`** が担う。`claim_target_hash ≠ slot hash` は frozen **`E_PROOF_MISBOUND`**）/ `E_BINDING_HASH_MISMATCH`（**v7 で検出モードを限定**: `sha256(格納 bytes) == slot.artifact_hash` は A′ の連鎖で保証されるため、本 code の**到達可能な**失敗は **格納 bytes が canonical WCJ でない**場合＝`H_WCJ(parse(bytes)) ≠ slot.artifact_hash`。到達性負例は §7 で必須）/ `E_BINDING_HASH_MALFORMED`（64-hex 違反）。詳細と根拠 = 本節の A′ 確定形。
...
>
> ✅**保持する実測（裁定に依存しない独立事実・pS 独立追認済）**: 下表の frozen 実測（`source_ref` の必須性・全 record 存在・hash 可視性・`claim_targets` の両 slot 束縛・frozen `:396` の provenance 用途・profile 非対称・`EXPLICIT_NONE` 免除）は **Rs 裁定の産物ではなく凍結パッケージの読み取り**であり、**破棄対象ではない**。これらは**訂正済みの前提で A/A′/B を再上程するための入力**として用いる。⛔ただし**どれか 1 案を採択したことにはならない**。

**以下は「A′ を採る場合の実装形」として測定済みの内容（採択されていない）**。**裁定 = A′（frozen `EvidenceRecord.source_ref` を束縛）**、対象 = **`tensor_binding` + `normalization` 両 slot**。以下は裁定後に凍結パッケージ上で**再実測**した確定内容（採用時に load-bearing になるため、過去の主張を再利用せず閉じた query で取り直した — 本 arc で 4 度踏んだ「存在 ≠ 十分」の再発防止）:

**B-1 確定形（frozen delta なし）**:

| `resolve_artifact` 引数 | 供給元 | 根拠（frozen・実測） |
|---|---|---|
| `expected_sha256` | `EvidenceRecord.claim_target_hash` | EP JSON `policy_definition.claim_targets`: `TENSOR_BINDING → execution_bundle.tensor_binding.artifact_hash` / `NORMALIZATION → execution_bundle.normalization.artifact_hash`。`_rule` = 「claim_target_hash MUST equal the hash derived from the component's target（不一致 = `E_PROOF_MISBOUND`）」⇒ **既に凍結側で束縛済み** |
| `ref` | `EvidenceRecord.source_ref` ← **本 doc が束縛（B1 の実体）** | `WMSO_D11A_CONTRACTS_V2_DESIGN…:252` = **`source_ref: str`（Optional でない必須 field）**／`:269` `records: tuple[EvidenceRecord, ...]` ⇒ **全 component が record を持つ ⇒ 全 grade に locator が存在**（rank 3 限定だった旧表を解消）／`:551` **`source_ref` は hash 対象 field** ⇒ locator の差し替えは `evidence_bundle_hash` に現れる（tamper-evident） |

**束縛の宣言（B 側の追加規則）**: TENSOR_BINDING / NORMALIZATION の `EvidenceRecord.source_ref` は、**当該 slot の artifact 実体（TensorBindingSpec / normalization payload）を解決する ref** でなければならない。**frozen は、TENSOR_BINDING / NORMALIZATION の artifact 解決用途における `source_ref` の指示対象を規定していない**（EvidencePolicy md 出現 0 件・EP JSON 出現 0 件 — 閉じた query）ため、本束縛は **frozen schema の変更でも意味論の上書きでもなく、未規定領域への B 側の宣言**である（prereg §2 IN-4 の範囲）。⚠**v7.1 scope 修正（pS N-1）**: v7 は「frozen が **一切** 規定していない」と書いたが**over-broad**。frozen DESIGN `:396` は **registry fixture が供給する非 bundle static field の provenance** として `source_ref` を要求しており（provenance 無き供給 = `E_MIGRATE_STATICS_ABSENT`）、`source_ref` の意味論はその文脈では規定済みである。本束縛が非 frozen-delta である根拠は「**artifact 解決経路という別文脈で未規定**」であり、かつ **provenance role と整合する refinement**（どちらも「この record の内容がどこから来たか」を指す）。⚠不在主張を用途で限定せず書いた点は、本 arc で繰り返した **over-broad な absence claim** の再発（§12 v7.1）。

**適用範囲（profile 別・実測）**:

| profile | min_grade_rank | TENSOR_BINDING | NORMALIZATION |
|---|---|---|---|
| CLOSED_LOOP | 3 | required | required |
| SHADOW | 2 | **required**（← locator が無かった grade。A′ で解消） | required |
| OFFLINE_REPLAY | 2 | **免除**（`not_applicable`「replay does not re-execute binding」） | **required**（⚠**両 slot は非対称** — replay でも normalization は要る） |

加えて **slot = `EXPLICIT_NONE` の component は required set から免除**（frozen EP §3c、SCRIPTED/WAIT が該当・免除は `UsageEligibilityReport` に loud 記録）。**`UNKNOWN` slot に免除は無い**。

**error code の整理（reuse-first・AGENTS.md 再利用ゲート）**:
- ⛔**`E_BINDING_ARTIFACT_UNRESOLVED` は撤回**。解決失敗は frozen の **`E_PROOF_ARTIFACT_UNRESOLVED`**（`:285` trust boundary）が既に担う。B 側に並行の error taxonomy を作らない。
- **`E_BINDING_HASH_MISMATCH` は存置するが検出モードが狭まる**。A′ の連鎖（`sha256(resolved bytes) == claim_target_hash == slot.artifact_hash`）により「bundle が別の binding を宣言している」モードは **上流の `E_PROOF_MISBOUND` / `E_PROOF_ARTIFACT_UNRESOLVED` が捕捉**する。本 code に残る**到達可能な**失敗は **「格納 bytes が canonical WCJ でない」** 場合 — このとき `sha256(raw) == slot hash` は成り立つが `H_WCJ(parse(raw)) ≠ slot hash` となる。⚠**選択肢 A で懸念された「store 整合性検査への縮退」は A′ では起きない**（A′ は hash を key に引かないため）。**到達性の実証は impl の負例で必須**（非 canonical bytes を格納 → 本 code が発火）— 到達不能 code を残さない規律（§12 D-18 / C2-2）と、**委譲先の到達性を検証する規律（§12 R1）** の両方を本 code に適用する。v2 は「frozen `resolve_artifact(ref, expected_sha256)` を用い slot hash を expected として解決する」と書き、pS もこれを「frozen delta 非要」と判定したが、**両者とも resolver 関数の実在を確認しただけで locator の実在を示していなかった**。実測: frozen `ArtifactSlot = {state, artifact_hash}`（`WMSO_D11A_CONTRACTS_V2_DESIGN…:47` 相当行）に **ref field は無い**。`ProofItem.ref` は evidence 側の参照であり、**TensorBindingSpec 実体の ref は全 grade の proof set に共通必須ではない**。よって `resolve_artifact` に渡す `ref` の出所が設計上未定義であり、**「frozen delta 非要」は NO-PROOF**。

---

<details>
<summary>⬇ <b>以下は Rs 裁定（14:39）以前の上程資料 — 記録として保持（SUPERSEDED by A′）</b></summary>

**v3 の over-claim 撤回記録（§12 B-1）**: v2 は「frozen `resolve_artifact(ref, expected_sha256)` を用い slot hash を expected として解決する」と書き、pS もこれを「frozen delta 非要」と判定したが、**両者とも resolver 関数の実在を確認しただけで locator の実在を示していなかった**。実測: frozen `ArtifactSlot = {state, artifact_hash}` に **ref field は無い**。`ProofItem.ref` は evidence 側の参照であり、**TensorBindingSpec 実体の ref は全 grade の proof set に共通必須ではない**。よって「frozen delta 非要」は当時 NO-PROOF だった。⚠**A′ 確定後の位置づけ**: 結論「frozen delta 非要」は最終的に正しかったが、**v2 の論証（resolver の実在）では成立せず**、成立させたのは `source_ref` の必須性・全 record 存在・hash 可視性という別の実測（上表）である。**正しい結論を誤った根拠で主張していた**ことは撤回の妥当性を変えない。
...
- **同型の未解決が `bundle.normalization` にもある（v4・cycle-2 CC4-C4）**: §4 の `E_BINDING_NORMALIZER_VALUE` は normalization artifact の payload を読む必要があるが、その slot も frozen `ArtifactSlot = {state, artifact_hash}` で **ref を持たない**。B1 と同一の locator 問題であり、本 open point の対象に **`normalization` slot も含める**。→ **A′ で同時解決**（両 slot とも `source_ref` を locator に束縛）。⚠ただし profile 適用は非対称（上表: OFFLINE_REPLAY で TENSOR_BINDING のみ免除）。
...
- ⛔**fail-closed 実行コマンド（事前登録・v6.1／pN H2）— 検証は必ず以下で走らせること**:
  ```
  python3 eval_runs/troot_optE_dapg_wholeroute_scope_20260701/wmso_d11b_fixtures/build_goldens.py \
      --verify eval_runs/troot_optE_dapg_wholeroute_scope_20260701/wmso_d11b_fixtures
  ```
  **要件 = ①Python インタプリタを直接呼ぶ（wrapper 経由にしない）②Python ≥ 3.8**（`math.prod`）。builder は **stdlib のみ**に依存し（`copy` `hashlib` `re` `unicodedata` `json` `sys` `math.prod` `pathlib`）Isaac / Newton / venv を要さないため、**任意の `python3` で可搬に fail-closed**。実測（本 pin・破損 fixture 注入時の rc）: `python3` = **1** / `/usr/bin/python3` = **1** / `env_isaaclab7/bin/python` = **1** / `env_isaaclab/bin/python` = **1**、clean 時は `python3` (3.12.3) で 4/4 PASS + 24/24 fired + rc=**0**。
  ⛔**`./isaaclab.sh -p` 経由で検証してはならない**: 同じ破損 fixture で **traceback を出力しながら rc=0 を返す**（本 pin 実測）＝ **fail-open**。これは AGENTS.md「Exit-code exception」が明記する既知 hazard（guard wrapper が `env_isaaclab/bin/python` を直接呼ぶ理由）であり、**本 doc は v6 までその規約を自らの計器に適用していなかった**。⚠v5 §12 の custody 行が `./isaaclab.sh -p` 経由の観測であった点は §12 で records-fix 済（pN H3）。
...
standalone unit（配布物のみで全実行）: 型 round-trip（encode→decode→encode で同一 bytes）/ **golden G-1/G-2/G-3 一致 + G-3 挿入順 variant が別 hash になること**（判別性の positive control）/ invalid corpus 全拒否 / coverage 算術 property test（ランダム layout → `E_BINDING_COVERAGE_INVALID` 検出）/ **G-4 を含む 4 fixture の一致 test**（v4 は §7 が G-4 を落としていた — pN R2）/ **flatten_order・history.layout の判別 test**（同一 field 集合で layout だけ異なる 2 spec が別 hash になること）/ cross-artifact 検査の赤→緑 pair（**v7: HASH_MISMATCH の条件付き扱いを解除** — A′ 確定により経路が定まったため成立。⚠**到達性の負例を必須化**: 非 canonical bytes を格納し `sha256(raw) == slot hash` を保ったまま `H_WCJ(parse(raw)) ≠ slot hash` となる case で本 code が発火すること／NORMALIZER_MISSING/ORPHAN/VALUE・NORM_COHERENCE・CONTROL_MODE_MISMATCH・LINEAGE_MISMATCH・IDENTICAL_HASH_PRESENT）/ **`source_ref` locator の赤→緑 pair**（解決不能 ref → frozen `E_PROOF_ARTIFACT_UNRESOLVED`／`claim_target_hash ≠ slot hash` → frozen `E_PROOF_MISBOUND`。**B 側 code でなく frozen code が発火することを確認** = reuse の positive control）/ Draft（slot UNKNOWN 混在）で cross-artifact 検査が発火しないこと。
...
- **v5**（2026-07-20 12:40 実測）— **pN exact-pin HOLD R1-R3 の fold**（custody/evidence は PASS: pN が隔離 archive + `./isaaclab.sh -p` で `--verify` の非書込・4/4 hash 再現を確認）。⚠**records-fix（v6.1・pN H3）**: 本行は当初「`--verify` で **negative control 全発火**を確認」と記載していたが、**v5 の code では構造上あり得ない**。v5 blob（`96f92af7c8`）の `main()` は verify 分岐が **`return` で抜けた後**に negative control ループを置いており（verify 分岐 = `if verify:` … `return`／ループはその後方）、**`--verify` 経路の発火数は 0** である。⇒ 当該観測は**生成経路**由来か転記誤りであり、`--verify` 由来としては成立しない。本行を「verify 経路 = 発火 0（生成経路のみ）」へ訂正（:285/:362 の v6 訂正と整合）。**教訓**: 私は peer の custody 報告を、自分の code の構造と突き合わせずに版歴へ転記した — 報告は on-disk 構造で検算してから記録する。
...
- **v6.1**（2026-07-20 13:17 実測、本版）— **pN exact-pin HOLD H1-H3 の fold**（custody = PASS: pN が `81f33ceefd` の DESIGN/builder/pS record 3 sha + fixture 4 sha を全一致確認、隔離 archive で `--verify` の 4/4 PASS + 24/24 fired + 前後 sha 不変 + 生成経路の同一 bytes を実測。R1 cast 禁止 = PASS、R3/S-1 の guard 配線 = mechanism PASS）:
  - **H2（CRITICAL 相当・fail-open）** §6 の `rc=1` 主張に **interpreter/entrypoint の指定が無かった**。実測: 破損 fixture に対し `python3` / `/usr/bin/python3` / `env_isaaclab7/bin/python` / `env_isaaclab/bin/python` は **rc=1** だが、**`./isaaclab.sh -p` は traceback を出しつつ rc=0**（= fail-open）。⇒ **fail-closed コマンドを §6 に事前登録**（素の `python3` 直呼び・Python ≥3.8・stdlib のみ依存ゆえ可搬）+ **`./isaaclab.sh -p` 経由の検証を明示禁止**。**自己評価**: これは AGENTS.md「Exit-code exception」が**既に文書化している既知 hazard**（guard wrapper が `env_isaaclab/bin/python` を直接呼ぶ理由そのもの）であり、**当該規約を常時ロードしていながら自分の計器に適用しなかった**。S-1（経路の被覆を見なかった）と同じ「到達性/経路の軸を見ない」盲点が、今度は**インタプリタ境界**で出た形。
  - **H3（records 矛盾）** §12 の v5 custody 行が「`--verify` で negative control 全発火を pN が確認」と記載する一方、§6 の v6 訂正は「v5 まで verify 経路の発火は 0」と述べており**同時成立不能**。**構造的に決着**: v5 blob（`96f92af7c8`）の `main()` は verify 分岐を `return` で抜けた**後**に negative control ループを置いており、`--verify` 経路の発火数は **0** で確定。⇒ v5 行を「生成経路のみ／verify 経路 = 未発火」へ records-fix。**自己評価**: peer の custody 報告を**自分の code の構造と突き合わせずに**版歴へ転記した。
  - **H1（順序 bypass）** :6 が拘束する順序は pS → pN の逐次だが、著者は v6 を **両者へ同時 dispatch**（13:10）した。pN の v6 検証は成立するが **pS の v6 readback が未 bank** ゆえ PASS-CLOSE 不成立。⇒ :6 に順序規律を明記し、**以後 final pin への pS addendum を bank してから pN へ回す**。
  - **B1 は依然 OPEN**（Rs 裁定 A/A′/B × `tensor_binding` + `normalization`）。**fixture 4 sha は v6.1 でも不変**（変更は doc の header/§6/§12/:6/:318 と、code 変更なし）。pN「追加 debate 不要」。
- ⛔**v7**（2026-07-20 14:43 実測）— ~~Rs 裁定 A′ の fold~~ **VOIDED（v8・Rs が裁定を破棄）**。以下は破棄された fold の記録（裁定受領 14:39。**v6.1 で two-key 充足済み**: pS `251e8c8516f2…` @ `ed60396e88` / pN exact-pin PASS-CLOSE transcript `a31498dd0582…` @ `ccff616b89`）:
  - **B1 CLOSED**: `resolve_artifact(ref, expected_sha256)` の両引数が確定 — `expected_sha256` = `claim_target_hash`（**凍結側で既に束縛済み**: EP JSON `claim_targets` が両 slot を `execution_bundle.*.artifact_hash` に固定・不一致 = `E_PROOF_MISBOUND`）／`ref` = **`EvidenceRecord.source_ref`（本 doc が束縛）**。
  - **裁定後に凍結パッケージ上で再実測**（過去の主張を再利用しない — 「存在 ≠ 十分」の 5 度目を防ぐため）: `source_ref: str` は **Optional でない必須 field**（frozen `:252`）／`records: tuple[EvidenceRecord, ...]` で**全 component が record を持つ**（`:269`）⇒ **全 grade に locator が存在**（rank 3 限定だった旧表を解消）／`source_ref` は **hash 対象**（`:551`）⇒ locator 差替が `evidence_bundle_hash` に現れる。**frozen は `source_ref` の指示対象を一切規定していない**（EP md / EP JSON とも出現 0 — 閉じた query）⇒ 本束縛は **未規定領域への B 側宣言であり frozen delta でない**。
  - ⚠**profile 適用の非対称を実測で発見**: OFFLINE_REPLAY は **TENSOR_BINDING を免除**するが **NORMALIZATION は required**（`not_applicable` = CONTROL_MODE / TENSOR_BINDING / TRAINING_DATASET / TRAINING_PROVENANCE）。**両 slot を「同型」と一括りにしていた v4-v6.1 の記述は不正確**であり、§4 の表で分離した。
  - **error code の reuse-first 整理**: `E_BINDING_ARTIFACT_UNRESOLVED` を**撤回**（frozen `E_PROOF_ARTIFACT_UNRESOLVED` が担う — B 側に並行 taxonomy を作らない）。`E_BINDING_HASH_MISMATCH` は**存置するが検出モードを「格納 bytes が canonical WCJ でない」に限定**（他モードは上流の frozen code が捕捉）。**A で懸念された store 整合性検査への縮退は A′ では起きない**。⚠**到達性の負例を §7 に必須化**（R1 の教訓 = 委譲先/残存 code の到達性を検証する）。
  - **DC-1 の BLOCKED 解除**（§2）: 構造性の根拠を「**artifact 不在**」と明示（「locator 不在」ではない — 後者は A′ で解消）。
  - ⚠**v5 以来はじめて設計 semantic 節（§2/§4/§7/§10）に触れた版** ⇒ **v6.1 の two-key PASS は v7 を覆わない**。pS → pN の逐次で再走してから freeze（H1 の順序規律を継続適用）。
  - **自己評価**: v2 の「frozen delta 非要」という**結論は最終的に正しかった**が、当時の**論証（resolver 関数の実在）では成立していなかった**。A′ を成立させたのは `source_ref` の必須性・全 record 存在・hash 可視性という**別の実測**である。**正しい結論を誤った根拠で主張していた**ことは v3 の撤回の妥当性を変えない（§4 に記録）。
- ⛔**v7.1**（2026-07-20 15:08 実測）— **VOIDED（v8）**。~~pS の v7 PASS-WITH-CONDITIONS の条件処理~~（pS record `a4620ac0449c…` @ `23fac01ec9`。pS は A′ 全前提を frozen 上で独立実測し、**profile 非対称を独立追認**した上で「両 slot 同型」とした自身の v4-v6.1 PASS を own）:
  - **N-1（scope 修正・§4）**: v7 の「frozen は `source_ref` の指示対象を**一切**規定していない」は **over-broad な不在主張**だった。実測: frozen DESIGN `:396` は **registry fixture が供給する非 bundle static field の provenance** として `source_ref` を要求している（provenance 無き供給 = `E_MIGRATE_STATICS_ABSENT`）。⇒ 主張を「**TENSOR_BINDING / NORMALIZATION の artifact 解決用途において未規定**」へ限定し、A′ が provenance role と**整合する refinement** である旨を明記。**非 frozen-delta という結論は不変**。⚠**自己評価**: 不在主張を用途で限定せず書いた — 本 arc で繰り返した型（[[feedback-absence-claims-need-closed-query-and-moving-tree-provenance-2026-07-18]]）の再発。EP md/JSON だけを閉じた query の対象にし、**frozen DESIGN 本体を同じ query に含めなかった**ことが原因。
  - **C-1（custody）**: **Rs の A′ 裁定を独立 file として bank** — `WMSO_RS_B1_RULING_APRIME_20260720.md`。⚠**裁定は構造化選択（single-select）で下されたため Rs の自由記述 prose は存在しない**。同 file は「提示した問い + 選択肢文言 + Rs が選んだ選択肢」を逐語保持し、prose 引用を捏造しない旨を明記。**併せて、問いの前置き「両 slot に同型で効きます」が後の実測で不正確（profile 非対称）と判明した事実も loud に記録**（Rs は不正確な前置きを含む問いに裁定した）。
  - **C-2（records freshness）**: LEDGER D1.1 行 + **DDR #27**（B1 BLOCKED / 「normalization も同型」）を **B1 CLOSED / A′ / profile 非対称**へ反映依頼 = p6 へ dispatch（#27 は **D1.1-C の [DEFER-RECON] を gate** するため優先）。
  - 設計 semantic の**追加変更なし**（N-1 は主張の scope 限定であり束縛内容は不変）。builder / fixtures 不変。
- **v8**（2026-07-20 16:0x 実測、本版）— ⛔**Rs が A′ 裁定を破棄**。**逐語「誤りが前提であるならそれは当然破棄にしろ」**:
  - **破棄の連鎖**: A′ 裁定は、CC が提示した問いに**不正確な前置き**（「両 slot に同型で効きます」）が含まれた状態で下された。⇒ **B1 = ⛔OPEN に復帰**・v7/v7.1 の A′ fold 無効・pS の v7/v7.1 設計軸 PASS 無効・pN exact-pin 中止（15:55 dispatch）・LEDGER DDR #27 は p6 が差し戻し済（`71622f65c9` + `93db26a737`。⚠STOP が間に合わず `470ef27c5e` が一度着地していた）。破棄裁定の記録 = `WMSO_RS_B1_RULING_APRIME_20260720.md`（⛔VOID・引用不可）。
  - ⛔**CC 側の判断の誤り（最重要）**: 私と pS は事後に「不正確部分は A/A′/B の選択に **orthogonal** ゆえ裁定は STANDS」と判定し、Rs には「軽い確認」を求めるにとどめた。**この判定自体に standing が無い** — **誤った前提を提示した側が、その誤りが相手の判断に効かなかったと決めることはできない**。orthogonality 分析が技術的に妥当かどうかは結論を変えない（判断の権威は Rs にある）。pS も同旨を own し、番人 default を「**偽前提下の裁定 = 即 tainted・正しい前提で再上程・結論を pre-judge しない**」へ訂正（pS §23）。
...
- **v12**（2026-07-20 22:5x 実測、本版）— **freeze 直前の records-fix 1 件（設計 semantics 不変）**:
  ...
  - **freeze 時に残る declared open（隠さず宣言）**: ①`stats_key` 一意性（per-feature 一意か共有可か・§1.2 / §10）— ⚠**v13 で精緻化**: **length 不一致での共有は fail-closed**（`E_BINDING_NORMALIZER_VALUE` が発火）だが、**同一 length での共有は検出 code が存在しない**（normalizer 系は `MISSING` / `ORPHAN` / `VALUE` のみ・重複/一意性 code は 0 件）⇒ **一意必須と読むなら silent-pass**。author 起因の correctness gap であり contract 層の hole ではないが、**certificate は通る**②§7 の到達性負例（impl leg）③§5 の必須化可否・class 台帳・判定器・窓幅（slice 詳細 prereg + Rs 裁定）④U-2 / U-5 / U-6（D1.1-C prereg の必須入力・DDR 登録済）。**D1.1-A も §10 に open を残したまま freeze しており、declared open を伴う freeze は本 project の確立した形**。
- **v13**（2026-07-20 23:0x 実測、本版）— **pS §30 の non-blocking precision を fold（records-only・設計 semantics 不変）**:
  - ⛔**私の過剰主張を訂正**: v12 は declared open ① を「**どちらの読みでも fail-closed**」と書いたが、これは **length 不一致の case にしか当てはまらない**。**同一 length で `stats_key` を共有する case を覆っていなかった** — 実測: normalizer 系 code は `MISSING` / `ORPHAN` / `VALUE` の 3 つのみで、**重複/一意性を検出する code は 0 件**（`E_BINDING_DUPLICATE_FIELD` は field 用・`E_BINDING_BELIEF_DUPLICATE` は belief 用でいずれも stats_key を覆わない）。⇒ **一意必須と読むなら同一 length 共有は silent-pass**。
  - **正しい risk 記述**: 「length 不一致 = fail-closed ／ 同一 length 共有 = 一意必須読みで **silent-correctness gap**（author 起因・contract 層の hole ではないが **certificate は通る**）」。⇒ 誤った normalizer を宣言しても identity hash は valid・certificate も発行され、mis-normalized な観測で policy が走り得る。**§1.2 で一意性規則を定めれば閉じる**。
  - ⚠**自己評価**: 「fail-closed」は**安全側の語**であり、それを 2 つの case のうち 1 つでしか成り立たないのに全称で書いた。**declared open の risk 記述は、open を残したまま freeze する判断の入力になる**ため、ここでの過剰主張は freeze 判断そのものを汚す。**検出者 = pS（自検出ではない）**。
  - ⚠**freeze 可否は変わらない**（pS も「freeze は止めない」と明示）— 変わるのは **Rs に渡す risk の記述**である。
