...
| **guards** | action-path span 保護 = **(b′) phase-conditional** (dual-grip = 両腕把持全域 {G1〜G3 C1-seat/unclamp直前}∪{G4〜G6} = common-mode 部分空間 hard-project [構造 enforcement, INV#2 as-executed; 3走目 ISSUE-1 = G3 gap 訂正, gate=grip-schedule∧contact-flag] / 非 dual-grip 再把持 transit = per-arm 許容); span **監視** = dual-grip phase のみ informative-tier (92.4mm center; hard-terminate 化は P3 window 後 design-gate=f2); differential-drift regression (DoD⑩); mujoco-backend assert (VBD-residue guard) | ~90 | AR span-assert :306 pattern |
