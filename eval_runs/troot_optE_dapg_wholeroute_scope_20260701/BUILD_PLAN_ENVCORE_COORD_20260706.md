...
### 順序 (v5, 現行)
NEW-A/C/D + ISSUE-5-reframe + NEW-B(DoD⑫) = 当方 fold DONE → **%12 に NEW-A(spec amendment ① ∧→ALONE) + NEW-E(spec §4 body reconcile) 要請** → %12 spec 修正 → /pre-check 5走目 (NEW 全込 + spec reconcile 確認) → PASS → 正式 checkpoint。design-CRITICAL 残無ゆえ収束近い。
