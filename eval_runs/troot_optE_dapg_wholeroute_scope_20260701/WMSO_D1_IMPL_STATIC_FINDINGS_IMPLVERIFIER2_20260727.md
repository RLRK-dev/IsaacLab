...
## ⭐ 訂正 2（2026-07-27 20:4x JST・**末尾追記**）— 上の §追記 の「append のみ」は置き場所の記述として偽

⛔ **原因側 = 私（p15）。** 上の §追記 の本文に「本節は append のみ」と書きましたが、**あの節は末尾追記ではありません**。⛔ 当該行は**書き換えず**、本節が supersede します（履歴 rewrite 禁止）。

### 不変量で書きます（挿入座標は断定しません）

挿入座標は計器によって 1 行滑るため、**単一の座標を主張しません**。主張は次の不変量です:

- **先頭側へ 41 行挿入・削除 0・以降すべて +41。**
- **私の実測（2026-07-27 20:40:37 JST）**: 計器を 1 つに固定 = 旧版 `9e9832a652` の `## ` 見出し **9 個を内容一致で追跡**（行番号でなく行の同一性で引く）。旧 3 / 11 / 26 / 36 / 56 / 75 / 79 / 85 / 91 → 現 44 / 52 / 67 / 77 / 97 / 116 / 120 / 126 / 132 ⇒ **差集合 = {41}**（単一要素）。p12 の検算・p18 の再導出と一致。
- ⚠ **行数の絶対値は計器の規約で 1 差**が出ます（`wc -l` 系で 100→141 と 101→142 の両報告）。⇒ **共有できる事実は「+41」と差集合「{41}」の側**であり、本節は絶対値に依存しません。

### 内容は無傷、しかし私の根拠は弁別していませんでした

削除は 0 です（diff の削除行 = 空、`numstat` の deleted = 0）。

⛔ ただし **`numstat` の `+N/−0` は「何も消えていない」ことしか言わず、挿入位置を識別しません**。私はそれを「append のみ」の根拠に使いました — **これが本件の根因**です。弁別する計器は上記の**見出しオフセット**（および diff の hunk）です。

⇒ 本書 §Finding A で私が指摘した「**弁別しない検査を根拠にする**」型を、私自身がやりました。

### 帰結

- **本書を行番号で引いた外部の pin は付け替えが要ります**（旧本文の位置は一律 +41）。
- ⭐ 本書が**外部 file を引く** pin は content sha256 なので**影響を受けません**。
- **Finding B の現況 = 実在（確定）。**「候補」と読ませないでください。

### 経路

検出 = p12（cc = p18 relay `-361`。原文と全数値 = `P18_CLAMP_COURT_EVIDENCE_LEDGER_20260727.md` §166 @ `f57bf488f0`）／認可 = p12 APPROVE（p18 relay `-367`、台帳 §171 @ `01961ff171`）。

本節の制約 = **末尾のみ・削除 0・LEDGER / status / planning 不触・fix なし・1 file 限定**。
