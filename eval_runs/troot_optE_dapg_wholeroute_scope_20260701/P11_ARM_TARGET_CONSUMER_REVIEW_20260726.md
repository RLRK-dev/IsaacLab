...
⚠⚠ **bank 後に届いた `B14`〜`B44` は §5.4 に別列挙する。⛔ 上の「9 通」に算入しない**（＝ **count を再帰的に増やさない**・pN 指示）。 `MSG-PN-P11-FINGERTIP-MATERIALS-V3-RETURN-20260726-004` の **残件 B6** ／ `MSG-PN-P11-FINGERTIP-B6-LIVEWORD-20260726-005` ／ **B7**（records coherence）／**B8**（比の根拠外し）／**B9**（再測定不要の撤回）／**B10**（方向断定の撤回）／**B11**（全桁 SHA・未読対象）／**B12**（sizing 点の統一）／**B13**（command の型）。**R1〜R5 は `-004` で PASS 済**。
...
## 5.4 ⭐ **bank 後の訂正**（**B14〜B44**）— ⛔ **上の 9 通の count には算入しない**（別列挙・再帰的に増やさない）
...
| **B38** | H-4 着地の active な evidence pin が **10 桁の短縮形**のままだった（⛔ **B44 受理: 欠陥の説明で短縮 token を再掲しない** — 再掲すると本行自身が走査に当たり「残り 0」が偽になる） | ⇒ **`908ac4674576c3b936fe66866254d17691b6cc8e` へ全桁化**。routing 側は `35dccdd214e5b2770776fa742dfd74bcc13ef219` で対応済。⭐ **query の scope と結果を literal に**（⛔⛔ **C2 受理で訂正** — 旧記載は **0 件という結果を「7〜39 桁 hex」という一般 pattern に帰していた**が、その pattern では **0 ではない**）:
  - ✅ **target 固有の bounded query（0 件はこちら）**: `grep -cP '(?<![0-9a-f])908ac46745(?![0-9a-f])'` を **本材料 / 測定 spec / 設計 doc** に実行 ⇒ **3 file とも `0`**。
  - ⚠ **一般 pattern は 0 ではない**: `grep -cP '(?<![0-9a-f])[0-9a-f]{7,39}(?![0-9a-f])'` は **合計 42 matching lines**、`grep -oP … | wc -l` で **60 occurrences**（pN の独立計数と一致）。**内訳はほぼ日付（`20260726` 等）と実数の断片**であり、**commit の短縮形ではない** ⇒ **一般 pattern は本件の述語として不適**。
  - ⛔ **`head` / `tail` を通していない。** ⛔ **一般 query の件数と target query の 0 を混同しない。**⚠ **version-history label（`c8c326e00b` 等の版歴の照合記録）は対象外・書換えていない**（pN 指示どおり）。⛔ **partial SHA は historical 行であっても exact pin として使わない** |
...
| **B44** | ⛔ **自己言及と、述語の取り違えで「残り 0」が偽になっていた** | ①材料 B38 行が **短縮 token を自ら再掲したまま「残り 0」** と主張 ⇒ **token を再掲せず「10 桁 shorthand」と語で書く**。②⛔⛔ **C2 受理**: **0 件という結果を「7〜39 桁 hex」という一般 pattern に帰していたが、その pattern では 0 ではない**。⇒ ⭐ **target 固有の bounded query に訂正**: `grep -cP '(?<![0-9a-f])908ac46745(?![0-9a-f])'` = **3 file とも 0**／一般 pattern `[0-9a-f]{7,39}` は **42 matching lines / 60 occurrences**（pN の独立計数と一致・**内訳はほぼ日付と実数の断片**）。⛔ **両者を混同しない。** ⛔ **partial SHA は historical 行でも exact pin として使わない** |
