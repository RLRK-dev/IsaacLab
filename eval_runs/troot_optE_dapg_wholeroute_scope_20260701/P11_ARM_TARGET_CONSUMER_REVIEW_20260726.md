...
| **B19** | 設計 doc の「**J-a と J-b は両腕とも約 127〜133 mm/rad 違う**」 | ⛔ **撤回**。**banked 表からの引き算 = arm0 `\|649.34 − 529.27\| = 120.07` ／ arm1 `\|224.26 − 310.65\| = 86.39` mm/rad**（**両腕で同程度ではない**）。⚠ **`127〜133` の出所を私は特定できなかった**（h2 report にも無く、J-c との差 `55.75`/`51.49`/`175.82`/`34.90` とも不一致）⇒ **出所不明のまま撤回**。⚠ **この引き算も proxy 上の算術**であり **bar への帰結は導かない** |
