...
**⭐ 本 v4 が応答する RETURN（**9 通を 1 つの最終 bundle に fold**）:** ⚠ **記録訂正（pN **B14** 受理・2026-07-26 18:31:33 JST）**: `d02385cca42e93c72a1aa92b60890ca1c495a8b8` の本行は **「8 通」と書いていたが、列挙は 9 要素**だった（`-004`(B6) / `-005` / B7 / B8 / B9 / B10 / B11 / B12 / B13）。⇒ **数え違いは私の欠陥。9 通に訂正。** ⛔ **その旧 commit（本行冒頭に全 40 桁で pin 済）は immutable のまま保全**（⛔ **`-017` B1 受理: 短縮 token を再掲しない** — 4 桁 + `…` は `-015` の sweep でも捕捉できないので、**語で指す**）。
⚠⚠ **bank 後に届いた `B14`〜`B44` ＋ `-013` の `R1` / `R3` ＋ `-014` ＋ `-017` の `B1` / `B2` は §5.4 に別列挙する（**`-017` B1 / B2 は該当箇所へ inline に fence した**）。⛔ 上の「9 通」に算入しない**（＝ **count を再帰的に増やさない**・pN 指示）。 `MSG-PN-P11-FINGERTIP-MATERIALS-V3-RETURN-20260726-004` の **残件 B6** ／ `MSG-PN-P11-FINGERTIP-B6-LIVEWORD-20260726-005` ／ **B7**（records coherence）／**B8**（比の根拠外し）／**B9**（再測定不要の撤回）／**B10**（方向断定の撤回）／**B11**（全桁 SHA・未読対象）／**B12**（sizing 点の統一）／**B13**（command の型）。**R1〜R5 は `-004` で PASS 済**。
...
| 14 | D-1 の evidence command に **10 桁 shorthand の SHA** を使ったこと（⛔ **C3 受理: 欠陥説明で partial token を再掲しない**）／§5.1 に残っていた「**T-4〜T-8 の行単位の意味 = 未測**」 | **B11**。⇒ command を **全 40 桁 `1a2b63450b312bac6aa7468d60422056d40d02ab`** へ。**未読対象は tier ではなく §R4-b の D-2 の 21 file**（D-1/D-2 分割と整合） |
...
## 5.4 ⭐ **bank 後の訂正**（**B14〜B44 ＋ `-013` R1 / R3 ＋ `-014` ＋ `-017` B1 / B2**）— ⛔ **上の 9 通の count には算入しない**（別列挙・再帰的に増やさない）
...
| **B25** | 設計 doc の **N5 以外**に残っていた無条件の把持方向の主張 | ⛔ 3 箇所を撤回/降格: ①「**楽観方向の要因が 3 つ重なる／厳しくなることはあっても緩くならない**」（②把持慣性は **B16 と競合**ゆえ「楽観側」と数えられない。⭐**B35 → B40/B41 で 3 項の scope を確定** = **①は「1 姿勢の標本」まで（⛔ 現行数値は狭い箱の *最大* ではないので、部分集合と最大の関係を当てない = B40。`ζ_min` は最小固有値側ゆえ最大の議論を流用しない）**／**②は方向・量とも UNVERIFIED**／**③は入力・実行可能集合の側でのみ方向が言える（集合の包含 ＋ `a_max` が `cap` に単調増加）が、合成 task metric は UNVERIFIED = B41**／**合成した task・requirement 上の全体方向は結論しない**）②「**単調枝の条件 `β·α < ζ_free²` は撤回しない構造**」（`α` が未検証仮定に依存 ⇒ **条件付きモデルへ降格・active 判定に使わない**）③「**真の余裕は ×2.09〜×8.16 の間**」（上端が未検証仮定に依存）⇒ ⭐ **残るのは `×2.09` のみ**。⛔⛔ **その scope（B30 を同じ行に fold = B37）**: **`×2.09` が保証なのは「宣言された free-arm / 現姿勢のスカラーモデルの中だけ」**（`λ_max(M)=2.393` に対する `5.0` の余裕）。⛔ **掴んだ cable に対する保証・「真の余裕」としては使わない**（grasped 側は `M`/`c`/`K`/`ζ` とも UNVERIFIED）。「把持慣性も同じスカラー」も **`ΔM` が PSD・`c`/`K` 不変**の仮定つきに明示 |
...
| **B31** | routing artifact の immutable chain が **短縮 SHA（8 桁）**／footer の全桁一覧が **直近 parent を 1 つ落としていた** | ⇒ **両方を全 40 桁・同一の 6 commit へ同期**（`35dccdd214e5b2770776fa742dfd74bcc13ef219`）。⚠ **当時 pN が bank を routing 1 path に限定していたため、本表には行が無かった**（私はその不整合を提出 message で surface した）⇒ **本表に行を追加した**。📎 **HISTORICAL（`-017` B2 受理）**: **範囲表記を `B14〜B38` に揃えたのは `e56fb3cd1cefad9754c01a29fad55d607dadca41` 時点の措置**であって **current range ではない**。⛔ **current range は §5.4 見出しが正**（本版では `B14〜B44 ＋ -013 R1 / R3 ＋ -014 ＋ -017 B1 / B2`）。**当時の値と混同しない** |
...
| **B35** | 設計 doc の隣接矛盾（「①③は向きが言える」 vs 「向きについて何も結論しない」） | ⇒ **1 つの整合した current state に揃えた**（**B40 / B41 で精密化**）: **①は「1 姿勢の標本」まで**（⛔ 現行数値は狭い箱の *最大* ではないので部分集合と最大の関係を当てない。⛔ `ζ_min` は最小固有値側ゆえ流用しない）／**②は方向・量とも UNVERIFIED**（B16）／**③は入力・実行可能集合の側でのみ方向が言える**（集合の包含 ＋ `a_max` の `cap` 単調性）**が合成 task metric は UNVERIFIED**／⇒ **合成した task・requirement 上の全体方向は結論しない**。**旧行は「削除の記録」だけにして current state を重複させない** |
...
| **B38** | H-4 着地の active な evidence pin が **10 桁の短縮形**のままだった（⛔ **B44 受理: 欠陥の説明で短縮 token を再掲しない** — 再掲すると本行自身が走査に当たり「残り 0」が偽になる） | ⇒ **`908ac4674576c3b936fe66866254d17691b6cc8e` へ全桁化**。routing 側は `35dccdd214e5b2770776fa742dfd74bcc13ef219` で対応済。⭐ **query の scope と結果を literal に**（⛔⛔ **C2 受理で訂正** — 旧記載は **0 件という結果を「7〜39 桁 hex」という一般 pattern に帰していた**が、その pattern では **0 ではない**）:
  - ✅ **target 固有の bounded query** — ⛔ **`-013` R1 受理: 短縮 literal を artifact に置かない**ので、**全桁から shell で切り出して pattern に渡す**形で記録する（⛔ **`-014` 受理: placeholder 入りを literal と称さない** = **B13 の再導入だった** ⇒ **exact repo path を 3 件とも列挙する**）:
    - `full=908ac4674576c3b936fe66866254d17691b6cc8e`
    - `target=${full:0:10}`
    - `grep -cP "(?<![0-9a-f])${target}(?![0-9a-f])" eval_runs/troot_optE_dapg_wholeroute_scope_20260701/P11_ARM_TARGET_CONSUMER_REVIEW_20260726.md eval_runs/troot_optE_dapg_wholeroute_scope_20260701/ARM_CONTROL_MEASUREMENT_HARNESS_SPEC_V1_ARMCONTROLDESIGN_20260721.md eval_runs/troot_optE_dapg_wholeroute_scope_20260701/ARM_CONTROL_CONTROLLER_DRIVEN_ROUTE_DESIGN_V1_ARMCONTROLDESIGN_20260721.md`（**producing commit の内容に対して実行する**ため、実測は `git show <commit>:<path>` で取り出した 3 file に対して行う）
  - ⚠ **結果は producing commit に対し post-bank で実測し、提出 message に載せる**（本書は自身の commit を pin できない）。📎 **parent `81779f2a3ec58c0f472ce6e0e9abf1065d6d03e9` での私の実測 = 材料 `2` / 測定 spec `0` / 設計 doc `0`** — **材料の `2` は本行と B44 行が command 内に短縮 literal を再掲していたため**（= `-013` R1 の欠陥そのもの）。
  - ⚠ **一般 pattern は 0 ではない**: `grep -cP '(?<![0-9a-f])[0-9a-f]{7,39}(?![0-9a-f])'` を**上と同一の exact 3 path**（材料 `P11_ARM_TARGET_CONSUMER_REVIEW_20260726.md` / spec `ARM_CONTROL_MEASUREMENT_HARNESS_SPEC_V1_ARMCONTROLDESIGN_20260721.md` / 設計 `ARM_CONTROL_CONTROLLER_DRIVEN_ROUTE_DESIGN_V1_ARMCONTROLDESIGN_20260721.md`・いずれも `eval_runs/troot_optE_dapg_wholeroute_scope_20260701/` 配下）に実行 ⇒ **parent `81779f2a3ec58c0f472ce6e0e9abf1065d6d03e9` 実測で 20 + 9 + 15 = 44 matching lines**、`grep -oP … | wc -l` で **23 + 19 + 20 = 62 occurrences**。**内訳はほぼ日付（`20260726` 等）と実数の断片**であり、**commit の短縮形ではない** ⇒ **一般 pattern は本件の述語として不適**。⛔ **旧記載「42 / 60」は撤回** — あれは **pre-bank 値を current として書いたもの**（`-013` R1）。
  - ⛔ **`head` / `tail` を通していない。** ⛔ **一般 query の件数と target query の件数を混同しない。**⚠ **version-history label（`c8c326e00b` 等の版歴の照合記録）は対象外・書換えていない**（pN 指示どおり）。⛔ **partial SHA は *scoped active exact-pin surface* で pin として使わない**（⛔ `-013` R3 受理: **「全 artifact に短縮 token が 1 件も残っていない」とは言わない** — version-history 例は残る） |
...
| **B44** | ⛔ **自己言及と、述語の取り違えで「残り 0」が偽になっていた** | ①材料 B38 行が **短縮 token を自ら再掲したまま「残り 0」** と主張 ⇒ **token を再掲せず「10 桁 shorthand」と語で書く**。②⛔⛔ **C2 受理**: **0 件という結果を「7〜39 桁 hex」という一般 pattern に帰していたが、その pattern では 0 ではない**。⇒ ⭐ **target 固有の bounded query に訂正**（**command は上の B38 行に literal で記録・短縮 token は再掲しない** = `-013` R1）。**件数は producing commit に対する post-bank 実測を提出 message に載せる**。📎 **parent `81779f2a3ec58c0f472ce6e0e9abf1065d6d03e9` 実測**: target 固有 = 材料 `2` / spec `0` / 設計 `0`／一般 pattern `[0-9a-f]{7,39}` = **44 matching lines / 62 occurrences**（pN の独立計数と一致・**内訳はほぼ日付と実数の断片**）。⛔ **両者を混同しない。** ⛔ **partial SHA は scoped active exact-pin surface で pin として使わない** |
| **B42** | ⛔ 本材料 `:5` の current range が `B14〜B30` のまま（§5.4 の範囲と不一致） | ⇒ **`B14〜B44` に同期**（⛔ **§5.3 / 各行の *当時の事実* は置換しない**） |
| **`-013` R1** | ⛔⛔ **自己言及の再発 ＋ pre-bank 値を current として記載** | ①**B38 / B44 の command が短縮 token を自ら再掲**していたため、**bounded target query が本材料で `2`**（＝「残り 0」を支える面が自分自身に汚されていた）。⇒ **command を `full=…` ＋ `target=${full:0:10}` の形に置換し、artifact に短縮 literal を置かない**。②旧「**42 matching lines / 60 occurrences**」は **bank *前* に測った値**で、**producing commit `81779f2a3ec58c0f472ce6e0e9abf1065d6d03e9` の post-bank 実測は 44 / 62**（pN の独立計数と一致）⇒ **旧値は撤回**。⭐ **規則 = 件数は必ず producing commit に対し post-bank で測る／self-pin できない値は提出 message に置く／pre-bank 値を current と書かない** |
| **`-013` R3** | ⛔ 提出 message の **overclaim**（「abbreviated commit token は残っていない」「残りは日付と実数のみ」） | **FALSE**。`81779f2a3ec58c0f472ce6e0e9abf1065d6d03e9` の bounded sweep には **本材料の `c8c326e00b`（version-history label）と target literal**、**routing の target literal** が残っていた。⇒ ⭐ **正確形 = 「*scoped active exact-pin surface* で partial SHA を pin として使わない」**。⛔ **「全 artifact で 0 件」とは言わない**（version-history 例は保持・書換えない） |
| **`-014`** | ⛔ **placeholder 入りを「literal に記録した command」と称していた**（**B13 で撤回済の欠陥の再導入**・commit 前に pN が捕捉） | `-013` R1 の訂正で置いた command の対象が **`<本材料> <測定 spec> <設計 doc>` という placeholder** のままなのに、B44 行が「**command は B38 行に literal で記録**」と書いていた。⇒ **B38 行の command に exact repo path を 3 件とも列挙**（`eval_runs/troot_optE_dapg_wholeroute_scope_20260701/` 配下の材料 / 測定 spec / 設計 doc）。⭐ **教訓 = 「literal」と書けるのは、そのまま実行できる文字列だけ。template なら template と呼ぶ** |
