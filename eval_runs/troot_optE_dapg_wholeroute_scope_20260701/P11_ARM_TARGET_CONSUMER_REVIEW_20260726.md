...
| **B38** | H-4 着地の active な evidence pin が **10 桁の短縮形**のままだった（⛔ **B44 受理: 欠陥の説明で短縮 token を再掲しない** — 再掲すると本行自身が走査に当たり「残り 0」が偽になる） | ⇒ **`908ac4674576c3b936fe66866254d17691b6cc8e` へ全桁化**。routing 側は `35dccdd214e5b2770776fa742dfd74bcc13ef219` で対応済。⭐ **query の scope と結果を literal に**: **対象 = 本材料 / 測定 spec / 設計 doc の 3 file 全体**、**pattern = 7〜39 桁の hex token（閉じた query・`head`/`tail` を通さない）**、**結果 = 当該 pin の短縮形 0 件**。⚠ **version-history label（`c8c326e00b` 等の版歴の照合記録）は対象外・書換えていない**（pN 指示どおり）。⛔ **partial SHA は historical 行であっても exact pin として使わない** |
...
| **B44** | ⛔ **自己言及で「残り 0」が偽になっていた** | 材料 B38 行が **短縮 token を自ら再掲したまま「短縮形の残り = 0」** と主張していた。⇒ **欠陥は「10 桁の短縮形」と語で記述し token を再掲しない**。**query の scope と結果を literal に**（対象 = 材料 / spec / 設計 doc の 3 file 全体・pattern = 7〜39 桁 hex・`head`/`tail` 不使用・結果 = 当該 pin の短縮形 **0 件**）。⛔ **partial SHA は historical 行でも exact pin として使わない** |
| **B42** | ⛔ 本材料 `:5` の current range が `B14〜B30` のまま（§5.4 の範囲と不一致） | ⇒ **`B14〜B44` に同期**（⛔ **§5.3 / 各行の *当時の事実* は置換しない**） |
| **B45** | — | ⛔ **routing-only**（routing artifact の exact-pin 節の分離と post-bank range）。**本材料に行は持たない**（pN 指示） |
