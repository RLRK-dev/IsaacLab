...
> ⚠ **D6 の pointer 訂正 2026-08-09 05:45（p11 発見・p18 経由・p4 採用）**: D6 の「§2 最終行」は指し先を外している ⇒ 正しくは **「§2 の表・clip 座標系の行」**（見出しで指す = 本 file 冒頭方針・番号と行は追記で動く）。⭐ **併せて trap の最強形を p11 から採る（p4 が実測して条件を 1 つ足す）**: **C1 は厳密な転置**（task `(0.350,0.150)` ↔ cell `(0.150,0.350)`）だが **C2 は転置でない**（task C2 の転置は `(0.075,0.400)`・cell は `(0.040,0.400)`）⇒ ⛔ **C1 だけの値照合は PASS して何も保証しない**。⚠ **p4 追加条件**: この転置は **`WORK_ROW_DY = 0`（既定・`ur15_cell_spec.py:387` で env 上書き可）でのみ成立** ⇒ env を変えれば C1 の一致すら消える ⇒ **「値が一致した」を移植の根拠にしない**は二重に成立する。

> ⚠ **status の読み方 (2026-08-09 06:17:13 追記)**: 本 chunk の node は **PENDING**（起票のみ・着手していない）。⛔ **計画面の生成器は PENDING を IN_PROGRESS に書き換える**（viewer に PENDING が無いため・p6 実測・p18 独立確認）⇒ **tracker / snapshot で「進行中」と見えても、それは状態の主張ではない**。⭐ **status は manifest と node の state.md を見る**。owner は生成器・viewer 側で、本 chunk の court ではない。
