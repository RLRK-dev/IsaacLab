...
- **B (FIRST-GATE — must precede A/C/D, which all assume a teachable restoring gap):** two checks:
  - **B0 obs-sufficiency (§運用21, %9-found root-cause, %12-verified on-disk):** the {11} obs seg_pos and the R re-grasp ACTION target select DIFFERENT segments — a **code-confirmed obs-target MISMATCH**:
    - **obs seg_pos** = `argmin(‖cable_xyz[:,:2] − c2xy‖)` = nearest the C2 clip **CENTER** (`_seg_rule` route_demo_to_bc.py:128).
    - **R action target** = `_kR = argmin(|cable_body[:,1] − (c2y+GHS)|)` = nearest R's **+Y grip lane** (c2y+**44mm**, GHS = 88mm-span half; LOCKED `test_newton:4480-4482`, R follows the ACTUAL bow X/Z at that lane).
    - cable ∥ Y ⇒ these are ~44mm apart in Y = generically different segments; esp. the **bow X** at the grip lane ≠ bow X at the clip-center (bow shape is not a fixed offset) → the policy is shown the wrong-target bow X and lacks the grip-lane following signal. This UNIFIES the {11}-kick failure AND the B2-diversity failure (neither moved seg 0.282 — the target signal was never in the obs).
    - **If this is the root cause, the fix is a NON-locked obs change** — align `_seg_rule`:128 for C2_REGRASP to the LOCKED action's grip-lane `_kR` (aligns the obs TO the lock, does NOT change the locked action) → **A/B/C/D (teacher redesign) become moot/downstream**.
    - **Cheap B0 test (~0 GPU, %9-proposed):** switch the {11} obs seg to argmin-to-grip-lane, re-measure seg-follow → learnable ⇒ obs-mismatch root-cause confirmed. **Honest scope:** the structural mismatch is code-confirmed = STRONG hypothesis; the fixed-Y-offset means the policy *might* partly compensate → the B0 **measurement** is the proof, not the code alone (don't turn inference into fact).
