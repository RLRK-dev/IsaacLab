...
Limited remaining diagnostic (if run): **(b) PROJECTED-only preliminary + current-pair sensitivity (FF@4 vs FF@10, same build) + pre-pin trace comparison** — value is LOW (confirms substep modulates drift extent, already largely known). **Recommendation: accept the known-open-loop-drift conclusion; do NOT chase a substrate-param root; (c) unneeded.** **PACKAGE STATUS = CLOSE** (pN readback 02:28): **NO diagnostic run authorized or pending; (b)/(c) dormant.** p6 custody disposed at commit `51abe5f625` (no new foundational row; #18-row motivation-note). **#18 arc returns to PRIMARY.** All results TRACE_ORACLE_ONLY / EXECUTION_NOT_COMPARABLE. Diagnostic only; no fix/B6/impl/training unlock. Re-open ONLY per the CLOSE-LABEL conditions (top).
