...
"""comp3 G1 A-probe: option A (per-frame base target + per-frame IK) empirical proof run.

Rs GO 19:44 (packet sec7 pre-registered falsification test; Rs adjudication (2)): the arm_q-direct run
proved the RECORDED arm path fixes L, but option A drives per-frame RECORDED EE TARGETS through the env's
own batched IK -- and the packet's leg-Q datum (all IK-path geometric deltas sub-mm/sub-deg yet L flips)
means A's effect is NOT analyzable, only probeable. This run IS that probe, at PRODUCTION 4-sub.

Mechanism (probe-level, env files untouched): monkeypatch route_executor.apply_arm_only_write_perworld
(single arm write site) to, per physics frame: (i) take the recorded ee_pos_l/r at rec_f =
min(counter+1, n-1) (the established sub-frame convention), (ii) apply the PRODUCTION lane-aware shared
floor clamp (ee_z_floor_ko_pair, dual window common-mode; machine-proven no-op on this path -- sweep
below-new-floor = 0 -- kept for A-faithfulness), (iii) solve the env's OWN batched IK
(env._solve_ik_batch, IK_ITERATIONS_RL=30, rot-weight/jlimit config UNCHANGED) warm-started from the
PREVIOUS frame's solution with fingers pinned OPEN (production conditioning, newton_route_env.py:972-976)
+ the production NaN guard (:1000-1002), (iv) write the per-frame solution (arm cols only consumed;
gripper stays servo-DYNAMIC; grip staircase untouched). The env's own per-step IK still runs upstream
(its output is bypassed at the write site) -> this probe measures A's drive FAITHFULNESS; A's COST is
already modeled in the packet (0.13x) and is NOT represented by this run's wall-clock. ik_resid is not a
tracking metric this run (step-level targets not driven); loud in JSON.

Pre-registered interpretation grid (%12 19:44): L recovers -> A PROVEN (trainer choice = A [0.13x] vs
B+D-b by cost) / L fails -> A/C EXCLUDED (trainer converges to B + D-b window design).
...
OUT_JSON = _EVAL_DIR / "comp3_g1aprobe_grasplift_result.json"
FRAMES_DIR = _EVAL_DIR / "comp3_g1aprobe_frames"
...
def main():
    print("[APROBE] building flag-ON + g1_scene_align env (world_count=1, device=cuda:0 as-coded substrate) ...")
    ...
    # A-PROBE bypass: per-frame recorded EE target -> production floor clamp -> per-frame batched IK.
    rec = np.load(GOLDEN_NPZ)
    rec_l = np.asarray(rec["ee_pos_l"], dtype=np.float64)
    rec_r = np.asarray(rec["ee_pos_r"], dtype=np.float64)
    n_rec = rec_l.shape[0]
    rex_mod = env._rex
    orig_write = rex_mod.apply_arm_only_write_perworld
    frame_counter = {"i": 0}
    prev_jq = {"jq": np.array(env._per_world_fk_jq[:1])}

    def aprobe_write(phys_jq, phys_jqd, jq_interp, arm_ow_q_idx, arm_ow_qd_idx):
        rec_f = min(frame_counter["i"] + 1, n_rec - 1)
        frame_counter["i"] += 1
        tl = rec_l[rec_f].astype(np.float32).reshape(1, 3).copy()
        tr = rec_r[rec_f].astype(np.float32).reshape(1, 3).copy()
        # production lane-aware shared floor (dual window common-mode; sweep-proven no-op on this path).
        fr, fl = nre.ee_z_floor_ko_pair(float(tr[0, 0]), float(tr[0, 1]), float(tl[0, 0]), float(tl[0, 1]), True)
        tr[0, 2] = np.clip(tr[0, 2], fr, nre.EE_Z_SAFETY_UPPER)
        tl[0, 2] = np.clip(tl[0, 2], fl, nre.EE_Z_SAFETY_UPPER)
        jq_start = prev_jq["jq"].copy()
        for base in (0, nre.JOINTS_PER_ARM):  # fingers pinned OPEN in the warm start (production :972-976)
            jq_start[0, base + nre.GRIPPER_DRIVER_JOINT_IDX[0]] = nre.FINGER_OPEN_POS
            jq_start[0, base + nre.GRIPPER_DRIVER_JOINT_IDX[1]] = nre.FINGER_OPEN_POS
        sol = env._solve_ik_batch(tl, tr, jq_start)
        if np.any(np.isnan(sol)):
            sol = jq_start.copy()  # production NaN guard (newton_route_env.py:1000-1002)
        prev_jq["jq"] = sol.copy()
        return orig_write(phys_jq, phys_jqd, sol, arm_ow_q_idx, arm_ow_qd_idx)

    rex_mod.apply_arm_only_write_perworld = aprobe_write
    print(f"[APROBE] per-frame target+IK bypass installed (n_rec={n_rec}; production 4-sub + env IK config)")
    ...
    result["preconditions_loud"] = {
        ...
        "a_probe": "PROBE-LEVEL option-A drive: per-frame recorded EE targets -> production floor"
        " clamp -> per-frame env batched IK (config unchanged) at production 4-sub. Measures A's"
        " drive FAITHFULNESS only; A's cost is the packet model (0.13x), NOT this wall-clock."
        " step-level ik_resid is not a tracking metric this run (step targets not driven).",
    }
    ...
    for tag in ("ctx", "zoom"):
        name = {
            "ctx": "comp3_g1aprobe_grasplift_context_20260710.mp4",
            "zoom": "comp3_g1aprobe_grasplift_clawzoom_20260710.mp4",
        }[tag]
        out = VIDEO_DIR / name
        cmd = [
            "ffmpeg",
