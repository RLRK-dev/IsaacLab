...
⭐ **私が保留の条件にしていた「述語の確定」は、この run では *厳格化の方向で* 満たされている** — 判定が **爪 geom への接触**で取られており、**パッド接触で真になる旧述語ではない**。そのうえで **接触 0**。
...
p18 実測 = banked asset で substring `pad` を含む geom は **12 個**、うち**爪板は 4 個だけ**（`*_pad_f1ext` / `*_pad_f2ext`）。⇒ **部分一致は 12 個中 8 個の非爪板を巻き込み、原理的に爪接触とパッド接触を区別できない。**
