...
**⇒ 追記 2026-07-28 15:5x（同じ形の 2 例目・p18 `-606` (1)）: `file:line` も同じ理由で時刻依存である。**
§27.2.133 (c) で私は 9 箇所の行を引いたが **版を書かなかった**。⇒ p4 が生きたまま編集しており、**私の読みと p18 の照合の間で 13 行ずれていた。**
⇒ ✅ **照合は通った。通った理由は 2 つ:** ① **私が行の *中身* を引用していた**（コードブロックで貼った）⇒ ⭐ **content が drift を越えた** ② ずれが **一様 +13** だったので版が自己同定できた。
⇒ ⛔ **② は運である。** p4 の編集が **私の引用した範囲の内側**に落ちていれば、ずれは一様でなくなり anchor は解決しない。⇒ ⭐ **通ったことを「この書き方で足りる」と読んではならない。**
⇒ ⭐⭐ **是正（以後）: 行を引く節には、同じ clause の中に版を書く。content が主（drift を越える）／版が従（照合を *運でなく* 確認にする）。**
⇒ **`sha256(file)` → `sha256(commit:file)` と同じ移動を、`file:line` → `file:line @ <commit>` でやるだけ。**
...
### 27.2.123 ⭐⭐ **t12 の分解が届いた — ⭐ 傾きは *ロール* だと code が既に書いていた ／ ⛔ 共通の鉛直誤差は符号で反証できる ／ ⛔ 私の R6(ii) は残差が乗っている軸を測っていない**

**入力:** p18 `-561` (1)(2)。⚠ **数値は p4 の走行中報告で trace は未 bank**（p18 明記）⇒ ⛔ **私は「読んだ」と言わない — 以下 §① は *報告の内部整合* の検査であって測定の検証ではない。**

**⇒ ① 内部整合（私の計算）= 合う。**

| 腕 | 成分 | √(along²+closing²+across²) | 報告値 |
|---|---|---|---|
| L | −1.28 / +1.87 / +0.48 | **2.316** | 2.31 ✅ |
| R | −0.37 / **+5.63** / +1.76 | **5.910** | 5.91 ✅ |

⇒ 3 成分は報告された大きさの直交分解として矛盾しない。⛔ **それだけ。**
閉じ軸の単位性も合う: `|û_L|` = 1.0005 ／ `|û_R|` = 1.0000。

**⇒ ② ⭐⭐ 傾きの正体は on-disk に既に書かれている — *ロール* である。**
水平からの傾き: **L = atan(0.317/0.949) = 18.5°** ／ **R = atan(0.500/0.866) = 30.0°**（2 軸の成す角 = **48.4°**）。
⇒ `ur15_steps_wired.py:653-656` 逐語（私の直読）: 「**The right hand's attitude is chosen at 32 degrees of roll and jams there every run; the left is chosen at 17 and clamps.**」
⇒ ⭐⭐ **18.5 ↔ 17 ／ 30.0 ↔ 32**（差 ~2°・その差は主張しない）⇒ **閉じ軸の傾き = ロール。** ⇒ 「鏡像でない」の原因は既知の名前を持つ: **両腕のロールが違う**（17 と 32）。⇒ ⭐ **そして詰まる方が転がっている方。** ⇒ これは私の §27.2.82（ロールが増えると使える収容が減る・L 95.6% / R 57.4%）と **同じ向き**に並ぶ。

**⇒ ③ ⛔ 2 つの測定を合わせるだけで、機構が 1 つ落ちる（新規測定なし・私の仕事）。**
- **共通の *鉛直* 誤差 = 反証。** 2 つの閉じ軸は **z 成分が逆符号**（−0.317 / +0.500）⇒ 共通 e_z は **逆符号**の閉じ残差を生む。⛔ 観測は **両方 正**（+1.87 / +5.63）。
- **共通の *−y* 誤差 = 符号は合うが大きさが合わない。** 係数 0.949 / 0.866 ⇒ **L の方が 1.10 倍 大きい**はず。⛔ 観測は L/R = **0.33**。
- ⚠ **一般の共通ベクトルなら解ける**（例 `e = (0, −3.63, +4.97) mm`・|e| = 6.16 が両式を厳密に満たす）— ⛔ **2 式 3 未知ゆえ、これは説明であって証拠ではない。**

⇒ ⭐⭐ **識別子（1 行・追加 run なし）= 残差を world 系でも印字する。** 2 本の world ベクトルが **一致 ⇒ 共通原因**（狙う先が動いた／指定が違う = §27.2.118 の線）／**不一致 ⇒ 腕ごとの狙い誤差**（= ②のロール差の線）。⇒ **今この 2 本は分離されていない。**

**⇒ ④ ⛔ across は最も厳しい帯を通っているが、閉じ軸には bar が無い — 私の要求の穴。**
- across L 0.48 / R 1.76 ⇒ ⭐ p18 の言う **3.00** は、私の読みでは **完全収容帯 `W−2r` = 14.00−8.00 = 6.00 の半分**（§27.2.67）。⚠ **3.00 は on-disk の名前付き定数ではない**（`ur15_cell_spec.py:428` `MOUTH_BAND_Z = (0.025, 0.039)` = 口 14.00 そのもの）⇒ **この対応づけは私の推定**・p4 の意図が別なら整合は変わる。⇒ 私の読みが正なら **両腕とも 3 帯のうち最厳を通っている。**
- ⛔ **しかし `cable_in_mouth`（`:695-716`）が読むのは pad-local **z の 1 成分だけ** = across。** ⇒ **R6(ii) は閉じ軸について沈黙している** — 残差が乗っているのはそちら。
⇒ ⭐ **自分の一般形が自分の要求に当たる: 1 本の脚を測る述語は連言を certify しない。** ⇒ R6(ii) は **閉じ軸の相棒**を持つか、**1 軸のみを覆うと明記**すべき。
- ⚠ ⭐ **ただし閉じ軸の 5.63 は「捕捉の失敗」ではない**: 捕捉時の爪先間は OPEN で **69.90 mm**（`sweep_raw_23points.txt:3`）⇒ 5.63 は楽に内側。⇒ ⭐⭐ **閉じ軸の残差は *閉じる仕事* の項である** — 先に届く側の爪がケーブルに **5.63 mm 早く**当たり、反対の爪が届くまでその分 **押す**。⇒ **したがって閉じ軸に要るのは帯ではなく「閉じ動作がケーブルを動かしてよい量」**（§27.2.118 の 10.8/12.7 mm と同じ通貨）。

**⇒ ⑤ ⭐ `P4_ROLL_CAP` の否定結果は、着座誤差を併記しないと読めない。**
`aim_both`（`:658-670`・直読）は **最初に着座した姿勢**を採る（`r[2] < 0.002`・角が立った順）⇒ **R が 32° に居るのは、それより角の立った候補が着座しなかったから** ⇒ ⭐ **ロールは着座の症状であって自由な選択ではない。**
⇒ 上限を掛けると **着座誤差とロールを交換する** ⇒ ⛔ **クランプしなかった cap run は「ロールが原因でなかった」と「cap が着座を壊した」の間で曖昧。**
⇒ ⭐ **読み出しは 2 つを併記すること: cap 後の着座誤差 ＋ クランプの成否。**（§27.2.103 の per-leg negative control と同型。）

⛔ **非主張:** 測定を検証していない（trace 未 bank）／「右腕は安全」と言わない（詰まりは閉じ軸）／`-101` の y 符号は open のまま触らない／定数を変更しない／run を求めない。**gate 不変。**

### 27.2.124 ⭐ **私の §27.2.122 ⑥ の鮮度 — 半分は既に直っている。⭐⭐ そして直し方が削除より良く、一般形になる**

**入力:** p18 `-567` (3)。⭐ **relay で更新しない** — 現 tree を直読して確認（2026-07-28 14:2x JST）。

**⇒ ① 実測 = ⑥ は連言で、**半分だけ**が動いた。**

| ⑥ の脚 | 現況（私の直読） | 出所 |
|---|---|---|
| **print が同じ数を 2 意で出す** | ✅ **直っている**（`5718746442` 14:24:16） | `:1083-1084` 現行 = `floor {SIGMA_FLOOR:.2f} removed {len(free) - len(well)} of them (it ranks, it does not exclude)` |
| **述語 `c[5] >= SIGMA_FLOOR` が恒真** | ⛔ **そのまま**（`well == free` 恒等は継続） | `:1062` 無変更 |

⇒ ⭐ **私が ⑥ を連言で書いたまま置けば、後の読み手が p5 の 2 値検査を掛けたとき答が割れる**（片脚は消えた・片脚は生きている）。⇒ **§27.2.103（連言は脚ごとに control が要る）が、今度は *私自身の finding* に当たった。** ⇒ 脚ごとの処分を付けたのは **p4 の修正と p6 の #52** であって私ではない。そう記録する。

**⇒ ② ⭐⭐ 直し方が削除より良い — 恒真の guard は *消す* のでなく *自分で報告させる*。**
p4 は恒真な filter を **消さなかった**。**それが何個落としたかを毎回印字する**ようにした（0 個なら `removed 0` と出る・`:1083-1084`）。
⇒ ⭐ **機構は再武装可能なまま残り**（床を入れれば同じ行が本物の数を出す）、**「filter が働いた」という見かけだけが消える。**
⇒ ⭐⭐ **これは私が 1 時間前に引いた ③-a／③-b（開示型／黙秘型）の分割と同じ動き**: 欠陥は **消されずに、黙秘から開示へ移された。**
⇒ **一般形: 今は識別できない guard の処方は、削除ではなく「何を落としたかを印字する」。** 削除は機構を失い、沈黙は読み手を失う。
⇒ ⚠ **p4 はこの手を 2 度使っている**（`fbc868cbe9`「拒否された測定の隣に無荷重の測定を置く」）⇒ 2 件は 1 つの型: **物は残し、真である文をその隣に置く。**

**⇒ ③ ⛔ 私からの唯一の設計上の付記（未依頼・記録のため）= `SIGMA_FLOOR` を再武装しないこと。**
床は「solver を飢えさせた」ので撤回された（`ur15_cell_spec.py:542-545` 逐語）。⇒ ⭐ **加えて §27.2.110 により、σ には絶対的な bar が置けない**（m/rad と無次元を積んだ量）。
⇒ ⭐⭐ **したがって条件数の門を戻すなら、戻す先は σ ではなく `‖Δq‖/‖Δx‖` [rad/m] の軸である。** ⇒ `:1062` が恒真であること自体は **今は正しい状態**であり、直すべきは「0 のまま黙っていたこと」だけ（それは ② で直った）。

⛔ **非主張:** filter の削除を求めない ／ 床の値を提案しない ／ #52 の閉じ方を決めない（p6 の court）／ run を求めない。**gate 不変。**

### 27.2.125 ⭐⭐ **p5 の「変位の予算」を受ける — ⚠ ただし差分は帰属しないし、腕ごとのスカラ 2 本は対を保証しない（48.4° は打ち消さない）**

**入力:** p18 `-569` (2)(3)。⭐ 以下 §④ は現 tree の直読（2026-07-28 14:3x JST）。

**⇒ ① p5 の精密化を採る — 私の言い方より強い。** 私は「R6(ii) に閉じ軸の相棒が要る」と書いたが、p5 は「**その相棒は単一 snapshot 述語の class では原理的に書けない**」と言う。⇒ **押しは 2 時点の量**であり、位置の述語では表せない。⇒ ✅ **正しく、私の定式を置き換える。**

**⇒ ② ⚠ しかし 2 時点の *差* は帰属しない — control が要る。**
2 snapshot の差には、その区間で ケーブルを動かした **全ての原因**が入る。⇒ ⭐ **§27.2.118 で既に測られているとおり、閉じる前の *狙い* の段階でケーブルは 10.8 / 12.7 mm 動いている。** ⇒ ⛔ **素の差分は、閉じ動作が起こしていない移動まで閉じ動作に課金する。**
⇒ ⭐ **要件 = 同じ区間・同じ持続・同じサーボ指令で、閉じだけを止めた control を並べる**（開いたまま同じ時間を走らせる）。その差が背景。⇒ **§27.2.103（連言は脚ごとに control）の、区間量への適用。**

**⇒ ③ ⭐⭐ 腕ごとのスカラ予算 2 本は、対を保証しない — 2 軸は 48.4° 離れていて打ち消さない。**
閉じ軸の成す角 = **48.4°**（§27.2.123 ②）⇒ 各腕に同じ予算 `b` を許すと、**剛体としての合成の上界は**

`|b·û_L + b·û_R| = b·√(2 + 2cos48.4°) = ` ⭐ **`1.82 b`**

⇒ ⛔ **両腕とも「予算内」でありながら、ケーブルは `1.82 b` 動き得る。**
⚠ **正直な但し書き:** 2 つの押しは **88 mm 離れた 2 点**に掛かるので、柔らかいケーブルでは全体並進ではなく **間の span が曲がる** ⇒ 失敗の *形* が変わる（間の着座が動く）だけで、**小さくはならない。**
⇒ ⭐⭐ **したがって予算は「腕ごとのスカラ 2 本」でなく、**ベクトルとして／対の水準で**述べねばならない。**
⇒ ⭐ **私がこれを言うのは、私自身が同じ合成誤りを踏んだから**（§27.2.82: 配置と トレードを独立に掛け算 → 重なりへ訂正）。**2 つの腕ごとの量を独立として合成する**、同じ形。

**⇒ ④ ⭐ そして 2 つの閉じは *同時* である — 選ばれたのではなく、表の既定。**
`ur15_steps_wired.py:1204` STEP 4「cable把持」= `CLAMP, CLAMP`（`:1214` STEP 14「両手クランプ」も同様・私の直読）⇒ **両顎が同じ段で指令される** ⇒ **2 つの押しは同時で、ケーブルを介して結合している。**
⇒ ⭐ **順序は生きた設計変数である**（片方を閉じてから他方を閉じれば、2 番目の押しは既にクランプされた把持に対して働く）。⛔ **どちらが良いかは主張しない。** ⛔ **段の順序は p5 の court** — 私は変数として名指すだけ。

**⇒ ⑤ t12 の集計一致について（p18 (2)）。** t11 と全一致 ⇒ ⭐ **同一構成の再走行が情報を増やさない**ことが p4 の実測でも出た。⇒ **STEP13 の集中が同じ 3/15・同じ段・同じ 2 body で再現** ⇒ ⭐⭐ **雑音ではなく choreography の固定的性質** ⇒ **設計で当たれる**（再試行では減らない）。⇒ §27.2.108 の特異点処分（**筋は正しいが狙いがずれている**）は、この再現性でむしろ強まる。

⛔ **非主張:** 予算の値を出さない ／ 段の順序を決めない ／ run を求めない ／ t13 の結果を先取りしない。**gate 不変。**

### 27.2.126 ⛔⛔ **§27.2.125 ⑤ の撤回 — 私は「同じ構成の再走行は情報を増やさない」と書いた 3 行あとで、その再走行を証拠に使った**

**入力:** p18 `-572` (2)（p5 `-127` + p18 の機械確認）。**原因側は私**ゆえ、私が引く。

**⇒ ① 2 つの欠陥。**

| 欠陥 | 私の書き方 | 正しい形 |
|---|---|---|
| **分子と分母が別の空間** | 「同じ **3/15**」＝ 率のように読める | ⛔ **3 本の非空印字は byte 同一 ＝ 1 事象の 3 回印字**。分母 15 ＝ 検査呼び出し数 ⇒ **「15 呼び出し中、非空は STEP13 右の 1 事象のみ（3 回印字）」** |
| **決定的な同一構成の反復を裏づけに使った** | 「再現 ⇒ 固定的性質」 | ⛔ **t11 ≡ t12 は弱い意味（計器追加に不変）のみ。独立再現ではない** |

**⇒ ② ⭐⭐ 2 つ目は自分で自分を踏んでいる — しかも同じ段落の中で。**
§27.2.125 ⑤ の 1 文目に私は「**同一構成の再走行が情報を増やさない**」と書き、**その次の文で その再走行を「再現」として証拠に使った。**
⇒ ⭐ **§27.2.117（決定性は私の依頼を退役させる）で既に立てた話を、1 節あとで作り直した。** ⇒ **同じ誤りの 2 度目である**と記録する。

**⇒ ③ ⭐ 残るもの／落ちるものを分ける（撤回の射程を測る）。**
- ✅ **「雑音ではない」は残る** — ⚠ ただし **決定性から自明に残るだけ**（平均すべき雑音が無い）。⇒ **強い主張ではない。**
- ⛔ **「固定的性質 ⇒ 設計で当たれる」は落ちる。** これに要るのは *同じ条件での反復* ではなく **変化に対する存続**であり、t11/t12 は同一構成 ⇒ **未検証。**
- ⇒ ⭐ **決める観測 = 何か 1 つを変えた run**（seed／ケーブル初期姿勢／姿勢メニュー）**で、同じ段に集中が残るか。** ⛔ **私は run を求めない** — 何が決めるかだけ言う。
- ✅ ⛔ **§27.2.108（特異点処分 = 筋は正しいが狙いがずれている）は本撤回の射程外** — あれは σ の構成と rad/m の逆転に載っており、**反復には載っていない。** ⇒ **無傷。**

**⇒ ④ ⭐ §27.2.117 の positive form（今回得たもの）:** **決定的な系では、反復は無料であり無価値である。情報を買うのは変化だけ。**
⇒ **系: 計器の差しか無い 2 つの run は 1 個のデータである。** ⇒ そこに「n=2」の語を当てるのは水増し。

**⇒ ⑤ p4 の別発見（`-119`・FYI）についての 1 行:** 報告側の `|dy|<8` が gate 実際の `groove_width/2 = 7.5` より 0.5 mm 甘い写しだった件は、⭐ **§27.2.122 ⑥ と同じ族**（**報告面が、報告対象の gate と違う定数を持っている**）。⇒ p4 の処方（`seat_tolerances` 単一定義・gate／走査／報告の 3 読者が同じ式）は「真である文を隣に置く」の構造版 ＝ **そもそも 2 つ書けなくする。** ⚠ `c1seat` は未変更・#52 ① は open のまま（p6 の court）。

⛔ **非主張:** run を求めない ／ 変化の中身を指定しない ／ #52 を閉じない ／ t13 を先取りしない。**gate 不変。**

### 27.2.127 ⛔ **§27.2.125 ④ の「STEP 14 も同様」を訂正 — 私は *状態* を読んで、機構が乗っている *遷移* を読まなかった。⭐ その結果、頼んだ帰属の半分は既に表の中に在る**

**入力:** p18 `-577` (2)（p5 `-128`）。**原因側は私。** ⭐ 現 tree を直読して確認（14:4x JST・⚠ 行が動いていた ⇒ **pin は content**）。
...
|---|---|---|---|---|
| 3 | OPEN | OPEN | — | `:1216` |
| **4** | CLAMP | CLAMP | ⭐ **両方が OPEN→CLAMP** | `:1217` |
| 12 | CLAMP | OPEN | 左が閉じる | `:1225` |
| 13 | CLAMP | OPEN | — | `:1226` |
| **14** | CLAMP | CLAMP | ⛔ **右だけが OPEN→CLAMP**（左は 段 12 から継続） | `:1227` |

⇒ ⛔ **私の「STEP 14 も同様」は誤り。** 指令の *対* は同じ `CLAMP, CLAMP` でも、**遷移は片側だけ。**

**⇒ ② ⭐⭐ 誤りの形 = 表の升目は *状態* で、押しは *隣り合う行の差* に乗っている。**
⇒ **一般形: 「いつ閉じるか・いつ動くか」の主張は、行の *値* でなく行の *差* から読む。**
⇒ ⭐ **そして これは p5 の議論の 1 段上で同じ失敗をしている**: p5 は「押しは 2 時点の量だ」と言った。**その押しを起こす *指令* もまた 2 時点の量である。** 私は片方を水準として読んだ。

**⇒ ③ 1.82b の射程が縮む（落とす所を明示）。**
- ⭐ **段 4 = 両顎が同時に遷移 ⇒ 対結合 1.82b が効く。**（✅ p5 が独立に 1.8242 を再導出 — **一致ではなく 2 本の導出**。）
- ⛔ **段 14 = 押しは 1 つ ⇒ 腕ごとのスカラで足りる。** しかもその押しは **既にクランプされた左の把持に対して**働く ⇒ ⭐⭐ **段 14 は、私が「代替」と呼んだ *順次* 形そのものである。**
⇒ ⭐ **振付は既に両方の型を含んでいる**（段 4 = 同時／段 14 = 順次）。⇒ §27.2.125 ④ の「順序は生きた設計変数」は残る。**落ちるのは 2 つ目の例だけ。**

**⇒ ④ ⭐⭐ 帰属（§27.2.125 ②）の半分は、新しい run 無しで既に在る。**
段 14 は **顎が 1 つだけ遷移する区間** ⇒ ⭐ **その区間のケーブル移動に「どちらの顎か」の曖昧さは無い。**
⇒ ⚠ ただし **閉じ抑止 control は依然 必要** — 腕はその間もサーボしており、**背景（閉じ以外の原因）は消えていない。** ⇒ **顎間の帰属 = 只で手に入る／背景の帰属 = 依然 control が要る。**
⇒ ⛔ **段 4 と段 14 の比較は controlled ではない**（狙う先・C1 に着座済か・再把持か初把持か・顎の初期状態 が同時に違う）⇒ ⭐ **見に行く場所であって、試験ではない。**
⇒ ✅ p5 の「**順序は押しを減らす手であると同時に、測れるようにする手**」はこの構造から出る。**同じ 1 run で両方が満たせる**という指摘に同意する。

**⇒ ⑤ 撤回の射程（下げるが、消さない）。**
✅ **残る:** 「検討されていない既定も決定である」（p5 §13-12 として一般化）／段 4 の 1.82b ／順序が設計変数であること。
⛔ **落ちる:** 「STEP 14 も同様」の 1 例のみ。
⇒ ⭐ **④ は同じ文の中に 正しい考えと 誤った実例を持っていた。** 考えは残し、実例は引く。

⛔ **非主張:** 段の順序を決めない（p5 court）／段 14 の測定を求めない／t13 を先取りしない。**gate 不変。**

### 27.2.128 ⛔⛔ **私が頼んだ識別子は、私が言ったことを決めない — ⭐ そして このデータは「共通原因 1 つ」で完全に説明できる（ケーブルが 3.8 mm 動き 3.1° 回った）**

**入力:** p18 `-582` (2)①。⚠ **p4 報告・trace 未 bank・1 瞬・1 構成。** 以下の算術は全て私が回した。

**⇒ ① 鎖は自分で閉じた — 腕あたり 3 つの数が互いに整合する。**

| 腕 | world 残差 | 大きさ | 閉じ軸への射影 `e·û` |
|---|---|---|---|
| L | (−1.28, −1.62, −1.04) | **2.312**（報告 2.31 ✅） | **+1.867**（報告 +1.87 ✅） |
| R | (−0.41, −5.75, +1.29) | **5.907**（報告 5.91 ✅） | **+5.627**（報告 +5.63 ✅） |

（`û_L` = [−0.000,−0.949,−0.317] ／ `û_R` = [−0.006,−0.866,+0.500]）⇒ **world・大きさ・射影の 3 者が閉じる。**

**⇒ ② ⛔⛔ 私の識別子は不足だった — 「不一致 ⇒ 腕ごと」は成り立たない。**
平均／差分に分けると: **共通（平均）= (−0.85, −3.69, +0.13)・|·| = 3.78 mm** ／ **差分 = ±(−0.44, +2.07, −1.17)・|·| = 2.41 mm**。
⇒ ⭐ **しかし 平均／差分が写すのは「ケーブルの並進／ケーブルの回転・曲げ」であって、「共通原因／腕ごと」ではない。**
⇒ ⛔ **ケーブルが剛体的に回れば、原因は 1 つでも 2 腕の誤差は不一致になる。** ⇒ **私が -121 で書いた「一致 ⇒ 共通原因／不一致 ⇒ 腕ごと」は、前半のみ正しく後半は無効。**
⇒ ⭐ **これは私自身の形である: 測っていないことを、測った量にぶら下げた。** 計器は正しい。**そこからの推論が強すぎた。**

**⇒ ③ ⭐⭐ そして このデータは 共通原因 1 つで説明し切れる。**
把持 2 点は **x に沿って 88 mm 離れている**（`:917-920` `cable_at(C1[0] ∓ GRIP_HALF_SPAN)`・`GRIP_HALF_SPAN = 0.044`・私の直読）。
⇒ 差分の x 成分は **−0.44** に対し y +2.07 / z −1.17 ⇒ ⭐ **差分はほぼケーブル軸に垂直** ＝ **回転・曲げの形**（軸方向の伸縮ではない）。
⇒ 中点まわりの剛体回転なら変位 = 44 mm × θ ⇒ **θ = 2.41/44 = 0.055 rad = 3.1°**。
⇒ ⭐⭐ **「ケーブルが 3.8 mm 並進し 3.1° 回った」だけで、両腕の誤差が再現する — 腕ごとの狙い誤差はゼロでよい。**
⇒ ✅ **これは §27.2.118 の形そのもの**（狙いは収束し、ケーブルが動く）。
⚠ ⛔ **原因だとは主張しない**: 腕ごとの誤差も同じ差分を作れるし、**2 点しか無ければ任意の 2 ベクトルが 平均＋反対称 に分解できる**ので、**反対称性は n=2 では検定にならない。**

**⇒ ④ ⭐ p18 の「共通は ~1.6 まで」との突き合わせ（矛盾ではない・読みが違う）。**
**1.6 = 成分ごとの共有ベクトル** (−0.41, −1.62, 0)・|·| = **1.67**（z は符号が逆ゆえ 0）。
⇒ これは「**両者に同一に在るのは幾らか**」の答であって、「**1 つの共通原因で幾ら説明できるか**」の答ではない。
⇒ ⭐ **最良の単一共通ベクトルは平均 3.78 mm** — 両点で *同じように外れてよい* ぶん大きくなる。
⇒ ⛔ **したがって「共通は高々 1.6」は成分ごとの読みでのみ成立**し、最小二乗の読みでは **共通項の方が大きい。**
⇒ ⚠ **どちらの読みでもロール線は否定されない** — ロールは「**なぜ口が外すか**」の面、これは「**ケーブルが何処に在るか**」の面。**両立する。**

**⇒ ⑤ ⭐ 本当に決める観測（また 1 行・分解が要らなくなる）= 同じ瞬間のケーブル自身の芯線姿勢（1 点＋向き）を、狙いが仮定した位置の隣に印字する。**
⇒ **仮定と対象を直接比べる** ⇒ ⛔ **分解も n=2 の縮退も要らない。** ⇒ ケーブルが仮定どおりの場所に在れば誤差は腕ごと、無ければ的の側。
⛔ **run は求めない** — 残差を印字する run に相乗りできる。

⛔ **非主張:** p4 の数値を否定しない（読みの違い）／ 1 瞬・1 構成・trace 未 bank ／ ロール線を否定しない ／ #52 の閉じ方は p6 の court（RESOLVED を受領）。**gate 不変。**

### 27.2.129 ⛔ **欠陥の入口は私である（p18 と p4 は私の規則を忠実に適用しただけ）— ⭐ そして防ぐ道具は私の記録の中に既に在り、使わなかった**

**入力:** p18 `-588`（p18 が Rs への headline を own・p4 の統合 artifact `10833f4ded` が 28 秒差で同じ過読を収録）。

**⇒ ① 帰属を入口に戻す（own の付け替えではなく、直す物が変わるから）。**
無効な推論「**不一致 ⇒ 腕ごと**」を書いたのは **私（`-121` = §27.2.123 ③）**。
⇒ **p4 はその規則を忠実に適用し、p18 はその出力を忠実に中継した。** ⇒ ⭐ **欠陥は *読み* にも *中継* にも入っていない — *仕様* に入っていた。**
⇒ ⭐⭐ **これは実務上の違いである:**
- 「p18 が過読した」と記録すれば、直す物は **中継の規律**。
- 「p11 が非同定な識別子を配った」と記録すれば、直す物は **識別子を配る前の検査**。
⇒ ⛔ **再発を止めるのは後者。** ⇒ **前者だけを記録してはならない。**（p18・p4 の own は中継・収録の脚として成立するが、**原因側は私。**）

**⇒ ② ⛔ 防ぐ道具は新しく作るものではない — 私が既に持っていて、使わなかった。**
私の記録にある形 = **仮説 × 読み の 2×2 同定表を、読みを pin する前に埋める。**
今回それを埋めれば 1 行で落ちた:

| | 読み「2 本が一致」 | 読み「2 本が不一致」 |
|---|---|---|
| **共通原因（的が動いた）** | ✅ 並進なら該当 | ⛔ **回転・曲げなら該当** |
| **腕ごとの誤差** | 起きにくい | ✅ 該当 |

⇒ ⭐ **「不一致」の列に印が 2 つ ⇒ その読みは同定しない。** ⇒ **配る前に見えた。**
⇒ ⛔ **したがって本件の教訓は「新しい形を得た」ではなく、「持っている形を適用しなかった」である。** そう記録する。

**⇒ ③ ⭐ だから今、置き換えの計器（芯線姿勢の併記・§27.2.128 ⑤）に同じ表を先に当てる。**
読み「ケーブル姿勢 ＝ 仮定」⇒ 腕ごと ／ 読み「≠」⇒ 的の側。**では「≠」を他に何が作るか:**
- ⚠ **座標系の食い違い** — 印字の frame と狙いの frame が違えば、どちらの仮説でも「≠」になる。
- ⚠ **時刻の食い違い** — ケーブルは動くので、仮定値が前の段のものなら「≠」は自動的に出る。
⇒ ⭐⭐ **したがって計器には条件を 3 つ付す:** **①同じ frame ②同じ瞬間 ③「仮定」は狙いが実際に使った変数そのものを出す**（別途 再計算しない）。
⇒ **この 3 つが揃って初めて「≠」が的の側を指す。** ⛔ 揃わなければ、また非同定な読みを配ることになる。

⛔ **非主張:** p18・p4 の own を取り消さない（脚として成立する）／ run を求めない ／ 実装形は p4 の court。**gate 不変。**

### 27.2.130 ⭐⭐ **p4 が私の計器を私の道具で直した（点でなく向き）— ⭐ ならばデータが出る前に予測を置く: 一様 2.7°（x-y）／1.5°（x-z）**

**入力:** p18 `-590`（p4 `-129`・ride #3 起動前 bank）。

**⇒ ① p4 の改良を採る — 私の比較は情報を持たなかった。**
`cable_at` は **link 中心**を返す（§27.2.107 の吸着）⇒ 閉ループの狙いは毎周その link を読み直す ⇒ ⭐ **収束時、狙いの「仮定した点」は現在の link 点と（収束残差の範囲で）一致する** ⇒ **点の比較は既に印字されている量を返すだけ。**
⇒ ⭐⭐ **測られていない仮定は *位置* ではなく *向き* である** — 座席・スロット・把持幅は、ケーブルを **world x に沿う棒**として扱っている。その扱いだけが値を持たない。
⇒ ✅ **私の計器は「測った量 対 測った量」、p4 のは「仮定 対 測った量」。** ⇒ **後者が正しい。**
⇒ ⭐ **p4 は私の 2×2 を私より先に当てた**（「違い得ない検定は検定でない」が *計器設計の段階* で効いた）。⇒ 検査が **仕様側と適用側の両方**に立った（p4 は自分の own を保持）。

**⇒ ② ⭐⭐ したがって データが出る前に予測を置く（事後の当てはめを避けるため）。**
§27.2.128 の差分 ±(−0.44, **+2.07**, **−1.17**) mm・把持間隔 88 mm（中点から ±44 mm）から、**剛体回転仮説**は次を予測する:

| 量 | 予測 | 由来 |
|---|---|---|
| x-y 面の角（ヨー相当） | ⭐ **≈ 2.7°**（2.07/44 = 0.047 rad） | 差分の y 成分 |
| x-z 面の角（ピッチ相当） | ⭐ **≈ 1.5°**（1.17/44 = 0.027 rad） | 差分の z 成分 |
| 合成 | **≈ 3.1°**（√(2.70²+1.52²)） | §27.2.128 と一致 ✅ |
| **左右・隣接 link 間の分布** | ⭐⭐ **一様**（同符号・ほぼ同値） | 剛体回転は全 link を同じだけ回す |

**⇒ ③ ⛔ 反証条件（先に書く）。**
- **左右で符号が逆** ⇒ 回転ではなく **曲がり**（S 字／たわみ）。
- **隣接 link 間の散らばりが左右差より大きい** ⇒ **局所曲率**が主で、全体回転ではない。
- **角が一様だが大きさが違う** ⇒ 回転は在るが **中心が中点でない**（仮説は棄却でなく精密化）。
⇒ ⭐ **この 3 行が先に在るので、出た数を後から物語に合わせることができない。**

**⇒ ④ ⚠ 射程（先に言う）。**
差分の元データは **1 瞬・1 構成・p4 報告（trace 未 bank）**。⇒ **予測は「もし回転なら」の形であって、私の世界についての主張ではない。**
⇒ ⛔ **一致しても「ケーブルが回っている」を確立しない**（§27.2.128 ②のとおり n=2 では平均＋反対称が常に作れる）— ⭐ **確立するのは「向きの仮定が値を持った」ことだけ**。それが今 欠けている物である。

⛔ **非主張:** run を求めない ／ 実装形は p4 の court ／ 出る前の数を主張しない。**gate 不変。**

### 27.2.131 ⛔ **予測の宛先を間違えた（次 run が正）— ⭐ そして p18 の小記録を代数で追ったら、私の反証条件 ③ が誤りだと出た（データが出る前に直す）**

**入力:** p18 `-592` (2)(3)。⭐ 2 commit を直読で確認: `581ae56935`「Print the aim residual in world too」＝ t13（**位置のみ**）／`9c141601f8`「Compare the aim's assumption against the target itself」＝ **次 run**（向き）。

**⇒ ① 宛先の訂正を受ける（原因側は私）。** §27.2.130 の予測 ②・反証 ③ が読めるのは **次 run**。t13 は **元データの瞬間を bank する側**。⇒ 予測はそのまま生きるが、**掛ける run が 1 つ後ろ。**

**⇒ ② ⭐ p18 の小記録を代数で追う — 結論は同じ向きだが、*領分* が違う。**
中心 `c⃗` まわりの剛体回転の **反対称成分**は

`(δ⃗_L − δ⃗_R)/2 = θ⃗ × (−44, 0, 0)` ⇒ **`c⃗` が消える。**

⇒ ⭐⭐ **反対称成分は中心に依らない**（中心が効くのは *共通* 成分だけ）。そして `θ⃗ × (−44,0,0)` の **x 成分は恒等的に 0**。
⇒ ⛔ **したがって反対称 x = −0.44 mm は、*どの中心の* 剛体回転でも作れない** — 軸上でも軸外でも。⇒ **中心位置の側にも読めない。**
⇒ ⭐ **残る読みは軸方向**: 両把持点が、狙いの仮定と **0.88 mm ずれている**（±0.44）。
⚠ **訂正（p18 `-594` (2)・原因側は私）:** 初出で私は「**離れている**」と書いたが、**離／近 の向きは印字規約（actual−assumed か assumed−actual か）に載る**。⇒ **規約 custody = t13 の未 bank trace** ⇒ ⛔ **今 言えるのは大きさ `|0.88|` だけ**（これは規約に依らない）。⇒ **向きは規約が bank されてから。**
⇒ ⚠ **同じ族・同じ向きの既知事実が在る**: §27.2.107 の link 吸着（15 mm 刻みゆえ 設計 88 → 実現 90.0・**+2.0 mm**）。⛔ **同じ数ではない**（片方は節点の吸着位置、片方は着座残差）が、**「狙いが仮定する span は、得られる span ではない」という同じ形。**

**⇒ ③ ⛔⛔ その代数の帰結として、私の反証条件 ③ は誤りである — 取り下げる。**
私は「**角が一様だが大きさが違う ⇒ 中心が中点でない**」と書いた。⇒ ⛔ **反対称成分は中心に依らないのだから、中心では大きさを説明できない。**
⇒ ⭐ **差し替え**: 角が一様で大きさが 2.7°/1.5° と違うなら、意味は **「把持間の区間の変位が剛体回転ではない」** か **「印字される link 方向が弦の方向と違う」**。
⇒ **反証条件（訂正版）:**
1. 左右で符号が逆 ⇒ **曲がり**（S 字／たわみ）
2. 隣接 link 間の散らばりが左右差より大きい ⇒ **局所曲率**が主
3. ⭐ **一様だが大きさが違う ⇒ 剛体回転そのものが不適**（中心の話ではない）
4. ⭐ **新規: 反対称 x の 0.44 は角では説明されない ⇒ 軸方向の項として別に読む**

**⇒ ④ ⭐ なぜ「事前登録を書き換えてよい」のか、明示する。**
⛔ **データが出た後に反証条件を変えるのは当てはめ**である。⇒ 本件が repair であって当てはめでないのは、**理由が純粋に代数で、データを 1 つも使っていないから**（`c⃗` が消えることは測定と無関係）。⇒ ⭐ **そして次 run はまだ走っていない。** ⇒ **直すなら今しかない。**

⛔ **非主張:** run を求めない ／ 0.88 の原因を主張しない（軸方向の項が在る、とだけ）／ §27.2.107 と同じ数だと言わない ／ 実装は p4 の court。**gate 不変。**

### 27.2.132 ⭐⭐ **規約が読めた（向き確定）／左右反転で私の garnish は倒れる — ただし当時から無効だった／Rs 指示 2 件の帰結と、追加検査 1 件の申告**

**入力:** p18 `-596`。⭐ 以下すべて私の直読（2026-07-28 15:2x JST）。

**⇒ ① 印字規約 ⇒ `0.88` の向きが確定した。**
`581ae56935` の追加行 逐語: `_w3 = (np.asarray(c, dtype=float) - aim_seat[t]) * 1000.0` ⇒ ⭐ **actual − assumed**（ケーブル点 − 顎の予測スロット）。
⇒ L の反対称 x = **−0.435**（L は −x 側）／R = **+0.435**（+x 側）⇒ ⭐⭐ **ケーブル側の 2 点は、顎の 2 スロットより 0.88 mm 広い。**
⇒ ✅ §27.2.131 の「離れている」は **結果として正しかった** — ⛔ **当時は根拠が無かった**（規約未確認）。**今は確定。**
⇒ ⭐ **§27.2.107（吸着で実現 span が設計より大きい）と符号が一致する。** ⛔ 数は別（0.88 vs 2.0）・機構は主張しない。

**⇒ ② ⛔⛔ 左右反転 — 私の `-117` の garnish は倒れる。しかも *当時から* 無効だった。**
⭐ **独立の裏づけ（私が読んだ）**: `1a1efe0ac5` commit message 逐語「**at 32 degrees the arm's lowest point sat 48.5 mm below its own mouth -- 25.5 mm through the table**」⇒ 32° の腕は **R**（私の測定 30.0°・§27.2.123 ②）⇒ **卓を抜けたのは R** ⇒ Rs が指したのが「卓を抜けた腕」なら **Rs の左 = 計器 R** ⇒ **反転は支持される**（⚠ inference・fact と言わない）。
⇒ ⛔ **よって私の rad/m 逆転（L が routinely 悪い）は、Rs が名指した腕と一致しない。**
⇒ ⭐⭐ **しかし真の欠陥は反転ではない — garnish は最初から無効だった。** 私は **顎の傾きについての視覚判定**を、**条件数の順位付け**の裏づけに使った ⇒ **別の量である。** ⇒ **私自身の形**（測っていないことを、測った量にぶら下げた）。**反転はそれを露出させただけ。**
⇒ ✅ ⛔ **bar 差し替えの根拠（σ は m/rad と無次元を積む ⇒ 絶対 bar 不可）は不動** — Rs 整合に一度も載っていない。

**⇒ ③ ⭐ Rs 指示 ①（座り段は全フィンガ垂直）の帰結 — pose/control 帯 = 私の court。**
(a) **座り段の roll が 0** ⇒ ⭐ **収容は最大**（§27.2.82 のトレードは roll=0 が最良）。
(b) ⭐⭐ **対結合の係数は 1.82 → 2.00 に *増える*。** 48.4° は **ロール差が作っていた**（§27.2.123 ②）⇒ 両腕 roll 0 なら 2 軸はほぼ平行・同符号（ともに −y）⇒ `|û_L+û_R| = 2.00`（**+10%**）。
⇒ ⭐ **代わりに差分（回転・曲げ）成分が消え、最悪値は純粋な並進になる。**
⇒ ⭐⭐ **並進は狙いで消せる／回転は目標を平行移動しても消せない。** ⇒ **設計としては好ましい交換**（⛔ 決めたのは Rs・私は変化を述べるだけ）。
(c) ⭐ **fallback 無し（解無しは raise）は正しい形** — 姿勢メニューが吸収していた劣化が表に出る ⇒ **座り段の raise は当面「期待される第一の結果」であって不具合ではない。**

**⇒ ④ ⛔ Rs 指示 ②（青爪 +2mm）の帰結 — 私の帯が全部動く。⭐ 追加検査 1 件を申告する。**
実測（asset 直読）: **blue = `f2ext`**（`rgba="0.20 0.45 0.95 1"`）が **0.0238 → 0.0218**（pad-local z −2.0 mm ＝ 世界で 2 mm 上）／`f1ext`（赤 0.0402）不変。
⇒ **スロット [25.00, 39.00] = 14.00 → [23.00, 39.00] = 16.00**、⭐ **中心 32.00 → 31.00**。

| 帯 | 旧（W=14） | ⭐ 新（W=16） | r 依存 |
|---|---|---|---|
| 完全収容 `W−2r` | 6.00（半 3.00） | **8.00（半 4.00）** | ✅ 依存 |
| 逃げ −1mm 余裕 `W−2` | 12.00 | **14.00** | ⛔ 非依存 |
| 逃げ `W` | 14.00 | **16.00** | ⛔ 非依存 |

⇒ ⛔⛔ **追加検査（申告）= `ur15_cell_spec.py:428` `MOUTH_BAND_Z = (0.025, 0.039)` が更新されていない。** `1a1efe0ac5` が触れたのは **driver と asset の 2 file のみ**（私の直読）。
⇒ **R6(ii) は物理より 2.00 mm 狭い帯で判定している。** ⇒ ⭐ **保守側の誤り**（物理的に口の中に在る 23–25 mm を「外」と言う）⇒ **危険ではないが誤り。** ⛔ 消費者は `cable_in_mouth` の 1 箇所のみ（閉じた grep）⇒ **影響は限定。**
⇒ ⚠ **中心の 1.00 mm 移動は別口で効く。** 現状 `MOUTH_BAND_Z` から中心を取る消費者は無く、座席は geom から従属するので **今は偏りが入っていない**（私の直読）。⛔ **ただし今後 中心基準の bar を書くなら 31.00 を使うこと。**

**⇒ ⑤ ⭐ t14 の数値目標についての規則（私の court）。**
t14 は **姿勢も爪も変わった構成** ⇒ ⛔ **2.7°/1.5° を持ち込まない。**
✅ **構造的反証（§27.2.131 訂正版 4 行）はそのまま生きる**（符号／一様性／剛体回転の可否／軸方向を別に読む）。
⇒ ⭐⭐ **数値は t14 自身の world 印字から、かつ *角の印字を見る前に* 再導出する**: `(e_L − e_R)/2` を取り、y 成分・z 成分を 44 で割って予測を **先に書く** → その後で角を読む。
⇒ ⛔ **順序が逆なら検定ではなく当てはめ。**

⛔ **非主張:** run を求めない ／ `MOUTH_BAND_Z` を私が直さない（p4/p5 の court・申告のみ）／ Rs 指示を評価しない（帰結のみ）／ 左右反転を fact と言わない。**gate 不変。**

### 27.2.133 ⭐⭐ **3 件の判定 — ① は aim 時のみ閉じる（t13 は無判定で退役）／私の順序規則は t14 では既に破れている（計器側で強制せよ）／垂直要件は相続で回避される（ただし今は発火しない）**

**入力:** p18 `-600` (3)(4)。⭐ コードは全て現 tree 直読（2026-07-28 15:4x JST）。

---

**⇒ (a) 反証 ① は「aim→close を跨ぐ剛体回転」を閉じるか = ⭐ *aim の瞬間については閉じる。区間については何も言わない。***

- 向き印字は **aim 時**。私の回転仮説（§27.2.128）も **aim 時の残差**に当てはめたもの ⇒ ⭐ **同じ瞬間・同じ面** ⇒ **その瞬間については ① が閉じる。**
- ⛔ **aim→close の区間は無傷** — そこを見る計器は無い。⇒ **閉じる最中の回転は未検査のまま。**
- ⭐ **決め手はもっと具体的**: 私の仮説は反対称 y = 2.07 mm ⇒ **x-y 角 2.7°** を予測した。⇒ **実測 x-y ≈ 0（両腕・y ≈ −0.000）** ⇒ ⛔ **ヨー半分は不在。**
- ⚠⛔ **ただし射程 — t14 の反証を t13 の仮説に移してはならない。** t14 は **姿勢も爪も違う構成**（私の §27.2.132 ⑤ の規則そのもの）。
  ⇒ ⭐⭐ **したがって t13 の「並進 3.8 mm ＋ 回転 3.1°」は *無判定のまま退役* する — 反証もされていないし確認もされていない。** 試せる計器は当時存在せず、その構成はもう無い。**そう書いておかないと、t14 の結果が黙って t13 を片付けてしまう。**
- ✅ **t14 自身については p4 の読みに同意**: ① 発火（x-z 符号逆）＋ ② 陰性（散らばり 0.7/0.6 < 左右差 2.5）⇒ **両手間の垂れ。**
- ✅ **p4 の上界を再導出**: `11 × sin 2.9° = 0.5565` ⇒ **0.56 mm** ✓。⭐ **保守方向も言う**: 接触中心まわりの傾きは **中心で 0・爪端で ±0.56** ⇒ **この上界は的側の取り分を過大に見積もる** ⇒ **狙い側は ~5.1 mm 以上**。⇒ **p4 の結論は弱まらず、強まる。**

---

**⇒ (b) t14 の `(e_L−e_R)/2` 再導出 = ⛔ 私の順序規則は t14 では *既に破れている*。**
私は「world 印字 → 予測 → 角」の順を課した。⇒ ⛔ **しかし角（L 0.4°／R 2.9°）が先に私に届き、world 残差はまだ手元に無い。** ⇒ ⭐ **答を知った後に書く予測は予測ではない。** ⇒ **t14 について私は clean な事前登録を持てない。**
⇒ **私がやること**: world 残差が来たら算術は回す。**ただし「事後の整合確認」と明記し、検定とは呼ばない。**
⇒ ⭐⭐ **本当の修正 = 順序を *読み手の規律* でなく *計器* に持たせる。** 同じ 1 行で **world 残差から計算した予測角**と**実測角**を並べて印字すれば、比較はコードが同じ瞬間に行い、**誰の順序も載らない。**
⇒ ⭐ **これは p4 の手と同じ形**（恒真 guard を消さずに「何個落としたか」を印字させた）: **規律を要求する代わりに、規律が要らない形にする。**

---

**⇒ (c) 垂直要件は相続された姿勢に適用されるか = ⛔ *されない*。⭐ ただし今は発火しない。**

**構造（直読）:**
```
:1610  if t in aimed:      # already solved by the closed-loop aim; do not re-solve
:1632      w[t] = aimed[t]
:1633      continue        ⛔ ここで抜ける
:1641  if t in _seating:
:1646      w[t] = solve_ik(..., pose_rd=(0.0, 0.0))   ← 垂直要件はここ
```
⇒ ⛔ **`continue` が垂直の指定より手前で抜ける** ⇒ **相続された姿勢に Rs 指示は掛からない。** ⇒ **packet (ii) と同じ相続面**（§27.2.112）。

**⭐ ただし現状は発火しない（ここも直読）:**
- `aimed = {}` は **段ループの中**（`:1437`）⇒ **段をまたいで残らない。**
- aim が走るのは **段 2–5**（`:1495`）と **段 13**（`:1443`）のみ。
- `_seating` は **`tgt[t][2] == Z_SEAT` で検出**（`:1591-1594`）⇒ 現 STEPS では **段 7 と 15**。
- ⇒ **{2,3,4,5,13} ∩ {7,15} = ∅** ⇒ ⭐ **同じ段で「相続」と「座り」が同時に成り立つことは今は無い。**

⇒ ⭐⭐ **したがって危険は *将来* の側に在る:** **aim を座り段へ広げた瞬間**（＝ p5 の R3／§13-3 が向かっている方向そのもの）**に、垂直要件は黙って掛からなくなる。**
⇒ ⛔ **これは最も悪い形の 1 つ: 今日は正しく、既に計画されている別の変更が着地した日に誤りになる guard。**

**⭐ 処方（設計・私の court）= 要件を「解く場所」でなく「使う場所」に置く。**
両経路が合流して `w[t]` が決まった **後** に、**座り段なら姿勢が垂直であることを検査して満たさなければ raise** する。⇒ **どちらの経路が姿勢を作ったかに依らなくなる。**
⇒ ⭐ **私の既存の形と同じ**（不在は sink で検証せよ・source の変数名で grep するな）。⛔ **実装は p4 の court・私は置き場所を言うだけ。**

⛔ **非主張:** run を求めない ／ コードを直さない ／ t13 の仮説を復活させない（無判定で退役）／ 左右反転を fact と言わない。**gate 不変。**

### 27.2.134 ⭐⭐ **垂直検査の許容 5.73° の批准 — ⛔ 値は通す／根拠は通さない。⚠ そして「メニュー最小非零ロールの半分」は事実として違う**

**入力:** p18 `-612` (2)。⭐ 版 = **`abff8d6c25`**（以下の行引用は全てこの版・worktree 一致を確認済）。

**⇒ ① ⛔ 事実の訂正が 1 件（批准の前提が動く）。**
`ur15_cell_spec.py:560 @ abff8d6c25` 逐語: `GRASP_ATTITUDES = [(y, r) for r in (0.0, 0.10, 0.20, 0.30, …)`
⇒ ⛔ **メニュー最小非零ロールは `0.10 rad`（＝ 5.73°）であって `0.20` ではない。**
⇒ ⭐ **したがって 5.73° は「最小非零ロールの半分」ではなく、*最小非零ロールそのもの*。** 記述どおりの「半分」は **`0.05 rad = 2.86°`**。
⇒ ⚠ **実害が 1 つ在る: 許容が選択可能な姿勢と *ちょうど等しい* と、合法な姿勢が境界の上に乗る** ⇒ **判定が `>` か `>=` かで変わる。** ⇒ **許容はメニュー値の *間* に置くべき。**

**⇒ ② ✅ 値は安全側にある（私の bound）。**
p4 の実測 1 点（`1a1efe0ac5` commit message）: **32° で腕の最下点が口の 48.5 mm 下・口は卓の 23 mm 上**（⇒ 貫通 25.5 ✓）。

| 仮定 | 卓に当たる傾き |
|---|---|
| 切片 0（比例） | `asin(23/(48.5/sin32°))` = **14.6°** |
| 切片 9.7 mm（CLAMP 時の roll 0 到達・`sweep_raw_23points.txt`） | **10.5°** |

⇒ ⭐ **当たる傾きは ~10–15°** ⇒ **5.73° は ~2 倍の余裕／2.86° は ~4 倍。** ⇒ **どちらも fail-safe 側。**

**⇒ ③ ⛔ 根拠は批准しない — 「メニュー値の一部」は *探索メニュー* の性質であって *卓* の性質ではない。**
⇒ ⭐ **メニューが変われば理由なく許容が動き、卓の余裕が変わっても許容は動かない。** ⇒ **`SIGMA_GOOD = 0.12` と同じ形**（数の保証が別の問いに属している・§27.2.122）。
⇒ ⭐⭐ **物理的根拠はこう書くべき: 許容は「座り高さで腕の最下点が卓に達する傾き」より下であること。** 現状その傾きは **1 点しか無く**（32°）、切片が未測ゆえ 10.5–14.6° の幅を持つ。

**⇒ ④ ⭐ 判定（3 部）。**
- ✅ **値 `5.73°` を、比較を厳密（`>` で不合格・境界は合格）にすることを条件に批准する。** 理由 = 当たる傾きの下限 10.5° に対し ~1.8 倍の余裕が在り、かつ **今日は誤停止の危険が小さい側**（垂直解の残差が未測ゆえ）。
- ⛔ **根拠文言は差し替えを要する**（上記 ③ の物理形へ）。⚠ 併せて「0.2 が最小非零」の記述を訂正のこと。
- ⭐ **目標値 `2.86°`（＝ 記述どおりの「半分」・メニュー値の間）へ、下記 ⑤ のデータが出てから締める。**

**⇒ ⑤ ⭐ 2 つの安い測定で、推定が数に変わる。**
(i) **合流点の検査は *合格時にも角を印字* すること。** ⇒ **余裕が「仮定」でなく「観測」になる** ⇒ 2.86° へ締められるかが即決まる。（⭐ 恒真 guard を自己申告させたのと同じ形。）
(ii) **腕込みの最下点 vs 傾きを 0° で 1 点**取れば、p4 の 32° と合わせて直線が決まり、**10.5–14.6° の幅が 1 つの数になる。** ⇒ **既存の掃引に 1 点足すだけ。**

**⇒ ⑥ ⚠ 新データ（右 rad/m 最悪 49.4 @ STEP15）の読み — 私の lane。**
**物理量に直す: `49.4 rad/m = 0.0494 rad/mm = 2.83°（関節） / 1 mm（EE）`。**
⇒ ⭐ **向きとしては予期される**: STEP15 は座り段で、t14 では **垂直が強制されている** ⇒ **選択器が条件数のために使っていた姿勢の自由度が消える** ⇒ **座り段の条件数は悪化する側。**
⇒ ⛔ **ただし確立していない**（1 run・以前の数とは構成が違う・control 無し）。⇒ ⭐ **control は安い かつ 具体的: 同じ t14 構成で、座り段だけ `pose_rd` の固定を外す。**
⇒ ⛔ **これは指示への異議ではない。** 垂直は **卓の貫通を消し、収容を最大にする**（§27.2.132 ③）。⇒ **言えるのは交換の中身が測れた、ということだけ**: **卓の余裕と収容を買い、座り段の条件数を払った。**

⛔ **非主張:** run を求めない ／ 値を私が実装しない ／ Rs 指示に異議を述べない ／ 49.4 を悪化の証明と言わない。**gate 不変。**

### 27.2.135 ⭐⭐ **p5 と基準を統一する — ⛔ 私の 5.73° を撤回して 0.05 rad を採る（p5 の bound の方が *きつい* から）／STEP13 の disposition は「leg は開いたが、埋まり具合は log の 1 事実で決まる」**

**入力:** p18 `-615` (2)(3)。版 = **`abff8d6c25`**（メニューの事実はこの版）。

---

## A. 許容の基準統一（p5 `-133` との割れ）

**⇒ ① ⭐⭐ 割れは「目的の層」ではない — *同じ量に対する 2 つの上界* であり、きつい方が拘束する。**

| 出所 | 述べていること | 上界 |
|---|---|---|
| 私（卓接触） | 座り高さで腕の最下点が卓に達する傾き | **< 0.183 rad**（10.5°） |
| ⭐ p5（メニュー分離） | 合法な傾き姿勢（`0.10 rad`）と垂直を **区別できる**こと | ⭐⭐ **< 0.10 rad**（5.73°） |

⇒ ⭐ **p5 の方が小さい ⇒ 拘束するのは p5 の側。** ⇒ **私の卓接触は「検査が存在する理由」であって「値を決める制約」ではない。**
⇒ ⛔⛔ **したがって私の 5.73° 批准を撤回する。** `5.73° = 0.10 rad` は **メニュー値そのもの** ⇒ **識別可能域の外** ⇒ ⭐ **私が付けた「比較を厳密に」は、そもそも範囲外の値に当てた patch だった**（境界の作法の問題ではなく、bound の外側の問題）。

**⇒ ② ⭐ 統一形（p5 へ提案・材料は両者 bank 済）。**

> **許容 τ は `r_max < τ < 0.10 rad` を満たすこと。**
> `r_max` = **垂直指令下の工具角残差**（未測・p5 の残差印字＝私の合格時印字が供給する同一計器）／`0.10 rad` = メニュー最小非零ロール（`ur15_cell_spec.py:560 @ abff8d6c25`）。
> ⇒ **`τ = 0.05 rad` を批准する。ただし `r_max < 0.05` が条件。**

**⇒ ③ ⭐ そして 2 値でなく 3 分岐で書く（p5 の条件を精密化）。**

| 実測 `r_max` | 帰結 |
|---|---|
| `< 0.05` | ✅ `τ = 0.05` が成立 |
| `0.05 ≤ r_max < 0.10` | ⚠ **区間は空でない**（`τ ∈ (r_max, 0.10)` が在る）が **余裕が痩せる** ⇒ ⭐ **これは閾値の問題でなく *解の側* の警告** |
| `≥ 0.10` | ⛔ **区間が空** ⇒ **垂直指令が隣のメニュー姿勢と区別できない** ⇒ **定式化の再構築**（p5 が私の court と名指し・**受ける**） |

⇒ ✅ **一致点の確認**: メニューを *根拠* にすることは両者とも拒否している。⭐ **私は p5 の *数* に移るが、理由は p5 のものより強い** — 「述べた趣旨の未実装」ではなく **「識別可能性という硬い上界」** である。

---

## B. STEP13 R が t14 でも陽性（disposition = 私）

**⇒ ④ ✅ leg は *開いた* — 同一構成の反復ではない初めての事例。** §27.2.126 ③ で私が「未検証」と書いた **変化に対する存続**の脚に、初めて何かが載った。

**⇒ ⑤ ⛔ ただし *どれだけ埋まったか* は、log に既に在る 1 事実で決まる。**
本事象は **腕どうしの接触**（`touching()` の非空印字）。⇒ 変化したのは **爪 `f2ext` の +2 mm**。
⇒ ⭐⭐ **決めるのは「接触対に `f2ext` が入っているか」:**
- **入っている** ⇒ **変えた幾何が、まさに接触している幾何** ⇒ ⭐ **意味のある存続。**
- **入っていない** ⇒ 変化は機構に触れていない ⇒ ⛔ **弱い**（＝ 変装した反復）。
⇒ ⛔ **run は要らない — 判定に要るのは log の接触対の body 名だけ。**

**⇒ ⑥ ⭐ 一般形（§27.2.126 ④ の必要な補正）:** 私は「情報を買うのは変化だけ」と書いた。⇒ ⚠ **正しくは「*仮説が感度を持つ量* の変化だけ」。** ⇒ **無関係な量を変えた反復は、反復である。**

**⇒ ⑦ ⭐ 間接の変化も 1 つ在る（存続を少し強める側）。** STEP13 は座り段ではないので垂直強制を直接は受けない。⇒ ⛔ **しかし段 7（C1 座り）が垂直強制を受ける** ⇒ **STEP13 に至る `prev`／`near` の履歴が変わっている** ⇒ **STEP13 の姿勢は同一とは限らない。** ⇒ ⭐ **これも log で見える**（選ばれた姿勢の `|q|max` / chosen pos）。

**⇒ ⑧ ⛔ それでも「構造的」とは言わない。決める変化は *STEP13 自身の的か seed*。**
STEP13 の的は `RX_MID`（設計定数）。⇒ ⭐ **それをずらす／seed を変える と、集中が *段* に付いてくるのか *その姿勢* に付いていたのかが分かれる。** ⇒ **段に付いてくれば振付の性質、消えれば その姿勢の性質。**
⚠ **そして整定ゲート未到達との因果は依然 未測** ⇒ ⛔ **存続は原因ではない。** 2 つを混ぜない。

⛔ **非主張:** run を求めない ／ 接触対を推測しない ／ 5.73° を p4 の誤りと言わない（私も一度採った）／ 因果を主張しない。**gate 不変。**

### 27.2.136 ⭐⭐ **入れ違いは「もう一度入れ替える」ことでは解けない — ⭐ 検査 *自身の* 用途が値を決める（0.05 rad）／B6 の私の前提は誤りで、拡張は body でなく geom を出さないと答えない**

**入力:** p18 `-623`。版 = **`3f5c7630ac`**（`touching()` の引用）／**`abff8d6c25`**（メニュー）。

---

## A. 入れ違い（私 `-135` 16:09:23 ⇄ p5 `-134` 16:08）

**⇒ ① ⛔ 双方が相手の旧位置へ同時に移った。⇒ *位置* で決めればまた入れ替わる。⇒ 論拠で決める。**

**⇒ ② ⭐ 検査は 2 つの別の仕事を持ち得る。どちらかで許容が変わる。**

| 仕事 | 要求 | 許容 |
|---|---|---|
| **J1 物理** — 卓を突かない | 傾きが接触傾き未満 | `< 0.183 rad`（5.73° で十分・余裕大） |
| ⭐ **J2 検証** — 垂直指令が *効いたか* を検出 | **解が返し得る最小の傾きを弁別できる** | ⭐⭐ `< 0.10 rad`（厳密） |

**⇒ ③ ⭐⭐ どちらが本件の仕事かは、私が §27.2.133 (c) で自分で立てた事実が決める。**
**この合流点検査は、そもそも *相続で解を迂回した姿勢* を捕らえるために置いた。**（相続経路は `continue` で垂直指定より手前を抜ける。）
⇒ **迂回して来る姿勢は `aimed[t]` = `aim_slot_at(..., pose_rd=(yaw, roll))` の産物 ⇒ *メニュー値そのもの*。**
⇒ **その最小の非零値は `0.10 rad`**（`ur15_cell_spec.py:560 @ abff8d6c25`）。
⇒ ⛔⛔ **`τ = 0.10 rad` は、検査が捕らえるべき最も安い実例に *ちょうど盲目* になる。**
⇒ ✅ **したがって `τ = 0.05 rad`。**

**⇒ ④ ⭐ これは私の旧主張の言い直しではない（そこは正直に言う）。**
`-135` で私が挙げたのは **抽象的な識別可能性**だった。⇒ 今回の根拠は **検査が現に捕らえる対象**に接地している ⇒ **より強く、かつ「層のどちらが目的か」に依存しない。** ⇒ ⭐ **入れ違いを閉じるべきなのは、この種の根拠だけ。**

**⇒ ⑤ ⭐ interim（code 5.73 継続）についての判定 = 条件付きで妥当。**
5.73° は **J1 では安全**（接触傾き 10.5° 以上に対し余裕 ~1.8 倍）。⛔ **J2 では盲目。**
⇒ ⭐⭐ **しかし J2 が要らないのは「相続が座り段で発火しない間」だけであり、それは §27.2.133 (c) で私が測った条件そのもの**（`{2,3,4,5,13} ∩ {7,15} = ∅`）。
⇒ ⭐⭐⭐ **interim の安全期限と、相続 hazard の潜伏期限は *同じ日に切れる*。** ⇒ **aim が座り段へ届いた瞬間、5.73 は「安全な暫定」から「盲目な検査」に変わる。**

**⇒ ⑥ `r_max` の 3 分岐（`-135` A4）はそのまま有効。** 合格時印字が `r_max` を出すまで、`τ=0.05` は条件付き批准。

---

## B. B6 — 私の前提が誤っていた

**⇒ ⑦ ⛔ 自認: 私は「答は既存 log に在る」と言ったが、*産出側のコードを読まずに* そう言った。**
`touching()` `@3f5c7630ac :318-328`（私の直読）— docstring 逐語「**What this arm is in contact with, other than itself.**」／`:325` 逐語 `other = g2 if a1 else g1`。
⇒ ⭐ **近側は「バグで落ちている」のではない — 関数は最初から *相手の名前* しか約束していない。** ⇒ ⛔ **誤ったのは私で、関数が持たないと明記している場を、持つと仮定した。**（**受け渡し面で確かめよ**、の 2 度目の形。）

**⇒ ⑧ ⭐ 拡張には 1 つ要件を足す: body ではなく *geom* を出すこと。**
`:326-327` は `geom_bodyid` → **body 名**を格納する（geom 名は fallback のみ）。⇒ ⛔ **近側を足しても body 名のままなら「`f2ext` が対に居たか」は依然 分からない**（pad body の中の どの板か が消える）。
⇒ ⭐ **したがって p4 の 1 行拡張は「両側」かつ「geom 粒度」でなければ、私の問いに答えない。**

**⇒ ⑨ ⚠ ただし — 近側の名が出ても、判定はさほど動かない。先に言っておく。**
変えたのは **2 mm**。腕どうしの接触（`L_forearm_link` / `L_wrist_1_link` 相手）という **寸法の大きい対**に対し、**2 mm は小さな摂動**である。
⇒ ⭐⭐ **どの仮説の下でも存続する程度の摂動** ⇒ **どちらに転んでも leg はほとんど埋まらない。**
⇒ ✅ **半分は既に確定している**（p18）: 触られた側は腕リンクで **pad/爪は無い** ⇒ **変えた幾何は「触られた側」ではない。**
⇒ ⛔ **決める変化は依然 `-135` ⑨ のまま**: **STEP13 自身の的（`RX_MID`）か seed。**
⇒ ⭐ **それでも拡張自体は支持する** — 「何に触れたか」だけを返し「自分のどこで触れたか」を返さない報告は、**問いの半分しか表現できない。** 本件の判定とは独立に、直しておく価値が在る。

⛔ **非主張:** run を求めない ／ p5 の位置を代弁しない（応答は p5 の court）／ 接触対を推測しない ／ 因果を主張しない。**gate 不変。**

### 27.2.137 ✅ **批准 CLOSED を受領 — ⭐ p5 の 2 つの規則を採り、実装前に穴を 2 つ塞ぐ（規則は「無い」と言えねばならない／bound は検査自身の量で書く）**

**入力:** p18 `-624`（私の `-136` と入れ違い・結論は独立に同着）。版 = **`abff8d6c25`**。

**⇒ ① ✅ 受領。** τ = 0.05 rad・条件 `r_max < 0.05`・3 分岐、すべて p5 と一致。⭐ **p5 が私の bound を自分の原則（違う結果が出得ない検査は検査でない）と同定した読みも正しい** — `τ ≥ 0.10` では述語「垂直である」に内容が無い。**役割分離（`τ < 0.10` = 検査が検査であるための条件／`τ < 0.183` = 存在理由に十分であるための条件・両方要り・きつい方が拘束）を採る。**

**⇒ ② ⭐ 規則 ①（`τ = (r_max + 0.10)/2`）を採る。⛔ ただし *停止条件* を付けねばならない。**
`r_max ≥ 0.10` を入れると、この式は **`τ ≥ 0.10` を返してしまう** ⇒ ⛔ **区間が空である場面で、無効な値を静かに産む。**
⇒ ⭐⭐ **導出規則は「値が無い」と言えなければならない。** ⇒ 形は `if r_max ≥ τ_max: raise（定式化の再構築）／else: τ = (r_max + τ_max)/2`。
⇒ **本日 3 度目の同じ形**（識別できない述語・恒真 guard・そして今回は *常に数を返す式*）。

**⇒ ③ ⚠ 中点そのものへの小さな注記（採用は妨げない）。** 中点は **2 つの誤りが等コスト**のときの最適。⇒ ⛔ 本件は等コストでない: **誤停止 = run が止まる（高価だが *声が出る*）／盲目 = 迂回姿勢を通す（*黙って* 通る）。**
⇒ ⭐ **非対称なら最適は中点でなく `r_max` 寄り**（厳しい側）。⚠ **ただし `r_max ≈ 0` なら両者は一致する**（どちらも 0.05）⇒ **効くのは中間分岐だけ。** ⇒ **中間分岐に入ったときだけ、この非対称を思い出すこと。**

**⇒ ④ ⭐⭐ 規則 ②（リテラルを書かない）を採る。⛔ ただし `roll ≠ 0` で書くと穴が残る。**
`GRASP_ATTITUDES` は **(yaw, roll) の対**であり、yaw も (0.0, 0.15, −0.15, 0.30, −0.30) を取る（`ur15_cell_spec.py:560-562 @ abff8d6c25`）。
⇒ ⚠ **yaw が工具を垂直から倒すかどうかを、私はここで判定しない。** ⇒ ⭐⭐ **規則がそれを知らなくて済む形にすべき、というのが要点である。**

> ⭐ **`τ_max = min{ tilt(a) : a ∈ GRASP_ATTITUDES, tilt(a) > 0 }`**
> ここで `tilt(a)` は **検査が実際に測るのと同じ量**（pinch→mouth と world −z の成す角）。

⇒ ✅ **これなら yaw が倒しても倒さなくても正しく、将来メニューに別の自由度が増えても正しい。**
⇒ ⭐ **私の既存の形と同じ: bar と読みは同じ measurement surface に載っていなければならない。** ⇒ `roll` は **パラメータ**、`tilt` は **検査の量**。**両者を同一視した瞬間に、パラメータ空間の穴が bound の穴になる。**

**⇒ ⑤ ✅ p5 の `touching()` 所見に同意** — 「計器が問いに答えられないとき、答えを推定せず計器を直す」。⇒ **私の `-136` ⑧ の geom 粒度要件は、その直し方の具体である**（両側かつ geom でなければ、直した計器がまだ同じ問いに答えない）。

⛔ **非主張:** 実装しない ／ `r_max` の値を予想しない ／ yaw の軸を判定しない ／ 中点の採用に反対しない。**gate 不変。**

### 27.2.138 ⭐⭐ **`ARM_CLEARANCE = 8.0 mm` の尺度レビュー — ⭐ 形は正しい／⛔ 量が足りない（追従誤差の項が無く、その項は同じか大きい）**

**入力:** p18 `-631` (1)。版 = **`fab643fda7`**（以下すべて私の直読）。

**⇒ ① 実装が何をしているか（確認）。**
`ur15_steps_wired.py:1131-1139` — `near_far_arm = min over ARMG[t] × ARMG[other] of mj_geomDistance(..., ARM_PAIR_CUTOFF, None)` ⇒ **両腕の *全 geom 対* の最小面間距離**。`< ARM_CLEARANCE` なら `hit = True`（＝ 衝突扱いで候補を落とす）。
`ur15_cell_spec.py:657` — **`ARM_CLEARANCE = 2.0 * CABLE_R`**（`CABLE_R = _tc.CABLE_RADIUS = 0.004` ⇒ **8.0 mm**）。
`:567` — `ARM_PAIR_CUTOFF = 4.0 * GRIP_HALF_SPAN = 176 mm` ⇒ ✅ **bar（8 mm）より遥かに大きいので、cutoff で頭打ちになる問題は無い。**

**⇒ ② ✅ 支持する点を先に 3 つ（形は正しい）。**
- ⭐ **定数がリテラルでなく *規則* で書かれている**（`2.0 * CABLE_R`）⇒ **τ で今日合意した形そのもの。**
- ⭐ **`other` 無しの呼び出しは測らず、黙って通さない**（`:1129-1130` の comment 逐語「a clearance of "no other arm" would read as infinite -- so the near-miss test says so rather than passing silently」）⇒ **測れない検査が「異常なし」と言わない** = fail-closed の正しい形。
- ✅ **計器はこの bar では健全** — 私が §27.2.74 で記録した `mj_geomDistance` の飽和は **負側**（箱の厚み ~−2.4/−2.6）であり、**+8.0 mm の判定は有効域**。

**⇒ ③ ⛔ しかし *量* が、制約が実際に課している問いと違う。3 つの食い違い。**

| # | 制約が課しているもの | 8.0 mm が答えているもの |
|---|---|---|
| (i) **場所** | **全 geom 対**の最小距離（腕のどこでも） | **ケーブルが通る所**に要る幅 |
| (ii) **時制** | 選択時の **予測姿勢**（scratch の `mj_forward`） | — （実現値との差が入っていない） |
| (iii) **予算** | 実現時に残っていてほしい隙間 | **追従誤差ぶんの余裕がゼロ** |

⇒ (i) は ⭐ **1 つの定数が 2 つの仕事をしている**（*ケーブルの通り道を空ける* と *腕どうしが当たらない*）= `SIGMA_GOOD` と同じ形。
⇒ (ii) は ⭐ **§27.2.118 と同じ形**（狙いは収束し、実体が動く）。

**⇒ ④ ⭐⭐ (iii) が本題 — 8.0 mm は、それが守るはずの誤差の *中* に在る。**
本セルで実測されている「指令と実現の差」の桁: **着座残差 L 2.31 / R 5.91 mm**（t13）／**狙いの収束 0.3 / 4.7 mm**／**狙いの間のケーブル移動 10.8 / 12.7 mm**。
⇒ ⚠ **これらは腕どうしの面で測った量ではない**（私自身の規則: 同じ数でも measurement surface が違えば別量）⇒ ⛔ **だから「8.0 では足りない」と断定はしない。**
⇒ ⭐⭐ **言えるのは: この機械が指令と実現の間で失う量は mm 単位で、8.0 と同じ桁かそれ以上である。⇒ 追従誤差の項をゼロと置いた定数は、その桁の中で無防備になる。** ⇒ **腕どうしの面での追従誤差が未測であること自体が、本レビューの結論。**

**⇒ ⑤ ⭐ 提案する形（数ではなく規則 — τ と同じ扱い）。**

> **`ARM_CLEARANCE = (その隙間を占める物) + (その面での追従誤差)`**
> 第 1 項 = `2.0 * CABLE_R`（**ケーブルが通る所については これで正しい**）／第 2 項 = ⛔ **未測。**

⇒ **安い測定**: 検査と同じ場所で **①選択時の予測 clearance ②整定後の実現 clearance** を印字する。⇒ **その差が第 2 項そのもの** ⇒ ⭐ **定数は選ばれるのでなく導出される。**

**⇒ ⑥ ⚠ 暫定の助言 = *上げない*。**
除外閾値を上げることには、このコードベースに **既知の失敗**が在る: `ur15_cell_spec.py:542-545` 逐語「the 0.12 floor **starved the solver**: it picked poses the servos could not hold」。
⇒ ⭐ **したがって「安全側だから大きく」は、ここでは安全側ではない。** ⇒ **8.0 のまま据え置き、上記 2 印字を足し、差から導出するのが両側に安全。**

**⇒ ⑦ ⚠ (i) の分離は急がない。** 2 つの仕事を 1 定数で持つのは正しくないが、**分けるには「ケーブルが在る領域」の定義が要る**（今 無い）。⇒ ⛔ **今は「1 定数が 2 仕事」と記録するに留め、⑤ の測定が入ってから分ける。**

⭐ **`:655-656` の comment 逐語「If p11 or p5 name a different scale, this reads from that instead.」に応える形で言うと — 私が名指す尺度は *別の物差し* ではなく、*同じ物差し ＋ 未測の第 2 項* である。**

⛔ **非主張:** 数を出さない ／ 上げろと言わない ／ 実装しない ／ 他の面の誤差を腕どうしの面に転用しない。**gate 不変。**

### 27.2.139 ⭐⭐ **特異点側の見直し = 要る。ただし *設計の再検討* ではなく *1 つの数* で足りる — しかもその数は既に計算されて捨てられている**

**入力:** p18 `-635` (2)。版 = **`fab643fda7`**。

**⇒ ① ⛔ まず手続きの話を閉じる — Rs の直接承認に対する異議は無い。**
§27.2.108(4) は **我々の提出作法**（片方だけ出すな）についての助言であって、**Rs の決め方を縛るものではない。** ⇒ ⭐ **そして危険は誰が起こしたかに依らない**: 直接承認が hazard を作ったのではなく、**両半分を運ぶはずの packet を誰も持っていない**という状態を残しただけ。⇒ ⭐⭐ **したがって手当ては *手続き* ではなく *観測* である。**

**⇒ ② ⭐ 相互作用の機構は在り得る（ただし主張しない）。**
clearance filter は **腕どうしが近い姿勢を落とす**。⇒ 2 本の腕は 88 mm 離れた的を同時に取る ⇒ **外側から伸び切って取る**（伸展 ＝ 条件数が悪い側）か、**畳んで近づいて取る**（条件数は良いが腕どうしが近い）かの傾向が在り得る。
⇒ ⭐⭐ **もし良条件の姿勢が「近い」側に住んでいるなら、clearance と条件数は逆を向く。**
⇒ ⛔ **これは仮説である**（配置空間のどこに良条件が在るかを私は測っていない）。⇒ **言えるのは「2 基準は独立でない・符号は不明・そして測るのが安い」だけ。**

**⇒ ③ ⭐⭐ 測り方 — 新しい run も新しい計器も要らない。数は既に在る。**
`ur15_steps_wired.py @ fab643fda7`: `hit` は `touching()` と clearance で決まり（`:1130-1139`）、**その後に全候補の `sv`（特異値）が計算される** ⇒ ⭐ **落とされた候補の条件数も計算済みで、そのまま捨てられている。**（`touching()` が近側を捨てていたのと同じ形。）
⇒ **要るのは 1 フィールド:** **「clearance が落とした候補のうち最良の条件数」を、「実際に選ばれた候補の条件数」の隣に印字する。**
⇒ ✅ **p5 が clearance に対して既に提案している「除去数の印字」と同じ行に乗る** ⇒ **新規の計器ではなく、追加される行のフィールド 1 つ。**

**⇒ ④ ⭐ なぜ run 間比較でなく run 内比較か（ここは譲らない）。**
既録の **R rad/m 最悪 49.4 @ STEP15 は t14**（`1a1efe0ac5`）で、**clearance は t16（`fab643fda7`）から** ⇒ ✅ **49.4 は filter の影響の証拠ではなく、filter 以前の基準値。**
⇒ ⛔ **しかし t14↔t16 は filter 以外も違う**（guard ①・guard ②・`touching` の粒度・clearance）⇒ **統制されていない** ＝ 本日 私が繰り返し引いた罠。
⇒ ⭐⭐ **run 内比較（落とされた最良 vs 選ばれた）は構成上 統制されている** — 同じ状態・同じ候補集合・違うのは filter だけ。⇒ **こちらが正しい計器。**

**⇒ ⑤ ⭐ 出る数で分岐が決まる。分岐は先に名指しておく（後で驚かないため）。**
- **落とした最良と選ばれたものの条件数がほぼ同じ** ⇒ 結合は弱い ⇒ **packet の第 2 半分は急がない。**
- **大きく違う** ⇒ ⭐⭐ **2 基準は直接競合している。** ⇒ **ここで構造上の問題が出る: *硬い filter は、順位付けされた目的と取引できない。***
  ⇒ 選択肢は 2 つだけ: **(A) clearance を cost に入れて重みで裁く**（⛔ すると `SIGMA_PENALTY` と同じ「未測の重み」問題に戻る）／**(B) filter は硬いままにし、条件数は残り物で我慢する**（⛔ 悪化を受け入れると明言する）。
  ⇒ **どちらも設計判断であり、私の court。** ⇒ **今は選ばない — 数が出てから選ぶ。**

**⇒ ⑥ 結論（p18 の問いへの直答）: 見直しは *要る*。形は「印字 1 フィールド ＋ 出た数で ⑤ の分岐」。⛔ run は求めない・設計の再検討は今 開かない。**

⛔ **非主張:** Rs の判断に異議を述べない ／ 機構（良条件がどちら側に住むか）を主張しない ／ 49.4 を filter の影響と言わない ／ ⑤ の分岐を今 選ばない。**gate 不変。**

### 27.2.140 ⭐⭐ **flag 2 点の分類 — ① 17.0 mm は *量子化では説明できない*（半ピッチ 7.5 を超える）⇒ 制御されていない軸の測定値である／② retarget は §0#5 の内側と読む（ただし §0 は Rs のもの・私は読みを出すだけ）**

**入力:** p18 `-639` (3)（p6 `-118` 提起・p6 は判定せず）。版 = **`91bf4d52c7`**。

---

## ① 保持点が溝中心でない（seat vs anchor = x **17.0 mm**・座り述語の 22 mm 内）

**⇒ (a) ⛔ まず私の最初の見立てを、確認して捨てる。**
「link 吸着の族だろう」と読みかけた。⇒ **link ピッチを確認した**: `ur15_cell_spec.py:57 @ 91bf4d52c7` `CABLE_SEG = _tc.CABLE_SEG_LEN` ⇒ `task_config.py:136` = **0.015 m = 15.0 mm** ⇒ **最近傍 link への吸着で生じ得る最大は 半ピッチ = 7.5 mm。**
⇒ ⛔⛔ **17.0 mm はそれを超える。** ⇒ **量子化だけでは説明できない。** ⇒ ⭐ **残差 ~9.5 mm が別物として在る。**

**⇒ (b) ⭐ その残差の正体は「制御されていない軸」である（推測でなく、位置の由来から）。**
把持の x は **設計値**（`C1[0] ∓ GRIP_HALF_SPAN` ＝ クリップの左右 44 mm）で、**溝に入る link は物理が置いたものが入る。** ⇒ ⭐ **anchor は「ケーブルが実際に居る所」を追っている** ⇒ **17.0 mm は pin の欠陥ではなく、C1 におけるケーブルの *軸方向位置誤差の測定値*。**
⇒ ⭐⭐ **本日 3 つ目の同じ話**: span 超過 **0.88 mm**（§27.2.132 ①）／狙いの間の移動 **10.8 / 12.7 mm**（§27.2.118）／そして **軸方向 17.0 mm**。⇒ **ケーブルの軸方向位置は、どこでも制御されていない。**

**⇒ (c) ⭐ 分類 = *欠陥* ではなく *制御されていない量* ＋ *それを隠している bar*。**
座り述語の窓 **22 mm** は **17.0 を通す** ⇒ ⛔ **窓が広いから、この軸が見えていない。** ⇒ **本日の他の例と同じ形**（測るべき物を隠すだけの幅を持つ bar）。
⇒ ⭐ **私からの助言 = 窓を狭めることではない**（狭めれば通らなくなるだけで、軸は制御されない）。⇒ **窓とは別に、軸方向位置を *数として出す*こと。** anchor と溝中心の差は **既に計算されている**（p6 が 17.0 を出せた）⇒ **また 1 フィールド。**
⇒ ⛔ **述語の幅は p5 の court** — 私は「幅を変えるな、量を出せ」とだけ言う。

**⇒ (d) ⚠ 半ピッチの床は依然 残る。** 仮に軸方向を制御しても、**保持点は 7.5 mm 以内には寄せられない**（最近傍 link）。⇒ **それより細かい要求が route に在るなら、動かせるのは *ピッチ* だけ。** ⇒ ⛔ **そしてピッチを下げると曲げ剛性が動く**（`EI = K·L`・§27.2.79/92）⇒ **UR15 driver が K を直書きしている面では黙って半分になる。** ⇒ **「位置精度をピッチで買う」は、剛性を売る取引である。**

---

## ② pin の retarget（`m.eq_obj2id` の runtime 変更）は §0#5 の内側か

**⇒ (e) ⭐ 判定の基準を先に置く（好みでなく述語にする）。** §0#5 が禁じている物（腕関節角の直接書込・指の kinematic close・`update_kinematic_bodies`・weld・cable-finger attachment）に共通するのは —

> ⭐⭐ **物理が作らなかった運動を、機構が作ること。**

**⇒ (f) ⭐ その基準を当てる。**

| 問い | retarget の場合 |
|---|---|
| 係合の瞬間に物が動くか | ⛔ **動かない** — anchor を係合時に再計算し拘束が厳密充足で入る（p6 が source 検証・docstring が teleport 回避を名指し） |
| 何を保持するか | **クリップがケーブルを保持する**（`cab30` → `cab27` ＝ *別の cable link*）⇒ **認可の対象そのもの** |
| 保持先が cable 以外へ動くか | ⛔ **動かない**（動けば cable-finger attachment の側で、それは不許可） |

⇒ ✅ **したがって私は「内側」と読む。**
⇒ ⭐⭐ **さらに強く言える: retarget は pin を *より忠実にする*。** build 時の推測（`cab30`）を保持すれば、**溝に居ない link を拘束する** ⇒ **物理が置かなかった場所にケーブルを留める** ⇒ ⭐ **そちらの方が kinematic trick に近い。**

**⇒ (g) ⛔ ただし §0 は Rs のものである。私は *読み* を出すだけで、閉じない。**
⚠ **Rs が指示したのは結果（「なおせ」）であって機構ではない** ⇒ **機構水準の確認を Rs から取る価値は在る。** ⛔ **ただし blocking ではない**（現行は代替より忠実であり、止める理由が無い）。
⇒ ⭐ **先に言っておく: ここに「より保守的な退避先」は無い。** 退避＝ build 時の推測に戻ることであり、それは (f) のとおり **より不忠実**。真の代替は「クリップが接触で保持する」＝ **RS71 §4 が到達不能として bank した fidelity 境界**。⇒ **「疑わしいから元に戻す」が使えない場面である**と明示しておく。

**⇒ (h) ⭐ 内側と読むための *継続条件* を 1 つだけ付ける。** 「係合時に動かない」は **数で確認できる**（係合直後の拘束残差／ケーブルの変位）。⇒ **その 1 行が出ている限り内側**、出ていなければ **主張は source 読みに留まる。**

⛔ **非主張:** §0 を裁定しない ／ 述語の幅を変えない ／ ピッチを変えろと言わない ／ run を求めない ／ 17.0 の原因を特定したと言わない（由来の説明であって測定ではない）。**gate 不変。**

### 27.2.141 ⛔ **未依頼の自己点検 — 私は「1 行・ほぼ無料」を今日 10 回言ったが、*集合として* 数えたことが一度も無い。⇒ 順位を付け、1 件を取り下げる**

**入力:** 無し（p18 `-643` は「要るもの = なし」）。⭐ **本節は依頼でなく、私が出した依頼の棚卸しである。**

**⇒ ① ⛔ 問題の形。** 個々の要求は **それぞれ正当**だった。⇒ ⚠ **しかし私は一度も合計を見ていない。** ⇒ 危険は 3 つ: **(a) 同じ量を別名で 2 度頼む ／ (b) 印字が増えて信号が埋まる ／ (c) 到着順に着手され、実際に詰まっている物が後回しになる。**
⇒ ⭐ **これは私が今日ずっと他人に当てていた形と同じ**（個々の検証を数えて、集合としての被覆を見ない）。

**⇒ ② 棚卸し（✅ = 着地・私が直読で確認した物のみ ✓ 印）。**

| # | 要求 | 出所 | 状態 |
|---|---|---|---|
| 1 | 芯線姿勢の併記 → **向き印字**（p4 が改良） | §27.2.128 ⑤ | ✅ `9c141601f8` ✓（直読） |
| 2 | 予測角と実測角を同一行 | §27.2.133 (b) | ✅（p18 報告 `5013fb8aa3`） |
| 3 | 垂直検査を合流点（use-site）へ | §27.2.133 (c) | ✅（同上） |
| 4 | ⭐ **合格時にも垂直角を印字** | §27.2.134 ⑤(i) | ⏳ **P1** |
| 5 | ⛔ 腕込み最下点 vs 傾き 0° の 1 点 | §27.2.134 ⑤(ii) | ⛔ **取り下げ（下記 ④）** |
| 6 | 接触印字を両側 × geom 粒度 | §27.2.136 ⑧ | ✅（p18 報告 `f1b9321771`） |
| 7 | clearance の **予測 / 実現** 2 印字 | §27.2.138 ⑤ | ⏳ **P4** |
| 8 | clearance が落とした最良候補の条件数 | §27.2.139 ③ | ⏳ **P3** |
| 9 | anchor と溝中心の軸方向差 | §27.2.140 ③ | ⏳ **P5** |
| 10 | 係合直後の拘束残差 | §27.2.140 ⑧ | ⏳ **P2** |

**⇒ ③ ⭐ 順位は「何が今 塞がっているか」で付ける（面白さでなく）。**
- **P1 = #4** — ⭐⭐ **批准済みの定数（τ = 0.05）が これ 1 つに条件付いている。** 着地した決定を条件付きのままにしている唯一の物 ⇒ **最優先。**
- **P2 = #10** — ⭐ **§0#5 の読み（§27.2.140 ②）の *継続条件*。** Rs へ機構確認が上がる以上、「動かない」を数で言えるかが直に効く。
- **P3 = #8** — 私が事前に名指した **設計の分岐（A/B）** を開く鍵。
- **P4 = #7** — `ARM_CLEARANCE` の第 2 項の導出。⚠ **暫定 8.0 据え置きが安全なので、待てる。**
- **P5 = #9** — 新しく見えた軸。⚠ **今 何も gate していない**（重要だが、急がない）。

**⇒ ④ ⛔⛔ #5 を取り下げる — 私自身の後の裁定が、それを不要にした。**
#5（腕込み最下点 vs 傾き）は **卓接触 bound を数にするため**に頼んだ。⇒ **しかし §27.2.136 で「拘束するのはメニュー分離 bound（`< 0.10 rad`）であり、卓接触は *検査の存在理由* であって値を決めない」と自分で裁定した。**
⇒ ⭐ **したがって卓接触 bound を精密にしても、τ は 1 mm も動かない。** ⇒ **測る理由が無い。**
⇒ ⛔ **取り下げる。**（§27.2.117 と同型 — **自分の依頼を、自分の後の結論が退役させたら、退役させる。**）
⚠ **戻す条件**: もし将来 τ が卓接触側で拘束される（例: メニューが 0.10 より細かくなり `τ_max` が下がって、卓 bound が相対的に効く）なら、その時に再依頼する。

**⇒ ⑤ ⭐ 以後の規律（自分に課す）。** **「1 行・ほぼ無料」と言うたびに、*これで何番目か* と *何を塞いでいるか* を併記する。** ⇒ **無料なのは 1 行の実装であって、注意ではない。**

⛔ **非主張:** p4 の順序を決めない（順位は助言・court は p4）／新しい要求を足さない／run を求めない。**gate 不変。**

### 27.2.142 ⛔ **その棚卸し自体が、未検証の前提で作られていた — 私は *他人の成果の状態* を、*自分の送信控え* から書いた**

**入力:** p18 `-648` (2)。⭐ 私自身の直読で確認した（版 = **`fab643fda7`**）。

**⇒ ① ⛔ 事実: 保留 5 のうち少なくとも 1 つ（P1）は、書いた時点で既に木の中に在った。**
`ur15_steps_wired.py:1814-1818 @ fab643fda7` 逐語（私の直読）:
`fingers {_off:4.1f} deg off straight down … allowance {VERTICAL_TOL_DEG:4.2f} deg -- margin {VERTICAL_TOL_DEG - _off:+5.2f}; tool axis {_tres:4.1f} deg off vertical; lowest point {_who3} at {(_lowest - TABLE_TOP)*1000:+6.1f} mm vs the table`
⇒ ✅ **角は `raise` の前に無条件で印字される ＝ *合格時にも出る* ＝ 私の P1。** ⇒ **`r_max` は t16 trace とともに届く。**

**⇒ ② ⛔⛔ 誤りの形 — 私は「まだ入っていない」を *受領通知が無いこと* から書いた。**
⇒ **不在の主張を、木でなく *自分の outbox* から作った。** ⇒ ⭐ **これは私自身の規則の裏返し**（不在は読んで書け・中継で書くな）。⇒ **何が実装されているかの権威ある出所は *コード* であって、私の送信控えでは決してない。**

**⇒ ③ ⭐⭐ 二次の帰結（こちらの方が重い）: 棚卸しの目的は p4 の負荷を下げることだった。⇒ 未検証の状態の上に建てた棚卸しは、負荷を *上げ得る*** — 既に終わった仕事を再点検させるからである。
⇒ ⚠ **今回上がらなかったのは、p18 が照合したから。** ⇒ ⭐ **「負荷を下げる提案」は接地規則の例外ではない — むしろ *他人の労力を差配する* のだから、規則が最も効く場所である。**

**⇒ ④ ✅ 取り下げ（#5）は維持。⭐ ただし理由が 1 つ増えた。**
- 従来の理由: **卓接触 bound は τ を拘束しない**（§27.2.136）⇒ 精密にしても τ は動かない。**変わらず有効。**
- ⭐ 追加: **その量は、より良い計器で既に届いている** — `lowest point … vs the table` は **掃引曲線ではなく、実際に使う姿勢での 1 点**を毎段出す。⇒ **私が頼んだ物より良い。**
⇒ ⭐ **したがって「取り下げたが、必要は満たされた」** — 取り下げは何も失っていない。

**⇒ ⑤ 実残（p18 の照合 ＋ 私の確認）= **P2（係合残差）と P5（軸方向差）の 2 件のみ**。** ⇒ ⭐ **τ = 0.05 の条件解除は t16 trace 待ちで、私の側に作業は無い。**

**⇒ ⑥ ⭐ §27.2.141 ⑤ の規律に 1 行足す。**
> 「1 行・ほぼ無料」と言うたびに *何番目か・何を塞いでいるか* を併記する（既存）。
> ⭐ **加えて: 他人が作った物の状態を数えるときは、木を読む。私の送信控えは進捗記録ではない。**

⛔ **非主張:** p18 の照合を追認だけで済ませない（P1 は自分で読んだ）／P3・P4 の着地は p18 報告として扱う（未直読）／run を求めない。**gate 不変。**

### 27.2.143 ⛔⛔ **私の「継続条件」は、書いたままでは *検定にならなかった* — p4 の 2 度目の読みがそれを検定にした。⭐ そして同じ欠陥は本日 3 度目である**

**入力:** p18 `-651` (2)。⭐ 私の直読で確認（版 = **`f9b5d45eb4`**）。

**⇒ ① ✅ 着地の確認。** commit 表題 逐語「**Check that the pin pulls nothing, rather than asserting it**」。
本文 逐語（`+` 追加分）: 「the two anchors are the same world point or they are not -- so it is **checked rather than asserted**, at the instant of engagement **and again after the solver has had a step to act**. A residual here is **a distance the cable would be pulled**.」
⇒ **P2（係合残差・2 回読み）と P5（溝中心からの軸方向差）が同じ行に載った。** ⇒ **私の実残 2 は 0。**

**⇒ ② ⛔ 私の書き方は検定になっていなかった。**
§27.2.140 ⑧ で私が書いたのは「**係合直後の**拘束残差／ケーブルの変位」。⇒ ⛔ **anchor は係合時に *拘束が満たされるように再計算される* ⇒ その瞬間の残差は構成上ほぼ 0。**
⇒ ⭐⭐ **つまり私は「違う答が出得ない読み」を継続条件に据えた。** ⇒ **本日ずっと他人に当てていた形、そのもの。**
⇒ ✅ **p4 の 2 度目の読み（solver が 1 手 動いた後）が、それを検定に変えた。** ⇒ **そして量の名前も正しくなった: 「ケーブルが引かれる距離」= §0#5 が問うている量そのもの。**

**⇒ ③ ⭐⭐ 3 度目である（一度ずつ直すのでなく、束ねて名指す）。**

| # | 私が指定した物 | 何が起きたか |
|---|---|---|
| 1 | 芯線姿勢を仮定の隣に印字 | ⛔ p4「点の比較は情報を持たない」（狙いは毎周 link を読み直すので一致は構成上） |
| 2 | 「不一致 ⇒ 腕ごと」の識別子 | ⛔ 非同定（ケーブルが回っても不一致になる） |
| 3 | 係合直後の拘束残差 | ⛔ 構成上ほぼ 0（本節） |

⇒ ⭐ **根は 1 つ: 私は「何を知りたいか」から測定を書き、「計器が何を返し得るか」から書いていない。**
⇒ ⛔⛔ **そして道具は既に自分で持っている**（仮説 × 読み の 2×2）。⇒ **§27.2.129 ③ で一度使い、2 節あと の §27.2.140 ⑧ で使わなかった。** ⇒ **欠けているのは知識ではなく *適用* である。**

**⇒ ④ ⭐ 機械的な直し方（心構えでなく手順にする）。**
> **測定を依頼する文には、同じ clause の中に 1 行を必ず入れる:**
> **「どの読みが出たらこれは偽か。計器はその読みを出し得るか。」**
> ⛔ **その 1 行が書けないなら、依頼を送らない。**

⇒ ⭐ これは私が p5 の τ 規則に付けた guard（**規則は「値が無い」と言えねばならない**）の、**依頼側の対**である。

**⇒ ⑤ ⚠ 鮮度 1 件（自節の訂正）: `t16` は存在しない。** p4 が 20 分の stall で停止し、厳密な事前除外を入れて **t17** を起動。⇒ **§27.2.142 ⑤ の「t16 trace」は `t17` に読み替える。** ⇒ **`r_max`（＝ τ = 0.05 の条件）は t17 trace から。**

⛔ **非主張:** p4 の実装を評価しない（確認のみ）／t17 の結果を先取りしない／run を求めない。**gate 不変。**

### 27.2.144 ⛔⛔ **その零点自己検査は、それを書かせた不具合を通す — 死んだ計器は零点を完璧に再現するから。⭐ 対になる span 検査が要る（+ min の空集合も）**

**入力:** p18 `-654` (2) の FYI。⭐ **私が直読して確認した**（版 = **`c9b5c3abdf`**）。⚠ **本節は依頼ではないが、t18 から効く guard の話なので今 出す。**
...
if upright and min(upright) > 0.5:
    raise RuntimeError(...)
```
⇒ **零ロールの姿勢が 0.5° より外れたら raise。**

**⇒ ② ⛔⛔ 起きた不具合は、この検査を通る。**
不具合は「回転の逆掛けで **全 attitude の tilt が 0**」。⇒ **零ロールの tilt も 0** ⇒ `min(upright) = 0.0` ⇒ `0.0 > 0.5` は **偽** ⇒ ⛔ **raise しない。**
⇒ ⭐⭐ **検査を書かせた当の失敗が、その検査を素通りする。** ⇒ ⛔ **死んだ計器は零点を *完璧に* 再現する。**
⚠ **誤解の無いように: 現行コードは正しい**（p4 が transpose を直した）。⭐ **問題は「再発を捕らえる仕組み」の側**である。

**⇒ ③ ⭐ 欠けているのは *span*。計器は零点と span の 2 点で校正する。**
現行 = **零点校正のみ**。⇒ 起きたのは **span の死**（全部 0 に潰れる）⇒ **零点検査は構造的に盲目。**
⇒ ⭐ **対になる 1 行（既存の 0.5° をそのまま使う対称形）:**
> **零でないロールの姿勢の tilt の最小値が `0.5°` を *下回れば* raise。**
⇒ 不具合下では `0.0` ⇒ ✅ **捕まる。** ⇒ **inversion / scale 0 / 符号潰れ を同時に覆う。**
⚠ **`tilt == roll` を要求しない**（両者は ~2° 違い得ることを §27.2.123 ② で私が測っている）⇒ **要求するのは「零でない入力が零でない出力になる」ことだけ。**

**⇒ ④ ⭐ もう 1 箇所 — `τ_max = min{ tilt(a) : tilt(a) > 0 }` の *空集合*。**
不具合下ではこの集合は **空**。⇒ **にもかかわらず `0.00` が印字された** ⇒ **既定値に落ちている。**
⇒ ⛔ **「傾いた姿勢が 1 つも無いメニュー」は有り得ない** ⇒ **それは値 0 ではなく *構成の破綻* である。**
⇒ ⭐⭐ **私は §27.2.137 ② で「規則は *値が無い* と言えねばならない」を *外側の式* に付けた。⇒ *内側の min* に付け忘れた。** ⇒ **同じ guard が 2 箇所要る。**

**⇒ ⑤ 自分の規則を自分に適用する（§27.2.143 ④）。**
> **どの読みが出たらこの提案は偽か:** 零でないロールの姿勢が `0.5°` を超える tilt を返す ⇒ **span は生きており、私の指摘は不要。**
> **計器はその読みを出し得るか:** ✅ **同じ関数を別の入力で 1 回呼ぶだけ。**

⛔ **非主張:** p4 の修正を否定しない（transpose は直っている）／t17 の有効性に異議を述べない（暫定定数 5.73 が効いていた）／実装しない／run を求めない。**gate 不変。**

**⇒ ⑥ 追記（p18 `-657`・私の `-144` と入れ違い）: ⭐⭐ p4 の役割分けを採る。そして それは本節と *同じ枠* である。**
p4: 「係合直後が ~0 でないなら **anchor 再計算自体が壊れており、2 度目の読みは解釈できない**」⇒ **1 つ目 = 前提検査／2 つ目 = 検定。**
⇒ ✅ **採る。** 私の §27.2.143 ② の own（**検定としては**違い得ない）は そのまま立ち、**1 つ目に別の正当な役**が立った。**相補であって矛盾ではない。**
⇒ ⭐ **役を認める前に、識別性を自分で確かめた**: 実装は **2 つの anchor を *world 点として* 比べる**（`f9b5d45eb4` 逐語「the two anchors as world points」）⇒ **再計算が壊れれば 2 点は離れる ⇒ 非零になり得る** ⇒ ✅ **前提検査としては「違い得る」。** ⇒ **役の主張だけを受け取らない。**

⇒ ⭐⭐⭐ **そして本節との関係が要点である:**

| 対象 | 零点（前提が生きているか） | span（仮説を分けるか） |
|---|---|---|
| **pin**（p4 の 2 読み） | ✅ 係合直後の残差 | ✅ 1 手後の残差 |
| **姿勢 tilt**（本節 ①-③） | ✅ 零ロール → upright | ⛔ **無い** |

⇒ ⭐ **pin の側は 2 点校正が揃っている。姿勢 tilt の側は零点だけ。** ⇒ **本節 ③ が求めているのは、pin で既に正しく行われていることを、tilt でも行うこと**に他ならない。

### 27.2.145 ⭐ **停滞への 1 つの仮説と、答を変えない削減 — ⚠ ただしそれは *私の依頼の 1 つと衝突する*（だから私が解く）**

**入力:** p18 `-661` (2) の watch（判定なし）。⛔ **私は profile していない — 以下は *コスト仮説* であって測定ではない。** 版 = **`fab643fda7`**。

**⇒ ① 桁は、走らせずに分かる（既に印字されている）。**
`ur15_steps_wired.py:318` は起動時に **`arm geom sets L=... R=...`** を出す。⇒ 既存 log 実測（`run_logs_20260728/*.txt`・私の直読）= **`L=38 R=38`**。
⇒ clearance の判定は `ARMG[t] × ARMG[other]` の全対 ⇒ ⭐ **候補 1 つあたり `38×38 = 1,444` 回の `mj_geomDistance`。**
⇒ 姿勢メニュー **65**（13 roll × 5 yaw）× `tries`（26／座り 44）× 補正 3 周 × 2 腕 ⇒ **1 回の aim で 10⁶ オーダー。** ⇒ **停滞の候補として桁が合う。**

**⇒ ② ⛔ 私が最初に思った「早期打切り」は、効きが *棄却率* に比例するだけ。**
候補が棄却されると分かった時点で min の完成は不要 ⇒ 打切り可。⇒ ⚠ **しかし *通る* 候補では 1 度も打切りが起きない** ⇒ **腕が離れている場面では何も速くならない。**
⇒ ⭐ **効くかどうかは既に測れる**: p5 が入れた **除去数の印字**がそのまま棄却率である。⇒ **実装前に log で決まる。**

**⇒ ③ ⭐⭐ もっと良い削減があり、こちらは *全候補* に効く — `ARM_PAIR_CUTOFF` を下げる。**
現行 `:567` = `4.0 * GRIP_HALF_SPAN` = **176 mm**、判定の bar は **8.0 mm** ⇒ **22 倍。**
⇒ `mj_geomDistance` は **cutoff を超える距離を cutoff として返す** ⇒ ⭐ **cutoff を bar より大きい任意の値（例 16 mm）に下げても、`< ARM_CLEARANCE` の判定は 1 件も変わらない**（遠い対は cutoff = 16 を返し、`16 < 8` は偽）。
⇒ ⭐ **そして cutoff は narrow-phase の枝刈りに使われる** ⇒ **遠い対が安くなる。**（⚠ **この枝刈り挙動は API の仕様理解であって、私が測った物ではない。**）

**⇒ ④ ⚠⚠ しかし ③ は *私自身の依頼* と衝突する。だから私が解く。**
私は §27.2.138 ⑤ で **「選択時の予測 clearance を印字せよ」** と頼んだ。⇒ ⛔ **cutoff を下げると、印字される min は真値でなく cutoff で飽和する。**
⇒ ⭐⭐ **解: 判定ループは小さい cutoff で回し、真の最小値は *選ばれた候補 1 つ* についてだけ大きい cutoff で計算する。**
⇒ **選ばれる候補は段あたり 1 つ** ⇒ **コストは無視できる** ⇒ ✅ **速度と私の印字の両方が立つ。**
⇒ ⭐ **これは §27.2.141 で私が心配した (a)(b) の実例である**（依頼を集合として見ていなかった）— **今回は衝突を私が見つけて解いた。**

**⇒ ⑤ 自分の規則の適用（§27.2.143 ④）。**
> **どの読みが出たら本節は偽か:** cutoff を下げても停滞が同じなら、**clearance ループはコストではない。**
> **計器はその読みを出し得るか:** ✅ **完走時間そのもの。** ⇒ **1 回の run で決まる。**

⛔ **非主張:** profile したと言わない ／ 実装しない（p4 の court）／ t17 を止めろと言わない ／ 停滞の原因を特定したと言わない。**gate 不変。**

### 27.2.146 ⭐⭐⭐ **A/B の裁定 — ⛔ A は原理で落ちる／⛔ B はまだ早い（データは *40 個の標本の中の* 競合しか示していない）。⭐ 順序は C → rad/m → D、そして D の交換比は *導出できる***

**入力:** p18 `-667`。⚠ **数は p4 の走行中報告・trace 未 bank** ⇒ ⭐ **以下の裁定は *構造* に載っており、比の正確な値には載っていない**（使うのは「2 例とも 1 より十分大きい」だけ）。

---

## 0. まず contamination の受領（私の側の訂正）

t16 停滞の主因は **p4 自身の監視 loop（99.5% CPU・15 分）** だった。⇒ ⭐ **私の §27.2.145 は t16 停止の *後* に出したので停止の原因ではない。⛔ しかし同じ方向を指していた。**
⇒ ⭐⭐ **欠けていた手順: コストをコードに帰する前に、*機械の上で他に何が走っているか* を見る。** ⇒ **算術（38×38 = 1,444／aim あたり 10⁶ オーダー）は *コスト見積り* として立つ。⛔ 「これが停滞だ」という帰属は落ちる。** ⇒ **交絡を除いて読み直す。**
⚠ **私の反証条件自体は正しく働く形だった**（cutoff を下げても停滞が同じなら clearance はコストでない）— **ただし正しい答に *不要な run 1 本* を経て着く形だった。** ⇒ **先に環境を見れば run は要らなかった。**

---

## 1. ⛔ A（clearance を cost に重み付きで入れる）— **原理で却下する。**

⇒ ⭐⭐ **重みを付けるとは「十分に良い条件数は、十分な接近を買える」と言うことである。**
⇒ ⛔ **clearance の内側には *交渉できない部分* が在る: 接触（隙間 0）。** ⇒ **1 つの重みは「余裕」と「接触」を区別できない。** ⇒ **順位付けは *柔らかい選好* に対してはできるが、*硬い制約* に対してはできない。**
⇒ ⚠ **加えて（副次的理由）**: σ は m/rad と無次元を積んだ量で **絶対尺度を持たない**（§27.2.110）⇒ **mm を σ と取引する重みは二重に根拠が無い。** ⇒ **ただし却下の主因は上の原理であって、こちらではない。**

## 2. ⛔ B（明示受容）— **まだ早い。データが示したのは「標本の中の競合」である。**

実測は **40 個中 10 個を除去し、生き残りの最良が除去された最良より 2.20 倍悪い**（STEP5 は 1.67 倍）。
⇒ ⭐ **これは「clear かつ良条件の姿勢が *存在しない*」を示していない。** ⇒ **示しているのは「40 個の標本の中には無かった」だけ。**
⇒ ⚠ **そして標本は小さい**: 6 自由度腕の IK 分枝は高々 8 程度、姿勢メニューは 65 ⇒ **把持段の探索空間は数百** に対し `tries = 26`／候補 40。
⇒ ⛔ **競合が本物か標本の産物かを、今のデータは分けられない。** ⇒ **B（恒久的に 2.2 倍を受け入れる）は、その区別の前には決められない。**

## 3. ✅ C（先にやること）— **標本の産物かを測る。既に印字されている数で決まる。**

> **把持段 1 つで `tries` を上げ（あるいは種を散らし）、*filter を硬いまま* 生き残りの最良 σ が改善するか見る。**

⇒ **改善する ⇒ 標本の産物** ⇒ ⭐ **A も B も要らない**（探索を厚くするだけ）。
⇒ **改善しない ⇒ 本物の競合** ⇒ **4 へ進む。**
⇒ ⭐ **自分の規則の適用（§27.2.143 ④）: 偽になる読み = `tries` を大きく上げても生き残り最良 σ が動かない。計器はそれを出し得るか = ✅ 勝者 σ は既に毎段印字されている。**

## 4. ⭐⭐ D（本物の競合だった場合の形）— **硬い床 ＋ 余裕の上の傾斜。⛔ そして傾斜の交換比は *選ばない・導出する*。**

- **床（硬い・交渉不可）= 接触**（隙間 > 0）
- **傾斜（柔らかい）= 0 から `ARM_CLEARANCE` までの余裕**
⇒ ⭐ **これなら 6 mm の隙間で良条件の姿勢が 9 mm の悪条件に勝てるが、0 mm は何にも勝てない。** ⇒ **A が壊した性質（安全は取引されない）が戻る。**

**⇒ 交換比の導出（ここが要点）:**
⛔ **mm と σ は交換できない**（σ に尺度が無い）⇒ ⭐ **先に σ を `‖Δq‖/‖Δx‖` [rad/m] へ替える**（§27.2.110・既に裁定済）。
⇒ ⭐⭐ **さらに一歩: 条件数の *費用* は「腕が消せずに残す EE 誤差」であり、それは mm で測れる。** ⇒ **両辺が mm になれば、重みは 1 になる。**
⇒ ⭐⭐⭐ **そしてその換算は、既に印字されている 2 つの数の相関から出る**: **段ごとの条件数** と **段ごとの狙い残差**（着座残差 2.31 / 5.91 mm・収束 0.3 / 4.7 mm を私は既に使っている）。
⇒ **したがって D の重みは「決める」対象ではなく「測る」対象である。** ⇒ **τ で確立した形（規則で書き、測定から導く）と同じ。**

## 5. 裁定（p18 の問いへの直答）

> ⛔ **A = 却下（原理）。** ⛔ **B = 保留（前提未確立）。** ✅ **C を先に。** ✅ **C が競合を確定させたら D、交換比は導出。**
> ⚠ **B が残る道は 1 つだけ**: C で本物と分かり、D の導出も失敗した場合 — **そのときは「2.2 倍の悪化を受け入れた」と *数つきで* 書く。黙って受け入れない。**

⛔ **非主張:** 数を確定として扱わない（trace 未 bank）／`tries` の値を指定しない／実装しない／run を求めない（C は次に走る run に相乗りできる）。**gate 不変。**

### 27.2.147 ⛔⛔ **`5.0°` を `r_max` にしてはならない — ⭐ 2 つの読みのどちらに転んでも同じ結論に着く（片方なら *残差でない*、もう片方なら *検査が捕らえるべき当のもの*）**

**入力:** p18 `-670`（p4 走行中報告・trace 未 bank）。⭐ コードは全て私の直読（版 = **`fab643fda7`**）。

**⇒ ① まず自分の最初の疑いを潰す。** 私は「STEP9 は垂直を指令していない段では」と疑った。⇒ ⛔ **違う。**
`:1784` `for t in _seating:` ／ `:1710` `abs(float(tgt[t][2]) - Z_SEAT) < 1e-9` ／ `:1300` `(9, "C1がcable固定", (LX1, C1[1], Z_SEAT), …)` ⇒ ⭐ **STEP9 は座り段であり、垂直は指令されている。** ⇒ **私の疑いは外れ。**（⇒ 座り段は 7・8・9・15・16・17 で、私が以前 7/15 と読んだのは不足だった。）

**⇒ ② ⭐⭐ しかし *同じ行の 2 つの角が食い違っている*。**
逐語（p4 報告）: **`fingers 5.0 deg off … tool axis 0.0 deg off vertical`**
⇒ ⭐ **解は *自分が駆動する量*（工具軸）では完全に垂直に着いている。** ⇒ **検査が読む量（pinch→mouth）だけが 5° 傾いている。**

**⇒ ③ ⭐ 有力な読み（⛔ 主張ではない）: このずれは *顎の開き* に依存する。**
`:1300` STEP9 = **`HALF, OPEN`** ／ `:1298` STEP7・`:1306` STEP15・`:1307` STEP16 = **`CLAMP, CLAMP`**。
⇒ `_down = slot_centre(t) - pinch(t)` は **pad の位置に依存**し、**pad は閉じながら振れる**（`CLOSE_RISE` の既存注記）⇒ ⭐ **HALF では口の向きが工具軸とずれ、CLAMP では一致し得る。**
⇒ ⚠ **決め手は同じ log 行に既に在る**: **STEP8 も `HALF, OPEN`**（`:1299`）⇒ **STEP8 が ~5° なら本読みは支持され、~0° なら死ぬ。** ⇒ **新しい測定は要らない。**

**⇒ ④ ⭐⭐⭐ そして τ については、③ が正しくても正しくなくても *同じ結論* になる。**

| ③ の可否 | `5.0°` の正体 | `r_max` に使えるか |
|---|---|---|
| **正しい** | **顎状態による系統的な幾何差**（解の誤差ではない） | ⛔ **不可 — `r_max` は「解が残す誤差」であり、これは解が動かせない量** |
| **誤り** | **垂直を指令した段で垂直に届いていない ＝ 要件違反** | ⛔ **不可 — 検査が捕らえるべき当のものを、閾値の側に吸収することになる（循環）** |

⇒ ⭐⭐ **どちらでも `5.0` は `r_max` ではない。** ⇒ ⭐ **`r_max` は *達成された群*（他段の 0.0–0.1°）から取る。**
⇒ ✅ **すると `r_max ≈ 0.1° ≪ 0.05 rad`** ⇒ **中間分岐は発火しない** ⇒ **`τ = 0.05 rad (2.86°)` が生きている。** ⇒ **`5.37°` の中点則も、`2.86°` 候補の死亡も、前提が違う。**

**⇒ ⑤ ⛔ 現状は 3 つのうち最悪の形である。** 暫定 `5.73°` は **既知・計算可能・系統的なずれを、公差で吸収している。** ⇒ ⭐ **その結果、検査は最大 5° の *本物の* 逸脱に盲目になる。**（⇒ §27.2.144 の「零点だけ」と同族: **公差が校正の代わりをしている。**）

**⇒ ⑥ ⭐ 設計上の帰結（私の court）= 要件が、解が駆動していない量で書かれている。**
`pose_rd=(0,0)` は **工具**を垂直へ駆動する。⇒ **要件は「指が真下」＝ pinch→mouth の向き。** ⇒ **顎が一致状態でなければ、両者は別物。**
⇒ **直す場所は閾値ではなく源**: **(a) 座り段では口の向きを目標にする／(b) 実際に指令される顎状態で評価し、差を目標へ織り込む。** ⛔ **(c) 公差で吸収する（現行）は採らない。**
⇒ ⭐ **これは §27.2.137 ④ と同じ分割が *解の側* に現れたもの**: あちらは「roll はパラメータ・tilt は検査の量」、こちらは「工具軸は解が駆動する量・口の向きは要件の量」。

**⇒ ⑦ ⚠ 正直な帰結: ④ を採れば、いま走っている run は STEP9 で止まり得る。**
`τ = 2.86°` なら **STEP9 L の 5.0° は raise。** ⇒ ⛔ **それは不具合ではなく、検査が仕事をしている姿である。** ⇒ **止まったら直すのは STEP9 の姿勢（または ⑥ の源）であって、閾値ではない。**

**⇒ ⑧ ✅ p18 `-670`(2) の実証を受ける。** 「工具軸 0.0° / 指 5.0°」は、**私が §27.2.137 ④ で「便利な量でなく検査が測る量で書け」と言った理由の、1 行の実測**である。⇒ ⭐ **本節はその同じ 1 行を、今度は *解の側* に当てている。**

⛔ **非主張:** ③ を主張しない（STEP8 の値で決まる）／`5.0` の原因を特定したと言わない／τ を確定しない（完走 trace 後）／run を止めろと言わない。**gate 不変。**

### 27.2.148 ⛔⛔ **③ は死に、⑥ も一緒に死ぬ — ⭐ しかし *機構* は生きていて、担い手が違った: 検査は「指令された腕 ＋ *実現した* 指」の混合状態で測っている**

**入力:** p18 `-679`（STEP8 = L 0.1° / R 0.0°）。⭐ コードは全て私の直読（版 = **`fab643fda7`**）。

**⇒ ① ✅ 私の基準どおり ③ は死ぬ。⛔ そして ⑥ も。**
STEP8 は `HALF, OPEN` で L の的も顎も STEP9 と同一なのに **0.1° vs 5.0°** ⇒ ⛔ **指令された幾何に差が無い** ⇒ **「顎の *指令* によるずれ」は反証。**
⇒ ⛔⛔ **§27.2.147 ⑥（要件が解の駆動しない量で書かれている・源は口の向きを目標にすること）は ③ の帰結だったので、一緒に撤回する。** ⇒ **前提が死ねば結論も死ぬ。両方を同じ turn で引く。**

**⇒ ② ⭐⭐ しかし機構は生きている — 担い手が *指令* ではなく *実現* だった（コードで確認済）。**
`:273` `QADR = {t: [... for j in J6] ...}` ⇒ ⭐ **`QADR[t]` は腕 6 関節のみ。**
検査は `_sv.qpos[:] = d.qpos` の後に **腕関節だけ** を `w[t]` で上書きする（`:1786-1788`）⇒ ⛔ **指の関節は *生の sim 値* のまま残る。**
そして `_down = slot_centre(t,_sv) - pinch(t,_sv)` は **pad body に依存** ⇒ ⭐⭐ **検査の答は「実現した指の開き」に依存する。**

**⇒ ③ ⭐⭐⭐ これで 0.1 と 5.0 が、同一指令のまま説明できる。**
STEP7 = `CLAMP, CLAMP` ⇒ **STEP8 の検査時点では指はまだ CLAMP 付近**（HALF は *今 指令された* ばかり）⇒ 口 ≈ 工具軸 ⇒ **0.1°**。
STEP8 の実行中に指は HALF へ動く ⇒ **STEP9 の検査時点では指は実際に HALF** ⇒ 口が傾く ⇒ **5.0°**。
⇒ ⭐ **③ は *機構* としては正しく、*変数* を取り違えていた**（指令された顎 ／ 実現した顎）。

**⇒ ④ ⭐ 只の反証条件（既に印字されている数で決まる）。**
> **STEP9 の R**（顎 `OPEN` ＝ CLAMP からより遠い）は、機構が正しければ **L の 5.0° と同等以上**になるはず。
> ⛔ **R が ~0 なら本読みは死ぬ。**
⇒ **決定打は 1 つ足すだけ: 角の隣に *その時点の指の開き* を印字する。**

**⇒ ⑤ ⭐⭐⭐ 源の修正（p18 が求めたもの）= 検査を「指令」か「実現」かに *決める*。今は混合である。**

| 形 | 何を答えるか | 使い道 |
|---|---|---|
| ⭐ **完全に指令状態**（腕 *と指* を指令値に置く） | **この姿勢を指令してよいか** | ⇒ **要件（Rs の「指は真下」）はこちら。⭐ 答が指令だけの関数になる** |
| **完全に実現状態**（整定後に測る） | **実際に卓を突かないか** | ⇒ **物理の問い。⭐ 別に要る** |

⇒ ⛔ **混合は、指令が同じ 2 段で違う答を出す** — まさに今起きたこと。
⇒ ⭐ **零点と span を両方要ったのと同じ構造**（§27.2.144）: **2 つ要るが、混ぜてはいけない。**

**⇒ ⑥ ⚠ τ への帰結は同じだが、根拠が変わり、そして 1 つ弱くなる。**
- ✅ **5.0 は依然 `r_max` ではない**（解の残差でなく *混合の産物*）⇒ **第 1 分岐が生存。**
- ⛔ **訂正（p5 `-147`・原因側は私）: 私は §27.2.147 ④ で「`τ = 0.05 rad (2.86°)` が生きている」と書いたが、`0.05 rad` は *分岐の境界* であって *τ の値* ではない。**
  `τ = (r_max + τ_max)/2` に `r_max ≈ 0.1°`・`τ_max = 5.73°` を入れると **`τ = 2.915°`**（私の再計算 ✓）。⇒ `0.05 rad = 2.865°` とは **0.05° 違い、出所が違う。**
  ⇒ ⭐⭐ **これは「同じ数字は同じ量ではない」の小型版であり、しかも私が p5 と *批准したばかりの規律*（数でなく式で持て）を、2 節あとで自分が破った形である。** ⇒ **本日 3 度目の「自分が立てた規則の非適用」。**
  ⇒ ✅ **正しい持ち方: 第 1 分岐が生存し、τ は式から出る（暫定 ~2.92°）。数は完走後の `r_max` から。**
- ⛔ **ただし私の §27.2.147 ④ の二分法は網羅していなかった** — 実際は **第 3 の場合（測定の産物）**だった。⚠ **結論が同じ所に落ちたのは *偶然に近い*。** ⇒ **「二分法が予見していた」とは言わない。**
- ⚠⚠ **そして弱くなる点**: 検査が混合なら、**0.0–0.1° の群も「その時 たまたま在った指の状態」で測られている** ⇒ ⛔ **`r_max` の母集団そのものが混合。** ⇒ ⭐ **`r_max` は ⑤ の修正 *後* に読み直すべきで、`τ = 2.86°` は暫定である。**

⛔ **非主張:** ④ を確定と言わない（STEP9 R で決まる）／指の開きを測ったと言わない／τ を確定しない／run を止めろと言わない。**gate 不変。**

### 27.2.149 ⛔ **variant は採らない（死んだ仮説を自由変数で救わない）— ⭐ ただし「指の状態が効く」は仮説でなく *算術で強制される*（1 つ条件つき）。⭐⭐⭐ そして源の修正は 2 つの検査に分ける**

**入力:** p18 `-685`（**STEP9 R = 0.0°**）。版 = **`fab643fda7`**。

**⇒ ① ✅ 述べた形の読みは死ぬ。⛔ variant は採らない。**
4 点の実測は **STEP8 L 0.1 / STEP8 R 0.0 / STEP9 L 5.0 / STEP9 R 0.0** ⇒ **開きに単調なら OPEN(R) は HALF(L) より大きいはず** ⇒ ⛔ **反証。**
⇒ ⚠ **非単調 variant（HALF だけ pad が振れる等）は *救済仮説* である。** ⇒ ⭐ **非単調性の裏づけは on-disk に 1 つも無い**（コの pad linkage を私は読んでいない）⇒ ⛔ **自由変数で死んだ仮説を救わない**（§27.2.87 と同じ形）。⇒ **variant は採らない。**
⇒ ⚠ **4 点のうち 3 点が 0.0 で、非零は 1 点だけ** ⇒ **1 点の上に機構の物語を建てるのは過適合。**

**⇒ ② ⭐⭐ しかし「指の状態が効く」は、仮説ではなく *算術で強制される*（条件 1 つつき）。**
- **口の向き** `slot_centre − pinch` は **腕関節 ＋ 指関節**で決まる（pad body 依存）。
- **工具軸**は **腕関節のみ**で決まる。
⇒ ⭐ **STEP9 で 工具軸 0.0° / 口 5.0°。もし STEP8 でも工具軸が ~0 なら、両段で腕側は同じ「垂直」であり、口の 4.9° の差は *指関節以外から来ようがない*。**
⇒ ⭐⭐ **したがって問いは「指が効くか」ではなく「どの指状態がどの傾きを生むか」であり、それは私が既に頼んだ 1 フィールド（角の隣に指の開き）で答が出る。**
⇒ ⚠⚠ **条件**: この論法は **STEP8 の *工具軸* の値**を要る。⛔ **未入手**（頂いたのは指の側の 0.1/0.0 のみ）。⇒ **同じ行に既に印字されている。** ⇒ **それが ~0 でなければ本論法は走らない。**

**⇒ ③ ⭐ 確立している物と、していない物を分ける（結合しない）。**
- ✅ **確立: 検査は「指令された腕 ＋ 実現した指」の混合状態で測っている。** — **これはコードの事実**（`:273` / `:1786-1788`）であって、データに依存しない。⇒ ⭐ **源の修正は、5.0° の説明が付かなくても これだけで正当化される。**
- ⛔ **未確立: `5.0°` が何であるか。** ⇒ **開いたままにする。⛔ 3 つ目の物語を作らない。**

**⇒ ④ ⭐⭐⭐ 源の修正（設計・私の court）= 1 つの混合検査を、2 つの純粋な検査に分ける。**

| | **検査 A「この姿勢を指令してよいか」** | **検査 B「実際に許容内に留まったか」** |
|---|---|---|
| 状態 | ⭐ **腕 *も指も* 指令値**（純粋に指令） | **整定後の実現値**（純粋に実現） |
| 性質 | **答が指令だけの関数** ⇒ 同じ指令は常に同じ答 | **物理の答**（卓を突いたか） |
| 役割 | ⭐ **実行前に gate する**（超えたら raise） | **報告する**（必要なら raise） |
| bar | ⭐ **`τ`（＝ 解が要件を満たしたか）** | ⭐⭐ **卓接触の bound** |

⇒ ⭐⭐ **そして `A − B` は、本日ずっと欠けていた量そのものである** — **この面での指令と実現の差**。⇒ **clearance で私が頼んだ「予測 vs 実現」と *同じ構造* が、ここにも要る。** ⇒ **1 つの形で 2 箇所を覆う。**
⇒ ⭐⭐⭐ **そして B の bar は、私が §27.2.141 で取り下げた *卓接触 bound* である。** ⇒ **τ を決めないから引いた測定が、B では *正しい bar* として戻ってくる。** ⇒ **取り下げは正しく、廃棄ではなかった。**

**⇒ ⑤ ⚠ `r_max` は 検査 A で測る。** 解の残差は「指令状態で解が残す誤差」⇒ **A の量。** ⇒ ⛔ **現行の 0.0–0.1° は混合で測られている** ⇒ **分割後に読み直す。** ⇒ **`τ` は式で持ち（`(r_max + τ_max)/2`）、値は分割後の `r_max` から。**

⛔ **非主張:** variant を否定しきったと言わない（**採らない**であって、非単調性が偽だとは言っていない）／`5.0°` の原因を提示しない／STEP8 の工具軸を推測しない／run を求めない。**gate 不変。**

### 27.2.150 ⭐⭐ **非一様分布は C/D の *形* を変える — ⛔ 単一の verdict は道具が合っていない。⭐ 第 5 の選択肢 E を立てる（段の的が強いているなら、順位付けでは直らない）**

**入力:** p18 `-689`。版 = **`fab643fda7`**（コード）／t17 log は p18 の banked 逐語読（`24f9180e13`）。

**⇒ ① ✅ 条件成立 ⇒ 指の依存は *確立* した（仮説から降ろす）。**
**STEP8 の工具軸 = 両腕 0.0°** ⇒ **両段で腕側は同一の「垂直」** ⇒ ⭐⭐ **口の 4.9° の差は指関節以外から来ようがない。** ⇒ **これは算術であって読みではない。**
⇒ **残る問いは「どの指状態がどの傾きを生むか」だけで、発注済 field の仕事。** ⇒ ⛔ **私はもう機構の物語を作らない。**

**⇒ ② ✅ A/B 分割の独立収束を受領。** ⭐ **B の bar が、私が取り下げた卓接触 bound の復帰であることも一致。** ⇒ **取り下げた測定が、正しい場所で戻った。**

**⇒ ③ ⛔⛔ t17 の把持 False を clearance filter に帰してはならない。**
**探索は 44/65**（p18 報告）⇒ **姿勢被覆が切れている。** ⇒ ⭐ **被覆が違う run どうしを比べるのは、本日 私が何度も引いた統制されていない比較そのもの。** ⇒ **t14/15（True）と t17（False）は比較にならない。**
⇒ ⚠ **右腕 54° は座り段ではない**ので垂直要件は掛からないが、⭐ **54° はロール/収容トレード（§27.2.82）では非常に悪い側**であり、**「best effort」＝ どの姿勢も着座させられなかった**ことを意味する。⇒ **これも被覆 44/65 の下での話。** ⇒ **t18（全被覆）まで読まない。**

**⇒ ④ ⭐⭐⭐ 非一様分布（勝者が良い段 2・悪い段 1）は、A/B/C/D の *形* を変える。**
⇒ ⭐ **「clearance は条件数を犠牲にするか」は、filter の性質ではなく *段ごとの答* を持つ問いだった。**

| 選択肢 | 非一様と分かった後の評価 |
|---|---|
| **A**（大域の重み） | ⛔ **さらに悪い** — **局所の問題に大域の道具**。競合していない 2 段を、競合している 1 段のために歪める |
| **B**（大域の受容） | ⛔ **さらに悪い** — **ほとんど存在しない罰を、全体として受け入れる**ことになる |
| **C**（標本の産物か） | ✅ **精密化して生きる（下記）** |
| **D**（床＋傾斜） | ⭐ **競合が実在する段に限って** 意味を持つ |
| ⭐ **E（新設）** | **段の *的* が腕を近づけているなら、どんな順位付けでも直らない** |

**⇒ ⑤ ✅ C の精密化 = `tries` を上げる前に、*どの段が競合しているか* を分類する。**
段ごとの 3 つ組（**除去数・勝者 σ・除去された最良 σ**）は **既に毎段印字されている** ⇒ ⭐ **分類は新しい測定なしで今できる。** ⇒ **競合段が 1 つなら、その 1 段だけを見ればよい。**

**⇒ ⑥ ⭐⭐ E の根拠（⚠ 仮説・確認手段つき）。**
把持段の的は **`C1[0] ∓ GRIP_HALF_SPAN`** ＝ **x で 88 mm** 離れた 2 点。⇒ ⚠ **旧 `task_config.py:237-241` の衝突目的は EE 球 35+35 mm ＋ 余裕 10 mm ⇒ 分離の床 ~80.5 mm** を敷いていた。⇒ ⭐ **設計 span 88 mm は その床の 8 mm 上に座っている。**
⇒ ⚠⚠ **EE 球の分離と腕 geom の面間距離は *別の量* なので等号では結べない。** ⛔ **私は結ばない。**
⇒ ⭐⭐ **しかし「把持段では span 自体が腕を近づける」という読みは検査できる**: **競合段で、腕 geom 最小距離が *的の指令分離* によって決まっているのか、*姿勢の選び方* によって決まっているのか。** ⇒ **前者なら E（直す場所は段の幾何 ＝ p5 の court）／後者なら D。**

**⇒ ⑦ ⭐ 自分の規則の適用（§27.2.143 ④）。**
> **⑤ が偽になる読み:** 3 つ組が **全段で同符号・同程度** ⇒ 非一様でなく一様 ⇒ 大域の答が正しい。
> **計器はそれを出し得るか:** ✅ **3 つ組は既に毎段出ている。**

**⇒ ⑧ 裁定（更新）。** ⛔ **単一 verdict を出さない。** ✅ **① 段の分類（既存データ）→ ② 競合段でのみ E か D かを分ける（⑥ の検査）→ ③ D なら交換比は導出（§27.2.146 ④）。** ⛔ **A は非一様でさらに落ちる。B は最後の手段のまま。**

⛔ **非主張:** t17 の後退を filter に帰さない ／ 88 mm と 80.5 mm を同じ量として扱わない ／ 段の幾何を変えろと言わない（p5 の court）／ run を求めない。**gate 不変。**

### 27.2.151 ⛔⛔ **私の E は、工程表を読むだけで死ぬ — 競合段と非競合段は *同じ 88 mm* を指令している。⭐ 判別設計は E′ の片側検定 ＋ 有限な網羅**

**入力:** p18 `-693`（p6 の 17 段分類）。版 = **`fab643fda7`**（私の直読）。

**⇒ ① ⛔⛔ E（「段の的が腕を近づけている」）は、新しい測定なしで死ぬ。**
`:1235-1236` 逐語: **`LX1, RX1 = C1[0] ∓ GRIP_HALF_SPAN`** ／ **`LX2, RX2 = C2[0] ∓ GRIP_HALF_SPAN`**、そして把持段は `GL/GR = C1[0] ∓ GRIP_HALF_SPAN`。
⇒ ⭐⭐ **段 2・3・4（競合）も 段 7・8・9（none-dropped）も 段 11（競合）も、指令している左右間隔は *すべて同じ 88 mm*。**
⇒ ⛔ **したがって「span が腕を近づけている」は 2 群を区別できない。** ⇒ **私の E は反証される。**（**自分の提案を、既に disk に在る表で潰す。**）

**⇒ ② ⭐ 実際に群を分けているのは *高さ* に見える（⚠ 相関であって機構ではない）。**
競合＝**高い側**（`Z_RISE_REST` / ケーブル高 / `Z_RISE_ROUTE`）／ none-dropped ＝ **低い側**（`Z_SEAT`）。
⇒ ⛔ **なぜそうなるかは述べない。** ⇒ **私は機構の物語をもう作らない**（§27.2.150 ①）。⇒ **観測として置く。**

**⇒ ③ ⭐⭐ E を作り直す（E′）— 「span が強いる」ではなく「*この段の的（位置と高さ）に到達する姿勢の中に*、隙間と条件数を両立するものが無い」。**
⇒ **これは段ごとの性質であり、⑤ の検定が測るのはこれ。**

**⇒ ④ ⚠ L 側だけに出ていることは、純粋に幾何的な E から遠ざける。**
clearance は **対の量**（`min over ARMG[L] × ARMG[R]`）⇒ **両腕で同じ数**。⇒ ⭐ **にもかかわらず条件数の罰は L 側だけ** ⇒ **差は共有の隙間からは来ない。腕ごとの条件数の地形から来る。**
⚠ **左右は鏡像ではない**（基部・閉じ軸とも・§27.2.123 ②）ので **非対称自体は起こり得る** ⇒ ⛔ **これだけで D と断定はしない。** ⇒ **言えるのは「純粋な E では説明が足りない」。**

**⇒ ⑤ ⭐⭐⭐ 判別設計（求められたもの）= 2 段構え。**

**第 1 段（只・1 フィールド・片側検定）:** **候補集合の `near_far_arm` の *最大値* を、min ベースの判定の隣に印字する。**

| 読み | 結論 |
|---|---|
| **max ≫ bar** | ⭐ **その的に隙間の在る姿勢は *存在する*** ⇒ ⛔ **E′ は反証** ⇒ **D**（または C = 良条件の物を引けていない） |
| **max ≈ bar** | ⚠ **E′ を *示唆* するだけ** — 標本不足と区別できない |

⇒ ⭐ **片側であることを明記する: この検定は E′ を *潰せる* が、*立証* はできない。**

**第 2 段（第 1 段が潰せなかった時のみ）= 網羅。** 6 自由度腕は EE 姿勢を固定すると解が **離散（分枝 ≤ 8）** ⇒ 姿勢メニューが自由な段でも **8 × 65 = 520/腕** ⇒ ⭐ **有限で列挙可能** ⇒ **E′ は原理的に *厳密に* 決まる**（統計でなく）。⇒ ⛔ **費用は p4 の court。**

**⇒ ⑥ ⭐ 自分の規則の適用（§27.2.143 ④）。**
> **⑤ 第 1 段が偽になる読み:** max が bar 近傍 ⇒ **E′ を潰せない**（＝ 検定が空振る）。
> **計器はそれを出し得るか:** ✅ **`near_far_arm` は全候補について既に計算されている** — **`min` を取っている所で `max` も取るだけ。**

**⇒ ⑦ 裁定（更新）:** ⛔ **E は死亡（①）。** ✅ **E′ を第 1 段で潰しにいく → 潰れれば D（交換比は §27.2.146 ④ で導出）／潰れなければ第 2 段で厳密に決める。** ⛔ **A は非一様下でさらに落ち、B は最後の手段のまま。**

⛔ **非主張:** 高さが原因だと言わない（相関のみ）／L 非対称から D と断定しない／網羅の費用を見積もらない／run を求めない。**gate 不変。**

### 27.2.152 ⭐ **t18 の落下は §27.2.144 の族ではない — §27.2.133 (c) の族である。⭐⭐ そして直し方が違う（しかも p4 は既にその直し方を持っている）**

**入力:** p18 `-697`（FYI・依頼なし）。⛔ **私は crash のコードを読んでいない** — 以下は p18 の記述（「guard 付き行の隣に無防備な元の行が残存・None 比較で落下」）に対する **分類** である。

**⇒ ① ⛔ 2 つは似て見えて、別の壊れ方である。**

| | 述語は動くか | 何が起きるか |
|---|---|---|
| **§27.2.144（零点検査）** | ✅ **動く** | ⛔ **動いた上で「異常なし」と言う** — 不具合が零点を完璧に再現するため（**盲目な述語**） |
| ⭐ **t18 の落下** | ⛔ **動かない** | **無防備な経路が横を通る**（**迂回される検査**） |

⇒ ⭐ **したがって t18 の親戚は §27.2.144 ではなく §27.2.133 (c)** — **相続経路が `continue` で垂直要件の手前を抜けていた**、あれと同じ形である。

**⇒ ② ⭐⭐ 直し方が違う。ここが分類の実益。**
- **盲目な述語** ⇒ **校正点を足す**（零点だけでなく span も）。
- ⭐ **迂回される検査** ⇒ **経路が合流する場所へ検査を移す。**
⇒ ⭐⭐⭐ **そして p4 は既にその処方を持っている** — **垂直要件の use-site 検査**（両経路が合流して `w[t]` が決まった *後* に測る）がまさにそれ。⇒ **t18 の落下は同じ class の別の場所なので、同じ処方が効く: guard を *生産者の隣* でなく *使用点* に置く。**

**⇒ ③ ⭐⭐ さらに 1 つ上の親戚: §27.2.122 の 第 3 の場合（量は生・数は死）の *コード版*。**
今回の変更は **guard 付きの行を *足し*、元の無防備な行を *消さなかった*** ⇒ ⭐ **「直した物が、直される前の物の隣に生き残った」** — comment の古い数が新しい数の隣に生き残るのと同じ形。

**⇒ ④ ⭐⭐⭐ そして重要な非対称: 同じ手が、記録では正しく、コードでは誤りである。**
p4 の `fbc868cbe9`「**拒否された測定の隣に無荷重の測定を置く**」は **記録に対しては正しい形**（消さず、真である文を隣に置く）。
⇒ ⛔ **しかし *実行される経路* に対しては、隣に置くとは「両方が走り得る」ということである。**
⇒ ⭐⭐ **記録は読まれる・コードは走る。** ⇒ **「消さずに隣へ」は前者の徳で、後者では欠陥。** ⇒ **同じ規律を両方に当てると、今回の形になる。**

⛔ **非主張:** crash のコードを読んだと言わない（分類のみ）／p4 の処方を評価しない／実装しない／run を求めない。**gate 不変。**

### 27.2.153 ✅ **count を批准する（私の max は *私自身の* cutoff 分割で壊れていた）— ⚠ ただし count が潰すのは E′ の *片脚* だけ。⭐⭐⭐ そして只の追加 1 つで、問い自体が消えるかもしれない**

**入力:** p18 `-700`（p4 `-167`）。

**⇒ ① ✅ 批准。⛔ そして私の field は「弱かった」のではなく *逆の問いに答える* ものだった。**
判定 loop は **16 mm cutoff**（私が §27.2.145 ④ で設計した分割）⇒ **余裕の在る候補は距離を持たずに返る** ⇒ ⭐ **内側の `max` は「混み合った候補の中の最大」** ＝ ⛔ **私が知りたいことの逆。**
⇒ ⭐⭐ **私の 2 つの設計が干渉し、実装者が出荷前に捕らえた。** ⇒ **§27.2.141 で私が心配した「依頼を集合として見ていない」の、実装側からの 2 例目。**（1 例目は私が自分で見つけた §27.2.145 ④。）

**⇒ ② ✅ count は代替でなく *上位互換* である。** 「16 mm を丸ごと超えた候補の数」は **距離値を必要としない** ⇒ **cutoff に影響されない** ⇒ **`count > 0` は「余裕のある姿勢が存在する」を直接に立てる。** ⭐ **片側性も保たれる。**

**⇒ ③ ⚠ ただし正確に言う: count が潰すのは E′ の *片脚* だけ。**
E′ ＝ **「隙間 *かつ* 条件数」を両立する姿勢が無い** ＝ **連言。**
⇒ **`count > 0` が立てるのは「隙間の在る姿勢は在る」だけ** ⇒ ⛔ **それら全部の条件数が悪ければ E′ は生きたまま。**
⇒ ⭐ **私自身の形（連言は脚ごとに）が、私の検定に当たった。**

**⇒ ④ ⭐⭐⭐ 修正案（只・そして これで問いが消える可能性がある）= 「生き残り集合の中の *最良 σ*」を勝者 σ の隣に印字する。**
現行の指標は **勝者 σ vs 除去された最良 σ**（例 STEP2 L = 0.0453 vs 0.1205・2.66×）。⇒ ⛔ **これは 2 つの別物を混ぜている:**

| 混ざっているもの | 分けるための数 |
|---|---|
| **filter が良い候補を落とした** | **除去された最良 σ** |
| ⭐ **cost 関数が、生き残った良い候補を選ばなかった** | ⭐⭐ **生き残り集合の最良 σ**（＝ *未印字*） |

⇒ ⭐ **勝者は `_cost`（ロール ＋ 移動量 ＋ σ 不足）で選ばれるので、生き残りの中の最良 σ とは限らない。**
⇒ ⭐⭐⭐ **もし「生き残りの最良 σ」≈「除去された最良 σ」なら、filter は何も奪っておらず、競合は *順位付けの中* に在る。** ⇒ **D でも E′ でもなく、cost の重みの話になる。** ⇒ **問いの所在が変わる。**
⇒ ⭐ **費用ゼロ**: σ は全候補について既に計算されている（`cands` に格納）。

**⇒ ⑤ ✅ tier-2（測定時のみ広半径）に同意。** ⛔ **私からは要求しない**（指示待ちのままでよい）。

**⇒ ⑥ 自分の規則の適用（§27.2.143 ④）。**
> **④ が無益になる読み:** 全段で **生き残りの最良 σ ＝ 勝者 σ** ⇒ cost は既に最良条件の生き残りを選んでおり、追加の情報は無い。
> **計器はそれを出し得るか:** ✅ **σ は全候補について既に在る。**

⛔ **非主張:** count を私が実装しない ／ tier-2 を要求しない ／ 競合が順位付けに在ると *主張* しない（④ は検査であって結論ではない）／ run を求めない。**gate 不変。**

### 27.2.154 ⭐⭐⭐ **E′ は死んだ — ただし count でなく「生き残りの最良 σ」が殺した。⭐ 手放されている条件数は filter でなく *しきい値の上で平らな cost* に行っている。⛔ そして垂直検査は、代理が margin 付きで通り、その代理が立っている量が落ちていた**

**0. 接地（私が読んだもの・中継からでない）。** `T20_RUN_TRACE_20260728.txt` @ `c65e0efc8b`、content sha256 `d2cd4bcccb2e0d277f9183be743348c46a5d3d701a736ecfcc7eb652711f33e8`（**私が `git show <commit>:<path> | sha256sum` で算出し、宣言値と一致**）。code は同 commit の `ur15_steps_wired.py`（**行番号は動くので、以下すべて `@ c65e0efc8b` を同じ節に付す**）。

---

#### ① ⭐⭐ E′ の片側検定 — 26 読みの全数（私の事前登録どおりの形）

13 段 × 2 側 = **26 読み**。定義: **Δ_rank = 生き残りの最良 σ − 勝者 σ**（順位付けが *生き残り集合の中で* 手放した分）／**Δ_filter = 除去された最良 σ − 生き残りの最良 σ**（filter が実際に奪った分）。⭐ **旧指標（除去された最良 − 勝者）はこの 2 つの和**であり、私が「filter の代価」と読んでいたものは和だった。

| 判定 | 件数 / 26 |
|---|---|
| 何も除去されていない | **9** |
| 除去は在るが、**除去された最良 ≦ 生き残りの最良**（filter は何も奪っていない） | **14** |
| **Δ_filter > 0**（filter が本当に良い候補を奪った） | ⭐ **3** — STEP2 L (+0.0335) / STEP11 L (+0.1344) / STEP12 R (+0.0577) |
| **Δ_rank > 0**（順位付けが生き残りの中で手放した） | ⭐⭐ **12** — 最大 STEP7 R **+0.1974**、次いで STEP12/13/14 L **+0.1037** 各 |

⇒ ⭐ **cleared-radius count は 26 読みすべてで ≥ 1**（最小 = 1、STEP3 R / STEP4 R）⇒ **E′ の「隙間」脚は一度も 0 にならない。**
⇒ ⭐⭐⭐ **決定的な 1 読み = STEP7 R** `:149`: `clearance removed 0 of 27 candidates, 27 of them roomy, winner sigma 0.1108 (best survivor 0.3082)`。⇒ **filter は何も除いていない。それでも 2.78 倍の条件数が手放されている。**
⇒ ⛔ **E′（「隙間 *かつ* 条件数」の連言）は棄却 — ⚠⚠ ただし *腕間 filter に対して* である。** **本表の除去会計は mast を含まない filter が出した数字**（mast は t21/t22 から filter に入る・p18 `-710` (2)）⇒ **件数は mast 込みへ自動的には持ち越せない。射程 = §27.2.155 ③。** 競合は *この filter では* 隙間と条件数の間には無い。**私の §27.2.153 ③ の懸念（count は片脚しか潰さない）は正しかったが、もう一方の脚は同じデータの別欄が潰した。**

#### ② ⭐⭐ では条件数はどこへ行ったか — **cost は SIGMA_GOOD の上で厳密に平ら**

`:1306-1307 @ c65e0efc8b`: `_short = max(0.0, SIGMA_GOOD - c[5]) / SIGMA_GOOD` ／ `2.0*c[4] + ‖c[0]-ref‖ + SIGMA_PENALTY*_short`。
⇒ **dCost/dσ = −SIGMA_PENALTY/SIGMA_GOOD = −3.0/0.12 = −25**（単位 = m / 単位 σ）**が σ < 0.12 の側だけ。σ ≥ 0.12 では厳密に 0。**
⇒ ⭐⭐ **σ = 0.3082 は σ = 0.12 に対して credit を一切得ない。** ⇒ STEP7 R は「重みの偏り」ではなく **cost が上側で条件数に盲目**なのである。
⇒ ⚠ **p4 の `SIGMA_FLOOR` コメント逐語「Ranking, not rejection, is the way to do this.」** — ⛔ **shortfall 項は片側の ranking** ＝ **床は filter を出て、cost に *半分だけ* 入り直した。** 意図は半分しか実装されていない。
⇒ ⭐ **D は生き残る。ただし軸が変わる:** 取引は σ 対 **隙間** ではなく、σ 対 **座り誤差・参照距離**（`2.0*c[4] + ‖c[0]-ref‖`）である。§27.2.146 ④ の「交換比を導出」は、**この対**について導出する。

#### ③ ⛔ **それでも私は k を出さない**（既存の勾配は上へ延ばせない・本 trace は σ が何を買うか測っていない）

既存勾配の素朴な延長（`max(0,·)` を外す）は **σ = 0.3082 に −4.7 m の credit** を与え、他項を全部支配する。⇒ **−25 m/単位σ は「わずかに足りない」ことへの罰としては通っても、「大きく上回る」ことへの報酬としては不合理。** ⇒ **単調項は自前の k を要し、k は継承でなく導出。**
⇒ ⛔ **`SIGMA_PENALTY = 3.0` は私のもので、依然として未測。本 trace はいかなる penalty 値も正当化しない。**（自分の数に自分の規則を当てる。）

#### ④ ⭐ σ の代価について trace が言うこと = **必要な向きには何も無い**

- 本 run 最低の σ（**L 0.0377** @ STEP3/4 `:70`/`:105`）で `GRASP L: joints vs commanded [-0. 0. -0. 0. 0. -0.] mrad -> servo arrived` `:92`。
- 到達しない側（R）は σ が **高い**（0.2059）が、原因は条件数ではない: `:100` `-1606.3 mrad` / 狙いから **112.5 mm** / mouth が `[+0.459 −0.662 +0.354]`（~1 m 外）。
⇒ ⭐ **本 trace には「低い σ が追従の失敗と同居する」読みが 1 つも無い。**
⚠ **射程（自分で狭める）:** joint err は **静止時**の読み。条件数の代価は**過渡**（関節速度・飽和・方向の喪失）で出る。飽和と act force は **STEP1 でしか印字されない** `:27-29`。⇒ **「静止時に代価は観測されない」までが言えること。過渡は未測。**
⇒ 決める読み = **移動中の飽和フラグと |q̇| 最大**（code は STEP1 で既に持っている形）。⛔ **要求はしない** — p4 のペースで、必要になった時に。

#### ⑤ ⭐⭐⭐ 垂直検査 — **代理が margin 付きで通り、代理が立っている量が落ちていた**

| 段 | 傾き | margin | **同じ行の直接量** |
|---|---|---|---|
| STEP7 L `:142` | 0.0° | **+5.69** | `lowest point … −0.1 mm vs the table` |
| STEP8 L `:158` | 4.2° | **+1.50** | `… −0.3 mm` |
| STEP9 L `:174` | 3.5° | **+2.23** | `… −0.5 mm` |

⚠ **過小主張しない:** 3 件はいずれも **1 mm 未満** ＝ 接触公差の域であり、この gate を産んだ欠陥（`1a1efe0ac5` の **25.5 mm** 貫通）ではない。
⭐ **効くのは R @ STEP10 の `−135.8 mm vs the table <- THROUGH THE TABLE`** `:193`。⇒ **そこで検査は走っていない。** `_seating` は `abs(tgt.z − Z_SEAT) < 1e-9` の段のみ（`:1896-1900`）＝ 工程表 `:1472-1484` より **7, 8, 9, 15, 16, 17**。**STEP10 は含まれない。**
⇒ ⭐⭐ **§27.2.152 の bypass 族に数値が付いた: 本 run 最大の貫通は、検査が覆っていない段で起きている。**
⇒ ⭐ **B 側の処方はデータで裏づいた** — **実現値を table 接触境界に対して *全段で* 報告**し、傾きの bar は診断へ降格する。（A は指令状態で gate、bar = τ。§27.2.148 の分割は変えない。）

#### ⑥ ⭐⭐ **-149 の純化は必要だったが十分ではない — 同じ指令が今も 2 つの読みを返す**

code は直っている: `:1988-1991 @ c65e0efc8b` が jaw を **その段自身の指令**へ駆動して `PREDICT_S` だけ settle させる（live qpos の丸写しではない）。
⚠ **それでも:** STEP8 L と STEP9 L は **同じ腕目標**（`(LX1, C1[1], Z_SEAT)`・表 `:1473`/`:1474`）**・同じ指令開度 HALF=214** でありながら、
`:158` **pads +10.78 mm / 傾き 4.2°** ／ `:174` **pads +11.73 mm / 傾き 3.5°**。
⇒ ⭐⭐ **読みは指令だけでは決まっていない。** 証明された担い手 = **jaw が cable に阻まれ指令へ到達できない**（同一指令で pad gap が違う）。⛔ **腕 solve 側の寄与は排除していない**（同じ目標でも継承経路で `w` が違い得る）。
⇒ **「この pose を *指令してよいか*」を問う gate が、指令が決めない量を読んでいる。** ⇒ A/B 分割は依然必要で、**A は「到達できない jaw をどう扱うか」を書かないと閉じない**（指令値で置く／到達不能を自己申告する、のいずれか）。

#### ⑦ ⚠ **allowance は自分の上界の *上* に座り、しかも code の誰も見ていない**

- **5.73° = degrees(0.10 rad) = 姿勢 menu の最小非零 roll**。傾き ≈ roll は §27.2.123（実測 18.5°/30.0° 対 記載 17°/32°）。
- 私が批准した規則は **τ < τ_max** を要求 ⇒ ⛔ **interim 定数は最終値として不適格**（境界そのもの）。p5 の規則が返す値は **2.915°**。
- ⭐⭐ **cap は印字されるだけで比較に入っていない:** `:1581-1583` が印字、`:2036` の判定は `if _off > VERTICAL_TOL_DEG` のみ。⇒ ✅ **だから deferred 404 (ii) の cap 0.00 は t20 のいかなる verdict にも触れていない**（「interim 定数だから無傷」より強い理由で無傷。**私自身が `:2036` で確認**）。⇒ ⛔ **裏返し = cap が直って ≈5.7° を返しても、allowance がその上に居ることに code は気づかない。**
- ⇒ **処方（§27.2.124 と同型・只）: 起動時に `allowance < cap` を assert し、破れたら raise。** 黙る guard を自己申告させる、の同じ形。
- ⚠⚠ **順序が効く:** 2.915° へ締めると **STEP8 L (4.2°) と STEP9 L (3.5°) が落ちる**。⛔ **だがそれは ⑥ の artifact 読みである。** ⇒ **読みを直してから bar を締める。逆順は偽の失敗を作る。**

#### ⑧ ⭐⭐⭐ **Rs の報告は *左* で、count は右で取られている**

Rs 逐語「**左ハンドが円柱に激突している**」。回送された count（R: 8/13 ＋ `−0.0` が 1）は **R 列**であり、**私も同じ数を独立に得た**（R = `−0.6` が STEP 3,4,5,6,11,12,13,14 の **8 件**、`−0.0` が STEP10）。
⭐ **L 列は STEP3 `:70` と STEP5 `:117` で `+0.0 mm`。** `column_gap` の定義は `:1100-1103 @ c65e0efc8b` 逐語「*Closest signed distance from this arm's geoms to the yoke column, in mm. Negative means the arm is inside the column.*」
⇒ ⭐ **印字は `%+7.1f`（`:2222`）ゆえ `+0.0` は「支柱まで 0.05 mm 未満」を意味する。** ⛔ **計器の床ではない**（負値は表現可能 — R が `−0.6` を出している）。⇒ **符号と接触は主張する。量は主張しない。**
⇒ ⭐⭐ **analyst の brief に L のこの 2 読みを載せないと、Rs が見た事象を反対側で探すことになる。**
⚠ 併せて `:45` `STEP 2 ARM-TO-ARM: closest +0.0 mm (11 <-> 67) <- TOUCHING OR THROUGH`（**腕同士**であって支柱ではない — 混ぜない）。
⚠ **走行が STEP15 R で落ちたため、run 全体の最悪値を出す summary（`:2345-2346` `WORST … column gap … <- INSIDE THE COLUMN`）は印字されていない。** ⇒ **現有は per-step の読みだけ**であり、「最悪はここ」とは誰も言えない。

#### ⑨ deferred 404 (ii) の形（私の court に触れる分だけ）

**「1 つの名前・2 つの選択」で、較正が *欠陥を見られない側* で選んでいた** ＝ ⑥ と同じ形（片方は指令、片方は実現）。⇒ ⭐ **一般形: 同じ集合を 2 通りに選ぶと、両方を見る述語が無い限り、食い違いは沈黙する。** ⇒ 私の側の適用は ⑦ の assert（2 つの量を **1 つの比較**に入れる）。

#### ⑩ 自分の規則の適用（§27.2.143 ④ / §27.2.131）

- **①-② を覆す読み:** 「filter が良い候補を奪った段が多数」＝ Δ_filter > 0 が過半。⇒ 実測 **3/26** ⇒ 覆らない。
- **⑤ を覆す読み:** STEP10 で検査が走っていた。⇒ `_seating` の定義（`:1896-1900`）で走らない。
- **⑧ を覆す読み:** L の `+0.0` が計器の床。⇒ 負値が表現可能ゆえ床でない。**ただし量は 0.05 mm 未満としか言えない。**
- **④ を覆す読み:** 低 σ と追従失敗が同居する段が在る。⇒ 本 trace には無い。**⚠ 過渡は未測ゆえ、これは「無い」ではなく「静止時には無い」。**

⛔ **非主張:** k を出さない ／ SIGMA_PENALTY を変えろと言わない ／ allowance を今すぐ締めろと言わない（順序が先） ／ run を求めない ／ 動画・物理妥当性の判定をしない（pC/pB/Rs） ／ 実装しない。**gate 不変。**

### 27.2.155 ⭐⭐⭐ **(b) の裁定 = *母集団分離*（時間分離では、母集団が変わった日を検出できない）。⭐⭐ mast の 8 mm 流用 review — 数字は受けるが、*その数字を意味づけていた理由* は移らない**

**接地:** p18 `-710`（台帳 407 @ `7eeafc3692`）／ p4 code 実測点 `:2049`/`:2105 @ ffbc1f15af`（**p4 の報告値・私は本 commit を読んでいない ⇒ 以下の裁定は実装事実に *依存しない* 形で書く**）。

#### ① ⭐⭐⭐ (b) 裁定: **母集団分離。時間分離は却下。**

**事実（p4 報告）:** `r_max` は合流点の **全読み** — solved も inherited も。`pose_rd` は aim の call site にしか無く、供給集合を選ばない。
**私の裁定（形の話であり、今日の数字の話ではない）:**

> **`r_max` の母集団 = 「上向きを *指令された* pose」のみ。gate の母集団 = 「座り段に到達した pose すべて」（solved / inherited を問わず）。この 2 つは同じ集合であってはならない。**

**理由（1 行で決まる）:** `r_max` の定義は「**上向きの指令が実際に残す残差**」である。⇒ **inherited pose は上向きを指令されていない**（`aim_both :658-670` は roll の menu から選ぶ ⇒ 構成上 *傾いている*）。⇒ **述語を満たさないものを、その述語の統計に入れている。**
⇒ ⛔ **帰結が悪い向きに出る:** inherited の傾きが `r_max` を押し上げ、`τ > r_max` ゆえ **bar が緩む** ⇒ **gate が捕まえるべき当のものが、gate の閾値を決めている。** ＝ 「bug の下で緑になった gate は bug に検証されている」／「検証集合を検証対象から作るな」と同じ形。
⇒ ⛔ **時間分離（先の run で測って凍結し、以後それを使う）は却下:** **category error を直さず、たまたまの混合比で凍結するだけ**。⇒ **今日 ∅（偶然純）であることが、まさに時間分離の危険の証拠** — 今日は正しく見え、相続が座り段に入る日に黙って壊れる（§27.2.133(c)）。

**実装可能性（自分の規則「決める量を私は測れるか」）:** ✅ **code は既に必要な情報を持っている** — 上向き solve の call site を通ったか否かは solve 側で分かる。⇒ **solve が返す pose に「上向き指令で作られた」flag を立て、`r_max` はその flag を持つ読みだけを集める。** 新しい計測も新しい定数も要らない。
**副次的な帰結（明示しておく）:** この分離の下では、**inherited pose の傾きが τ を超えれば落ちる。それが正しい** — 上向きを指令されていない pose に、上向きである資格は無い。⇒ ⭐ **t20 の STEP15 R（11.2°）の停止は、本裁定の下では gate が仕事をした形であり、bug ではない。** ⛔ **残る欠陥は 2 つだけ:** (i) **報告位置**（solve でなく check で出る）(ii) **相続が、判定されていない pose を座り段まで運べること**（§27.2.152 の bypass 族）。
**私の falsifier（先に置く）:** 「inherited pose も別経路で上向きを指令されている」なら分離は誤り。⇒ **`aim_both` は roll menu から採るので、構成上そうならない。** ⇒ 発火しない。⚠ **ただし `ffbc1f15af` の当該 2 行は私は読んでいない** ⇒ **実装がこの形かの確認は p4/pZ の court。**

#### ② ⭐⭐ mast が selection filter に入った件 — **数字は受ける。理由は移らない。**

p4 自身が commit message で design call として flag（逐語）: *"It is tested now exactly where the far arm is tested, by the rule Rs approved for the far arm, and unconditionally … The distance is the same cable diameter; reusing it rather than inventing a second number is a design call and is flagged as one"*。

**✅ 受ける部分:** ⭐ **無条件に、遠い腕と同じ場所で試す** — これは私の §27.2.152（検査は経路が合流する所へ置く）と同じ形であり、**bypass を作らない置き方**である。⛔ **第 2 の数字を発明しないという判断も支持する**（未測の定数を増やさない）。
**⚠ 移らない部分（これが私の review の中身）:** **8.0 mm = 2×CABLE_R を腕間で意味づけていたのは「ケーブルがそこを通れなければならない」という理由**（`ur15_cell_spec.py:657` 前段のコメント）。⇒ ⛔ **腕と支柱の間をケーブルが通る必要は無い。** ⇒ **同じ数値が、別の測定面で使われている**（「同じ定数 ≠ 同じ measurement surface」）。⇒ **支柱側の要求は「通す」ではなく「当てない」＝ 追従誤差の余裕**であり、**§27.2.138 で私が「欠けている」と言った項そのもの**。
⇒ ⭐ **裁定: interim として受け入れる。ただし *正当化されている* とは記録しない。** 正当化する読み = **腕が実際に示す最悪の追従誤差**（trace が腕間で既に印字している `predicted minus realised` と同じ計器）。⛔ **私はその測定を要求しない。**
⇒ ⚠ **向きの明示:** 腕間の 8 mm は「最低通過幅」＝ 下限。支柱の 8 mm は「接近禁止」＝ 別種の下限。**数値が同じでも保守側かどうかは別に決まる** ⇒ **現時点では未決**（追従誤差が 8 mm を超えれば非保守）。

**⭐⭐⭐ 本件が t20 のデータと出会う所（これは強い）:** t20 の実測では **L が支柱まで 0.05 mm 未満（STEP3 / STEP5）**、**R は 8 読みで −0.6 mm 内側**（§27.2.154 ⑧）。⇒ **8 mm を bar とする filter が t20 時点で効いていれば、これらの pose を生んだ候補は除去され得た。** ⇒ ⭐ **Rs が見た事象（左ハンドが円柱に当たる）に対して、本 design call は正しい向きである。**
⚠ **ただし厳密には「除去された」とは言えない:** filter は **solve 時の候補**を見、`column_gap` は **静止時の実現値**を読む。⇒ **両者の差＝追従誤差**は、まさに ⚠ で欠けていると言った項。⇒ **向きは確か、量は未証明。**

#### ③ ⛔ **私の §27.2.154 ① の射程を、私が狭める**

**t20 の E′ 材料（26 読みの除去会計）は、mast を含まない filter が出した数字である。** mast が filter に入れば **除去は増える** ⇒ ⭐ **「Δ_filter > 0 は 3/26」は mast 込みの filter へは自動的に持ち越せない。** ⇒ **E′ は *腕間 filter に対して* 棄却された、と読む。**
⭐⭐ **持ち越せる部分と持ち越せない部分を割る（撤回の範囲も測定・§07-26）:**
- ✅ **持ち越せる = §27.2.154 ②（cost が `SIGMA_GOOD` の上で厳密に平ら）。** これは `:1306-1307` の **code の事実**であり、filter が何を除去しようと変わらない。⇒ **「条件数は filter でなく cost の平坦部で手放されている」という *機構* の主張は生きる。**
- ⛔ **持ち越せない = ① の件数と、STEP7 R の `best survivor 0.3082` の値。** filter が候補を除けば、生き残りの最良も下がり得る ⇒ **Δ_rank の大きさも変わる。**
⇒ ⭐ **必要な再測は t21/t22 の同じ 3 欄で足りる（新しい計器は要らない）。** ⛔ **要求はしない。**

### 27.2.156 ⭐⭐⭐ **(b) の最終形を閉じる — 母集団分離は必要だが足りない。⛔ gate は自分の母集団を bar の所で打ち切る ⇒ 「τ > r_max」は完走 run では恒真。⭐ 記録上の計画は、自分の言葉で自分を打ち消している**

**接地:** p18 `-715`（台帳 409 @ `2cc1ae6601` / p5 bank #16 @ `45b539a901` §18・**p5 の入力は裁定でないと明記されている**）。数値は `T20_RUN_TRACE_20260728.txt` @ `c65e0efc8b`（私が読み、pin 自算・一致）。

#### ① ⭐⭐⭐ p5 の「残る自己参照」に名前を付ける — **打ち切り（censoring）である**

trace `:35` 逐語: *"the allowance is an interim **until a run reports the worst residual an upright command actually leaves**"*。
⇒ **`r_max` は run から来る。** ⇒ そして `:2036 @ c65e0efc8b` は `_off > VERTICAL_TOL_DEG` で **raise して run を終わらせる**。
⇒ ⭐⭐⭐ **gate が armed である限り、run が記録できる読みは τ 以下だけである**（τ を超える読みは、run の最後の 1 行として 1 個だけ現れる）。⇒ **完走 run から取った `r_max` は必ず τ 以下** ⇒ **「τ > r_max」は恒真** ＝ **異なる結果になり得ない検査**。
**t20 の実測がその形をしている:** 記録された傾きは **8 個** — `0.0`×3 / `0.1`×2 / `3.5` / `4.2` / **`11.2`（これが run を終わらせた）**。⇒ **5.73 を超える読みは 1 個だけで、それは打ち切り点そのもの。** ⇒ ⛔ **t20 は、どの母集団を取っても `r_max` の推定に使えない。**
⇒ ⭐⭐ **記録上の計画（`:35`）は自分を打ち消している:** 「run が最悪残差を報告するまでの interim」だが、**gate が armed な run は最悪残差を報告できない**。

#### ② ⭐⭐ 最終形 = **直交する 2 軸**（p5 の「時間分離は取り直し条件へ格下げ」を、捨てずに第 2 軸として吸収する）

> **軸 1 — 母集団分離（どの読みを数えるか）:** `r_max` の母集団 = **上向きを指令された pose のみ**。gate の母集団 = **座り段に到達した pose すべて**。⇒ **category error を直す**（§27.2.155 ①）。
> **軸 2 — 武装分離（測る間、bar は噛んでいないこと）:** `r_max` を測る pass では **gate は raise しない（報告のみ）**。⇒ **打ち切りを直す**（本節 ①）。

⇒ ⭐ **この 2 つは別の欠陥を直しており、片方では閉じない。** 母集団だけ直しても、armed な run は τ 以下しか記録しない。武装だけ外しても、傾いた相続 pose が `r_max` を押し上げる。
⇒ ⭐⭐ **軸 2 は私が §27.2.148 で既に切った A/B 分割そのもの**（A = 指令状態で gate、B = 実現値を報告）であり、**§27.2.124 の形**（恒真になる guard は消さず、自己申告させる）でもある。⇒ **新しい機構は要らない。**

**⇒ 閉じた順序（4 段・厳密に順序つき。各段は既に別々に規定済み）:**
1. **計器を直す** — 同一指令が 2 つの読みを返す状態を解消（§27.2.154 ⑥）。⛔ **これより前に測った数はすべて死ぬ。**
2. **武装を外して測る** — 座り段の全 pose を報告のみで通し、**上向き指令の pose だけ**から `r_max` を取る（軸 1 ＋ 軸 2）。
3. **批准済みの規則で τ を導出** — `r_max < τ < τ_max`、`τ = (r_max + τ_max)/2`、⛔ **値が無いと言えること**（`r_max ≥ τ_max` なら raise）。
4. **武装する。** ⭐ **同時に `allowance < cap` を起動時 assert**（§27.2.154 ⑦）。

#### ③ ✅ **τ の数値はすべて do-not-bank に同意 — ⭐ そして順序の帰結を足す**

p5 の `r_max = 0.1` 撤回に同意する。⭐ **正しい撤回理由は「STEP9 が相続かどうか」ではない** — **計器が混合状態を読んでいたから、0.1 も 5.0 も一緒に落ちる**（§27.2.148/-149）。⇒ **これは私の「同じ壊れた計器から出た数は、個別にでなくまとめて死ぬ」形。**
⇒ ⭐ **帰結（上の順序 1 が効く所）: 計器を直す前に測り直せば、新しい数も同じ理由で死ぬ。** ⇒ **interim 5.73 の継続に同意**。⛔ **ただし 5.73 は自分の上界そのものであり（§27.2.154 ⑦）、最終値の候補ではない。** ⇒ **「暫定として使う」と「候補として残す」を混ぜない。**

#### ④ ⭐ **§27.2.154 ⑥ の *強化*（撤回ではない）** — p18 の t17 実測（相続段 = {3,4,13}・STEP9 = 0）を受けて

私の ⑥ は「同一指令（`(LX1, C1[1], Z_SEAT)`・`HALF=214`、工程表 `:1473`/`:1474`）で pads +10.78 / +11.73 mm、傾き 4.2° / 3.5°」であり、**どちらが相続かに依存していなかった**（担い手 (a) 解の差 と (b) jaw が cable に阻まれる、を並記し (a) を排除していないと明記した）。
⇒ ⭐ **p18 の実測で STEP8 も STEP9 も *解かれた枝* と分かった** ⇒ **相続は担い手から外れる。** ⇒ **残るのは (a) 同一目標でも solve が違う `w` を返す（seed = solve 時の live 状態）と (b) jaw の阻止**。⇒ **⑥ は狭まって強くなる: 指令が同じで、相続も無く、それでも読みが違う。**
⛔ **私は p18 の t17 を再測していない** — 上の帰属は **p18 の実測に依存する**（依存を明示する）。

#### ⑤ ✅ p18 の訂正（t20 は未完走）を受ける — ⭐ **それは ① の証拠でもある**

私の §27.2.154 は t20 が STEP15 R の raise で終わったことを読んで書いている（trace `:280-284`）ので、節の内容に訂正は要らない。
⇒ ⭐⭐ **むしろ「未完走」は本節 ① の実例である:** **run を終わらせたのは gate 自身**であり、**その終わらせ方が、`r_max` を測るための標本を打ち切った。** ⇒ **「未完走」は事故の記述ではなく、armed な gate の下では *設計どおりの* 帰結。**

#### ⑥ 自分の規則の適用（falsifier を先に置く）

- **① を覆す読み:** `r_max` が run でなく menu / 別の非破壊 pass から来ている。⇒ trace `:35` は run と言っている。⚠ **ただし `r_max` を集める code を私は読んでいない**（`ffbc1f15af` 未読）⇒ **もし非破壊 pass が既に在るなら軸 2 は充足済みで、私の処方は「既に在る」と読み替わる。実装の court は p4/pZ。**
- **② 軸 1 を覆す読み:** 相続 pose も別経路で上向きを指令されている。⇒ `aim_both :658-670` は roll menu から採る ⇒ 発火しない（§27.2.155 ①）。
- **④ を覆す読み:** STEP8/9 の L 目標か jaw 指令が実は違う。⇒ 工程表 `:1473`/`:1474` を読んで同一。

⛔ **非主張:** 測定を要求しない ／ τ の数値を出さない ／ 実装しない ／ p18 の t17 を追認したとは言わない（依存として引用しただけ）。**gate 不変。**

### 27.2.157 ⭐⭐⭐ **交点の答: 死んだ数は問いを *開ける* が、*閉じる* ことはできない。⭐⭐ そして「どれが死んでいるか」は既に印字されている — 同じ行の 2 本目の計器が、指を読んでいない**

**接地:** p18 `-722`（台帳 413 @ `cd129d6d7b`）／ 数値は `T20_RUN_TRACE_20260728.txt` @ `c65e0efc8b`、code は同 commit の `ur15_steps_wired.py`（**すべて私が読んだ**）。⛔ t17 の値（p5 の二項機構の例 5.0/0.0）は **私は読んでいない — p18/p5 の実測として引く**。

#### ① ⭐⭐⭐ 交点の一般形

> **bar を *置く* には値が要る。bar が *建たない* と言うには存在だけで足りる。**

- **τ の算出**（`τ = (r_max + τ_max)/2`）は `r_max` の **値**を使う ⇒ ⛔ **死んだ数ではできない。私の -156 ③ はそのまま立つ。**
- **rebuild 分岐**（`r_max ≥ τ_max` ⇒ raise）は **不等式**であり、**下界が 1 つ在れば発火する** ⇒ ⭐ **値を知る必要が無い。**
⇒ ✅ **答: rebuild 条件は宣言できる。** ⛔ **ただし 11.2 を `r_max` として bank することも、それから τ を計算することもできない。**
⇒ **標語: 死んだ数は問いを開ける。閉じられるのは生きた数だけ。**

**⚠ ただし条件が 1 つ要る:** 汚染が **下界を膨らませ得ない**こと。⇒ 次項がそれを与える。

#### ② ⭐⭐⭐ 同じ行の 2 本目の計器 — **tool axis は指を読んでいない**

`:279` 逐語（私が読んだ）: `STEP15 R: fingers 11.2 deg off straight down …; tool axis 11.2 deg off vertical`。
**code の基礎:** `:2027-2028 @ c65e0efc8b` `_tool = np.array(_sv.xmat[TOOLB[t]]).reshape(3,3)` / `_tres = degrees(acos(-_tool[2,2]))` ⇒ **工具 *body* の姿勢**。⇒ ⭐ **`pinch()` も `slot_centre()` も通らない ＝ 私が -149 で指摘した混合（指の状態）を経由しない。**
⇒ ⭐⭐ **11.2 は指の artifact ではない。腕が本当に 11.2° 傾いている。** ⇒ **汚染は下界を膨らませていない** ⇒ ①の条件が満たされる。
⚠ **正確に言う:** 両者は同じ scratch `_sv` を共有するので、**腕関節の代入が誤っていれば両方汚染される。** ⇒ **「私が同定した欠陥に対して独立」であって「絶対に独立」ではない。**

#### ③ ⭐⭐ **判別子は既に印字されている — 8 読みの *全数* が分離した**（標本ではない）

| 段 | fingers | tool axis | 判定 |
|---|---|---|---|
| STEP7 L `:142` / R `:143` | 0.0 / 0.0 | 0.1 / 0.0 | 一致・言うことなし |
| **STEP8 L `:158`** | **4.2** | **0.3** | ⛔ **指が支配（14 倍）⇒ 死** |
| STEP8 R `:159` | 0.1 | 0.1 | 一致 |
| **STEP9 L `:174`** | **3.5** | **0.8** | ⛔ **指が支配（4 倍）⇒ 死** |
| STEP9 R `:175` | 0.0 | 0.0 | 一致 |
| STEP15 L `:278` | 0.1 | 0.1 | 一致 |
| **STEP15 R `:279`** | **11.2** | **11.2** | ✅ **腕側・厳密一致 ⇒ 存在主張として生きる** |

> **判別子: `_off ≈ tool axis` なら腕側（欠陥を越えて生き残る）／ `_off ≫ tool axis` なら指が支配（死）。両方とも既に印字されている ⇒ 費用ゼロ。**

⇒ ⭐⭐ **これは私の -156 ③ の *狭め* である（私は消しすぎた）。** 「stage 1 前の数は全部死ぬ」は**強すぎた** — 正しくは **「指が支配する読みが死ぬ。腕側の裏づけを持つ読みは *下界* として生きる」**。⛔ **それでも値としては使えない**（①）。
⇒ ⚠ **p5 の t17 例（STEP9 L 5.0 / 0.0）は、この判別子で *指支配 ＝ 死*** ⇒ **p5 の除外は正しく、理由もこの列で説明できる**（⛔ t17 の値自体は私の未読・p18/p5 の実測）。

#### ④ ⭐⭐ 副産物 — **§27.2.154 ⑥ の担い手が絞れた**（自由に、既存の列だけで）

⑥ で私は「担い手 (a) 同一目標でも solve が違う `w` を返す / (b) jaw が cable に阻まれる、(a) は排除していない」と書いた。
⇒ ⭐ **tool axis が (a) を測っている:** STEP8 L で **0.3°**、STEP9 L で **0.8°** ⇒ **腕はどちらも鉛直から 1° 以内** ⇒ **(a) は ≤0.8° に有界で、4.2° の読みを説明できない。**
⇒ ⭐⭐ **(b) が支配する、と言える。** ⇒ ⑥ は再び狭まって強くなる（指の状態が読みを作っている）。

#### ⑤ **rebuild 条件が *認可するもの* を限定する**

✅ **条件は MET と宣言できる**（①②③）。⛔ **それが認可するのは *再導出* であって、姿勢 menu / spec の編集ではない。**
⇒ **編集は生きた数（stage 2 の測定）を待つ。** ⇒ **死んだ数は問いを開けるが、閉じない**（①）。
⭐ **保守側の向き（§運用15）:** 偽の発火 = 不要な再導出（安価・設計 review であり run ではない）／ 見落とし = **存在しない bar を出荷する**。⇒ **非対称ゆえ、不確かな読みでも *開ける* 側に倒すのが正しい。** ⛔ **ただし「開ける」までで、閉じる側には倒さない。**

#### ⑥ ✅ p5 の「被覆は母集団の上流」を採る ⇒ **形は 3 軸になる**

> **軸 0 — 被覆（どの段を検査するか）** ⇒ **軸 1 — 母集団（どの読みを数えるか）** ⇒ **軸 2 — 武装（測る間 bar は噛まない）**

⇒ **p5 は正しい:** 検査されない段には母集団の問いが起きない。⇒ ⭐ **私の STEP10 の −135.8 mm（検査が走らない段での最大貫通・§27.2.154 ⑤）は、軸 0 の実例**である。⇒ **-156 ② の 2 軸を 3 軸へ更新する。**（4 段順序は不変。軸 0 は順序 1 の前に効く = **何を検査するかが決まっていなければ、直す計器の範囲も決まらない**。）

#### ⑦ ⭐⭐⭐ 鋭い帰結 — **打ち切りを逃れた唯一の読みが、bar は建たないと言っている**

§27.2.156 ① の形（armed な gate は自分の標本を bar で打ち切る）の下で、**τ を超える読みは run につき 1 個 = 終端の 1 個だけ**。⇒ **t20 でそれは STEP15 R の 11.2°。**
⇒ ⭐ **そしてそれは *上向きを指令された* pose である**（`:277` に aim 行・相続印字は {3,4,13} のみ = **p18 の実測に依存**）⇒ **軸 1 の母集団に入る。**
⛔⛔ **本項は §27.2.158 ① で撤回された**（11.2 は座り段の解きの受理窓 17.19° の内側 ⇒ 機構の残差でなく設定の残差）。以下は撤回された推論として残す。
⇒ ⭐⭐⭐ **`r_max ≥ 11.2° > cap ≈ 5.73°`** ⇒ **批准済み規則の第 3 分岐（値が無い ⇒ raise）が、下界だけで発火する。**
⇒ **`:35` の計画（「run が最悪残差を報告するまでの interim」）に対する答は、皮肉な形で出ている: run は報告した。報告された数は、bar が建たないことを意味する。**

#### ⑧ 自分の規則の適用

- **② を覆す読み:** tool axis も指を読んでいる。⇒ `:2027-2028` は工具 body の `xmat` のみ ⇒ 発火しない。⚠ **腕関節代入が誤っていれば両方倒れる**（明示済み）。
- **③ を覆す読み:** `_off ≫ tool axis` なのに指が原因でない段が在る。⇒ 8 読み全数を見て無い。⚠ **8 読みは t20 のみ ⇒ 他 run へは持ち越さない。**
- **⑦ を覆す読み:** cap が 5.73 より大きい。⇒ cap は現在 0.00 を返しており（404(ii)）、**修正後の値が 11.2 を超えるなら分岐は発火しない** ⇒ ⭐ **これが唯一の生きた反証経路であり、cap の修正結果を待つのが正しい。**⛔ **要求はしない。**

⛔ **非主張:** τ の値を出さない ／ menu・spec の編集を認可しない ／ t17 を再測したと言わない ／ 実装しない ／ run を求めない。**gate 不変。**

### 27.2.158 ⛔⛔ **交差 1 を認め、撤回する — 私は *自分が既に持っていた* 汚染経路だけを潰し、それで受理判定が閉じたことにした。⭐⭐⭐ そして 2 つの交差は同じ 1 つの原理で、その原理の一覧は *開いている*（今それを言えるのは、閉じていると言って外したばかりだからである）**

**接地（すべて私が自分の copy で読んだ）:** `ur15_steps_wired.py @ c65e0efc8b` — 座り段の解き `:1952` `quiet=True, re_max=0.30, wide=True, pose_rd=(0.0, 0.0)`（同型が `:1964`）／ aim 側 `:645` `re_max=0.02` ／ 既定 `:1176` `re_max=0.05` ／ 棄却判定 `:1254` `if pe > 0.002 or re_ > re_max:`。⇒ **座り段の解きは向き残差 0.30 rad = 17.19° まで受理する。検査許容 5.73° = 0.10 rad の 3 倍・aim の 15 倍・既定の 6 倍。** ✅ **p5 / p18 の実測どおり。**

#### ① ⛔ **撤回する 1 点**

⛔ **「t20 が rebuild 条件の充足を示した」を撤回する。**
**理由:** 11.2° は **解きの受理窓の内側**にある。⇒ **解きは設定どおりに振る舞っただけ** ⇒ **11.2 が測っているのは「上向き指令が機構に残す残差」ではなく「この解き設定が受理した残差」。** ⇒ ⭐ **私の恒常の失敗形そのもの: 測った量に、それが測っていない結論をぶら下げた。**

**✅ 残るもの（撤回の範囲も測る）:** ①**値 / 存在の非対称**（bar を置くには値、建たないと言うには下界）／②**判別子**（`_off` 対 `tool axis`）／③**rebuild 分岐そのもの**（消えていない）／④**11.2 は指の artifact ではなく本物の腕の傾き**（`:2027-2028` の基礎は有効）。
⛔ **落ちるもの:** 「**t20 が示した**」の 1 点のみ。⇒ **p5 の射程宣言と一致する。**

#### ② ⭐⭐⭐ **なぜ間違えたか（機構を正確に）**

**私は、自分が既に手に持っていた汚染経路（指）を潰し、それで *受理判定が閉じた* ことにした。** ⇒ **§27.2.157 ③ の「8 読みの全数が分離した」は、*1 つの判別子についての* 全数**であって、**受理可能性についての全数ではない。**
⇒ ⭐ **これは私自身の「✅ には射程を付ける」に、自分の ✅ が違反した形。** ⇒ **「全数」という語が、検査した軸の名前を落として運ばれた。**

#### ③ ⭐⭐⭐ **2 つの交差は同じ 1 つの原理 — そして一覧は開いている**

> **`r_max` が測れるのは、pose を決めているのが *指令だけ* である読みに限る。**
> **pose を共同決定する経路 — 指の状態（私 -149）／解きの受理窓（p5 ④）／飽和した関節・外部接触（p4）— はいずれもその読みを `r_max` について無効にする**（他の用途については無効にしない）。

⛔ **この一覧が完全だとは主張しない。** ⇒ **完全だと言った直後に 1 つ増えたからである。** ⇒ **受理は「既知の共同決定経路それぞれについて、非活性の証拠を *その読みが携えている*」形で書く。**
⭐ **各経路には既に印字可能な証人が在る:** 指 → **tool axis**（`:2027-2028`）／窓 → **`re_max` 対 許容**（`:1952`）／jam → **`saturated` と接触**（trace `:27-29` に STEP1 の実例が在る）。

#### ④ ⭐⭐⭐ **rider ④ の帰結を足す — 母集団 C は *空* になる**

現行の座り段の解きは **すべて `re_max = 0.30`**（`:1952` / `:1964`）＝ **17.19° > 許容 5.73°**。⇒ **rider ④ の下で、座り段の読みは 1 つも資格を持たない。**
⇒ ⭐⭐ **`r_max` は「未知」ではなく「現行構成では *測れない*」。** ⇒ **stage 1 に 1 項足す必要がある:**

> **stage 1 = 計器を直す ＋ *受理窓を検査許容以下にする*。** ⛔ **これを飛ばすと stage 2 は受理可能な読みを 1 つも産まない。**

⚠ **窓を締めると、解きは「傾いた pose を返す」代わりに「**解けない**」に変わり得る。⇒ ⭐ **それが正しい挙動であり、p4 はその計器を既に作っている** — code コメント逐語「*(2) the tool's own orientation residual against the vertical it was commanded, so an empty window -- a solve that cannot reach upright at all -- is visible as itself rather than as a tilt*」。⇒ **締めた後の世界の計器は既に在る。**

#### ⑤ ✅ **交差 2（p4「自由な腕」）を受ける — ただし run 単位の前提でなく *読み単位の受理条件* に直す**

⛔ **run 単位の「腕が自由であること」は検証できない**（いつ？どこで？）。✅ **読み単位の「この読みの関節は上限に居らず、外部の物体が pose を支えていない」は検証でき、既に印字できる。**
⇒ ⭐⭐ **これは staging の変更ではない** — **stage 2 の定義（「*受理可能な* 読みで測る」）の中に入る。** ⇒ **p4 の躊躇（staging 変更を主張しない）は、これで解ける。**
⇒ ⭐ **§27.2.154 ④ との合流:** 私はそこで「条件数が代価を持つかを決める読み ＝ 移動中の飽和と `|q̇|`、STEP1 でしか印字されない」と書き、**要求しないと決めた。** ⇒ **いま同じ欄が `r_max` の受理条件になった。** ⇒ **独立な 2 つの必要が 1 つの既存欄に着地した。**

#### ⑥ ⭐⭐ **jam の署名は t20 に既に在る（私が読んだ）**

R の `column gap` は **STEP11 / 12 / 13 / 14 で −0.6 mm**（`:204`/`:216`/`:235`/`:265`）、STEP10 で **−0.0**（`:192`）。⇒ **`column_gap` の定義（`:1100-1103`「負は支柱の内側」）により、R 腕は 4 段連続で支柱の内側に在った。** ⇒ **11.2° の読みはその直後の段（STEP15 R）。**
⛔ **jam が 11.2 を作ったとは言わない**（因果は立てていない）。✅ **言えるのは: 同じ読みが 3 つの受理条件のうち 2 つで落ちる** — 指（✅ 通った）／窓（⛔ 落ちる）／jam（⛔ 疑いがあり、非活性の証拠が無い）。
⇒ ⭐⭐ **撤回は 1 経路でなく 2 経路で正しい。** ⇒ ⭐ **そして p4 の t22 jam は t22 で生まれたのではない — 私が読んだ t20 の banked data に既に写っている。**

#### ⑦ ⭐ **いま言える唯一の判定**

⛔ 「rebuild 条件は満たされた」でもなく ⛔ 「満たされていない」でもない。
✅ **「現行構成では判定できない」** — 根拠は `:1952` の 1 行（受理窓 17.19° > 許容 5.73°）。⇒ ⚠ **これは「未知」より強い主張である**（測っていないのではなく、**この設定では測れない**）。
⇒ **「死んだ数は問いを開ける、閉じられるのは生きた数だけ」は保つ。⭐ 足すのは: 生きた数を作るには、まず *窓* を閉めねばならない。**

#### ⑧ 自分の規則の適用

- **①/④ を覆す読み:** 座り段の解きに `re_max ≤ 0.10` の経路が別に在る。⇒ `:1952`/`:1964` の 2 か所とも 0.30。⚠ **grep は `re_max` の綴りに閉じている ⇒ 別名で渡す経路は排除していない。**
- **⑤ を覆す読み:** 飽和も接触も無いのに pose が指令から外れる経路が在る。⇒ ⛔ **排除していない。それが ③ の「一覧は開いている」の意味である。**
- **⑥ を覆す読み:** `−0.6` が計器の床。⇒ 同じ列が `−0.0` と `+328.2` を出しており床ではない（§27.2.154 ⑧ と同じ論法）。

⛔ **非主張:** τ の値を出さない ／ rebuild を宣言しない（撤回した） ／ `re_max` の新しい値を提案しない（窓 ≤ 許容 という *関係* だけを言う） ／ 実装しない ／ run を求めない。**gate 不変。**

### 27.2.159 ⭐⭐ **1 語の答: 採らない — 同一裁定ではない。⭐ p5 の主張は真で、しかも私のより *決定可能* である。2 つは「次の行為」で一致するが、「窓を閉めた後」で分かれる**

**接地:** p18 `-730`（台帳 420 @ `f641e328b8` / p5 `-151` 台帳 422）。code は `:1952 @ c65e0efc8b`（私が読んだ）。

#### ① **2 つは別の問いである**

| | 主張 | 何に依存するか | いつ決まるか |
|---|---|---|---|
| **p5 −151** | 「現構成に両立する許容は無い」 | **`:1952` の 1 行のみ**（解きが 17.19° を受理する）＋ cap 5.73 | ⭐ **今**（run 不要） |
| **私 −158** | 「`r_max` は現行構成では測れない ⇒ 分岐の真偽が判定できない」 | **実現された最悪残差**（規則が実際に消費する量） | 窓を閉めた後 |

⇒ ⭐ **p5 の方が決定可能性で強い。私はそれを真として採る。** ⛔ **しかし同じ主張ではない。**

#### ② ⛔ **同一にすると 1 つ消える — rebuild 分岐**

- **p5 の読み**では、窓を 5.73 以下に締めれば **両立不能は構成上ほどける** ⇒ **「窓を閉めれば問題は解ける」と読める。**
- **私の読み**では、窓を締めて初めて **`r_max` が測れるようになる** ⇒ **測った `r_max` が `τ_max` 以上なら、分岐は *その時に本当に* 発火する。**
⇒ ⭐⭐⭐ **窓を閉めても、許容が存在する保証は無い。** ⇒ **2 つを 1 つの裁定に畳むと、「窓を閉めれば済む」に丸まり、批准済み規則の第 3 分岐（値が無いと言えること）が黙って退役する。**
⇒ ⛔ **だから採らない。** ✅ **採るのは「次の行為が一致する」ことまで — 窓を閉める、そして menu/spec は編集しない。**

#### ③ ⭐⭐ **p5 の再接地が撤回済みの数を密輸していないか（私が確かめた）**

**保証の主張として読むなら密輸していない** — 「解きが 17.19° を *受理し得る*」は `:1952` の仕様であり、11.2 を要しない。✅
⚠ **実現系の主張として読むと密輸する** — 「解きが実際に 5.73 を超える pose を返す」という前提が要り、**それを供給できた唯一の観測が 11.2（受理不能）** だからである。
⇒ ⭐ **従って「1 語」の中身は、賛否ではなく *読みの固定* である:** **p5 の主張は「保証についての主張」として採る。**（**可能性についての主張であって、実現についての主張ではない。**）

#### ④ **区別する検定（両者を分ける唯一の読み）**

> **窓を 5.73 以下に締めた後、上向き指令の読みで観測される最悪残差が `τ_max` 未満か否か。**
> **未満 ⇒ 許容は存在する**（p5 の不能は構成由来であり、解けた）。**以上 ⇒ 分岐が本当に発火する**（私の未判定が「充足」に確定する）。
⇒ ⛔ **今は答えられない。それが私の −158 ⑦ の意味であり、変わらない。**

#### ⑤ FYI 3 件の取り込み（⛔ いずれも私は再測していない・出所に帰属する）

- **(i) `g43` = `R_forearm_link`**（p4 `-177`・t23 live）⇒ ⭐ **私の §27.2.154 ⑧ / §27.2.158 ⑥ で見た R の支柱内側は *前腕* である。** ⇒ **Rs が報告した「左ハンド」とは *別の部品・別の事象*。** ⛔ **混ぜない。** ⚠ **左が t23 live で +73.3 clear なのは t23 の話**であり、**私の L +0.0（STEP3/STEP5）は t20 の読み** ⇒ **run を跨いで持ち越さない**（§27.2.157 ⑧ で自分に課した射程がそのまま効く）。
- **(ii) pB `-506`**: `g43` が 14 本中 11 勝・両符号 ／ 第 2 の部品 `g67` −1.1 (STEP18) ／ **t8 の −1728.4 mrad と t22 の j1 −1.728 rad が 4 桁一致** ⇒ ⭐⭐ **jam は t22 で生まれたのではなく、同じ関節・同じ値で反復している** ⇒ **私の §27.2.158 ⑥（jam の署名は t20 に既に在る）と同じ向きの、独立で *より古い* 証拠。**
- **(iii) producing pin 訂正**（t22 = `ffbc1f15af` 産・`4c2cd5dc77` は着地）⇒ ✅ **私は t22 を pin で引いていない** ⇒ **私の側に訂正は無い。**
