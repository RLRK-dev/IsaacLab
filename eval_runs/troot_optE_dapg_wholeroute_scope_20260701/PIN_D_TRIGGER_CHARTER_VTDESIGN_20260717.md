...
- **escape guard 実在**: `_c1_escape_after_seat`（`:1443-1457`）= c1_latched（G3 = `_g_latched[w,2]`、`:1652`）∧（dx_c1 == MISS `9.0`〔m sentinel、⚠R3 訂正: 全 dx は m 単位ゆえ 9.0 m・「9.0mm」は単位誤り、`:1301`〕∨ dx_c1 > `60mm`（`:412`））→ `c1_escape` → `dropped`（`:1652-1660`）→ `r = TERM_PENALTY -10`（`:1714-1715`）→ terminate（`:1720`）。materials 一致。
...
- **⭐ 非-crutch の構造保証（⚠R3 訂正 = pN catch・§9.7.9、2026-07-17 22:5x — 訂正前「fire capture ~5mm > retention 3.5mm の loose-weld margin」は誤り: match_tol=5mm は eq world-position resolution〔`route_executor.py:1014` 逐語〕で capture 幅でない）**: 正 = `clip_capture_check`（`:981`）と `authorize_clip_pin`（`:1024`）は両方 `clip_capture_predicate(…, rc.SEAT_LAT_BAR_M, …)` = **3.5mm 同一 lateral bar + [821,836] z band**（containment-by-identity）+ fire は depth ≤831 を追加 ⇒ **fire strictness ≥ retention**（lateral 同 3.5mm・z は fire [821,831] ⊆ retention [821,836]）⇒ pin は retention より緩い seat で発火できない = **false c1_retained を機構的に捏造不能**（元 margin 論より強い保証・loose-weld band 不在ゆえ L-DB-I は不能 → §9.7.9 で fire⊆retention leg L-DB-I′ に置換）。post-fire escape-inertness は faithful（clipped cable は escape しない = clip の定義）。
- 定数 10 項（SEAT_LAT 3.5mm / Z 821·836mm / Z_FIRE 831mm / K 3 / MISS 9.0 **m** sentinel / DROP 60mm / G bonus 5 / G6 200 / TERM -10 / K_ROUTE 10）= materials と on-disk 一致（rc:174-185/104-107、env:412/1301; ⚠MISS 単位 = m、R3）。
...
K=3（`rc.PIN_TRIGGER_DWELL_K`）を design-time 定数 RATIFY。**design-time safety（今）**: fire = capture ∧ depth(≤831) ∧ K=3 は genuine depth-seat（rim-fire は §8.11.2 除外）+ same-snapshot containment（fire ⇒ authorizer-accept、§8.10.2）+ §9.7.0 の 3.5-5mm margin。K=3 physics frame は sub-RL-step（1 RL step 内の PHYSICS_STEPS_PER_RL frame の一部）ゆえ seated RL step で満たされる（policy が RL step 跨ぎ dwell を「選ぶ」必要なし）。**⭐ spurious-fire = load-bearing リスク**: K が非 settling transit で発火 → weld が bad seat lock → escape 永久不発 → false c1_retained（Q-Db6 gate② corruption と連結・K の仕事 = spurious fire を殺す）。**empirical DoD（deferred、trainer 要）RATIFY**: policy rollout で fire-vs-first-true-seat lag ≤ small bound ∧ 同一 RL step 内に volume を出る transit で不発火 — 違反なら K↑。worst-case transit bound = capture-volume extent ÷ max cable speed（α-6D residual 15mm/step `:401` + capture geom）= options 入力（裁定でない）。(d-a) DR-ON M1 / cell-2 DoD-7 と同 deferred pattern（trainer = fork-B V0）。
  - ⚠ **deferral 境界 refinement（§9.7.8、2026-07-17 22:5x）**: 本項の「empirical DoD 全 deferred」は pN B1 circularity 指摘により **over-broad** と判定 → **§9.7.8 が supersede**: spurious-rejection **MECHANISM** = binding-now（prelaunch two-key leg L-DB-G、R1/R2 精密化付き）/ deferred = **live-policy fire-rate のみ**。
...
### §9.7.8 🔒 Q-Db4 deferral 境界 refinement（pN B1 circularity → CONFIRM、v1.10、2026-07-17 22:5x — prereg v0.2 `9b85e2dec8` §0-B1/§4-B1 の p5-confirm 要請を受けて。全 cite p5 自読 @ HEAD `469435014f`＝`5b0de67402` と code 同一）
**pN B1（CRITICAL）= 妥当**: §9.7.3 の「empirical DoD deferred」は **over-broad** だった。spurious-fire 棄却 K を trainer 後へ全 defer すると、(d-b) two-key が K 未検証で PASS → training-ready 解除〔(d-a)∧(d-b) two-key∧cell-2〕が K 未検証で成立し、K 検証は trainer を要す = **循環**（load-bearing property が、それが gate する当のもの＝訓練の後にしか検証されない = 「test that cannot come out differently / gate-validated-under-the-bug」class）。
**refinement = CONFIRM（reversal でなく強化）**: K の spurious-rejection **MECHANISM** は deterministic-testable ゆえ **binding-now**（prelaunch (d-b) two-key leg = **L-DB-G**、prereg §3.1:101-107 / §4:126-129）。**deferred は live-policy fire-rate のみ**（実 policy が spurious transit を生むかは policy を要す = post-launch monitor、§8.2/§9.7.3 re-open trigger）。
**⚠ L-DB-G leg 精密化（2 点、design-axis 必須 — CONFIRM の条件）**:
- **R1（discriminating 変数）**: leg (ii)「same-RL-step transit → no fire」は不正確。dwell counter は **consecutive physics frame** を数え（`:1854`）、RL-step 境界で reset されず across-RL-step 持続（reset = gap `:1852` と episode clear `:1906/:471` のみ）。かつ PHYSICS_STEPS_PER_RL=10（`:404`）≫ K=3 ゆえ **1 RL step 内の transit も ≥K dwell すれば正しく発火**。⇒ 判別変数 = **consecutive-frame dwell < K**（RL-step 境界でない）。sweep は fire/no-fire を **dwell 長（in-volume 連続 frame 数）**に対し map: dwell ≥ K → fire / dwell < K → no fire。
- **R2（gap-reset negative control 追加）**: strict-consecutive（`:1852`「any gap resets」）を突く leg = dwell K−1 → 1-frame gap → dwell K−1 → **no fire**（総 in-volume ≥K だが K 連続でない）。これが無いと total-count（非 consecutive）実装 bug が L-DB-G を素通り ⇒ leg の discriminating 力に必須。
**その他 v0.2 の §9.7 接触点（design-axis 読み、CONFIRM）**: L-DB-A（WITH/WITHOUT deadlock contrast; ⛔「FAIL before G3 不可」+「reachable ≠ G6 SUCCESS latch 必須」= §9.7.0 deadlock + アンカー方法論に厳密一致、強く confirm）/ L-DB-I（loose-weld 3.5-5mm → no false G6 = §9.7.0 safety margin を named falsifiable leg 化、confirm）/ **sync Q-1（§9.7.1 probe item）= RESOLVED 受理**（warp 1.13.0 `array.numpy()` auto-sync `types.py:4083-4086`；(d-a) FF pin が explicit sync なしで body_q を読み two-key PASS 済＝corroboration；version-pin warp 1.13.0 を probe が assert）/ **L-DB-K perf**（per-frame `body_q.numpy()` sync copy 10×/RL-step）: 0.8 throughput floor = recommended 妥当だが **final bar は probe-time の measured baseline に対し設定**（V0 implicit 継承しない、%12 flag に concur）；full-array read は最適化 carry（seat body slice / reward-site read 再利用 / physics-step body_q defer = %12 列挙 fallback で足る、設計は full-array を mandate せず）。
**M2（DAPG loader consumer assert）→ V0/prelaunch acceptance 明示化 = CONFIRM**（曖昧な training-era でなく named acceptance item、prereg §1 / §carry）。
⇒ **B1 refinement + R1/R2 = CONFIRM〔p5 設計軸〕**。本節が §9.7.3 deferral 境界を supersede（deterministic spurious-rejection = binding-now / live-policy = deferred）・§9.7.6 negative legs に L-DB-G（R1/R2 精密版）を含む・carry C1 更新（「empirical K deferred」→「live-policy fire-rate deferred；deterministic spurious-rejection binding-now L-DB-G」）。⛔ training-ready 未解除は不変。0-commit — bank = %12。

### §9.7.9 🔒 pN v0.2 readback（HOLD）design-axis 3 点 = R3 訂正 / R1 再定義 / R4 freeze（v1.11、2026-07-17 22:5x — 入力 = pN v0.2 readback via %12 + p5 on-disk 再検証 `route_executor.py:977-1028`。R2/R5+%12 doc 訂正は %12 v0.3 fold）

**R3〔CRITICAL・§9.7.0 factual 訂正〕= pN 正・CONFIRM（forward-only）**:
...
**R1〔B1 spurious 再定義〕= §9.7.8 R1 が governing + 残余/境界 明記**: 「same-RL-step transit → no fire」は K=3 から導けない（1 RL step=10 frame `:404`、inside ≥3 frame は現 predicate が正しく fire）。**spurious の定義 = inside capture∧depth の consecutive physics frame < K のみ**（<K=no-fire / ≥K=fire、§9.7.8 R1/R2）。⚠**残余 risk + training-ready 境界の明記（pN 要請）**: deterministic sweep は **MECHANISM**（dwell-count 棄却 + gap-reset）を validate するが **live-policy が生む fire 分布**（頻度・timing）は捕捉不能 = post-launch monitor（deferred）。**training-ready 解除は live-policy leg を待たない** — 根拠 = **R3 の fire strictness ≥ retention**（≥K dwell の fire は capture∧depth = genuine seat ゆえ早期 fire でも genuine retention・spurious でない）⇒ 循環なし。live-policy monitor が pathology を示せば §8.2/§9.7.3 re-open（re-open trigger 明記）。

**R4〔perf bar freeze + N framing 訂正〕= CONFIRM**:
- (a) **throughput ratio bar = ON/OFF ≥ 0.8 を run 前に凍結**（p5 設計 bar・Rs override 可）。ratio は matched ON vs OFF の self-normalizing 比ゆえ pre-known absolute baseline 不要・pre-run freeze 可。<0.8 なら batch/slice-read 最適化を landing 前に mandate。
...
- **RETENTION surface = 補間 crossing**: `c1_retained = _seated_in_groove(dx_cross, z_cross)`（`:1625/:1629`）、`_seat_metrics(cable_pos,_C1_XY)` → `_seat_crossing`（`:1304-1317` = y=C1Y での polyline 補間、reward `:1623`）。**⭐ 決定的**: `_seat_crossing` の docstring（`:1307-1310`）は gate② fix が「nearest-in-Y **NODE**」（Y-quantization residual を off-axis に漏らす旧 metric）から **意図的に補間 crossing へ移した**と明記 ⇒ body-fire（node）と crossing-retention（補間）は **設計上別 surface**。⚠**precision（pN v0.4 R4、materials 4e3d39080b と sync）**: 両 surface の gap は **proven distance bound を持たない** — docstring の ~7.5mm は Y-quant residual **class** の magnitude であって本 gap の bound でない（surface が別である事実が design-relevant で、bound 値でない）。
...
| (d-b) deadlock-removal verify（§9.7.0） | `newton_route_env.py:1443-1457`（`_c1_escape_after_seat`: c1_latched ∧ dx==9.0 ∨ >60mm）/ `:1652-1660`（`c1_escape`→`dropped`）/ `:1714-1715`（drop→-10）/ `:1629`（`c1_retained`=c1_seated=幾何）/ `:1704-1711`（G6 = c1_retained∧c2_honest∧¬drop∧span_ok×10）/ `:1687-1688`（G4/G5 = policy route）/ `RS71-System-Spec-SSOT.md:66`（§4 FIDELITY BOUNDARY: planar bender・horizontal routing kinematic w/ pin）/ `:56`（pin = Y-slide insurance）/ 定数 rc:174-185·104-107, env:412·1301（MISS 9.0 = m sentinel、R3）|
| R3 fire-strictness≥retention（§9.7.9） | `route_executor.py:981`（`clip_capture_check` → `clip_capture_predicate(…, rc.SEAT_LAT_BAR_M=3.5mm, …)`）/ `:1024`（`authorize_clip_pin` 同 predicate 同 3.5mm bar）/ `:1004-1006`（containment-by-identity docstring）/ `:1014`（`match_tol_m=5e-3` = eq world-position resolution・**capture 幅でない**）⇒ fire lateral=retention 3.5mm・fire z⊆retention |
