# W1 B1 (route_t 骨格) — conformance 表 v2 (5体 [VERIFY] fold 済、%12+%9 verify 対象)
...
| R12 | disposition 5/6 state_bank world-slice case | B1 追加。**判別力条件 (CC3-CH2)**: per-world **相違**の synthetic bank (tile 由来の同値 bank では slice バグが不可視) で world w の書込値 == w の block を assert + 他 world sentinel 不変。**+ `assert 0 not in state_bank`** (k=0 不変量 pin、CC3/CC4/CC5 3者収束) **+ flag-OFF `route_t ≡ episode_length_buf` unit assert (強制 per-world reset 後)** (CC6 条件 — done-mid-run 経路の穴を閉じる) | B1 |
