...
**著者 / 原因側:** SKILL-DETAIL-DESIGN (`w2:p5`)。**本節の執筆時刻:** 2026-07-26 20:17:26 JST（`date` 実測）。⚠ 本節は **uncommitted** ゆえ **bank 時刻は存在しない** — landing 後に author date を追記する（無い時刻を書かない）。
...
- **process:** N1-N9 の編集と本節は **uncommitted**（pN 指示 = **§7 冒頭「契機」の 5 通一覧**〔`-004` / `-005` / `-006` / `-007` / `-008`。全桁 stable ID は当該行に記載 — 重複を避けて参照する。⚠ 旧記述は `-004`/`-005`/`-006` で止まり **N1-N9 を生んだ `-007`/`-008` が欠落**していた: pN `MSG-PN-P5-FINGERTIP-H4-DRAFT-RETURN-20260726-009` R2 で訂正〕）。⛔ **本 leg で私が編集した path = 3 件**（pN `MSG-PN-P5-FINGERTIP-H4-DRAFT-RETURN-20260726-010` S1 で訂正）:
  1. **correction artifact 2 path**（tracked / modified / staged 0）= 本書 ＋ 旧 artifact `P5_ESCALATION_fingertip_offset_franka_legacy_20260721.md`。
  ...
  ⚠ git **外**の memory index `MEMORY.md` も同様に固定（scope report は pN へ別掲）。⇒ 「repo 内 / repo 外」「tracked / untracked」を混ぜずに数える。⛔ commit / `--no-verify` / 無関係 file の編集を **行っていない**。source / `[CHANGE]` / impl / RUN / verify / status / gate flip は **CLOSED**。
- ⚠⚠ **未 bank ゆえ本節は未発効** — **banked 記録の上では N1-N9 の各節がまだ active**。⇒ landing までの間、**N1-N9 を根拠にした下流主張は成立しない**ものとして扱うこと（working tree の観測を committed artifact の代わりにしない）。発効の条件 = 本 2 file が bank され、その commit と author date が本節に追記されること。
