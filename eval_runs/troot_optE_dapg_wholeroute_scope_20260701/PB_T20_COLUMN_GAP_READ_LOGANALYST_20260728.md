...
**きっかけ:** p18 `-725`（FYI・依頼なし）。**私が自分で読んだもの:** `T22_RUN_TRACE_20260728.txt`・内容 sha256 `a6bc06a4cb841fee3e1437d509fd95b6d1d05c23b6b2f2d11cec8486dd066867`（p18 申告と一致・349 行）。
⚠ **版 pin（§8-7 で訂正済）:** 取得 pin = `4c2cd5dc77`（**この trace はこの commit にしか無い** — `ffbc1f15af` には存在しません／私が `git ls-tree` で確認）。⛔ **producing = `ffbc1f15af`**（p4 -177・p18 `-728`）。⇒ **code の照合は `ffbc1f15af` で行うのが正しく、初版の私は着地 commit の code を読んでいました。**
...
## 8-2. ⛔ **本節は §8-7 で訂正しました（原因 = 私）。以下の見出しと主張は誤りです — 読む前に §8-7 へ。**

### ~~8-2. mount は、除外を外しても一度も最小値を取りません~~ ⛔ SUPERSEDED
...
⚠ 関節番号の対応は自分で確認しました（⚠ **行番号は §8-7 で訂正**: producing 版 `ffbc1f15af` では `:1762-1763`。`:1770` は着地 commit `4c2cd5dc77` の座標で、⭐**機構は両版で同一**）: driver が `_worst = int(np.argmax(np.abs(qerr)))` で `j{_worst}` と印字 ⇒ **0 始まりの添字**。⇒ `j1` = 6 要素ベクトルの **index 1** = `J6`（`:64` = URDF の運動学順）の **2 番目の関節**。`at its limit` と `act force` の True / 433 も同じ index 1 に立っています。
...
| | t8（`run_t8.txt:199` @ `823ddf963e`） | t22（`:262`・取得 pin `4c2cd5dc77` / producing `ffbc1f15af`） |
|---|---|---|
...
# 追記 §8-7 — 2026-07-28 20:18 JST ⛔ **私の §8-2 の訂正（原因 = 私。指摘 = p18 `-728` 経由 p4 `-177`）**

## 何を間違えたか

§8-2 で私はこう書きました — 「**t22 では mount geom も query に入っています。それでも R の勝者は … mount は一度も勝ちません**」。
⛔ **これは誤りです。** 私は **trace が着地した commit（`4c2cd5dc77`）の driver を「走った code」として読みました**が、t22 を産んだのは **`ffbc1f15af`** です（p4 -177）。両版は除外の有無がまさに違います:

| 版 | `COLFREE` | mount は query に |
|---|---|---|
| **`ffbc1f15af`**（19:18・**producing**） | `:1086` `sorted(ARMG[t] - MOUNTG)` | ⛔ **入っていない（除外されている）** |
| `4c2cd5dc77`（19:49・trace の着地） | `:1093` `sorted(ARMG[t])` | 入っている |

⇒ ⛔ **t22 は mount を除外した状態で走っています。**⇒ 「除外を外しても勝たない」は**観測されていません**。「勝たない」は、そもそも参加していなかったのだから**空虚**です。⇒ **§8-2 を取り下げます。**

⚠ 私は print の書式（`mast … (gN vs stem)`）で版を見分けようとしましたが、⛔ **この書式は両版に在ります**（`ffbc1f15af:2302` / `4c2cd5dc77:2310`）⇒ **書式は識別子になりません。** trace 自身から producing を決める手掛かりを、私は持っていませんでした。

## ⭐⭐⭐ 訂正の結果、逆に **§3-3 の予測が run そのもので確認されました**

§3-3 で私はこう書きました — 「⛔ **`MOUNTG` を除外しても −0.6 は消えません**」。そして反証テストの第 2 枝を「**除外を入れた版で −0.6 が残るなら、除外は −0.6 に対して無効だったと確定します**」と定義しました。

⇒ ⭐ **t22 はまさに「除外を入れた版」の走行です**（`ffbc1f15af`）。そして **−0.6 は残っています** — STEP 3 / 4 / 6 / 13 / 14 が `R=−0.6 mm (g43 vs stem)`、STEP 5 が −0.5、STEP 18 が `−1.1 mm (g67 vs stem)`。
⇒ ⭐⭐⭐ **反証テストの第 2 枝が満たされました。**⇒ **除外は −0.6 に対して無効**、すなわち **§3-3 の結論は「議論」ではなく「走行」で確認されました。**
⇒ ⛔ **§8-2 の「反証テストは前提消滅で不要」も取り下げます。** 不要になったのではなく、**既に走って私の側に出ていた**のに、私が版を取り違えたせいでそれを読み落としていました。

⚠ **第 1 枝（+387.5 は mount だったか）は t22 では評価できません** — 除外により mount は最小値を取り得ず、t22 の R 最大 +432.9 は `g42` に帰属します。⇒ ⛔ **t20 の +387.5 の帰属は依然 未決**（§8-3 の取り下げは維持）。

## 影響しない節（範囲を明示します）

- **§8-1**（g43 が両符号を出す ⇒ 動く部品で確定・第 2 の部品 g67）= ⭐ **不変**。除外が外すのは左右の `shoulder_link` geom 2 個だけで、g43 / g67 / g42 / g44 はそれに当たりません（⭐ p4 t23 live 読みでは **g43 = `R_forearm_link`**・p18 `-728` 経由。⚠ **trace 未 bank なので私は未検証**）。
- **§8-4 / §8-5**（接触の実在・t8 との接続）= ⭐ **不変**。数値は trace 側にあり、除外の有無に依存しません。⚠ 行番号のみ訂正（`:1770` → producing `ffbc1f15af:1762-1763`。⭐ 機構 `_worst = int(np.argmax(np.abs(qerr)))` は両版で同一）。
- **§1-§7（t20 本体）** = ⭐ **不変**。t20 は `0b0d4c462f` より前の版で、除外はそもそも存在しません。

## ⭐ 私が持ち帰る型

⛔ **「その artifact を含む commit」と「その artifact を産んだ commit」は別物で、bank-after-run では設計上ずれます。**私は §0 で t20 についてはこの区別を書いた（「走った code と表示された source はずれ得る」）のに、⛔ **§8 では自分でその区別を落としました。**
⇒ **code を引くときは、artifact の着地 commit ではなく producing 版を pin する。**着地 commit は **取得 pin** としてのみ有効です。

---
**pB LOG-ANALYST correction / 2026-07-28 20:18 JST**
