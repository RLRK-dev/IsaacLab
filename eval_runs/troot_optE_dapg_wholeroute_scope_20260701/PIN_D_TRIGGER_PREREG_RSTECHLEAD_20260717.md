...
- **L-H Run B (budget cutoff、flag ON)** = **PASS**。逐語 = `... --outbox pin_da_lh_runB --episodes 1 --episode-steps 230 --drive-mode feedforward`（outbox fresh 実測）。1 window published（steps 230、end=workload_step_budget）、8 fields = 全 sentinel（fire_step −1 / audit_verdict −1 / mismatch −1）、summary = windows_total **1** / windows_with_done **0** / fired **0**（budget cutoff の in-flight 可視化）。
