...
| 積分刻み | `timestep="0.002"` (500 Hz) | `DT = 1/480` (`test_newton_clip_routing.py:123`) ÷ `SIM_SUBSTEPS = 10` (⚠ **`task_config.py:101`**) = **2.083e-4 s** | ⭐ **私が 9.6 倍 粗い** |

⚠⚠ **上表の出典を 1 つ訂正します (2026-07-27 21:2x)**: 私は `SIM_SUBSTEPS` を producer の `:101` と書き、`-074` でも p18 へそう伝えました。⛔ **実際は `task_config.py:101`** で、producer は `:111` で import し `:124` で割っています。⇒ ⭐ **数値 (2.083e-4) は正しいまま**ですが、⛔ **出典が違うと「その行を見に行った」ことにならない** ⇒ 訂正します。⚠ 気づいたのは、この量を**実際にコードから読ませようとして落ちた**ためです (自分の grep 2 本の出力を 1 つの file の話として読んでいました)。
