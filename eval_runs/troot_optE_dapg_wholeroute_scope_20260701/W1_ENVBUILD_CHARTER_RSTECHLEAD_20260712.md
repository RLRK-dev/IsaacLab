...
**Status:** v0.2 **BANKED** — p1 verify 14:04 = **PASS-WITH-CORRECTIONS** (転記忠実性 全数照合 PASS / MED 2 = W-1 governance + W-2 device、両方 v0.2 で fold 済) → **B0 即時着手可 + B1+ 解禁** (p1 verdict verbatim)。改版: v0.1 13:51 DRAFT → ERRATUM-1 14:04 (B0-1 = 3 files、%11 catch) → v0.2 14:0x (W-1/W-2 fold) → **ERRATUM-2 16:3x (B1 5体 CC4-CRIT/CC5-HIGH + 3者収束: flag-OFF byte-preserve は diff 表面適合 [env 側 = ⑨a′/DoD⑤⑥⑩/cablediag anchor、producer subset は route_executor 側のみ] + B1 writesite セルの「期待値更新」→「不変-PASS assert (反転 = B3)」帰属訂正)** → **ERRATUM-3 23:4x (B2 5体 CC5-1: spec §9 :192 tail 4 checks の owner が v0.2 で未割当 — (iii)(iv) 機構 = B2 / (i)(ii) 検証 = B7 に pin)**。
...
|---|---|---|---|
| **B1 route_t 骨格** | per-world `route_t` 新設 + 時計消費者 4 系統付替え (①step_target ②grip staircase ③ff replay ④obs[50]) + interface v2 `reset_to_phase(k, world_ids=None)` (v1 consumer 5 点 disposition + **新規 consumer #6** = step() per-world done-reset の re-fork 呼出) + **flag-gate default-OFF** (spec §4.1) | §10 行1 の route_t 側 ~200-400 | route-clock 単一源 grep leg (episode_length_buf を route 系が参照 = 0) / flag-OFF byte-preserve legs (**表面適合、ERRATUM-2 — §4-2 参照**) / **state_bank world-slice case 追加 + writesite 不変-PASS assert (期待値反転 = B3、spec §4.1 :76 verbatim「bank v2 後に」— v0.2 の「期待値更新」は B1 帰属の誤転記 = ERRATUM-2)** (spec §4.1 H6) |
| **B2 HOLD + oracle API** | recording contract v2 (`cable_xyz`+`held_seg_l` 必須 key 昇格、spec §4.2 N9) / div_grip metric (s=held_seg_l[f]、f=chunk 終端 clamp) / arming = G1-latch 後 / resume hysteresis + chatter telemetry / 非有限 = fail-HOLD + loud event / grip = chunk 終端値 定値 hold / query() API 形状 (spec §4.2/§4.3) / **tail 機構 = spec §9 :192 の (iii) HOLD 無効 [終端域 div 計算 skip] + (iv) 予定 release 後 drop terminal 無効化 (ERRATUM-3: v0.2 で no-owner だった帰属を B2 に pin — B2 が route_t 終端域とHOLD/tail 相互作用を所有するため)** | §10 行1 残 ~200-300 + 行8 50-100 | **hold-quiet-pre-onset** (健全域 f0-336 発火 0) + **hold-fires-at-onset** (f343±1 chunk) — 両 leg = residual≡0 replay を **cuda:0 pin で discharge** (499-step 1 本、数分。**p1 W-2 fold: f343±1 bar は cuda:0 trace 固有 + FORK-1 = micro-diff カオス増幅 + canonical device-fragile banked ゆえ、CPU replay discharge は onset 移動による false-verdict risk — v0.1 の 0-GPU 主張を撤回**) / claim ① 実測 (drift-under-HOLD < 0.58mm/RL-step) は B7 に委譲可 / flag-OFF byte-preserve subset |
...
| **B6 OG port** | legs A′B′C′ 再設計実装 (no-sim 契約保全、nominal cell scope) / og_gate.json schema_version / **13x-replicate null artifact を 62D/α-6D 契約で再生成** (leg-C′ precondition) / 旧 schema 消費者 (dagger_loop:203-208 等) = retire-or-migrate 判断は R3 build 時 (触らない)。**⚠carry (B0-3、%11 14:08): og_offline_gate.py:32 の pre-existing Japanese-text (§ 混じり comment) が pre-commit Layer2 CHECK4 を BLOCK — B6 が file を触る際に line-32 を同時修正しないと commit 不能** | 250-450 | legs A′B′C′ が canonical artifact 上で数値を返す (band 判定はしない — α/β/γ = Rs 後決) / no-sim 契約 grep leg (solver/mujoco/newton import 0) |
| **B7 smoke 一式** | spec §8 残 leg 全数: throughput ≥9.7fps / device-parity cuda:2 + cg-GPU whole-route screen / HOLD 込み horizon 設計算術 leg (paper) / bank-start per-k 正制御 / bank-G3 fork 摂動 (claim ② 実行 leg) / DR-corner (drift-rate 再計測 alert>10 / breach>20 mm/RL-step + per-cell div profile) / IK 残差分布 probe / claim ① 実測 / **tail 検証 (i) seated 枝 drop held-z 非発火 + (ii) G6 sustain release 境界連続性 (spec §9 :192、ERRATUM-3 帰属) + R3h 境界算術 771-vs-770 照合 carry** | 150-350 | spec §8 表の bar が正 (転記しない — spec 参照)。**MAX_HOLD・HOLD 閾値系の再導出値 + curriculum mix 比率提案 + OG band α/β/γ 提案 → W1 完了報告で Rs 単独項として提示** (§6) |
...
2. **post:** 層3 = **single-file lint のみ** (`./isaaclab.sh -f` tree-wide 禁止、spec §10 M12) + chunk DoD legs (§3 表) + **flag-OFF byte-preserve legs = diff 表面適合 (ERRATUM-2、spec §8 verbatim 準拠): env 側 diff → ⑨a′ EXACT 再実行 + DoD⑤⑥⑩ 再実行 + cablediag env-trajectory byte-anchor (pre-B1 baseline = 311f18cb9b、既 bank) / route_executor.py 側 diff → producer subset vs ca33d1e1a0 (5-cell 毎 chunk、25-slice = B7)** — v0.2 の「subset (毎 chunk、pinned baseline)」単独指定は、producer 経路が env を実行しない (on-disk 実証、B1 5体 CC4-CRIT/CC5-HIGH) ため env 側 diff には感度ゼロの偽検証となる誤転記 → 表面適合に訂正 + 層5 (L3 該当) + %10 audit + %12+%9 verify → **explicit-path atomic commit** → p6 relay (milestone 毎、batch 溜め禁止)。
...
| multi-cell bank capture option | **W0-a 一括「A」により採 (解釈 LOUD: §2a 行の提案 = P3 収録 piggyback → 発効 = W3。veto 可)**。W1 影響 = B3 capture の cell-parameterize (前方互換のみ、追加 build なし)。**W3 発効前の明示 Rs confirm = W1 完了報告に 1-line 同載 (p1 W-1 fold)** | %12 |
