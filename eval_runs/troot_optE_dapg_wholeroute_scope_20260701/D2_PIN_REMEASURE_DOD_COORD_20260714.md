...
> ⛔⛔ **C-3 は不十分でした (当方 own、2026-07-14 07:3x)。** 本 parity は **実装の忠実性**しか証明しません。
> **実際に走らせた結果: 4.18 vs 4.16 で PASS。しかし 4.16mm は *節点量子化のゴミ* でした** (dx = −0.15mm / dy = −4.16mm)。
> ⇒ ⭐**parity check は、構成上、それを捕まえられません。両側が *同じ (誤った) 定義* を encode しているからです。**
> ⇒ **捕まえたのは「dx と dy に分解する」= *定義*水準の検査**であって、「同じ数を再現する」= *実装*水準の検査ではありませんでした。
> ⭐⭐ **これは %9 が特定した「two-key 検証は実装の忠実性しか見ない ⇒ 両鍵が同じ定義を encode していれば一致は必然で、何も証明しない」の *第 2 の実例* であり、**しかも本 arc の当夜に、その罠を狩っていた当人 (私) が踏みました。**
> ⇒ ⭐**独立性は *実装* でなく *定義* の水準で要る。** ⇒ **C-3 は「parity を取る」ではなく「*定義が何を測っているかを分解して問う*」に置換されるべき。** (本 (d2) では実際にそう修正済 — `centerline_offset_mm` + `cable_NOT_ESCAPED_*`。)
