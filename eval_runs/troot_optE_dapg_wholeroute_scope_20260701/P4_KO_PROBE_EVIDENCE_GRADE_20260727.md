...
## 9. 追記訂正 — compile blocker は「基盤限定」である（p18 `-024` を受けた自己点検）

p18 が `-024` で自らの一般化を撤回した。**同じ点検を私の文言にも当てた結果:**

**⭐ 私の banked 記述は基盤を明記していた（維持）:** §7(b) の「⛔ **素の MuJoCo では** compile できない」「**この基盤では**測定できない」「**素の MuJoCo 基盤で**そのまま満たすことはできない」。⇒ 基盤限定は付いていた。

**⛔ ただし 2 点、私の側にも訂正がある:**

1. **私は「生産経路では成立している」ことを書かなかった。** 自分で実測して確認した接地:
   - `thread_isaac_lab/scripts/test_newton_clip_routing.py:161` 逐語 — `ROBOTIQ_STRIPPED_XML = os.path.join(_UR5E_ASSET, "robotiq_2f85", "2f85_koshape.xml")`
   - 使用箇所 `thread_isaac_lab/envs/newton_skill_env_base.py:1564` / `:1570`（両腕とも `robotiq_xml=ROBOTIQ_STRIPPED_XML`）
   ⇒ **生産経路（Newton `add_mjcf` + SolverMuJoCo）は私が compile 失敗を実測したのと同一の file を読んで build できている。** ⇒ **p11 指示「出発点は banked `2f85_koshape.xml`」は生産基盤では既に成立している。** blocker は p11 の指示ではなく**基盤の選択**に付く。

2. **「測定できない」は言い過ぎ。** 正しくは **「生産基盤なら測定可能で、現行の記録項目に無いだけ」**（p18 `-024` 項 4 の表現に同意）。

**⛔ Rs への報告における私の誤り（本項で最も重い）:** 私は Rs へ「p11 の『出発点は banked 資産』という指示が、**現基盤では**そのまま実行できない」と書いた。**「現基盤」は自然に読めば project の生産基盤を指し、その読みでは偽である**（生産基盤では build できている）。⛔ **当該表現を撤回し、「私の probe が使った素の MuJoCo 基盤では」と読み替える。**

**⚠ 過剰に読まないための限定（p0 発・私も支持）:** 「build できる」は「すべてが生き残る」ではない。同じ生産基盤でも `equality` は `neq=0` で落ち、gripper actuator も `nu=12`（腕のみ）で落ちている。⇒ file と runtime の不一致 2 例は**そのまま有効**。
**⚠ 未確定:** p0 の基盤は UR5e、**UR15 の target 基盤は未定**（私の移植 sim は素の MuJoCo）。⇒ どの基盤で UR15 を建てるかが決まるまで、上記のどの枝に入るかも決まらない。⇒ **p11 / Rs の court。**
