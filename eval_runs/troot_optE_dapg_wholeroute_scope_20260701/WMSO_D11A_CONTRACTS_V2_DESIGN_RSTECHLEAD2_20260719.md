# WMSO D1.1-A `contracts_v2` — DESIGN (v2.9)

- node: `T-WMSO`; author = w2:pQ (RS-TECH-LEAD2)
- created: v1 = 2026-07-19 09:09 JST / v1.1 = 09:24 / v2 = 09:32 / v2.1 = 09:43 / v2.2 = 10:59（editing 完了実測; 旧「10:5x」表記を確定 — 以後 x-mask 廃止）/ v2.3 = 12:07 / v2.4 = 12:41（12:31 に v2.3.1 として着手後、Rs review v3 着信により拡張）/ v2.4.1 = 13:07 / v2.5 = 13:36 / v2.6 = 14:16 / v2.7 = 14:36 / v2.7.1 = 15:46 / v2.8 = 16:07 / **v2.9 = 16:52 JST（editing 完了実測; 16:31 着手）**
- **status: DESIGN DRAFT v2.9 — 統合 fold = **Rs review v6〔RV6、必須 7 領域〕+ pN 再 verify ⛔HOLD C1-C3**（16:26・交差着信 — C ⊂ RV6 と同根多数）。⚠**Rs の RV6 §10 は本 fold の目標版を「v2.8/EP v1.6」と命名したが、当該ラベルは直前の pS-G fold（交差）が消費済み → 実版 = v2.9/EP v1.7**（版名 collision の loud 記録）。⭐RV6 §10 逐語「これ以上一般的に精緻化せず、証拠束縛・handoff・migration の閉包だけを直して freeze」 → pS 照合 → pN 再 verify（最終 SHA 宛）→ **freeze** PENDING**
...
- 兄弟 normative artifacts（pN DESIGN verify の対象; full 64-hex — B-CH5）:
  ...
  - **pN HOLD C1-C3（v2.8 宛・16:26）** = 本 fold の入力として pN が保持（pN 16:39 readback — C1-C3 の as-received 転記 bank は pN の v2.9 verdict と併せて次 round で確定）
  - **EvidencePolicy v1.7** = `WMSO_D11A_EVIDENCE_POLICY_V1_RSTECHLEAD2_20260719.md`（sha = bank commit で確定・毎版更新）+ **機械可読 fixture `WMSO_EvidencePolicy_v1.7.json`**（二層 {metadata, policy_definition}; **`evidence_policy_definition_hash` = `066eed1049f4f51a89dd86e9d50614a65adba070b2ff650424ea7cef05dec4ea`**〔実算出・掲載 — pN C2〕; file sha = manifest + bank commit）
