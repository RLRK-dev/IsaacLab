# WMSO D1.1-A `contracts_v2` — DESIGN (v2.7)

- node: `T-WMSO`; author = w2:pQ (RS-TECH-LEAD2)
- created: v1 = 2026-07-19 09:09 JST / v1.1 = 09:24 / v2 = 09:32 / v2.1 = 09:43 / v2.2 = 10:59（editing 完了実測; 旧「10:5x」表記を確定 — 以後 x-mask 廃止）/ v2.3 = 12:07 / v2.4 = 12:41（12:31 に v2.3.1 として着手後、Rs review v3 着信により拡張）/ v2.4.1 = 13:07 / v2.5 = 13:36 / v2.6 = 14:16 / **v2.7 = 14:36 JST（editing 完了実測; 14:24 着手）**
- **status: DESIGN DRAFT v2.7 — fold = **Rs PLAN_STATUS review v5〔RV5、14:11 原本〕WMSO 全項**（v2.6 先行治癒 2 項 + 新規 fold 6 項 + custody 2 項）。⭐**RV5 §6-5 の Rs 方向 = 本 bounded fix 群で D1.1-A を freeze → D1.1-B（tensor binding）/ D1.1-C（artifact manifest）/ 2-3 skill boundary-only 実験へ** → pS delta 照合 → pN 再 verify（最終 SHA 宛）→ freeze 判定 PENDING**
...
- **全表 = EvidencePolicy v1.4**（sha 冒頭; **total map〔(component, grade, applicability_class)〕・ApplicabilityResolver・3 cell exact 化・DC-3 準拠復元・ProofItem 順序/conflict・§3d 束縛規則・EXPLICIT_NONE 免除・per-component 意味論・二層 hash** を含む）。
...
`certify_definition(definition, evidence_bundle, evidence_policy, schema_registry, proof_artifact_resolver) → DefinitionCertificationResult`
〔**RV5-W-P0-3 gap 3 — trust boundary**: resolver = `(ref, expected_sha256) → bytes | UNRESOLVED`（fail-closed = `E_PROOF_ARTIFACT_UNRESOLVED`）。hash 構造一致に加え asserted bytes の実在・内容整合（manifest 列挙 / TTCB 束縛 / reproduced-output equality）を検査 — grade を self-asserted にしない。evaluator allowlist = `E_EVALUATOR_UNKNOWN`。詳細 = EP §3d trust boundary〕
...
```
検査内容（inline）: `validate_invocation_start` = initiation predicate / freshness_policy 評価（明示 `now`）/ required_control_resources vs 実 ownership（比較 = **required ⊆ offered** の bool 包含 — D-CH5）。`validate_outcome` = terminal ∈ declared_classes / interrupt checkpoint ∈ checkpoint_specs / duration・cost 有限非負 / invocation_id 一致。`validate_handoff` = offer.handoff_schema_id ∈ producer 宣言 / producer_invocation_id 一致 / control_epoch 単調 / §5D 互換。**certificate を無効化しない**。
...
| accepted_incoming_handoff_set | → accepted_handoff（producer/schema/version へ正規化 + **producer_handoff_schema_hash は registry fixture 供給 or UNKNOWN→Draft**〔RV4 §2.6〕; 不足情報は UNKNOWN 扱い） |
...
- **RV5→fold-map（Rs PLAN_STATUS review v5、対象 = v2.5+EP v1.3 期 package）**:
  - **RV5 C-P0-1**（pN HOLD artifact 不在）= **本版で as-received transcript を bank**（`WMSO_PN_DESIGN_VERIFY_HOLD_B1B7_TRANSCRIPT_20260719.md`、pN 確認 PENDING）。package 同梱 = p6。
  ...
  - **RV5-W-P0-6**（機械可読 policy object 不在）= **`WMSO_EvidencePolicy_v1.5.json` を normative fixture として新設**（total map + binding + profiles + resolver 規則）。semantic hash 値は WCJ 実装を要するため **impl 時 golden test で確定**（手計算 hash の先行掲載はしない — 検証不能な自己申告を作らない）。
