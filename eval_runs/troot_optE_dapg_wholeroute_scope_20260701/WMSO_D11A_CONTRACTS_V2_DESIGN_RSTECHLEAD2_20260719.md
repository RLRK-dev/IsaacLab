# WMSO D1.1-A `contracts_v2` — DESIGN (v2.11)

- node: `T-WMSO`; author = w2:pQ (RS-TECH-LEAD2)
- created: v1 = 2026-07-19 09:09 JST / v1.1 = 09:24 / v2 = 09:32 / v2.1 = 09:43 / v2.2 = 10:59（editing 完了実測; 旧「10:5x」表記を確定 — 以後 x-mask 廃止）/ v2.3 = 12:07 / v2.4 = 12:41（12:31 に v2.3.1 として着手後、Rs review v3 着信により拡張）/ v2.4.1 = 13:07 / v2.5 = 13:36 / v2.6 = 14:16 / v2.7 = 14:36 / v2.7.1 = 15:46 / v2.8 = 16:07 / v2.9 = 16:52 / v2.9.1 = 17:21（pS H-1..H-3 fold）/ v2.9.2 = 17:35（pN R1-R3 fold: EP 旧重複行統一 / enum member 全数 inline / 版数 3 面同期）/ v2.10 = 20:29（RV7 fold — pS 工程 12 = design-axis PASS〔record §16〕→ pN 工程 13 = ⛔HOLD B1-B4）/ **v2.11 = 21:23 JST（実測 — pN 工程 13 B1-B4 + R1-R2 fold: applicability 2 段分割 / EXACT×非学習 機械拒否 / projection 構造化 + golden 7 本 / method descriptor + METHOD_REGISTRY〔algorithm-neutral — charter 整合復元〕。EP = v1.9）**
- **status: DESIGN DRAFT v2.11（版数 3 面同期 — R3 規則）— fold 系譜 = … → RV7 fold（v2.10）→ pS 工程 12 PASS → **pN 工程 13 ⛔HOLD B1-B4 の fold = 本版**。残 gate = **pS 再照合 → pN exact-pin 再検証 → Rs freeze 判定**。⚠**B4 は批准済み §3 表（ExecutionFamily 5 行 allowlist）の再構成を含む** — charter『algorithm-agnostic』逐語 + Rs の RL+IL 非限定要件が典拠（register ⑩ に loud 記録・Rs confirm 対象）**
...
```python
class IdentityKind(Enum): LEARNED | SCRIPTED | WAIT   # RV7 P0-7 明示 inline（member 全数; .value = member 名 ASCII 文字列）。certify/eligibility の kind 分岐キー — §3 coherence（LEARNED⇔{PPO,BC}, SCRIPTED⇔SCRIPTED, WAIT⇔WAIT）
...
- **RV5 fold 追加**: EP markdown ↔ **JSON fixture** ↔ code 定数の**三面一致試験**（**`WMSO_EvidencePolicy_v1.8.json` = normative fixture**〔RV7 R-1 で現行版へ同期 — 導入時の版名 v1.6 は §11 RV5 fold 履歴に残置〕・semantic hash はこの test が確定 — RV5-W-P0-6）/ **B3 mutation 13 面**（semantic_obs/action_schema の 2 変異 → E_BEHAVIOR_REVISION_STALE — RV5-W-P0-4）/ `E_PROOF_ARTIFACT_UNRESOLVED`（resolver 取得不能・sha 不一致・zero-length — RV5-W-P0-3）/ `E_EVALUATOR_UNKNOWN` / **TP applicability 判別 case**（SCRIPTED の TP = N/A で CLOSED_LOOP 可〔他 required 充足時〕・LEARNED の TP 不在 = UNKNOWN 失格 — RV5-W-P0-2）/ **registry transitive projection case**（definition 非参照の schema 追加 → schema_registry_hash 不変・参照 schema 変更 → 変化 — RV5-W-P0-5）/ final_training_config_hash / final_source_commit の lineage 別 null/必須 corpus（RV5-W-P0-3）。
