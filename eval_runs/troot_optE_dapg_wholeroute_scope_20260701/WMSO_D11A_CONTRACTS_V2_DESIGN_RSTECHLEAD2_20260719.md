# WMSO D1.1-A `contracts_v2` — DESIGN (v2.10)

- node: `T-WMSO`; author = w2:pQ (RS-TECH-LEAD2)
- created: v1 = 2026-07-19 09:09 JST / v1.1 = 09:24 / v2 = 09:32 / v2.1 = 09:43 / v2.2 = 10:59（editing 完了実測; 旧「10:5x」表記を確定 — 以後 x-mask 廃止）/ v2.3 = 12:07 / v2.4 = 12:41（12:31 に v2.3.1 として着手後、Rs review v3 着信により拡張）/ v2.4.1 = 13:07 / v2.5 = 13:36 / v2.6 = 14:16 / v2.7 = 14:36 / v2.7.1 = 15:46 / v2.8 = 16:07 / v2.9 = 16:52 / v2.9.1 = 17:21（pS H-1..H-3 fold）/ v2.9.2 = 17:35（pN R1-R3 fold: EP 旧重複行統一 / enum member 全数 inline / 版数 3 面同期）/ **v2.10 = 20:29 JST（実測 — Rs review v7〔RV7 ⛔HOLD、v2.9.2 宛〕fold: P0-1..P0-7 + R-1..R-4。EP = v1.8〔semantic 版 bump — 旧 pin `066e…`/`ed10…` 退役、RV7 指示〕）**
- **status: DESIGN DRAFT v2.10（版数 3 面同期 — R3 規則）— fold 系譜 = … → pN 最終 pin PASS-CLOSE（v2.9.2、17:43）→ **Rs review v7 = ⛔HOLD（RV7; §11 fold-map）の fold = 本版**。freeze 上程は RV7 で差し戻し — 残 gate = **pS 差分照合 → pN exact-pin 再検証（最終 SHA のみ渡す — RV7 工程 13）→ Rs freeze 判定**（RV7 工程 12-14）。RV7 逐語「次に許可される作業: 設計・EvidencePolicy 限定修正のみ」に従い本版は当該修正のみ**
...
- 兄弟 normative artifacts（pN DESIGN verify の対象; full 64-hex — B-CH5）:
  ...
  - **EvidencePolicy v1.8** = `WMSO_D11A_EVIDENCE_POLICY_V1_RSTECHLEAD2_20260719.md`（sha = bank commit で確定・毎版更新）+ **機械可読 fixture `WMSO_EvidencePolicy_v1.8.json`**（二層 {metadata, policy_definition}; **`evidence_policy_definition_hash` = `bdc508200889005142eaab5ac15c48cdfb8281df63bbb35537542f1f248891a8`**〔実算出・掲載・metadata 編集で不変 — 導出 command 埋込〕; file sha256 = bank commit 後に manifest が確定値を pin。⚠**旧 v1.7 系 pin `066eed1049f4…` / `ed10c77a…` は RV7 の指示どおり最終 pin から退役** — P0-2/3/4 の semantic 変更につき）
...
- **全表 = EvidencePolicy v1.8**（sha 冒頭; **total map〔(component, grade, applicability_class)〕・resolve_applicability・3 cell exact 化・DC-3 準拠復元・ProofItem 順序/conflict・§3d 束縛規則〔G-4 kind 条件付き + RV7 P0-4 CONFIG_HASH total map + P0-3 projection 確定形込み〕・EXPLICIT_NONE 免除・per-component 意味論・二層 hash〔17 member〕** を含む — G-1 fix: 本行の版数は EP header と同期更新する〔RV7 R-1 で 2 度目の履行〕）。
- **applicability**（v3 W-P0-1 → **RV7 P0-2 で署名・優先順位を修正**）= EP §3c **`resolve_applicability(identity_kind, training_lineage, explicit_none_components, requested_profile, component) → REQUIRED(min_grade) | NOT_APPLICABLE | OPTIONAL`**、優先順位 = **1. instance EXPLICIT_NONE → 2. profile「—」→ 3. lineage 条件 → 4. default**（旧 instance→lineage→profile 順は lineage REQUIRED が profile「—」cell に先行する defect — register ⑨。certification は requested_profile = None で層 2 skip = profile 中立を保持）。**N/A ≠ UNKNOWN**（N/A = validated 不存在・集約除外 / UNKNOWN = 知識不足・失格）。
...
- 判定 = EP §3c `resolve_applicability` → applicable へ per-component **`assigned_grade ≥ min_grade`**（EP §4; **達成 grade の出所 = evaluation certificate のみ** — RV7 P0-6。evaluation の無い required component = UNKNOWN(0) = 失格）。resolver 入力 = `certificate.identity_kind` / `certificate.training_lineage` / `certificate.explicit_none_components` / `requested_profile`（F-1 — definition 実体なしで全 4 層が計算可能）。
...
- **RV7 fold 追加**: **resolver 優先順位 regression**（OFFLINE_REPLAY × BC_ONLY × TRAINING_DATASET → NOT_APPLICABLE〔層 2〕— 旧順の REQUIRED 誤判定を負例に; learned × OFFLINE_REPLAY × TRAINING_PROVENANCE も同型）+ certification profile-neutral case（requested_profile = None → 層 2 skip）/ **projection golden 2 本**（control_mode: `{"state":"KNOWN","value":"DIFF_IK_EE_TARGET"}` → `4f0b26d5…`・handoff_schema: 1 schema + accepted 空 → `00ffb015…` — EP §3d/JSON `projection_rules` の掲載値と一致検証）+ 並べ替え coherence case（handoff tuple reorder → SkillDefinitionHash・claim_target 双方変化）/ **CONFIG_HASH total map cases**（LEARNED×BC_THEN_RL: TD claim → bc_config_hash・POLICY_ARTIFACT claim → final_training_config_hash / LEARNED×{RL_ONLY,BC_ONLY} → final / SCRIPTED → runtime_config_hash; 誤 target → E_PROOF_MISBOUND）/ **`E_GRADE_MISMATCH`**（record.grade ≠ assigned_grade → fail-closed）+ evidence_evaluation_bundle_hash 再現 + **eligibility が assigned_grade を使う判別 case**（record 自己申告 grade を上書きした入力 → E_CERT_INPUT_MISMATCH）/ **`E_HANDOFF_EPOCH_STALE`**（offer.control_epoch ≠ authority_epoch_snapshot）。
...
## 10. Open points
**RV7-P0-1..P0-7 / R-1..R-4 = 本版で全 fold（自己申告 — §11 RV7 fold-map に per-item 処置）。未充足 gate = pS 差分照合 → pN exact-pin 再検証 → Rs freeze 判定（RV7 工程 12-14）** — 「open = 0」の無条件宣言は行わない（RV7 末尾の指摘: 検証未通過の自己申告を open 0 と書かない）。RV7 transcript の fidelity = Rs 確認 PENDING（chat 原文・byte N/A）。review-4 transcript の fidelity は **RV4 §1.4 で Rs 確認済み: semantic fidelity CONFIRMED**（byte fidelity = N/A — 原本が chat message であり独立 file でないという限定付き。この限定を明記して PENDING 解消 — RV4 逐語「Rs確認PENDINGは、この限定を明記した上で解消してよい」）。W-review v2/v3/v4 は byte-identical file copy のため確認不要。
