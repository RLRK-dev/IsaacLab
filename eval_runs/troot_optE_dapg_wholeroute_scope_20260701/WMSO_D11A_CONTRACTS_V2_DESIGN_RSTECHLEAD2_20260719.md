# WMSO D1.1-A `contracts_v2` — DESIGN (v2.11.2)

- node: `T-WMSO`; author = w2:pQ (RS-TECH-LEAD2)
- created: v1 = 2026-07-19 09:09 JST / v1.1 = 09:24 / v2 = 09:32 / v2.1 = 09:43 / v2.2 = 10:59（editing 完了実測; 旧「10:5x」表記を確定 — 以後 x-mask 廃止）/ v2.3 = 12:07 / v2.4 = 12:41（12:31 に v2.3.1 として着手後、Rs review v3 着信により拡張）/ v2.4.1 = 13:07 / v2.5 = 13:36 / v2.6 = 14:16 / v2.7 = 14:36 / v2.7.1 = 15:46 / v2.8 = 16:07 / v2.9 = 16:52 / v2.9.1 = 17:21（pS H-1..H-3 fold）/ v2.9.2 = 17:35（pN R1-R3 fold: EP 旧重複行統一 / enum member 全数 inline / 版数 3 面同期）/ v2.10 = 20:29（RV7 fold — pS 工程 12 = design-axis PASS〔record §16〕→ pN 工程 13 = ⛔HOLD B1-B4）/ v2.11 = 21:23（pN 工程 13 B1-B4 fold — pS §17 PASS-W-C → pN 再 verify ⛔HOLD R1-R4〔bounded consistency のみ〕）/ v2.11.1 = 21:58（再 R1-R3 fold — pS §18 PASS → **pN 再判定 = 設計/証拠 ✅PASS-CLOSE・custody ⛔HOLD M1-M4**〔record-only〕）/ **v2.11.2 = 22:18 JST（実測 — M2 構造修正のみ: pS record 行の dynamic bank 記載を撤去し manifest authoritative 化〔循環断ち〕。設計内容・EP/JSON = 不変）**
- **status: DESIGN DRAFT v2.11.2（版数 3 面同期 — R3 規則）— fold 系譜 = … → pN 再 verify R1-R4 fold（v2.11.1）→ **pN 再判定 22:15 = v2.11.1 設計/証拠 ✅PASS-CLOSE・exact-pin custody ⛔HOLD M1-M4（record-only・pN 再走不要）→ M 系 fold = 本版 + manifest 同 bank**。残 gate = **manifest 自己整合 readback → pN exact-pin PASS-CLOSE（宣言済み条件）→ Rs freeze 判定 + register ⑩ confirm**（⑩ 未確認では freeze 不可）**
...
- 兄弟 normative artifacts（pN DESIGN verify の対象; full 64-hex — B-CH5）:
  ...
  - **pN HOLD C1-C3 transcript** = `WMSO_PN_DESIGN_VERIFY_HOLD_C1C3_TRANSCRIPT_20260719.md`（as-received 転記 — pS §14 H-3 の解消; **pN 著者 readback = SEMANTIC FIDELITY CONFIRMED / BYTE N/A**〔工程 13 verdict 内 R1、21:08〕）
  - **pN R1-R3 HOLD + ✅PASS-CLOSE transcript** = `WMSO_PN_DESIGN_VERIFY_R_AND_PASSCLOSE_TRANSCRIPT_20260719.md`, sha256 `2847e2aa9d30a9b57093232425c8fdbef2aa09a87d0291b489f79e471f911139`（as-received 転記・bank `3a8ae46f32`; **pN 著者 readback = SEMANTIC FIDELITY CONFIRMED / BYTE N/A**〔同上 R1〕）
  - **EvidencePolicy v1.9** = `WMSO_D11A_EVIDENCE_POLICY_V1_RSTECHLEAD2_20260719.md`（sha = bank commit で確定・毎版更新）+ **機械可読 fixture `WMSO_EvidencePolicy_v1.9.json`**（二層 {metadata, policy_definition}; **`evidence_policy_definition_hash` = `e7ca43093084c167a209b008533a66d26a1fd3223d2a3c11274d28306c3ff803`**〔実算出・掲載・metadata 編集で不変 — 導出 command 埋込〕; file sha256 = bank commit 後に manifest が確定値を pin。⚠**旧 pin 系譜（v1.7 系 `066eed1049f4…`/`ed10c77a…`・v1.8 系 `bdc5082008…`）は semantic 変更につき退役**）
  - **Rs review v7（RV7 ⛔HOLD; 対象 = v2.9.2 + 旧 zip 世代 companion）** = `WMSO_RS_REVIEW_V7_HOLD_TRANSCRIPT_20260719.md`（as-received 転記 — chat 原文につき byte identity N/A・**Rs fidelity 確認 PENDING**。転記者注に custody 事実確認〔review 環境 companion = 旧 zip 世代、現 Downloads/repo は v1.7.1/最終 JSON — 20:10 実測〕を付記）
  - pS design-verify record = `WMSO_D11A_DESIGN_VERIFY_WMSODESIGN_20260719.md`（**path のみを stable host として掲載 — 最新 blob pin / bank commit は外部 manifest が authoritative**〔M2 構造修正: design 内の dynamic「現行 bank」記載は design hash → pS re-readback → 新 record hash → 再 stale の**相互 hash 循環**を作るため撤去。G-3 の re-bank 義務は維持・記載先を manifest に一本化〕; 現行版は pS 管理 — B-CH1 の host pin）
- version 履歴: v1 (prefix `8bec472a6ac1` — content 非保全) → v1.1 (prefix `b807ce2abbd7` — 同) → v2 (prefix `4880dc0d6c6d` — 同) → v2.1 (full `c49ff132ff200257085f072c875c58e3200754ac74ee28de8634b786ab2681e7`, bank `cd5482310d`) → v2.2 (full `3a1577f89dfa3d3c7fa642c938f56a4ff05ab60e14b57194cb3368f7ddcb95f3`, bank `ac5865b66d`) → v2.3 (full `94a42a0ea73a08d2b85be562373322c5ebfd65f49393183b9cf7fff68040f192`, bank `efc355e52d`; **pS 全行照合 = PASS-WITH-CONDITIONS R-1..R-3**〔verify record §9〕) → v2.4 (full `772f346c32cf15f11e3c62fa14ed1342f87b7bcf0acc9e8d0ea2a93f885986d7`, bank `130813e934`; **pS 全行照合 = PASS-WITH-CONDITIONS F-1/F-2**〔verify record §10, sha `609e7a35722b…`; R-1..R-3 = 3/3 discharge・register ⑥ 解 = 検証済で pS 推奨より正当・v3 9 項 = faithful〕) → v2.4.1 (full `8310cb6fd3f8f23929fc1479f734141e481d9b3641f3227d798e311084dad15c`, bank `ba10218549`; **pS FINAL CONFIRM = PASS**〔record §11〕→ **pN DESIGN verify = ⛔HOLD B1-B7**〔13:26 実測・時刻訂正済; 全項 pQ on-disk 検証で真〕) → v2.5 (full `2f0a706833b024f25e992c29791dfd2ab0837622ac5e58f2bea2abfd97c1dfac`, bank `59b7720408` — B1-B7 fold + pS record bank) → v2.6 (full `9c8a18f83bbf1555b9adaadf72f9244a3ec8ea09dc26922e83b9a1435e98f975`, bank `51008ace1f` — RV4 fold + full self-contained 化) → v2.7 (full `5b4568903e6c1ac321106d418a8fa3e583067cffb770a994fd3fd1f402a46c59`, bank `e0257b5648` — RV5 fold) → v2.7.1 (full `64e2b005d6dd7f7a64dc884356e860da43762638c328a215ba5b464fd8164ac3`, bank `86d127b5c0`; **pS §12 全区間 re-check = PASS-WITH-CONDITIONS G-1..G-4**〔record sha `98e47cace83a…`〕) → v2.8 (full `24f5fd3d8e8ec85034bdb53cfb65d47a6636529c2aa366e9212cf1d271515fef`, bank `ba99db30f6` — G fold; **pS §13 FINAL CONFIRM = PASS** → **pN 再 verify = ⛔HOLD C1-C3**〔16:26〕) → v2.9 (full `e64c192b62e754da8050041985dd9028a67bf12c287d55bcbc1579c660b8de46`, bank `8caa19a7a4` — RV6+C 統合 fold; **pS §14 = PASS-WITH-CONDITIONS H-1..H-3**〔record sha `f6028b096032…`; hash 独立再計算一致 = airtight〕) → v2.9.1 (full `0da79321038950a916b64c36096e9bbd23d095f7e09f13cfaea24696917a956e`, bank `ea30e7fdc3` — H fold; **pN 最終 pin = ⛔HOLD R1-R3**〔17:30; custody/hash/transcript legs = PASS・C2 CLOSE〕) → v2.9.2 (full `e83a29061400b42c18a6607c96295e0ede2984cda8b9b047f338512ccb0b7f96`, bank `ba69702cb9` — R fold; **pN 再 verify = ✅evidence-axis PASS-CLOSE**〔17:43〕→ **Rs review v7 = ⛔HOLD**〔RV7 — companion custody FAIL + P0-2..P0-7/R-1..R-4; P0-1 semantic 部分は EP v1.7.1 で既修正・custody 部分を本 round で解消〕) → v2.10 (full `86a882219780dc43ad10dd38988c9933454a6892fb2f2721e83d843841e749c3`, bank `a87525cc15` — RV7 fold; **pS 工程 12 = design-axis PASS**〔record §16 `383c3e3a0efc…`@`dfeb6c1e57`・must-fix 0・hash 3 本独立再計算一致〕→ **pN 工程 13 = ⛔HOLD B1-B4**〔21:08; custody/hash legs = PASS — 6 blob pins・definition hash・golden 2 本・manifest pins・ancestry 検証済〕) → v2.11 (full `8d1f356024f7e6f4a14f7d2f36d94a9e6958905d65920078b30831e76642759f`, bank `37ddb72284` — B1-B4 + R1-R2 fold; **pS §17 = PASS-WITH-CONDITIONS**〔C-1 transcript bank = 解消 `81065b9b5c` / C-2 = register ⑩ Rs confirm〕→ **pN 再 verify = ⛔HOLD R1-R4**〔21:55; B1-B4 mechanism fold = PASS・6 blob pins/def hash/golden 7/7・260 cell totality 検証済・残差 = bounded consistency〕) → v2.11.1 (full `698bfc9e3c7eabd8ac9cdfa681aef4700cb466df3a71d80a8e6ebe83d978d066`, bank `c628e58697` — 再 R1-R3 fold; **pS §18 = v2.11.1 design-axis PASS**〔stale-live 0 sweep・§17 :374 strike 訂正 = 再 R4〕→ **pN 再判定 = 設計/証拠 ✅PASS-CLOSE・custody ⛔HOLD M1-M4**〔22:15; 7 blob pins/ancestry/R1-R4 fold 全 PASS・M 系 = record-only〕) → **v2.11.2 (本版 — M2 構造修正のみ; M1/M3/M4 = manifest/transcript 側・同 bank)**。以後毎版 bank。**注記（RV7 custody 指摘への恒久規則）: 各版の full sha / bank commit は「後継版の本履歴 + manifest」が記録する — file は自身の sha を内包できない。**
...
Certification: certify_definition() → DefinitionCertificationResult {report, (valid のみ) certificate, evidence_evaluations}   … profile 中立（W-P0-2 + RV7 P0-6）
...
```python
class IdentityKind(Enum): LEARNED | SCRIPTED | WAIT   # RV7 P0-7 明示 inline（member 全数; .value = member 名 ASCII 文字列）。certify/eligibility の kind 分岐キー — §3 coherence（LEARNED⇔{ONLINE_RL, IMITATION, IMITATION_PLUS_RL}, SCRIPTED⇔SCRIPTED, WAIT⇔WAIT — 工程 13 再 R1: B4 の method_class 化に同期）
...
### 1.3 TrainingProvenance（learned-conditional; **B4 で method descriptor 化 — algorithm-neutral**）

```python
@dataclass(frozen=True)
class TrainingMethodDescriptor:       # pN 工程 13 B4 — charter『PPO or another online RL / DAPG or another imitation-plus-RL』(algorithm-agnostic) の型実装
    method_class: TrainingMethodClass # 中立クラス — §3 許容表の key（lineage との許容対を判定）
    method_id: str                    # 具体 algorithm 識別子（"PPO"/"BC"/"DAPG"/…）。METHOD_REGISTRY ∋ 必須 — ∉ = E_METHOD_UNREGISTERED（fail-close）。registry 行の追加 = Rs 承認事項（SKILL_ID_REGISTRY と同権 — schema 変更なしで algorithm を拡張可能）

@dataclass(frozen=True)
class TrainingProvenance:
    method: TrainingMethodDescriptor  # B4 — 旧 `execution_family: ExecutionFamily`（enum-as-allowlist）を置換: enum 固定は charter の algorithm-agnostic 要件・Rs の RL+IL 非限定要件と不整合（DAPG/他 RL を型で拒否していた）
...
## 3. method × lineage 許容表（full inline — RV4 §2.4; **B4 で method_class key 化 — algorithm-neutral**）

**許容表（validator の唯一の許容源; 表外 = `E_LINEAGE_FORBIDDEN`、unknown enum = `E_ENUM_UNKNOWN`、method_id ∉ METHOD_REGISTRY = `E_METHOD_UNREGISTERED`; 照合対象 = TrainingProvenance。key = method_class — 具体 algorithm は registry が method_class へ写像するため、**新 algorithm の追加 = registry 行追加のみ**〔fail-close allowlist は registry 段で維持〕）**:

| # | method_class | training_lineage | 必須 hash | null 必須 | source-closure |
|---|---|---|---|---|---|
| 1 | ONLINE_RL | RL_ONLY | final_artifact_hash + **final_training_config_hash + final_source_commit**〔RV5〕 | bc_base / bc_config / demo_dataset = null | null |
| 2 | ONLINE_RL | BC_THEN_RL | bc_base + bc_config + final_artifact_hash + **final_training_config_hash + final_source_commit**〔RV5〕 | —（demo は hash か absent_reason — §1.3） | null |
| 3 | IMITATION | BC_ONLY | final_artifact_hash + **final_training_config_hash（= 唯一の BC stage config）+ final_source_commit**〔RV5〕 | **`bc_base_hash = null / bc_config_hash = null`**（C-CH10/A-CH7 — BC 単独訓練に RL stage も base 概念も無い。v1 "BC" migration で bc 系 config hash 出現 = `E_MIGRATE_BC_CONFIG_UNEXPECTED` fail-close） | null |
| 4 | **IMITATION_PLUS_RL** | **DEMO_PLUS_RL** | final_artifact_hash + **final_training_config_hash（単一 joint stage config）+ final_source_commit** + demo_dataset（hash か absent_reason — 定義上 demo を消費する系〔EP demo 系〕） | **bc_base = null / bc_config = null**（逐次 BC stage が存在しない — B4） | null |
| 5 | SCRIPTED | NOT_APPLICABLE | —（executable = closure aggregate） | final/bc/demo 全 null | **必須** |
| 6 | WAIT | NOT_APPLICABLE | — 同上 | 同上 | **必須** |
...
- **prereg §5 表（7 行）との関係（B4 で改訂 — register ② 修正 + ⑩）**: 旧 v2.10 以前は「DAPG 2 行は DC-5 により除外（5 行が supersede）」だったが、**pN 工程 13 B4 が charter『algorithm-agnostic』『PPO or another online RL / DAPG or another imitation-plus-RL』逐語 + Rs の RL+IL 非限定要件との不整合を CRITICAL/L0 と判定** → 本表は method_class key の 6 行 + METHOD_REGISTRY へ再構成（**imitation-plus-RL 系を再収容**）。DC-5 の除外は「v1 corpus に DAPG instance が無い」corpus 事実として読み直す（**Rs confirm 対象** — register ⑩）。
- coherence: **IdentityKind ↔ method_class（LEARNED⇔{ONLINE_RL, IMITATION, IMITATION_PLUS_RL}, SCRIPTED⇔SCRIPTED, WAIT⇔WAIT; `E_KIND_FAMILY_MISMATCH` — error code 名は継続〔family ≒ method_class・test 名の churn 回避〕）** + method_id ↔ method_class（registry 写像と一致 — 不一致 = `E_METHOD_UNREGISTERED`）+ kind ↔ control_mode 値（§1.2 表 — U15）。
...
- **全表 = EvidencePolicy v1.9**（sha 冒頭; **total map〔(component, grade, applicability_class)〕・classify/overlay 2 段・kind×grade 機械拒否〔B2〕・3 cell exact 化・DC-3 準拠復元・ProofItem 順序/conflict・§3d 束縛規則〔G-4 kind 条件付き + RV7 P0-4 CONFIG_HASH total map + P0-3/B3 projection 構造化 field map〕・EXPLICIT_NONE 免除・per-component 意味論・二層 hash〔17 member〕** を含む — G-1 fix: 本行の版数は EP header と同期更新する〔RV7 R-1 → B 系で 3 度目の履行〕）。
- **applicability**（v3 W-P0-1 → RV7 P0-2 → **pN 工程 13 B1 で 2 段分割**）= EP §3c **`classify_applicability(identity_kind, training_lineage, explicit_none_components, component) → NOT_APPLICABLE | OPTIONAL | REQUIRED_CLASS`**（profile 中立 — certification が使用・REQUIRED_CLASS は min_grade を持たない）**+ `resolve_applicability(classification, requested_profile, component)`**（eligibility 専用 overlay — profile「—」cell の N/A 化 + min_grade 付与。requested_profile 非 None 必須）。branch は **component-scoped・互いに排他**（TD/TP 専用 branch の else が他 component を吸収しない — B1 defect の閉塞）。RV7 P0-2 の優先順位とは観測上同値（register ⑨）。**N/A ≠ UNKNOWN**（N/A = validated 不存在・集約除外 / UNKNOWN = 知識不足・失格）。
...
- **出力型（RV7 P0-6 — field 集合を明示）**:

```python
@dataclass(frozen=True)
class DefinitionCertificationResult:
    report: ValidationReport                                        # issues 安定順 = (code, field_path, message) — C-CH10
    certificate: ContractCertificate | None                         # valid のみ発行（Draft / invalid = None）
    evidence_evaluations: tuple[EvidenceEvaluationCertificate, ...] # 評価した record ごとに 1 通（canonical 順 = component_kind.value bytes 昇順）
```

- **grade 測定の閉包（RV7 P0-6）**: certify は record 毎に evaluator の `assigned_grade` を測定し `EvidenceEvaluationCertificate` を発行、**`EvidenceRecord.grade == assigned_grade` を証明**する（不一致 = **`E_GRADE_MISMATCH`** fail-closed — grade は自己申告でなく測定値）。**`ContractCertificate.evidence_evaluation_bundle_hash` = H_WCJ(evidence_evaluations)** が evaluation 束を certificate に固定（EP §3d wiring）。
- valid のみ ContractCertificate:
...
class ContractCertificate:
    ...
    evidence_evaluation_bundle_hash: str  # RV7 P0-6: H_WCJ(evidence_evaluations) — grade 測定結果の束を certificate に固定（eligibility の入力検証先）
    identity_kind: str                 # F-1: IdentityKind.value（certify 時に definition から抽出）— EP §3c resolver の kind 依存規則を certificate 単独で計算可能に
    training_lineage: str              # F-1: TrainingLineage.value — classifier lineage branch（TD の demo 系条件〔B4 拡張〕）の入力。explicit_none_components と同じ completion class
...
evaluate_usage_eligibility(
    ...
    evidence_evaluations,      # RV7 P0-6（register ⑧ — 旧 evidence_bundle を置換）: EvidenceEvaluationCertificate 列。certificate.evidence_evaluation_bundle_hash と一致検査（不一致 = E_CERT_INPUT_MISMATCH、fail-closed）— grade は assigned_grade（測定値）のみを使用
...
- 判定 = EP §3c **classify（certificate.identity_kind / training_lineage / explicit_none_components から — F-1）→ overlay（requested_profile 非 None）** → applicable へ per-component **`assigned_grade ≥ min_grade`**（EP §4; **達成 grade の出所 = evaluation certificate のみ** — RV7 P0-6。evaluation の無い required component = UNKNOWN(0) = 失格）。certification 側は classify のみ使用（min_grade 不存在 — B1 の None-profile defect を型で閉塞）。
...
validate_handoff(invocation, offer, producer_definition, consumer_definition,
                 authority_epoch_snapshot: int) -> HandoffValidationReport
    # RV7 P0-5: authority manager（O0）が読み取った現在 epoch の snapshot 値を明示引数で渡す —
    # 純 validator に隠れ状態参照をさせない（W-P1-6 の now と同型の決定論境界）
```
...
**`validate_handoff`（RV6 §6 で identity 検査を完全化）** = producer_invocation_id 一致 / **`offer.producer_action_id == invocation.skill_action_id`** / **`offer.producer_definition_hash == invocation.skill_definition_hash == H_WCJ(producer_definition)`** / offer.handoff_schema_id ∈ producer 宣言 schema / **consumer 側 `AcceptedHandoffSpec.producer_handoff_schema_hash == producer schema の計算 hash** / **`offer.control_epoch == authority_epoch_snapshot`**（RV7 P0-5 — 比較対象は明示引数で受領・等値検査のみ。不一致 = `E_HANDOFF_EPOCH_STALE`）/ §5D 互換。**certificate を無効化しない**。
**責務分離（RV6 §6 + RV7 P0-5 — 純 validator では epoch 単調・再利用拒否を保証できない）**: epoch の CAS 更新・新 epoch 発行・旧 epoch command 拒否・**使用済み offer 拒否（`handoff_offer_id` 単位）**・**validator へ渡す `authority_epoch_snapshot` の読出し** = **authority manager（O0 層）の状態所有責務**として契約側から要求仕様を pin（validator は snapshot との等値検査・manager は状態遷移 — 二層で fail-closed）。
...
| v1 文字列 | → (method_id / method_class, TrainingLineage)〔B4 で method descriptor 形へ〕 |
|---|---|
| "BC+RL" | (PPO / ONLINE_RL, BC_THEN_RL) |
| "PPO" | (PPO / ONLINE_RL, base/config null なら RL_ONLY、else BC_THEN_RL) |
| "BC" | (BC / IMITATION, BC_ONLY) |
| "DAPG" | (DAPG / IMITATION_PLUS_RL, DEMO_PLUS_RL) — **B4 で有効化**（旧 v2.10 以前 = `E_LINEAGE_FORBIDDEN` は register ⑩ で supersede; demo/final hash 要件 = §3 行 4。corpus case は「valid Draft 生成」へ反転 — §8） |
| "SCRIPTED" | (SCRIPTED / SCRIPTED, NOT_APPLICABLE) |
| "WAIT" | (WAIT / WAIT, NOT_APPLICABLE) |
...
- invalid corpus（base）: review-1 実証 negative + DC-4 追加分 + `E_DECIMAL_NONCANONICAL`（`-0`・anchoring-trap: `junk-0.5`・`0.0`・`1.50`・`03`・`1e3`・`5.` — U16）+ lone surrogate + duplicate key + non-NFC + int>2^53−1 + slot 不整合（E_SLOT_INCONSISTENT / E_SLOT_FORBIDDEN_NONE / E_KIND_CONTROL_MISMATCH: LEARNED×KNOWN(WAIT)）+ **未登録 method_id → E_METHOD_UNREGISTERED**〔工程 13 再 R2: 旧「"DAPG" → E_LINEAGE_FORBIDDEN」期待は B4/register ⑩ で撤去 — "DAPG" は valid Draft の**正例**へ反転（§6-5 + §8 B4 cases に単一化・二重期待の衝突を解消）〕+ identity-less v1 行 → E_MIGRATE_IDENTITY_ABSENT + float-bearing v1 → 変換 or E_MIGRATE_FLOAT_FORM。
...
- **#4b は hashed fields（grade / source_ref / proof / claim_target_hash / evaluator_artifact_hash）限定 + notes-mutation 行追加（全 hash 不変）**（C-CH7; **field 名は RV7 R-2 で現行 EvidenceRecord に同期** — 旧 artifact_hash は RV6 §3 で claim_target_hash に改名済み・evaluator_artifact_hash は hash-visible につき変異対象）。
...
- **RV5 fold 追加**: EP markdown ↔ **JSON fixture** ↔ code 定数の**三面一致試験**（**`WMSO_EvidencePolicy_v1.9.json` = normative fixture**〔RV7 R-1 → 工程 13 再 R3 で現行版へ再同期。歴史ラベル: v1.6 = 導入時（§11 RV5 fold）・v1.8 = superseded〕・semantic hash はこの test が確定 — RV5-W-P0-6）/ **B3 mutation 13 面**（semantic_obs/action_schema の 2 変異 → E_BEHAVIOR_REVISION_STALE — RV5-W-P0-4）/ `E_PROOF_ARTIFACT_UNRESOLVED`（resolver 取得不能・sha 不一致・zero-length — RV5-W-P0-3）/ `E_EVALUATOR_UNKNOWN` / **TP applicability 判別 case**（SCRIPTED の TP = N/A で CLOSED_LOOP 可〔他 required 充足時〕・LEARNED の TP 不在 = UNKNOWN 失格 — RV5-W-P0-2）/ **registry transitive projection case**（definition 非参照の schema 追加 → schema_registry_hash 不変・参照 schema 変更 → 変化 — RV5-W-P0-5）/ final_training_config_hash / final_source_commit の lineage 別 null/必須 corpus（RV5-W-P0-3）。
...
- **RV7 fold 追加**: **resolver 優先順位 regression**（OFFLINE_REPLAY × BC_ONLY × TRAINING_DATASET → NOT_APPLICABLE〔dash 優先〕— 旧順の REQUIRED 誤判定を負例に; learned × OFFLINE_REPLAY × TRAINING_PROVENANCE も同型）+ certification profile-neutral case / **CONFIG_HASH total map cases**（LEARNED×BC_THEN_RL: TD claim → bc_config_hash・POLICY_ARTIFACT claim → final_training_config_hash / LEARNED×{RL_ONLY,BC_ONLY,DEMO_PLUS_RL} → final / SCRIPTED → runtime_config_hash; 誤 target → E_PROOF_MISBOUND）/ **`E_GRADE_MISMATCH`**（record.grade ≠ assigned_grade → fail-closed）+ evidence_evaluation_bundle_hash 再現 + **eligibility が assigned_grade を使う判別 case**（record 自己申告 grade を上書きした入力 → E_CERT_INPUT_MISMATCH）/ **`E_HANDOFF_EPOCH_STALE`**（offer.control_epoch ≠ authority_epoch_snapshot）。
- **工程 13 B 系 fold 追加**: **B1 applicability 全域 corpus** = 13 component × 5 lineage × (3 profile + 中立) の**生成型 golden 期待表**（classify/overlay 合成の全 cell を機械検証 — 排他 branch の吸収なしを含む: 非 TD/TP component が TD/TP else に落ちない事）/ **B2 negative** = SCRIPTED × EXACT_TRAIN_TIME + synthetic TRAIN_RUN_MANIFEST → **E_GRADE_INAPPLICABLE**（proof 検査前の kind×grade 拒否; WAIT 同型）/ **B3 projection golden 7 本** = CM 全 3 member + HS {empty `76f0ddf3…` / single `00ffb015…` / multi+accepted `2c1d7760…` / **fields reorder `7a686477…`（≠ multi の実証対）**} — EP §3d/JSON 掲載値と一致検証 + 並べ替え coherence（reorder → SkillDefinitionHash・BehaviorSignature・claim_target 整合変化）/ **B4 cases** = "DAPG" v1 行 → **valid Draft（(DAPG/IMITATION_PLUS_RL, DEMO_PLUS_RL)）へ反転**（旧 E_LINEAGE_FORBIDDEN 期待を置換 — register ⑩）・method_id ∉ registry → **`E_METHOD_UNREGISTERED`**・method_id↔method_class 不一致 → 同 error・TD required @ DEMO_PLUS_RL（demo 系）・IMITATION_PLUS_RL × bc_config 非 null → E_MIGRATE_BC_CONFIG_UNEXPECTED 同型 fail-close。
...
## 10. Open points
**RV7 全項（v2.10）+ pN 工程 13 B1-B4/R1-R2（本版）= fold 済（自己申告 — §11 の各 fold-map に per-item 処置）。未充足 gate = pS 再照合 → pN exact-pin 再検証 → Rs freeze 判定** — 「open = 0」の無条件宣言は行わない（RV7 末尾の指摘: 検証未通過の自己申告を open 0 と書かない）。⚠**register ⑩（B4 — 批准済み §3 表の method registry 化）= Rs confirm 対象**。RV7 transcript の fidelity = Rs 確認 PENDING（chat 原文・byte N/A）。pN C1-C3 / R+PASS-CLOSE transcript = **pN 著者 CONFIRMED（工程 13 verdict 内 R1、21:08）**。review-4 transcript の fidelity は **RV4 §1.4 で Rs 確認済み: semantic fidelity CONFIRMED**（byte fidelity = N/A — 原本が chat message であり独立 file でないという限定付き。この限定を明記して PENDING 解消 — RV4 逐語「Rs確認PENDINGは、この限定を明記した上で解消してよい」）。W-review v2/v3/v4 は byte-identical file copy のため確認不要。
...
- **RV7→fold-map（Rs review v7 ⛔HOLD、対象 = v2.9.2 + 旧 zip 世代 companion; transcript = `WMSO_RS_REVIEW_V7_HOLD_TRANSCRIPT_20260719.md`）**:
  - **RV7-P0-1**（EP v1.7.1 実体不在・v1.7 の重複行）= **semantic 部分は v1.7.1（17:34、bank `ba69702cb9`）で先行修正済み** — pN R1 CLOSE が独立確認（「旧 FINAL 束縛残0」逐語）。RV7 の review 環境 companion が旧 zip 世代（EP `586fec…`）だったため観測不能だった（transcript 転記者注 — pQ の bundle 納品 gap）。**custody 部分 = 本 round の完全 bundle 納品（v1.8 系 + manifest 全 pin）で解消**。v1.7.1 の規則自体は v1.8 が supersede。
  - **RV7-P0-2**（resolver 入力・優先順位）= **EP §3c `resolve_applicability` 5 引数 + 優先順位 instance→profile「—」→lineage→default**（Rs sketch 逐語採用; 旧順の defect 例〔OFFLINE_REPLAY×BC_ONLY×TD〕を訂正記録 + regression corpus。register ⑨）+ 設計 §4 の参照更新 + JSON `applicability_rules` 同期。
  - **RV7-P0-3**（projection 未定義）= **EP §3d + JSON `projection_rules`（新 member — 16→17）に確定形**: control_mode = §2.3 slot 直列化（state 含む・KNOWN のみ〔EXPLICIT_NONE 型禁止/UNKNOWN 非 certify〕）/ handoff_schema = {handoff_schema, accepted_handoff}（**accepted 含む・宣言 tuple 順保持** — 第二 canonical 順を作らない）/ fields 要素型 = SemanticFieldSpec を §1.4 で pin / 空 tuple = [] / **golden vector 2 本を実算出・掲載**（`4f0b26d5…` / `00ffb015…`）。
  - **RV7-P0-4**（CONFIG_HASH stage 入力不在）= Rs 提示 2 案中 **total map 案採用**: **(identity_kind, training_lineage, component) 全域 map**（LEARNED×{RL_ONLY,BC_ONLY} → final / LEARNED×BC_THEN_RL → TD = bc_config・他 12 = final / SCRIPTED・WAIT → runtime_config）。**stage field は導入しない**（EvidenceRecord 不変・1 component = 1 claim 維持; BC config は BC_THEN_RL の TD claim が attest — 判別子を record 外に持たない）。JSON の旧 `stage` when 節は撤去。
  - **RV7-P0-5**（epoch 入力不在）= **`validate_handoff(..., authority_epoch_snapshot: int)`**（Rs sketch 逐語; W-P1-6 `now` と同型の決定論境界）+ `E_HANDOFF_EPOCH_STALE` + manager 責務に snapshot 読出しを追記。
  - **RV7-P0-6**（evaluation certificate 未接続）= Rs 提示の最低 3 点を**全て採用**: `DefinitionCertificationResult` field 集合明示（report / certificate | None / **evidence_evaluations**）+ **`ContractCertificate.evidence_evaluation_bundle_hash`** + **eligibility は assigned_grade 使用**（§5A2 第 2 引数 = evidence_evaluations — register ⑧）+ **`E_GRADE_MISMATCH`**（record.grade == assigned_grade の証明）。
  - **RV7-P0-7**（IdentityKind 未 inline）= §1.2 code block に class 定義 + §1.4 enum inventory に収載（member = {LEARNED, SCRIPTED, WAIT}）。
  - **RV7-R-1**（v1.6 参照残存）= §4 冒頭「全表 = EP **v1.8**」/ §8 三面一致「**v1.8.json**」（履歴中の v1.6 言及は Rs 許容どおり残置）。
  - **RV7-R-2**（mutation test 旧 field 名）= §8 #4b を {grade, source_ref, proof, claim_target_hash, evaluator_artifact_hash} に同期。
  - **RV7-R-3**（manifest 全面旧版）= manifest 全面書換（v2.10 系 pin + bank commit + pS/pN record + review 系譜 — RV7 工程 10-11）。
  - **RV7-R-4**（JSON 型記述）= metadata を「ASCII key / str・int・**bool** のみ・float/null/非 ASCII なし」に訂正（機械検証済み — 実 value 型 = {str, int, bool}）。
  - **hash 帰結**（RV7「066e…/ed10… を最終 pin にしない」）= EP semantic 版 v1.8 へ bump・`evidence_policy_definition_hash` = **`bdc508200889005142eaab5ac15c48cdfb8281df63bbb35537542f1f248891a8`**（実算出）・JSON file 改名 v1.8。
- **B(工程 13)→fold-map（pN exact-pin reverify ⛔HOLD B1-B4、21:08 — custody/hash legs = PASS; 全項 pQ on-disk 検証で真と確認の上 fold）**:
  - **B1 CRITICAL**（applicability_rules の分岐 semantics — 同 order 2 本の else が component-scope を欠き逐次評価で他 component を吸収 + requested_profile=None で min_grade 未定義）= **EP §3c を classify（profile 中立・component-scoped 排他 branch・明示 default fallthrough）+ profile overlay（eligibility 専用・非 None 必須）に分割**（pN 提示 2 案の後者〔classifier 分離〕を採用 — 5 引数形は合成として保存）+ JSON `applicability_rules` を classifier/overlay 構造に全面書換 + **全域 corpus（13×5×4）を §8 に義務化**。
  - **B2 HIGH**（EXACT×非学習の到達不能が prose のみ — synthetic manifest で充足可能）= **`identity_kind ∈ {SCRIPTED, WAIT} ∧ grade = EXACT → E_GRADE_INAPPLICABLE` を proof 検査前の機械規則に**（EP §3d + JSON `proof_policy._kind_grade_inapplicable`）+ negative corpus。
  - **B3 HIGH**（projection_rules.object が prose placeholder）= **構造化 field map**（handoff_schema_item / semantic_field_item / accepted_handoff_item の型付き列挙 + array 順序規則表）**+ golden 7 本**（CM 全 3 member〔total〕・HS empty/single/multi+accepted 非空/reorder 対 — 全て実算出・JSON に input 全文掲載・自己再現検証済み）。
  - **B4 CRITICAL/L0**（ExecutionFamily 固定が charter『algorithm-agnostic』『PPO or another online RL / DAPG or another imitation-plus-RL』+ Rs RL+IL 非限定要件と不整合）= **TrainingMethodDescriptor {method_class, method_id} + METHOD_REGISTRY**（§1.3/§1.4 — 中立クラス 5 種・具体 algorithm は registry 行〔Rs 承認制・fail-close `E_METHOD_UNREGISTERED`〕）+ **§3 表を method_class key 6 行に再構成**（IMITATION_PLUS_RL × DEMO_PLUS_RL 行を新設）+ TrainingLineage += DEMO_PLUS_RL（EP demo 系に参加）+ §6-5 DAPG 写像の有効化。**register ⑩ = Rs confirm 対象**（批准済み表の再構成 — charter 整合の復元）。
  - **R1**（transcripts fidelity 表記）= C1-C3 / R+PASS-CLOSE の 2 transcript を **pN 著者 CONFIRMED** に更新（header + §10; R+PASS-CLOSE は sibling list に本版で正式収載）。RV7 transcript = Rs-only PENDING 維持。
  - **R2**（header line 19 の pS bank 表記 stale）= 「本 v2.8 commit」→ **現行 bank `dfeb6c1e57`（§16 込み）+ 本行同時更新規則**。
  - **再判定 M1-M4（pN 22:15 — v2.11.1 設計/証拠 PASS-CLOSE 後の custody 残差・record-only; v2.11.2 + 同 bank manifest で fold）**: **M1** = manifest 最終 pin 表/lineage/bundle が v2.11 のまま → v2.11.2 現行候補に自己整合化 / **M2** = design :20 の dynamic「pS 現行 bank」記載 = design↔record 相互 hash 循環の構造欠陥 → **撤去・manifest authoritative 化**（本版の唯一の design 変更）/ **M3** = manifest の §17 引用 cell に「dangling ExecutionFamily 0 = §18 strike 訂正済」を明記（false を現行事実として読ませない）/ **M4** = R1-R4 transcript fidelity = pN CONFIRMED 反映。
  - **再 verify R1-R4（pN 21:55 — B1-B4 mechanism fold PASS 後の bounded consistency 残差; v2.11.1 で fold）**: **再 R1** = §1.2 IdentityKind comment の coherence 例示 {PPO,BC} → **{ONLINE_RL, IMITATION, IMITATION_PLUS_RL}**（B4 同期漏れ）/ **再 R2** = §8 base corpus の旧「"DAPG" → E_LINEAGE_FORBIDDEN」期待を撤去 → **未登録 method_id → E_METHOD_UNREGISTERED に置換・DAPG 正例は §6-5/§8 B4 cases に単一化**（二重期待の正面衝突を解消）/ **再 R3** = §8 三面一致 fixture v1.8 → **v1.9**（歴史ラベル明示）/ **再 R4** = pS record §17 :374「dangling ExecutionFamily=0」の訂正 = **pS 著者権 — 訂正依頼を dispatch**（+ manifest/transcript header の pN fidelity PENDING → **CONFIRMED**〔pN 21:55 逐語「SEMANTIC FIDELITY CONFIRMED / BYTE IDENTITY N/A」〕）。
