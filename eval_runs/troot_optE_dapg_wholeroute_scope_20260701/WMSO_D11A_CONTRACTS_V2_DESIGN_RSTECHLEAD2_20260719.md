# WMSO D1.1-A `contracts_v2` — DESIGN (v2.8)

- node: `T-WMSO`; author = w2:pQ (RS-TECH-LEAD2)
- created: v1 = 2026-07-19 09:09 JST / v1.1 = 09:24 / v2 = 09:32 / v2.1 = 09:43 / v2.2 = 10:59（editing 完了実測; 旧「10:5x」表記を確定 — 以後 x-mask 廃止）/ v2.3 = 12:07 / v2.4 = 12:41（12:31 に v2.3.1 として着手後、Rs review v3 着信により拡張）/ v2.4.1 = 13:07 / v2.5 = 13:36 / v2.6 = 14:16 / v2.7 = 14:36 / v2.7.1 = 15:46（records-only: pN transcript CONFIRMED 反映）/ **v2.8 = 16:07 JST（実測）**
- **status: DESIGN DRAFT v2.8 — fold = pS §12 全区間 re-check PASS-WITH-CONDITIONS の **G-1..G-4**（G-1/G-2/G-3 = records・G-4 = 非学習 evidence path〔EP v1.6 kind 条件付き束縛〕）。⭐**RV5 §6-5 の Rs 方向 = bounded fix 完了で D1.1-A freeze → D1.1-B/C へ** → pS final confirm → pN 再 verify（最終 SHA 宛）→ freeze 判定 PENDING**
...
- 兄弟 normative artifacts（pN DESIGN verify の対象; full 64-hex — B-CH5）:
  ...
  - **EvidencePolicy v1.6** = `WMSO_D11A_EVIDENCE_POLICY_V1_RSTECHLEAD2_20260719.md`（sha = bank commit で確定・毎版更新）+ **機械可読 fixture `WMSO_EvidencePolicy_v1.6.json`**（RV5-W-P0-6; filename = 内容版数と同期 — C-P0-4）
