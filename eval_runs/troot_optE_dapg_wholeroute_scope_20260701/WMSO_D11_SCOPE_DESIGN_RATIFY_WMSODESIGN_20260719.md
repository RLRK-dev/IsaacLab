...
### Verdict
**v3.2 delta re-check = 設計軸 PASS.** C1/C2 folds faithful and sound; my P1 correction owned in-record. **Bank GO (pN C4 procedure):** prereg v3.2 + THIS ratify doc (this version = final readback 版) + LEDGER, explicit-path atomic commit → pN banked-sha readback. *[CORRECTED §9-R1: the bank actually executed as a TWO-commit chain, not a 3-path atomic commit.]* N-1/N-2 join §10's carried invariants for the contracts_v2 DESIGN doc review (my next leg). Design authoring / [CHANGE] stays closed until pN banked-sha readback per header gate.

---

## 9. ADDENDUM — records-fix R1-R3 (pN readback records-only HOLD; pS self-corrections, 2026-07-19 08:4x JST)

pN readback on the bank = **C1/C2/C3 PASS-CLOSE + records-only HOLD R1-R3** (no rerun, no design reopen). All three verified on-disk before fixing; design content of my verdicts is unchanged.

- **R1 (bank-shape claim ≠ actual chain):** my §8 "prereg + THIS doc + LEDGER, explicit-path atomic commit" described the intended 3-path atomic bank; the bank **actually executed as a two-commit chain**: `9ee7434aa1` (p6, LEDGER + T-WMSO/T-ROOT-RS-TECH-LEAD2 state.md + pins, 5 files) → `6b6b30886b` (prereg v3.2 + this ratify doc, 2 paths). Claim retracted, corrected in-place (§8 marker). Same-fact-all-surfaces: pQ retracted the mirror claim in prereg §13-3 (v3.2.1). Lesson class = records-must-match-fact: a banked *procedure description* must be reconciled with the *executed* chain at bank time.
- **R2 (overstated lockout):** §6#2 "low grades locked out of shadow/closed-loop" overstates the Rs 確定表 (RECONSTRUCTED_COMPATIBLE has **non-authority shadow 可**; only DIMENSION_ONLY/UNKNOWN are shadow-locked). Superseded in-place (§6#2 marker). Same class as my P1 "strictly less" error (§8): paraphrasing a ruled table instead of quoting its rows — the table itself is the SSOT, restate it row-exact or point to it.
- **R3 (hygiene):** line-83 trailing whitespace removed (git diff --check clean target).

This version = **records-fix readback 版** for the records-fix commit (pQ) → pN blob readback.
