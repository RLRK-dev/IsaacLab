...
# The clips move with the hands: Rs asked for them to match the new spacing, so they are placed
# one span apart about the same midpoint they had (0.095).
C1 = (0.150, CLIP_Y_ODD)    # table :1253 C1 = 0.35 / +0.150
C2 = (0.040, CLIP_Y_EVEN)   # table :1254 C2 = 0.40 / +0.075
...
# Rs: double the spacing between the left and right hands, and place the clips to match.
# 88 mm (task_config.py:235 @ 843084ae5e47ddc9f17bfe33c2dbc3f46ea53562) -> 176 mm.  This is the
# grasp span named in RS71 section 0 item 2, so the change is Rs's, recorded here as such.  It is
# what forced the right arm to roll 34 degrees to reach: at 88 mm the two wrists meet unless the
# tool is rolled, and rolling is what tips the ko's mouth off the cable.
GRIP_HALF_SPAN = 0.044
...
CLIP_RISER = 0.040
GROOVE_Z = TABLE_TOP + CLIP_RISER + 0.030   # where the cable must end up, world z
...
def seat_legs(clip, cx, cy, link):
    ...
    legs = {"x": abs(p[0] - cx) < 0.022,
            "y": abs(p[1] - cy) < GROOVE_W / 2.0,
            "z": abs(p[2] - (TABLE_TOP + CLIP_RISER + 0.030)) < 0.006}
...
for num, name, lt, rt, lf, rf, secs, gate in STEPS:
    ...
    for t in SIDES:
        if abs(float(tgt[t][2]) - Z_SEAT) < 1e-9:
            off = seat_offset(t, prev[t])
            want = np.array([float(tgt[t][0]), float(tgt[t][1]), GROOVE_Z])
            tgt[t] = want - off
            print(f"[steps] STEP{num:2d} {t}: aiming the MOUTH at the groove "
                  f"(measured mouth-minus-pinch offset {np.round(off*1000,1)} mm; the old "
