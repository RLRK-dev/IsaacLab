...
**· v2.1 PATCH 2026-07-03 10:14 JST (%9 RE-VALIDATE build-review fold, spec-clarification only — 0-code/0-GPU):** C1 command-keyed mask/assert (§C, §F.1, §F.3) + C2 per-phase expand-not-clip affine (§F.2) + §H.1 verdict-notation pin (anti-(iv)-confound). **two-key UNLOCKED** (Rs Option-A direct GO + %9 CONCUR PASS); build gated on **Rs loud notify only**.
...
- **converter assert (U3, add to regression — %9 C1 command-keyed, VERDICT-CRITICAL):** the KEEP/DROP discriminator + the regression assert key on the **COMMANDED target (offset==0 = released) / `injection_windows` membership**, NOT achieved-ee_pos. A KEPT row is asserted to have commanded-offset==0 AND to lie OUTSIDE every injection window; a kick/outbound row (commanded = script+offset, in-window) is DROPPED. ⚠ **Do NOT assert `achieved ≈ script`:** a recovery frame's achieved ee_pos is LEGITIMATELY off-path (in transit script+offset→script) — an achieved-keyed assert would drop the restoring teachers → validity passes but γ⊥ static = a **FALSE Outcome-B** (a mask bug misread as mechanism failure; the inverted form of silent-wrong-teacher, %9).
...
1. **mask (U2, command-keyed per %9 C1):** DROP rows whose **COMMANDED target carries the offset** (kick/outbound, in `injection_windows`); KEEP post-release recovery rows (commanded-offset==0, out-of-window; label=achieved-moving-back=restoring). Key on **commanded-target / window-membership, NOT achieved position**. No hold window exists to keep.
2. **affine AFTER masking (U7 — %9 C2 per-phase, expand-not-clip):** build `_build_abs_affine` over **kept-p2r + clean** frames. The masking (§F.1) already removed the kick/outbound excursions; the KEPT recovery frames of **{1}/{11}** still LEGITIMATELY excurse ~offset (≤20mm) off the clean path → the **{1}/{11} per-phase boxes EXPAND** to encompass them (⚠ do NOT clip — clipping distorts the restoring label); `amax≤0.95` (`:451`) held by the **widened** box. **p2r regression (per-phase, %9 C2):** the **NON-injected** phase boxes assert within tol of the clean-only b2 box (unchanged, `:156-157`/`:519`); the **{1}/{11}** boxes assert widened by ~offset ONLY (expected recovery excursion, NOT contamination) + `amax≤0.95` recheck on the masked set.
3. **converter assert (U3, %9 C1 command-keyed):** each KEPT p2r row asserted **commanded-offset==0 AND out-of-`injection_windows`** (NOT achieved≈script — see §C rationale: an achieved-keyed assert drops restoring teachers = FALSE Outcome-B).
...
### §H.1 verdict-notation pin (%9 RE-VALIDATE, anti-(iv)-confound)
v1 injects ONLY {1},{11}. The other movable cells are **un-injected in v1**: {2,3} = SKIP-**un-injectable** (close/lift choreography), {0} = single-call generalization-only (U4), {4,5,10,12,13} = fills deferred to v3. Therefore the verdict template MUST pin:
- **v1's TRUE success criterion = the §H directional-γ⊥ check on the load-bearing INJECTED cells {1},{11}** (did their γ⊥ move ↓ toward the restoring bar). This is the kick-and-recover mechanism verdict.
- If the full B2 §3.3 movable table STOPs because **{2,3}/{0} γ⊥ did not move, that STOP is EXPLICITLY attributed to un-injectability** (no teacher for those cells in v1) — **NOT to a kick-and-recover mechanism failure, and must NOT be read as「(ii) が (iv) 同様に失敗」.** (Distinct from (iv), whose failure was synthetic-obs model-consistency; here the un-injected cells simply have no v1 teacher by scope, by design.)
- The full §3.3 GO gate remains the eventual bar (reached when fills land in v3); v1's honest verdict **separates injected-cell γ⊥ movement (the mechanism verdict) from un-injected-cell γ⊥ static (expected-by-scope, not a failure)**.
