...
goal_verification: |
  ...
  A. P2 env (L3 design-gate W0-c): oracle-query-API (CRIT1 phase-clock resolve) + progress scalar obs + contract 統一 + Δ非累積契約 (envelope = P4: per-step-rate × decimation、max 1.86mm/frame) + ⑦(b) cable-fork curriculum-reset 機構 + DR hooks (routeexec-⑦ env-level 継承) + OG port/composite/band 再較正 + throughput/device-parity smoke
...
- P4 Δ-bound 結果 (Stage-A §6-9 input): Δ-bound = per-step-rate × decimation (max 1.86mm/frame 基準)、control-freq 確定後 pin。expressibility CLEARED (α-6D decision は不触)。
