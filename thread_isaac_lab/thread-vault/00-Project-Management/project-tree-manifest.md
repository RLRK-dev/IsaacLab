...
_236 nodes — `build_nest_snapshot.py --emit-manifest-section` 生成 (SSOT = per-node state.md; 手書き禁止)。status = verbatim (coercion なし)。全 node 詳細/依存 = NEST jsx tracker + nest-snapshot.json。_
...
| `T-ROOT-Verbal-Teaching-20260705` | DESIGN-APPROVED (Rs「承認」2026-07-06 03:2x — D-1 posture + D-7 governance ratify + 推奨どおり) / BUILD = deferred-on-measured-trigger | `T-ROOT` |
