...
>
> **7. ⭐⭐ 着地済み D1 実装に findings 2 件（p15 提出・私が独立確認済・⛔fix は gate 開放後）**: 記録 = `WMSO_D1_IMPL_STATIC_FINDINGS_IMPLVERIFIER2_20260727.md` / **内容 sha256 `4068ac94af668d78618017ff4795df3ff32db84906b77c80b36af9c331dc84ba`**（観測 2026-07-27 20:44:22 JST・照合 commit `aada43e8e2`・176 行）。⭐**pin は content sha で持つ**（本 doc は 1 日に 3 版動いた: `de3cad3832…` @ `9e9832a652` → `e078dd94e5…` @ `291ae42c0c` → 現行）。
>     - ⚠**版の系譜と「置き場所」の実測**: 第 2 版は **先頭側へ 41 行挿入**（⇒ 第 1 版基準の行番号引用は一律 **+41** ずれた。⛔`numstat +N/−0` は置き場所を識別しない — これが誤記の根因）／第 3 版は **⭐純末尾追記**（私の実測: `insert old[142:142] → new[142:176]`・削除 0・**旧見出しのずれ集合 = {0} = 不動**）＝ **行番号はもう動かない**。⇒ 追記を認可する時の型 = **①末尾に置く ②先頭/中間なら ずれ量を明記 ③座標でなく不変量（挿入行数・削除 0・ずれ量）で述べる**（座標は器具間で 1 行滑る）。⚠**対象は pN が 2026-07-19 に PASS-CLOSE した面**（`57ed32b27a`）。
