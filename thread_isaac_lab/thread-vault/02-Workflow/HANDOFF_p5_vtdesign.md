...
## 43. 追記（date 実測 13:19:21 系列・witness = bank commit）— **m-p18-85/86 受領: item 8 close（Rs 直接裁定 (a)）・item 7 SETTLE = C-2（Rs 委任下 p4 決定）・#18 DoD 処分 = (b)。当卓 2 注記を納品**

**(1) first-hand 照合（書く前に測った集合）**: ①Rs 裁定「8 は (a) で close して」= `2dbed74a`:20133・user・2026-08-08T04:07:05.015Z（=13:07:05 JST）— 当卓で同 line を print・p18 pin（m-p18-85）と逐語一致 ②Rs 委任「君が判断していい」= `9e3d21d6`:6237・user・04:09:27.452Z（=13:09:27 JST）— 同上 ③custody bank `8599c6627d`（`P4_DELEGATED_DECISIONS_ITEMS7_9_DOD_20260808.md` +53 行）content sha 当卓再計算 = `8d113c5a1eb87edb…835cf01f` 一致・§1 列挙 3 面一致（**#54 再測は (b) のみ・(a) は認可しない**）を実読 ④C-2 数値 = `GRID240_READING_20260802.md` §3 実読（L5/R30・+14.7mm・240 draws・「chosen pose pair」の語も §3 系に在る）。
**(2) 納品 2 件（m-p18-86 の当卓宛 2 指示）**: ①menu → **v0.2**（§0-A SETTLE 記録 + §2 C-2 行 marker + §4 全 4 点に ⇒決着 注記: 1=**C-2**／2=**crown 不変**／3=**選択後再測・carrier=item 7 単独**／4=**不要**）②DoD v0.3 §11 → **処分行 1 行追記**（**(b) design-side accept**・決定者 = p4 委任下・残余は /reward-design 全走→/pre-check が carry・**次段 D-list 数値設計は commission 待ち**）。両編集とも**追記のみ・組成時本文不変**。
**(3) 等級の保存**: menu header「settle は Rs」は**委任下の p4 決定**で満たされた — §0-A に「Rs 直接発話でない」を明記（Q2 と同型: 裁定と読みの差を、読者が読む場所に置く）。DoD §11 の (b) も同じ等級を明記。
**(4) 出荷しなかった中間観測（規律記録）**: m-p18-86 到着前、裁定 line 前後の調査中に「`2dbed74a` の裁定直前 screen（:20130・12:01:07 footer）には item-8 列挙が無く**別物の (a)/(b)**（#18 DoD 処分）が表示されていた」を観測。token query（このまま／追い測定／existence 4）は裁定前 0 hit だったが、**query 未完**（別 wording・別 session file 未走査）のまま custody が 3 面 bank で close ⇒ **不在主張として出荷せず**（closed query になっていない）。p4 記録 §1 で提示 = 11:00/11:11 の別 wording と判明 — 半走の query を主張にしない（実地 1 件）。
**(5) 教訓受領**: ①m-p18-85(4) **count-class 一般化**（slot を閉じると習慣は隣の未測 field へ移る — 修理単位 = compose-time 測定量 class 全体。当卓は shell 注入運用を継続）②m-p18-86 冒頭（**authority objection も premise を測ってから** — p18 が 2 分の測定で自らの誤 objection を止めた）。
**(6) 盤面（m-p18-86 count・当卓視点）**: CLOSED 1-6, 8／OPEN = 7（settled C-2・当卓 surface 反映済 — LEDGER 側 p6 待ち）・9（p6 reconciliation・裁定者 = p4 は p18 の読み〔Rs が自ら取れば消える〕）／#18 DoD = **disposed (b)**。**当卓 open 0・owed = bank 依頼のみ・HOLD 不変・実行 0。**
