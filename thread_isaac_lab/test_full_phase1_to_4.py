#!/usr/bin/env python3
"""
Full Phase 1 -> 2 -> 3 -> 4 Integration Test
...
4. Grasp maintained throughout all phases
5. No cable drop

Reference:
- test_cumulative_lift.py for Phase 2->3
- test_phase3_to_4_diffik.py for Phase 3->4
"""
...
print("=" * 70)
print("FULL PHASE 1 -> 2 -> 3 -> 4 INTEGRATION TEST")
print("=" * 70)
...
LIFT_TARGET_CM = 5.0       # Target lift in cm (5cm to stay within stable range)
...
# Set Phase 1 position with grippers open
...
# Move to grasp position with grippers open
...
# Stabilization phase - ensure grasp is stable before recording
...
# Calculate lift target (lift from current position by LIFT_TARGET_CM)
# Instead of using Phase 3 waypoint, lift from current position to maintain grasp
lift_amount_m = LIFT_TARGET_CM / 100.0
target_phase3_left = phase2_ee_pos_left_tensor.clone()
target_phase3_left[0, 2] += lift_amount_m  # Add lift to current Z
target_phase3_right = phase2_ee_pos_right_tensor.clone()
target_phase3_right[0, 2] += lift_amount_m  # Add lift to current Z
...
# Pre-lift stabilization: maintain grasp position while IK initializes
print("  Pre-lift stabilization...")
for _ in range(50):
    # Keep commanding grasp position and gripper closed
    set_robot_joints(robot_left, PHASE2_LEFT_JOINTS, GRIPPER_CLOSE)
    set_robot_joints(robot_right, PHASE2_RIGHT_JOINTS, GRIPPER_CLOSE)
    sim.step()
    scene.update(sim.get_physics_dt())

# Re-record state after stabilization
phase2_ee_pos_left_tensor = robot_left.data.body_pos_w[:, jacobian_body_left].clone()
phase2_ee_quat_left_tensor = robot_left.data.body_quat_w[:, jacobian_body_left].clone()
phase2_ee_pos_right_tensor = robot_right.data.body_pos_w[:, jacobian_body_right].clone()
phase2_ee_quat_right_tensor = robot_right.data.body_quat_w[:, jacobian_body_right].clone()

# Update targets with fresh data
target_phase3_left = phase2_ee_pos_left_tensor.clone()
...
pre_lift_force_l = get_contact_force(contact_left)
pre_lift_force_r = get_contact_force(contact_right)
print(f"  Pre-lift Force: L={pre_lift_force_l:.2f}N, R={pre_lift_force_r:.2f}N")

# Track contact during lift
min_force_during_lift = float('inf')
contact_lost_step = None
consecutive_low_force = 0  # Count consecutive steps with low force
CONTACT_LOST_THRESHOLD = 10  # Require 10 consecutive low-force steps to declare contact lost
...
for i in range(LIFT_NUM_STEPS):
    ...
    # Linear interpolation to Phase 3 target
    ...
    # Track minimum force (only when force > 0 to filter sensor noise)
    ...
    # Detect contact loss with hysteresis (require consecutive low-force steps)
    status = "OK"
    if force_l < 1.0 and force_r < 1.0:
        ...
        consecutive_low_force = 0  # Reset counter if force is OK
        if contact_lost_step is not None:
            status = "recovered?"

    # Log every 50 steps or on contact loss
...
grasp_maintained_phase3 = contact_lost_step is None

print(f"  Lift:      {'SUCCESS' if lift_success else 'FAIL'} (threshold > 5cm)")
print(f"  Grasp:     {'MAINTAINED' if grasp_maintained_phase3 else 'LOST at step ' + str(contact_lost_step)}")
...
# ============================================================
print("\n" + "=" * 70)
print("PHASE 4: TRANSITION (Cumulative Diff IK)")
print(f"  NUM_STEPS: {TRANSITION_NUM_STEPS}")
print("=" * 70)

# Calculate transition target (Phase 4 target with Z offset)
...
dist_left = np.linalg.norm(np.array(WAYPOINT_PHASE4_LEFT) - np.array(WAYPOINT_PHASE3_LEFT))
dist_right = np.linalg.norm(np.array(WAYPOINT_PHASE4_RIGHT) - np.array(WAYPOINT_PHASE3_RIGHT))
...
# Track contact and joint changes during transition
min_force_during_transition = float('inf')
max_joint_change_per_step = 0.0
contact_lost_step_transition = None
consecutive_low_force_transition = 0
...
for i in range(TRANSITION_NUM_STEPS):
    ...
    if force_l > 0 or force_r > 0:
        min_force = min(force_l, force_r) if force_l > 0 and force_r > 0 else max(force_l, force_r)
        if min_force < min_force_during_transition:
            min_force_during_transition = min_force

    # Detect contact loss with hysteresis
    status = "OK"
    if force_l < 1.0 and force_r < 1.0:
        consecutive_low_force_transition += 1
        if consecutive_low_force_transition >= CONTACT_LOST_THRESHOLD and contact_lost_step_transition is None:
            contact_lost_step_transition = i - CONTACT_LOST_THRESHOLD + 1
            status = "*** LOST ***"
        elif contact_lost_step_transition is not None:
            status = "lost"
        else:
            status = f"low({consecutive_low_force_transition})"
    else:
        consecutive_low_force_transition = 0
        if contact_lost_step_transition is not None:
            status = "recovered?"

    # Log every 100 steps or on contact loss
...
phase4_joints_left = get_joint_angles(robot_left)
phase4_joints_right = get_joint_angles(robot_right)
...
ee_error_left = np.linalg.norm(phase4_ee_left - target_left_with_offset) * 100
ee_error_right = np.linalg.norm(phase4_ee_right - target_right_with_offset) * 100
...
print(f"  EE Left Error:   {ee_error_left:.2f}cm")
...
print(f"  EE Right Error:  {ee_error_right:.2f}cm")
print(f"  Force:           L={phase4_force_l:.2f}N, R={phase4_force_r:.2f}N")
print(f"  Cable Z:         {phase4_cable_z:.4f}")
print(f"  Max joint change/step: {max_joint_change_per_step:.2f}°")

transition_success = ee_error_left < 2.0 and ee_error_right < 2.0
joint_continuity_ok = max_joint_change_per_step < 5.0
grasp_maintained_phase4 = contact_lost_step_transition is None

print(f"  EE Error:        {'SUCCESS' if transition_success else 'FAIL'} (threshold < 2cm)")
print(f"  Joint Continuity:{'SUCCESS' if joint_continuity_ok else 'FAIL'} (threshold < 5°/step)")
print(f"  Grasp:           {'MAINTAINED' if grasp_maintained_phase4 else 'LOST at step ' + str(contact_lost_step_transition)}")
...
results["phase4"] = {
    ...
    "joint_continuity_ok": joint_continuity_ok,
    "max_joint_change_per_step": max_joint_change_per_step,
    "grasp_maintained": grasp_maintained_phase4,
    "min_force_during_transition": min_force_during_transition
}
...
# ============================================================
print("\n" + "=" * 70)
print("FINAL SUMMARY - Phase 1 -> 2 -> 3 -> 4")
print("=" * 70)

# Verification items from TASKS_T1.md
print("\n検証項目:")
...
print(f"  3. Phase 3→4遷移成功（EE誤差 < 2cm）: {'✅ PASS' if transition_success else '❌ FAIL'} (L={ee_error_left:.1f}cm, R={ee_error_right:.1f}cm)")
print(f"  4. 把持維持（全Phase通じて接触力 > 5N）:")
print(f"       Phase 2→3: {'✅ PASS' if grasp_maintained_phase3 else '❌ FAIL'} (min={min_force_during_lift:.1f}N)")
print(f"       Phase 3→4: {'✅ PASS' if grasp_maintained_phase4 else '❌ FAIL'} (min={min_force_during_transition:.1f}N)")

# Cable drop check
cable_dropped = phase4_cable_z < phase2_cable_z - 0.02  # More than 2cm drop from grasp position
print(f"  5. ケーブル落下なし:                  {'✅ PASS' if not cable_dropped else '❌ FAIL'} (Z={phase4_cable_z:.4f})")
...
print("\n" + "=" * 70)
if all_passed:
    print("✅ OVERALL: SUCCESS - Full Phase 1→2→3→4 completed!")
else:
    ...
    if not transition_success:
        print("  - EE position error too large")
