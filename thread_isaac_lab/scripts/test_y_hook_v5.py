...
def create_cable_hanging(base_path, v_valley_pos, table_z, num_segments=24,
                          cable_total_length=0.40, segment_radius=0.006,
                          color=(1.0, 0.5, 0.0)):
    """
    Create cable with correct shape:
    - Hook (center) is highest point
    - Ends hang down to table level (inverted U shape)
    """
    stage = omni.usd.get_context().get_stage()
    ...
    # Hook top height (V-valley + a bit higher)
    hook_top_z = v_valley_pos[2] + 0.01

    # End height (just above table)
    end_z = table_z + 0.03
    ...
    for i in range(num_segments):
        ...
        # Y: from left robot (-Y) to right robot (+Y)
        y = v_valley_pos[1] + (t - 0.5) * cable_total_length

        # X: at hook center
        x = v_valley_pos[0]

        # Z: Inverted U shape (parabola with max at center)
        # Center (t=0.5) is highest, ends (t=0, t=1) are lowest
        dist_from_center = abs(t - 0.5) * 2  # 0 at center, 1 at ends

        # Smooth curve using power function
        z = hook_top_z - (hook_top_z - end_z) * (dist_from_center ** 1.5)
    ...
    print(f"[Cable] {num_segments} segments, ~{cable_total_length*100:.0f}cm")
    print(f"  Shape: Inverted U (hook at top)")
    print(f"  Center (hook): z = {hook_top_z:.3f}m")
    print(f"  Ends (table):  z = {end_z:.3f}m")
...
def main():
    print("\n" + "=" * 60)
    print("Y-HOOK GOAL STATE v5 (Correct Shape)")
    print("Hook is highest point, ends hang down to table")
    print("=" * 60)
    ...
    # Cable (inverted U shape)
    print("[Status] Creating cable...")
    create_cable_hanging(
        base_path,
        v_valley_pos,
        table_z,
        num_segments=24,
    ...
    print("\n" + "=" * 60)
    print("GOAL STATE v5 (Correct Shape)")
    print("")
    print("Side view:")
    print("")
    print("       ╲ ╱   ← Y-hook (blue)")
    print("        ●    ← Cable highest point")
    print("       ╱ ╲")
    print("      ╱   ╲")
    print("     ╱     ╲  ← Cable hanging down")
    print("    ●       ●  ← Cable ends (near table)")
    print("  ═══════════  ← Table")
    print("")
    print("  [Left Robot]  [Right Robot]")
    print("   grabs here    grabs here")
    print("")
