...
"""Collect precision grasp demonstrations for DAPG (1mm action scale).

Uses the standard ApproachCable env with POS_ACTION_SCALE=0.001 (1mm).
Scripted proportional controller: action = clip(-error / scale, -1, 1).
Starts from default P0 cache (~10mm above cable) or custom precondition.

Usage:
    source ~/env_isaaclab6/bin/activate
    cd ~/IsaacLab
    python thread_isaac_lab/scripts/collect_precision_grasp_demos.py \
        --num-episodes 80 --world-count 4 \
        --output thread_isaac_lab/data/bc_demos/precision_grasp_demos_v1.npz \
        --device cuda:2
"""
...
def main():
    parser = argparse.ArgumentParser(description="Collect precision grasp demos (1mm action scale)")
    parser.add_argument("--num-episodes", type=int, default=80)
    parser.add_argument("--world-count", type=int, default=4)
    parser.add_argument("--device", type=str, default="cuda:2")
    parser.add_argument("--output", type=str,
                        default="thread_isaac_lab/data/bc_demos/precision_grasp_demos_v1.npz")
    ...
    parser.add_argument("--rot-action-scale", type=float, default=0.003,
                        help="ROT_ACTION_SCALE [rad] (default: 0.003 = 0.17deg, clamp shift <0.66mm)")
    ...
    NewtonApproachCableEnv.ROT_ACTION_SCALE = args.rot_action_scale
    NewtonApproachCableEnv.TERMINAL_STEPS_OVERRIDE = args.terminal_steps
    if args.precondition_cache:
        NewtonApproachCableEnv.PRECONDITION_CACHE_OVERRIDE = os.path.abspath(args.precondition_cache)

    print(f"[PrecisionDemo] Config: POS_SCALE={args.pos_action_scale*1000:.1f}mm, "
    ...
    for batch_idx in range(batches):
        ...
        for step in range(args.terminal_steps):
            ...
            # obs[33:36] = pos_error = clamp_r_pos - seg_pos (positive = overshoot)
            # obs[30:33] = ori_error_aa (axis-angle from hand to grasp target)
            # obs[39:42] = pos_error_l, obs[36:39] = ori_error_aa_l

            actions = np.zeros((args.world_count, 12), dtype=np.float32)

            for w in range(args.world_count):
                # Right arm: position
                pos_err_r = obs_np[w, 33:36]  # clamp - target
                pos_act_r = -args.pos_gain * pos_err_r / args.pos_action_scale
                actions[w, 0:3] = np.clip(pos_act_r, -1.0, 1.0)

                # Right arm: rotation
                ori_err_r = obs_np[w, 30:33]  # axis-angle error
            ...
            # obs: [N, 42] tensor (post-reset if done)
    ...
    # Reshape: interleave worlds correctly
    # obs_all is [batches * terminal_steps, N, 42] after concat
    # We want [total_transitions, 42]
    ...
    elapsed = time.perf_counter() - t0
    print(f"\n[PrecisionDemo] Done in {elapsed:.1f}s")
