...
from envs.dual_arm_cfg_opt_c import DualArmSceneCfg, TABLE_HEIGHT
from utils.d6_grasp_manager import D6GraspManager

# Cable segment indices (20-segment articulated cable)
# Option C: X-axis cable layout (SphericalJoint v5 with rigid body damping)
# NOTE: Articulation structure creates branching pattern, NOT linear ordering
# seg_0 is root (near center)
# With Y-axis -90° rotation, cable extends along X-axis:
#   seg_17: Near MIN X (left end along X)
#   seg_19: Near MAX X (right end along X)
# Left robot targets seg_17, Right robot targets seg_19
CABLE_BASE_IDX = 17  # "Left" gripper target (MIN X, left end)
CABLE_TIP_IDX = 19   # "Right" gripper target (MAX X, right end)

# Grasp offset: move grippers 3cm inward from cable ends for stable grasping
# Cable is along X-axis: seg_17 at min X (left), seg_19 at max X (right)
# Inward offset is applied along X-axis toward cable center
GRASP_INWARD_OFFSET = 0.03  # 3cm inward from cable ends (along X-axis)
...
class DualArmController:
    ...
    def __init__(self, scene: InteractiveScene, device: str):
        ...
        # Cable runs along X axis, so fingers must open along Y axis (perpendicular)
        # X=180° makes gripper point down, Z=0° keeps fingers to open along Y (perpendicular to X-axis cable)
        from scipy.spatial.transform import Rotation as R
        final_rot = R.from_euler('xz', [180, 0], degrees=True)  # Z=0 for X-axis cable
        quat_xyzw = final_rot.as_quat()  # scipy returns [x, y, z, w]
        quat_wxyz = [quat_xyzw[3], quat_xyzw[0], quat_xyzw[1], quat_xyzw[2]]
        print(f"[Controller] Gripper quaternion (wxyz): {quat_wxyz}")
        print(f"[Controller] Gripper orientation: pointing down, fingers perpendicular to X-axis cable (Option C)")
...
class GoalImageCollector:
    ...
    def execute_phase(self, phase: TaskPhase) -> bool:
        ...
        if phase == TaskPhase.REACH_CABLE:
            ...
            # Option C: X-axis cable layout
            # Cable extends along X axis: seg_17 at min X (left), seg_19 at max X (right)
            # Y is roughly constant at cable spawn Y position
            # Apply inward offset to X axis (toward cable center)
            HOVER_HEIGHT = 0.10
            # seg_17 (left) is at min X, so inward = +X
            # seg_19 (right) is at max X, so inward = -X
            left_grasp_x = cable_left[0].item() + GRASP_INWARD_OFFSET  # Move toward center (+X)
            right_grasp_x = cable_right[0].item() - GRASP_INWARD_OFFSET  # Move toward center (-X)
            left_grasp_y = cable_left[1].item()  # Keep Y at cable position
            right_grasp_y = cable_right[1].item()  # Keep Y at cable position
            print(f"  [X-axis cable] Grasp position (inward offset applied to X):")
            print(f"  [Grasp Y] Left Y: {left_grasp_y:.4f}, Right Y: {right_grasp_y:.4f} (unchanged)")
            print(f"  [Grasp Offset] Left X: {cable_left[0].item():.4f} -> {left_grasp_x:.4f} (+{GRASP_INWARD_OFFSET}m)")
            print(f"  [Grasp Offset] Right X: {cable_right[0].item():.4f} -> {right_grasp_x:.4f} (-{GRASP_INWARD_OFFSET}m)")
            ...
            # Use same inward X offset as Stage 1 (X-axis cable)
            ...
            # X-axis cable: inward offset applied to X, Y unchanged
            left_grasp_x_updated = cable_left_updated[0].item() + GRASP_INWARD_OFFSET  # seg_17 at min X, move +X
            right_grasp_x_updated = cable_right_updated[0].item() - GRASP_INWARD_OFFSET  # seg_19 at max X, move -X
            left_grasp_y_updated = cable_left_updated[1].item()  # Keep Y at cable position
            right_grasp_y_updated = cable_right_updated[1].item()  # Keep Y at cable position
            print(f"    Updated targets: Left X={left_grasp_x_updated:.4f} (inward +3cm), Y={left_grasp_y_updated:.4f}")
            print(f"    Updated targets: Right X={right_grasp_x_updated:.4f} (inward -3cm), Y={right_grasp_y_updated:.4f}")
            ...
            # X-axis cable: seg_17 (left) at MIN X, seg_19 (right) at MAX X
            left_target_x = cable_left[0].item() + GRASP_INWARD_OFFSET  # Move toward center (+X)
            right_target_x = cable_right[0].item() - GRASP_INWARD_OFFSET  # Move toward center (-X)
            left_target_y = cable_left[1].item()  # Keep Y at cable position
            right_target_y = cable_right[1].item()  # Keep Y at cable position
            ...
            if left_target_dist > 0.005 or right_target_dist > 0.005:
                ...
                # X-axis cable: Apply inward offset to X, keep Y at cable position
                # seg_17 (left) at MIN X: move +X (toward center)
                # seg_19 (right) at MAX X: move -X (toward center)
                left_align_x = cable_left[0].item() + GRASP_INWARD_OFFSET  # Move toward center (+X)
                right_align_x = cable_right[0].item() - GRASP_INWARD_OFFSET  # Move toward center (-X)
                left_align_y = cable_left[1].item()  # Keep Y at cable position
                right_align_y = cable_right[1].item()  # Keep Y at cable position
                print(f"    [Y unchanged] Left Y: {left_align_y:.4f}, Right Y: {right_align_y:.4f}")
                print(f"    [Offset] Left X: {cable_left[0].item():.4f} -> {left_align_x:.4f}")
                print(f"    [Offset] Right X: {cable_right[0].item():.4f} -> {right_align_x:.4f}")
