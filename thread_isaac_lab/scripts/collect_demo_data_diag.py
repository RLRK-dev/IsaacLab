...
PHASE1_STEPS = 100
PHASE12_STEPS = 300
GRASP_CLOSE_STEPS = 300  # Increased 100→300 for contact force spike prevention
GRASP_STABILIZE = 100
LIFT_STEPS = 1600  # Double slow lift (diagnostic) for NaN prevention
PHASE34_STEPS = 400
PHASE445_STEPS = 300
PHASE455_STEPS = 300
PHASE55_STEPS = 200
...
# Store initial cable state for reset (FULL state: root + joints)
initial_cable_root_state = cable.data.root_state_w.clone()
initial_cable_joint_pos = cable.data.joint_pos.clone()  # Joint positions for all spherical joints
initial_cable_joint_vel = cable.data.joint_vel.clone()  # Joint velocities
...
cable.write_joint_state_to_sim(initial_cable_joint_pos, initial_cable_joint_vel)  # Reset joints too
...
def reset_cable_to_initial(max_retries: int = 3) -> bool:
    ...
    for attempt in range(max_retries):
        # Reset FULL cable state: root state + joint states (critical for Articulation!)
        cable.write_root_state_to_sim(initial_cable_root_state)
        cable.write_joint_state_to_sim(initial_cable_joint_pos, initial_cable_joint_vel)
        ...
        if check_cable_nan():
            ...
            # Extra stabilization with re-write
            cable.write_root_state_to_sim(initial_cable_root_state)
            cable.write_joint_state_to_sim(initial_cable_joint_pos, initial_cable_joint_vel)
            for _ in range(CYCLE_RESET_STABILIZATION_STEPS):
                sim.step()
