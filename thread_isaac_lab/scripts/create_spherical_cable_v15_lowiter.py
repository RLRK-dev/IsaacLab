#!/usr/bin/env python3
"""
Create SphericalJoint Cable USD v15_lowiter - Low Iteration Version
====================================================================

Based on v15_veryhighdamp but with reduced solver iterations to prevent
TGS solver hangs.

Key changes from v15:
- SOLVER_POS_ITER: 128 → 32
- SOLVER_VEL_ITER: 32 → 4 (TGS compliant - warning says >4 causes issues)

Root cause of hangs:
- Warning: "more than 4 velocity iterations being added to a TGS scene"
- PhysX TGS solver deadlocks with high iteration counts during contact

All other parameters (damping=60, mass=100g/seg) remain unchanged.
"""

import argparse
from isaaclab.app import AppLauncher

parser = argparse.ArgumentParser(description="Create SphericalJoint Cable USD v15_lowiter")
...
# Cable parameters (same as v15)
NUM_SEGMENTS = 20
SEGMENT_LENGTH = 0.03  # 3cm per segment
SEGMENT_RADIUS = 0.005  # 5mm radius
TOTAL_LENGTH = NUM_SEGMENTS * SEGMENT_LENGTH  # 60cm total

# Same mass and damping as v15
SEGMENT_MASS = 0.1  # 100g per segment
CONE_ANGLE_LIMIT = 30.0  # degrees

# LOW ITERATION - key change for v15_lowiter (TGS compliant)
SOLVER_POS_ITER = 32   # Reduced from 128
SOLVER_VEL_ITER = 4    # Reduced from 32 (TGS max is 4)

# Same damping as v15
LINEAR_DAMPING = 60.0
ANGULAR_DAMPING = 60.0

OUTPUT_PATH = "/home/rlrk/IsaacLab/source/extensions/isaaclab_tasks_thread/data/cable/spherical_cable_v15_lowiter.usd"


def create_cable_usd(output_path: str):
    """Create a TGS-compliant cable USD with low solver iterations."""

    print("=" * 60)
    print("Creating SphericalJoint Cable USD v15_lowiter (Low Iteration)")
    print("=" * 60)
    ...
    print(f"  Solver iterations: pos={SOLVER_POS_ITER}, vel={SOLVER_VEL_ITER} (TGS compliant)")
    ...
    for i in range(NUM_SEGMENTS):
        ...
        # Blue-green color for v15_lowiter (low iteration version)
        capsule.GetDisplayColorAttr().Set([(0.1, 0.6, 0.8)])
