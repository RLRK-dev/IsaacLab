...
def run_diff_ik_motion_with_collection(start_pos_left, start_pos_right, start_quat_left, start_quat_right,
    ...
    for i in range(num_steps):
        ...
        if phase == 4 and i % 100 == 0:
            delta_l = (joint_cmd_l[0] - joint_pos_l[0]).abs().max().item()
            delta_r = (joint_cmd_r[0] - joint_pos_r[0]).abs().max().item()
            print(f"  [IK Debug step {i}] joint_delta_max: L={delta_l:.4f}rad R={delta_r:.4f}rad")
...
def run_cycle(cycle_idx: int):
    ...
    # v48b: Stage 2a split into 2a-1 and 2a-2 (midpoint approach)
    # Stage 2a-1: First intermediate point (Z=1.05 constant)
    print("  --- Stage 2a-1: First intermediate (Z=1.05) ---")
    MID_X_LEFT_1 = 0.49  # Current position (~0.40) to ~1/3 of the way
    p45_mid1_l = torch.tensor([[MID_X_LEFT_1, -0.18, PRE_ROTATION_Z]], device=device)
    p45_mid1_r = torch.tensor([[WAYPOINT_PHASE45_RIGHT[0], WAYPOINT_PHASE45_RIGHT[1], PRE_ROTATION_Z]], device=device)
    print(f"  Stage 2a-1 Target: L=({p45_mid1_l[0,0]:.3f}, {p45_mid1_l[0,1]:.3f}, {p45_mid1_l[0,2]:.3f})")
    print(f"                     R=({p45_mid1_r[0,0]:.3f}, {p45_mid1_r[0,1]:.3f}, {p45_mid1_r[0,2]:.3f})")

    PHASE45_MID1_STEPS = 500
    p45_mid1_pos_l, p45_mid1_pos_r, p45_mid1_quat_l, p45_mid1_quat_r = run_diff_ik_motion_with_collection(
        p45c_pos_l, p45c_pos_r, p45c_quat_l, p45c_quat_r,
        p45_mid1_l, p45_mid1_r, PHASE45_MID1_STEPS, GRIPPER_CLOSE, phase=4
    )

    # Log Stage 2a-1 completion
    s2a1_ee_l = robot_left.data.body_pos_w[:, jacobian_body_left]
    s2a1_ee_r = robot_right.data.body_pos_w[:, jacobian_body_right]
    s2a1_force_l, s2a1_force_r = get_contact_force()
    s2a1_cable_z = cable.data.body_pos_w[0, :, 2].cpu().numpy()
    print(f"  Stage 2a-1 Complete: L=({s2a1_ee_l[0,0]:.3f}, {s2a1_ee_l[0,1]:.3f}, {s2a1_ee_l[0,2]:.3f})")
    print(f"  [P4.5 Stage2a-1] Contact: L={s2a1_force_l:.1f}N, R={s2a1_force_r:.1f}N, cable_Z: {s2a1_cable_z.min():.3f}-{s2a1_cable_z.max():.3f}")

    # Check cable NaN after Stage 2a-1
    if not check_cable_nan():
        print("  [ERROR] Cable NaN after Stage 2a-1!")

    # Stage 2a-2: Second intermediate point (original MID_X_LEFT)
    print("  --- Stage 2a-2: Second intermediate (Z=1.05) ---")
    MID_X_LEFT = 0.575  # Original intermediate point
    p45_mid_l = torch.tensor([[MID_X_LEFT, WAYPOINT_PHASE45_LEFT[1], PRE_ROTATION_Z]], device=device)
    p45_mid_r = torch.tensor([[WAYPOINT_PHASE45_RIGHT[0], WAYPOINT_PHASE45_RIGHT[1], PRE_ROTATION_Z]], device=device)
    print(f"  Stage 2a-2 Target: L=({p45_mid_l[0,0]:.3f}, {p45_mid_l[0,1]:.3f}, {p45_mid_l[0,2]:.3f})")
    print(f"                     R=({p45_mid_r[0,0]:.3f}, {p45_mid_r[0,1]:.3f}, {p45_mid_r[0,2]:.3f})")

    PHASE45_MID2_STEPS = 500
    p45_mid_pos_l, p45_mid_pos_r, p45_mid_quat_l, p45_mid_quat_r = run_diff_ik_motion_with_collection(
        p45_mid1_pos_l, p45_mid1_pos_r, p45_mid1_quat_l, p45_mid1_quat_r,
        p45_mid_l, p45_mid_r, PHASE45_MID2_STEPS, GRIPPER_CLOSE, phase=4
    )

    # Log Stage 2a-2 completion with contact force
    s2a_ee_l = robot_left.data.body_pos_w[:, jacobian_body_left]
    s2a_ee_r = robot_right.data.body_pos_w[:, jacobian_body_right]
    s2a_force_l, s2a_force_r = get_contact_force()
    s2a_cable_z = cable.data.body_pos_w[0, :, 2].cpu().numpy()
    print(f"  Stage 2a-2 Complete: L=({s2a_ee_l[0,0]:.3f}, {s2a_ee_l[0,1]:.3f}, {s2a_ee_l[0,2]:.3f})")
    print(f"                       R=({s2a_ee_r[0,0]:.3f}, {s2a_ee_r[0,1]:.3f}, {s2a_ee_r[0,2]:.3f})")
    print(f"  [P4.5 Stage2a-2] Contact: L={s2a_force_l:.1f}N, R={s2a_force_r:.1f}N, cable_Z: {s2a_cable_z.min():.3f}-{s2a_cable_z.max():.3f}")
    # v47: Target diff logging
    target_diff_x = MID_X_LEFT - s2a_ee_l[0,0].item()
    target_diff_y = WAYPOINT_PHASE45_LEFT[1] - s2a_ee_l[0,1].item()
    target_diff_z = PRE_ROTATION_Z - s2a_ee_l[0,2].item()
    print(f"  Stage2a Target Diff: dX={target_diff_x*1000:.1f}mm, dY={target_diff_y*1000:.1f}mm, dZ={target_diff_z*1000:.1f}mm")

    # Check cable NaN after Stage 2a
    if not check_cable_nan():
        print("  [ERROR] Cable NaN after Stage 2a!")

    # Stage 2b: Second half (Z fixed at 1.05 - v46 fix)
    # v46: Z下降を削除。Z=1.05を維持したままX移動のみ
    print("  --- Stage 2b: Second half (Z=1.05 fixed, v46) ---")
    ...
    print(f"  Stage 2b Target: L=({p45_target_l[0,0]:.3f}, {p45_target_l[0,1]:.3f}, {p45_target_l[0,2]:.3f})")
    print(f"                   R=({p45_target_r[0,0]:.3f}, {p45_target_r[0,1]:.3f}, {p45_target_r[0,2]:.3f})")

    PHASE45_FINAL_STEPS = 1000  # v47a: 500→1000に増加（NaN回避）
    p45_pos_l, p45_pos_r, p45_quat_l, p45_quat_r = run_diff_ik_motion_with_collection(
        ...
        p45_target_l, p45_target_r, PHASE45_FINAL_STEPS, GRIPPER_CLOSE, phase=4
    )
    ...
    print(f"  [P4.5 Stage2b] Contact: L={s2b_force_l:.1f}N, R={s2b_force_r:.1f}N, cable_Z: {s2b_cable_z.min():.3f}-{s2b_cable_z.max():.3f}")
