...
def run_arc_rotation_with_collection(
    ...
    for i in range(num_steps):
        ...
        JOINT_DELTA_MAX = 0.03  # v52b: 0.03rad + X=0.55 target
...
def run_cycle(cycle_idx: int):
    ...
    # Stage 2a-2: v52b: skip this stage (go straight to 2a-4/mid)
    print("  Stage 2a-2: Target (0.46, -0.19, 1.05)")
    p2a2_target_l = torch.tensor([[0.46, -0.19, PRE_ROTATION_Z]], device=device)  # v52b: reduced
    ...
    # Stage 2a-3: v52b: reduced target
    print("  Stage 2a-3: Target (0.47, -0.20, 1.05)")
    p2a3_target_l = torch.tensor([[0.47, -0.20, PRE_ROTATION_Z]], device=device)  # v52b: reduced
    ...
    # Stage 2a-4: (0.535, -0.215) → (0.48, -0.22) [v52b: 中間点縮小]
    print("  Stage 2a-4: Target (0.48, -0.22, 1.05)")
    MID_X_LEFT = 0.48  # v52b: 0.575→0.48
    ...
    # Stage 2b-1: v52b: (0.48, -0.22) → (0.52, -0.22) 縮小目標
    print("  Stage 2b-1: Target (0.52, -0.22, 1.05)")
    p2b1_target_l = torch.tensor([[0.52, -0.22, FINAL_Z]], device=device)  # v52b: 0.64→0.52, Y=-0.22
    ...
    # Stage 2b-2: v52b: (0.52, -0.22) → (0.55, -0.22) [縮小最終位置]
    print("  Stage 2b-2: Target (0.55, -0.22, 1.05)")
    p45_target_l = torch.tensor([[0.55, -0.22, FINAL_Z]], device=device)  # v52b: X=0.70→0.55, Y=-0.24→-0.22
