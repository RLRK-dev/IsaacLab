...
# =============================================================================
# RIGHT ARM ONLY TEST MODE
# Left arm stays at initial position (no grasping)
# Purpose: Isolate right arm physics for NaN debugging
# =============================================================================
RIGHT_ONLY_MODE = True
...
def set_right_arm(robot_right, arm_joints, gripper_val):
    """Set right arm joints (normal operation in RIGHT_ONLY_MODE)."""
    set_robot_joints(robot_right, arm_joints, gripper_val)


def set_left_arm(robot_left, arm_joints, gripper_val):
    """Set left arm joints, respecting RIGHT_ONLY_MODE."""
    if RIGHT_ONLY_MODE:
        # Keep left arm at initial position with gripper open
        set_robot_joints(robot_left, LEFT_ARM_INIT_JOINTS, GRIPPER_OPEN)
    else:
        set_robot_joints(robot_left, arm_joints, gripper_val)
...
def reset_cable_to_initial(max_retries: int = 3) -> bool:
    """Reset cable to initial position with NaN check and retry.
...
def run_cycle(cycle_idx: int):
    ...
    for i in range(PHASE1_STEPS):
        set_left_arm(robot_left, LEFT_ARM_INIT_JOINTS, GRIPPER_OPEN)
        set_right_arm(robot_right, RIGHT_ARM_INIT_JOINTS, GRIPPER_OPEN)
        sim.step()
        scene.update(sim.get_physics_dt())
        if i % FRAME_COLLECT_INTERVAL == 0:
    ...
    for step in range(PHASE12_STEPS):
        ...
        set_left_arm(robot_left, left_joints, GRIPPER_OPEN)
        set_right_arm(robot_right, right_joints, GRIPPER_OPEN)
        sim.step()
        scene.update(sim.get_physics_dt())
    ...
    for i in range(GRASP_CLOSE_STEPS):
        ...
        set_left_arm(robot_left, PHASE2_LEFT_JOINTS, current_grip)
        set_right_arm(robot_right, PHASE2_RIGHT_JOINTS, current_grip)
        sim.step()
        scene.update(sim.get_physics_dt())
    ...
    for i in range(GRASP_STABILIZE):
        set_left_arm(robot_left, PHASE2_LEFT_JOINTS, GRIPPER_CLOSE)
        set_right_arm(robot_right, PHASE2_RIGHT_JOINTS, GRIPPER_CLOSE)
        sim.step()
        scene.update(sim.get_physics_dt())
        if i % FRAME_COLLECT_INTERVAL == 0:
    ...
    # Use Phase 2 joints as start (after lift, gripper is down)
    ...
    for step in range(DIRECT_HOOK_STEPS):
        ...
        # Interpolate from Phase 2 joints directly to Phase 5.5 joints
        left_joints = [p2 + alpha * (p55 - p2) for p2, p55 in zip(PHASE2_LEFT_JOINTS, PHASE55_LEFT_JOINTS)]
        right_joints = [p2 + alpha * (p55 - p2) for p2, p55 in zip(PHASE2_RIGHT_JOINTS, PHASE55_RIGHT_JOINTS)]
        set_left_arm(robot_left, left_joints, GRIPPER_CLOSE)
        set_right_arm(robot_right, right_joints, GRIPPER_CLOSE)
        sim.step()
        scene.update(sim.get_physics_dt())
        if step % FRAME_COLLECT_INTERVAL == 0:
    ...
    for i in range(RELEASE_STABILIZE_STEPS):
        set_left_arm(robot_left, PHASE55_LEFT_JOINTS, GRIPPER_CLOSE)
        set_right_arm(robot_right, PHASE55_RIGHT_JOINTS, GRIPPER_CLOSE)
        sim.step()
        scene.update(sim.get_physics_dt())
        if i % FRAME_COLLECT_INTERVAL == 0:
    ...
    for i in range(RELEASE_GRIPPER_STEPS):
        ...
        set_left_arm(robot_left, PHASE55_LEFT_JOINTS, gripper_val)
        set_right_arm(robot_right, PHASE55_RIGHT_JOINTS, gripper_val)
        sim.step()
        scene.update(sim.get_physics_dt())
        if i % FRAME_COLLECT_INTERVAL == 0:
    ...
    # Stabilize at home
    for i in range(100):
        set_left_arm(robot_left, PHASE7_LEFT_JOINTS, GRIPPER_OPEN)
        set_right_arm(robot_right, PHASE7_RIGHT_JOINTS, GRIPPER_OPEN)
        sim.step()
        scene.update(sim.get_physics_dt())
        if i % FRAME_COLLECT_INTERVAL == 0:
