#!/usr/bin/env python3
"""
Create SphericalJoint Cable USD v12 - Lightweight + 90 degree
==============================================================

v12: Combines successful v5 mass (50g) with high flexibility (90°)
- Lightweight: 50g per segment (same as v5 which achieved 13.6cm lift)
- High cone angle: 90° to prevent joint limit violations
- Moderate damping: 10.0 (same as v5)
"""

import argparse
from isaaclab.app import AppLauncher

parser = argparse.ArgumentParser(description="Create SphericalJoint Cable USD v12")
...
# v12: Lightweight + high flexibility parameters (based on v5 success)
SEGMENT_MASS = 0.05  # 50g per segment (same as v5 - SUCCESS config)
CONE_ANGLE_LIMIT = 90.0  # degrees - high flexibility
SOLVER_POS_ITER = 128
SOLVER_VEL_ITER = 32
LINEAR_DAMPING = 10.0  # Same as v5
ANGULAR_DAMPING = 10.0  # Same as v5

OUTPUT_PATH = "/home/rlrk/IsaacLab/source/extensions/isaaclab_tasks_thread/data/cable/spherical_cable_v12_light90.usd"


def create_cable_usd(output_path: str):
    """Create a lightweight cable USD with 90° cone angle."""

    print("=" * 60)
    print("Creating SphericalJoint Cable USD v12 (Lightweight + 90°)")
    print("=" * 60)
    print(f"  Segments: {NUM_SEGMENTS}")
    print(f"  Segment length: {SEGMENT_LENGTH * 100:.1f} cm")
    print(f"  Segment radius: {SEGMENT_RADIUS * 1000:.1f} mm")
    print(f"  Segment mass: {SEGMENT_MASS * 1000:.0f} g (LIGHTWEIGHT - same as v5)")
    ...
    for i in range(NUM_SEGMENTS):
        ...
        # Green color for v12 (lightweight)
