...
def run_cycle(cycle_idx: int):
    ...
    # ============================================
    # v50b: シンプル2段階版（目標Y=-0.20）
    # ============================================
    print("  --- Stage 2a: Simple 2-step (v50b Y=-0.20) ---")

    # Stage 2a: 中間点へ
    MID_X_LEFT = 0.55  # 中間点
    p45_mid_l = torch.tensor([[MID_X_LEFT, -0.18, PRE_ROTATION_Z]], device=device)
    ...
    print(f"  [P4.5 Stage2a] L=({s2a_ee_l[0,0]:.3f}, {s2a_ee_l[0,1]:.3f}, {s2a_ee_l[0,2]:.3f}), Force: L={s2a_force_l:.1f}N R={s2a_force_r:.1f}N, cable_Z: {s2a_cable_z.min():.3f}-{s2a_cable_z.max():.3f}")
    if not check_cable_nan():
        print("  [ERROR] Cable NaN after Stage 2a!")

    # Stage 2b: 最終位置へ（Y=-0.20）
    print("  --- Stage 2b: Final position (v50b Y=-0.20) ---")
    FINAL_Z = PRE_ROTATION_Z
    FINAL_Y = -0.20  # v50b: 目標Yを-0.24→-0.20に緩和
    p45_target_l = torch.tensor([[0.70, FINAL_Y, FINAL_Z]], device=device)
    ...
    p45_pos_l, p45_pos_r, p45_quat_l, p45_quat_r = run_diff_ik_motion_with_collection(
        ...
        p45_target_l, p45_target_r, PHASE45_STEPS, GRIPPER_CLOSE, phase=4
    )
    ...
    print(f"  [P4.5 Stage2b] L=({final_ee_l[0,0]:.3f}, {final_ee_l[0,1]:.3f}, {final_ee_l[0,2]:.3f}), Force: L={s2b_force_l:.1f}N R={s2b_force_r:.1f}N, cable_Z: {s2b_cable_z.min():.3f}-{s2b_cable_z.max():.3f}")
