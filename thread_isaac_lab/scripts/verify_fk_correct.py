#!/usr/bin/env python3
"""
Correct FK Verification using IK module's FK function

Uses the same FK implementation and coordinate transforms as the IK solver.
...
def compute_world_ee_position(joint_angles, base_pos, base_quat_wxyz):
    """
    Compute EE position in world frame (matching IK module's coordinate transform).
    ...
    """
    # Get EE transformation in base frame (4x4 homogeneous matrix)
    ...
    R_base = R.from_quat(base_quat_xyzw).as_matrix()
    ...
    # Transform EE from base frame to world frame
    # T_world_ee = T_base @ T_base_ee
    T_world_ee = T_base @ T_base_ee

    # Extract world position
    ee_world = T_world_ee[:3, 3]
...
def main():
    ...
    print("=" * 70)
    print("Correct FK Verification using IK Module - All Phases")
    print("=" * 70)
