#!/usr/bin/env python3
"""
Create SphericalJoint Cable USD v5
==================================

v5: Rigid body damping approach
- Since articulation joint damping is ignored, apply damping to rigid bodies instead
- Use PhysxRigidBodyAPI's linearDamping and angularDamping attributes
- 20 segments x 3cm = 60cm total (Y-axis layout)
- High linear/angular damping on each segment

Purpose: Fix cable drift by damping individual segment motion.
"""
...
# Parse arguments
parser = argparse.ArgumentParser(description="Create SphericalJoint Cable USD v5")
parser.add_argument("--linear_damping", type=float, default=10.0, help="Linear damping per segment")
parser.add_argument("--angular_damping", type=float, default=10.0, help="Angular damping per segment")
...
# Cable parameters (same as v3/v4)
NUM_SEGMENTS = 20
SEGMENT_LENGTH = 0.03  # 3cm per segment
SEGMENT_RADIUS = 0.005  # 5mm radius
TOTAL_LENGTH = NUM_SEGMENTS * SEGMENT_LENGTH  # 60cm total

# v5: Rigid body damping parameters
SEGMENT_MASS = 0.05  # 50g per segment
CONE_ANGLE_LIMIT = 30.0  # degrees
SOLVER_POS_ITER = 64  # High solver iterations
SOLVER_VEL_ITER = 16
LINEAR_DAMPING = args_cli.linear_damping  # Linear velocity damping
ANGULAR_DAMPING = args_cli.angular_damping  # Angular velocity damping

OUTPUT_PATH = "/home/rlrk/IsaacLab/source/extensions/isaaclab_tasks_thread/data/cable/spherical_cable_v5.usd"


def create_spherical_cable_usd(output_path: str):
    """Create a flexible cable USD with rigid body damping."""

    print("=" * 60)
    print("Creating SphericalJoint Cable USD v5 (Rigid Body Damping)")
    print("=" * 60)
    ...
    for i in range(NUM_SEGMENTS):
        ...
        # Set color (green for v5)
        capsule.GetDisplayColorAttr().Set([(0.0, 0.8, 0.2)])
