...
def run_cycle(cycle_idx: int):
    ...
    # ============================================
    # v50a: シンプル2段階版（目標X=0.60）
    # ============================================
    print("  --- Stage 2a: Simple 2-step (v50a X=0.60) ---")

    # Stage 2a: 中間点へ
    MID_X_LEFT = 0.52  # 縮小版
    p45_mid_l = torch.tensor([[MID_X_LEFT, -0.20, PRE_ROTATION_Z]], device=device)
    ...
    # Stage 2b: 最終位置へ（X=0.60）
    print("  --- Stage 2b: Final position (v50a X=0.60) ---")
    FINAL_Z = PRE_ROTATION_Z
    FINAL_X = 0.60  # v50a: 目標Xを0.70→0.60に縮小
    p45_target_l = torch.tensor([[FINAL_X, -0.24, FINAL_Z]], device=device)
