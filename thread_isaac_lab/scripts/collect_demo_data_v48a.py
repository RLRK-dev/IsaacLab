...
diff_ik_cfg = DifferentialIKControllerCfg(
    ...
    ik_method="dls",
    ik_params={"lambda_val": 0.1},  # v48a: Reduced for faster tracking
)
...
def run_cycle(cycle_idx: int):
    ...
    # Stage 2a: First half (Z=1.05 constant)
    print("  --- Stage 2a: First half (Z=1.05) ---")
    # 左腕: X 0.40 → 0.575 (中間点)
    # 右腕: X固定（0.30）
    MID_X_LEFT = 0.575  # 中間点
    ...
    print(f"  Stage 2a Target: L=({p45_mid_l[0,0]:.3f}, {p45_mid_l[0,1]:.3f}, {p45_mid_l[0,2]:.3f})")
    print(f"                   R=({p45_mid_r[0,0]:.3f}, {p45_mid_r[0,1]:.3f}, {p45_mid_r[0,2]:.3f})")

    PHASE45_MID_STEPS = 1000  # v47: 500→1000 for Stage2a convergence
    p45_mid_pos_l, p45_mid_pos_r, p45_mid_quat_l, p45_mid_quat_r = run_diff_ik_motion_with_collection(
        ...
        p45_mid_l, p45_mid_r, PHASE45_MID_STEPS, GRIPPER_CLOSE, phase=4
    )

    # Log Stage 2a completion with contact force
    ...
    print(f"  Stage 2a Complete: L=({s2a_ee_l[0,0]:.3f}, {s2a_ee_l[0,1]:.3f}, {s2a_ee_l[0,2]:.3f})")
    print(f"                     R=({s2a_ee_r[0,0]:.3f}, {s2a_ee_r[0,1]:.3f}, {s2a_ee_r[0,2]:.3f})")
    print(f"  [P4.5 Stage2a] Contact: L={s2a_force_l:.1f}N, R={s2a_force_r:.1f}N, cable_Z: {s2a_cable_z.min():.3f}-{s2a_cable_z.max():.3f}")
