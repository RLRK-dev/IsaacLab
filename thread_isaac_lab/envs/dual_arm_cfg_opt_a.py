...
# Robot base positions - wall-mounted configuration (asymmetric)
# 最終構成: 左右で異なるX/Y位置
ARM_Z_POS = 1.10     # Z position: 共通
# Left robot position
ARM_LEFT_X = 0.10    # Left arm X (IK optimized: same as right)
ARM_LEFT_Y = -0.40   # Left arm Y
# Right robot position (offset for optimal reach)
ARM_RIGHT_X = 0.10   # Right arm X (10cm forward)
ARM_RIGHT_Y = 0.20   # Right arm Y
...
class LeftFrankaCfg(ArticulationCfg):
    ...
    # Initial state - Wall-mount rotation aware IK solution
    init_state: ArticulationCfg.InitialStateCfg = ArticulationCfg.InitialStateCfg(
        pos=(ARM_LEFT_X, ARM_LEFT_Y, ARM_Z_POS),
        rot=(0.7071, 0.0, 0.7071, 0.0),  # Y+90° only: 前方（+X）を向く
        joint_pos={
            "panda_joint1": LEFT_ARM_INIT_JOINTS[0],  # IK for EE at (0.30, -0.285, 0.905)
            ...
            "panda_finger_joint1": GRIPPER_INIT,
            "panda_finger_joint2": GRIPPER_INIT,
        },
        joint_vel={".*": 0.0},
    )

    actuators: dict = {
        # DifferentialIK用: 位置制御（超高stiffness, 超高damping - 壁掛け重力対策）
        "panda_shoulder": ImplicitActuatorCfg(
            joint_names_expr=["panda_joint[1-4]"],
            effort_limit=87.0,
            velocity_limit=2.175,
            stiffness=2000.0,   # 壁掛け重力対策
            damping=200.0,      # 振動抑制
        ),
...
class RightFrankaCfg(ArticulationCfg):
    ...
    # Initial state - Wall-mount rotation aware IK solution
    init_state: ArticulationCfg.InitialStateCfg = ArticulationCfg.InitialStateCfg(
        pos=(ARM_RIGHT_X, ARM_RIGHT_Y, ARM_Z_POS),
        rot=(0.7071, 0.0, 0.7071, 0.0),  # Y+90°: 前方（+X）を向く
        joint_pos={
            "panda_joint1": RIGHT_ARM_INIT_JOINTS[0],  # IK for EE at (0.30, +0.285, 0.905)
    ...
    cable: ArticulationCfg = ArticulationCfg(
        ...
        spawn=UsdFileCfg(
            ...
            articulation_props=sim_utils.ArticulationRootPropertiesCfg(
                ...
                solver_position_iteration_count=128,  # Keep high position iterations for cable articulation stability
                solver_velocity_iteration_count=4,    # TGS-compliant velocity iterations; >4 is unstable
            ),
        ),
        init_state=ArticulationCfg.InitialStateCfg(
            # Y-axis cable layout: cable extends in +Y direction
            # Cable spans Y=-0.285 to Y=+0.285
            # 過去の成功設定: cable at X=0.30
        ...
        actuators={
            "cable_joints": ImplicitActuatorCfg(
                ...
                damping=5.0,    # High damping for drift prevention (A+C test)
            ),
        },
    )
