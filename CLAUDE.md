...
- **⚠ Option-E（mujoco-substrate S-series: S1-S8）の venv は `env_isaaclab7`。版 = 実測 2026-08-03: Newton 1.4.0 / mujoco 3.10.0 / mujoco-warp 3.10.0.3 / warp 1.15.0。** 2026-07-27 20:05 に Rs 依頼で p5 が更新（更新前 = Newton 1.2.1 / mujoco 3.8.1 / warp 1.13.0。記録 = `eval_runs/troot_optE_dapg_wholeroute_scope_20260701/P5_ENV7_UPGRADE_20260727/REPORT.md` + `pip_freeze_BEFORE/AFTER.txt`）。⛔**更新前に測られた evidence は as-measured で真 — 過去 doc の版表記を書き換えない**（code sha 一致は必要十分でない・env は別軸）。⛔**本行の版は実測日つき。引用前に `/home/rlrk/env_isaaclab7/bin/python -c "import newton,mujoco,warp;print(newton.__version__,mujoco.__version__,warp.__version__)"` で再測せよ。** ⚠ 旧記載の `env_isaaclab6`（VBD/RL track・Newton 1.0.0）は **on-disk に存在しない**（2026-08-03 実測・preflight P9 WARN）⇒ fallback として引かない。Option-E の smoke/test/build は `/home/rlrk/env_isaaclab7/bin/python` で実行する
