...
"""Deliver the verified v05b folder without modifying earlier review versions."""
...
def main():
    ...
    print(f"V05B_DELIVERY_PREFLIGHT files={len(rows)} mp4={len(manifest['mp4_files'])}", flush=True)
    if not args.execute:
        return
    ...
    print(f"V05B_DELIVERY_COMPLETE files={len(rows)} mp4=1 destination={DESTINATION}")
