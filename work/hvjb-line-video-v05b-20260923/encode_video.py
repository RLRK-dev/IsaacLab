...
"""Encode the complete saved-sample PNG sequence directly into one review MP4."""
...
def main() -> None:
    plan_path = ROOT / "data/concept_v05b.json"
    plan = json.loads(plan_path.read_text())
    assert not plan["preview_only"]
    assert len(plan["expected_source_samples"]) == 1785
    ...
    encode_process_review_video.encode(plan_path, ["concept_v05b"], "HVJB_line_split_process_concept_v05b_review")
    for path, expected in pins.items():
        assert hashlib.sha256(Path(path).read_bytes()).hexdigest() == expected, path
