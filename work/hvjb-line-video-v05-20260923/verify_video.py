...
STEM = "HVJB_line_split_process_concept_v05_review"
...
def main() -> None:
    ...
    plan = json.loads((ROOT / "data/concept_v05.json").read_text())
    manifest = json.loads((ROOT / "previews/concept_v05/manifest.json").read_text())
    ...
    assert encoded["presentation_sha256"] == sha(ROOT / "data/concept_v05.json")
    ...
    defaults = [int((row["start_s"] + 0.62 * (row["stop_s"] - row["start_s"])) * 15) for row in plan["scenes"]]
    preview = json.loads((ROOT / "previews/concept_v05_preview02/manifest.json").read_text())
    indices = sorted(set(defaults) | {row["original_sample_index"] for row in preview["images"]})
    selected_scenes = [
        next(row for row in plan["scenes"] if row["start_s"] <= index / 15 < row["stop_s"]) for index in indices
    ]
    ...
    rows = []
    for number, (index, scene) in enumerate(zip(indices, selected_scenes, strict=True), start=1):
        path = images_path / f"frame-{number:02d}.png"
        source = ROOT / "previews/concept_v05" / manifest["images"][index]["file"]
    ...
    print(f"VIDEO_READBACK_COMPLETE frames=1785 seconds=119 samples={len(indices)} jobs=20", flush=True)
